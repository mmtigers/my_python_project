...
def data_dir(tmp_path, monkeypatch):
    """site_failures.jsonの保存先をテスト用一時ディレクトリに差し替える。"""
...
class TestSiteFailureLifecycle:
    def test_record_increments_consecutive_count(self, data_dir):
        count1, alerted1 = DataManager.record_site_failure("site_a")
        count2, alerted2 = DataManager.record_site_failure("site_a")

        assert (count1, alerted1) == (1, False)
        assert (count2, alerted2) == (2, False)

    def test_counts_are_tracked_per_site(self, data_dir):
        DataManager.record_site_failure("site_a")
        count_b, _ = DataManager.record_site_failure("site_b")

        assert count_b == 1

    def test_mark_alerted_is_returned_by_subsequent_record(self, data_dir):
        DataManager.record_site_failure("site_a")
        DataManager.mark_site_failure_alerted("site_a")

        _, alerted = DataManager.record_site_failure("site_a")

        assert alerted is True

    def test_clear_resets_count_and_alerted_flag(self, data_dir):
        DataManager.record_site_failure("site_a")
        DataManager.mark_site_failure_alerted("site_a")

        DataManager.clear_site_failure("site_a")
        count, alerted = DataManager.record_site_failure("site_a")

        assert (count, alerted) == (1, False)

    def test_clear_without_existing_record_does_not_create_file(self, data_dir):
        """正常巡回のたびに全サイト分のNAS書き込みが発生しないこと。"""
        DataManager.clear_site_failure("never_failed_site")

        assert not (data_dir / "site_failures.json").exists()

    def test_corrupted_failures_file_is_treated_as_empty_state(self, data_dir):
        failures_file = data_dir / "site_failures.json"
        failures_file.write_bytes(b'{"site_a": \xf9broken')

        assert DataManager.load_site_failures() == {}


class TestHandleSiteNetworkFailure:
    def _fail_once(self, notifier, site):
        module._handle_site_network_failure(
            notifier, site, requests.RequestException("connection refused")
        )

    def test_below_threshold_logs_error_and_does_not_alert(
        self, data_dir, caplog, propagating_logger
    ):
        site = _make_site()
        notifier = MagicMock()

        with caplog.at_level(logging.WARNING, logger="newface_monitor"):
            self._fail_once(notifier, site)
    ...
    def test_alert_sent_once_when_threshold_reached(self, data_dir):
        site = _make_site()
        notifier = MagicMock()
    ...
    def test_failed_alert_send_is_retried_on_next_failure(self, data_dir):
...
class TestCheckSiteIntegration:
    def test_network_failure_records_and_success_clears(self, data_dir, monkeypatch):
        ...
        assert DataManager.load_site_failures()[site.site_id]["count"] == 1
