...
# #581: 以前は`Prompt:\s+([^\n]+)`のように`Prompt:`直後を`\s+`で受けていたため、
# 内容が空(末尾空白のみ)の`Prompt:`行では、次の「番号. タイトル」行までの
# 改行をすべて`\s+`が飲み込み、次項目のタイトル行自体をこの項目の内容として
# 誤って取り込んでいた(次項目はその分丸ごと消失する)。`[ \t]*(.*)`で行内の
# 空白のみを許容し、改行を跨がないようにする(内容が空の項目も正しく1件として扱う)。
