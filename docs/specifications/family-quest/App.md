...
* **完了音・無限クエストのクールダウン発火タイミングの修正（Issue #102）**: 以前は`QuestList.tsx`の`QuestItem`が、タップ即時（`ConfirmModal`を開く前）に完了音の再生と（無限クエストの場合の）60秒クールダウンの開始を行っていた。そのため確認モーダルで「キャンセル」しても完了音が鳴り、無限クエストは60秒間タップ不能になる不具合があった。修正後は、`runQuestAction`が`completeQuest`の成功レスポンスを受け取った時点で初めて完了音（`clear`/`submit`）を再生し、対象クエストの`quest_id`・完了した本人の`user.user_id`（**Issue #363で追加**: 横画面4人パネルで同じシグナルが全員に届いても、`QuestItem`側が自分のパネルの完了にだけ反応できるようにする）・発火のたびに変わる`nonce`(`Date.now()`)を`completedSignal`(state)にセットする。**（#291で修正）** `id`へのフォールバック（`completedQuest.id ?? completedQuest.quest_id`）は、`id`がAPIから一度も送られてこない幽霊フィールドであったため廃止され、`quest_id`のみを参照するようになった。`completedSignal`は`FamilyDashboard`（横画面）・`QuestList`（縦画面、直接使用）の両方のJSXへpropとして渡され、`FamilyDashboard`→`FamilyPanel`→`QuestList`→`QuestItem`まで転送されたのち、`QuestItem`側の`useEffect`が`completedSignal.id`と自身のクエストIDの一致・`completedSignal.userId`と自パネルのユーザーの一致（Issue #363）・`isInfinite`を条件にクールダウンを開始する（判定ロジック自体は`QuestList.tsx`側の管轄）。子どもの完了報告（`status: 'pending'`）も鳴動・クールダウン対象から除外されない。
* 根拠: `completedSignal`宣言・更新箇所 (181〜188, 249〜252行目 / 抜粋: "setCompletedSignal({ id: idForSignal, userId: user.user_id, nonce: Date.now() });")、JSXへの受け渡し (554, 603行目)
...
* **[修正済み] クエスト完了の送信中ガードとカードのローディング表示（Issue #391 / F-M3）**: `executeConfirm`は確認モーダルを閉じてから`await runQuestAction`するため、応答が返るまでカードは未完了のまま再タップでき、2回目の確認モーダルが1回目の完了後も開いたまま残って「はい」で400「本日は完了済み」/429のエラーモーダルになっていた。修正後は送信中の`(user_id, quest_id)`キー（`getQuestProcessingKey`、`useQuestStatus.ts`）の集合`processingQuestKeysRef`（同期判定用ref）と`processingQuestKeys`（`QuestList`へ渡す表示用state。`syncProcessingQuestKeys`でrefから写す）を持ち、`runQuestAction`は同一キーが送信中なら何もせず、それ以外はキーを登録して`runQuestActionInner`（従来の本体）を`try`/`finally`で実行し解除する。`handleQuestClick`は送信中キーのタップを冒頭で静かに無視する（`play('select')`も鳴らさない）。`processingQuestKeys`は`FamilyDashboard`（横画面）と`QuestList`（縦画面）へpropとして渡され、該当カードは「送信中...」オーバーレイになる。
* 根拠: (186〜193, 248〜262, 304〜305, 629, 683行目 / 抜粋: "const processingQuestKeysRef = useRef<Set<string>>(new Set());\n  const [processingQuestKeys, setProcessingQuestKeys] = useState<string[]>([]);", "const processingKey = getQuestProcessingKey(user.user_id, target.quest_id);\n    if (processingQuestKeysRef.current.has(processingKey)) return;\n    processingQuestKeysRef.current.add(processingKey);\n    syncProcessingQuestKeys();\n    try {\n      await runQuestActionInner(user, mode, target);\n    } finally {\n      processingQuestKeysRef.current.delete(processingKey);\n      syncProcessingQuestKeys();\n    }", "if (processingQuestKeysRef.current.has(getQuestProcessingKey(user.user_id, q.quest_id))) return;", "processingQuestKeys={processingQuestKeys}")
* **[修正済み] 一括承認と個別承認の競合（Issue #391 / F-L8）**: 以前は`handleApproveAll`のループが`approvingHistoryIdsRef`と連携しておらず、一括承認中に個別の「承認」をタップすると同じ履歴に2回目の承認POSTが飛び400のエラーモーダルになっていた。修正後は`handleApproveAll`が(1)個別承認の応答待ち中（既に集合にある）の履歴を対象から外し、(2)残りの全idを処理開始前に`approvingHistoryIdsRef`へ入れ（`claimedIds`）、`finally`でまとめて解除する。一括処理中の個別タップは`handleApprove`の既存ガード（`has(history.id)`）で静かに無視される。あわせて`approvingHistoryIds`/`isApprovingAll`をstateにも写し（`syncApprovingHistoryIds`/`setIsApprovingAll`）、`ApprovalList`（横画面は`FamilyDashboard`経由）へ`busyHistoryIds`/`isApprovingAll`として渡して該当行のボタンをローディング/無効化する。
* 根拠: (180〜184, 424〜429, 454〜457, 466〜485, 524〜528, 630〜631, 660〜661行目 / 抜粋: "const [approvingHistoryIds, setApprovingHistoryIds] = useState<ID[]>([]);\n  const [isApprovingAll, setIsApprovingAll] = useState(false);", "const targets = [...pendingQuestsRef.current].filter(h =>\n        h.id == null || !approvingHistoryIdsRef.current.has(h.id)\n      );", "for (const h of targets) {\n        if (h.id != null) {\n          approvingHistoryIdsRef.current.add(h.id);\n          claimedIds.push(h.id);\n        }\n      }", "for (const id of claimedIds) approvingHistoryIdsRef.current.delete(id);", "busyHistoryIds={approvingHistoryIds}\n            isApprovingAll={isApprovingAll}")
