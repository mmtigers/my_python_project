...
* 根拠: (324〜326, 331〜340, 344〜357, 372〜375行目)
...
* **役割**: クエスト承認処理を実行する。記録名義は`getRepresentativeParent(users)`で「親」に固定する（要件5）。**Issue #119の修正により**、先頭で`history.id`が`approvingHistoryIdsRef.current`（`Set<ID>`）に既に含まれていれば即座に`return`し、同一履歴への多重送信（連打・スワイプ承認とボタン承認のほぼ同時操作）を静かに無視する（エラー表示は出さない）。ガードを通過すると`history.id`を`approvingHistoryIdsRef.current`に追加し、`try`ブロックで`approveQuest`を呼び出し、`finally`で必ず`approvingHistoryIdsRef.current`から取り除く。承認は複数の異なるクエストを並行して処理できる必要があるため、`executeConfirm`（Issue #101）のような単一の`boolean`ではなく、処理中の履歴idの集合（`Set`）で個別に多重送信を防いでいる。成功時、承認APIのレスポンス`res.earnedMedals`が1以上であれば`medal`音とメダル獲得トーストを表示する（バグ修正M-6-1: 以前は承認経由のメダル獲得演出が一切反映されなかった）。失敗時はエラーメッセージ（`onRetry`で同じ引数で自身を再実行するコールバック付き）を表示する。
...
* 根拠: メダル獲得演出のバグ修正 (390〜395行目 / 抜粋: "// ★バグ修正(M-6-1): 承認APIのearnedMedalsを見て、完了フロー(runQuestAction)と\n      // 同様にメダル獲得演出を出す(以前は承認経由だと一切反映されなかった)。\n      if ((res.earnedMedals ?? 0) > 0) {\n        play('medal');\n        showToast({ title: \"ちいさなメダル獲得！\", text: `ちいさなメダルを ${res.earnedMedals} 枚手に入れた！`, icon: \"🏅\" });\n      }")

* **引数/リクエスト**: `history: QuestHistory`
* **戻り値/レスポンス**: `Promise<void>`
* **副作用**: `approvingHistoryIdsRef.current`への履歴idの追加・削除（Issue #119の多重送信ガード、`finally`で必ず削除）、`approveQuest`の呼び出し、`approve`/`medal`/`cancel`音の再生、成功時（メダル獲得時）の`showToast`、失敗時の`messageData`更新
...
* **メダル獲得演出のバグ修正（完了・承認・一括承認の3経路）**: 以前はサーバー側で計算されていた`earnedMedals`をフロントが一切参照していなかったため無反応だった。現在は`runQuestAction`内（クエスト完了時、252〜256行目）に加え、`handleApprove`（個別承認、390〜395行目、バグ修正M-6-1）と`handleApproveAll`（一括承認、443〜447行目）でも`(res.earnedMedals ?? 0) > 0`（一括承認は合計値`totalEarnedMedals`）を判定し、`medal`音とトーストを表示する。以前は承認経由のメダル獲得演出が一切反映されていなかった。
