...
- [game_logic.md](../../../MY_HOME_SYSTEM/game_logic.md) — `getNextLevelExp`と同一の経験値計算式（`100 × 1.2^(level-1)`の切り捨て）をPython側で実装した`GameLogic.calculate_next_level_exp`の実装元。game_logic.md側の解析で、この計算式が2言語間で重複実装されている旨が指摘されている。

本ファイルを実際にインポートしているfamily-quest内の他コンポーネント・フックは、解析対象の仕様書一覧の中では確認できなかった。
...
| 項目 | 理由 | 必要なファイル |
| --- | --- | --- |
| 時刻・曜日情報の使用目的 | 本ファイルは時刻・曜日を返すのみであり、それがUI表示用か、ゲーム内のイベント判定用かは読み取れない。（`family-quest/src`配下を`gameHelpers`および`DAYS`/`getDayIndex`/`getCurrentTime`の各識別子で検索したが、本ファイル自身を除きインポート・呼び出し箇所は1件も存在せず、解消不可。未使用コードの可能性がある） | 本関数をインポートしているファイル |
| `level` 引数の制約 | 引数の最大値や、レベル0以下が渡されるケースが存在するかどうかは呼び出し元に依存する。（`family-quest/src`配下を`getNextLevelExp`で検索したが、本ファイル自身を除き呼び出し箇所は1件も存在せず、解消不可。未使用コードの可能性がある） | `getNextLevelExp` を呼び出しているファイル |
