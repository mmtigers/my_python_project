...
| `earnedMedals`の付与条件 | `MY_HOME_SYSTEM/game_logic.py`と`MY_HOME_SYSTEM/services/quest_service.py`を直接確認した。`GameLogic.calculate_drop_rewards`(64〜79行目)は`medal_chance = 0.05`（5%固定、71行目）とし、`earned_medals = 1 if random.random() < medal_chance else 0`(72行目)で0か1を決定する。この結果は**（Issue #550で`services/quest/quest_service.py`へ移設された）**`_apply_quest_rewards`(522〜575行目)内で`rewards['medals']`(533行目)として取り出され、`quest_users`テーブルの`medal_count`列を`medal_count + ?`で加算するUPDATE文(543〜546行目)に使われたうえで、戻り値の`"earnedMedals": earned_medals`(573行目)としてAPIレスポンス(`CompleteResponse.earnedMedals`)に含まれることを確認した。あわせて**（Issue #409で追加）** メダルドロップ数は`quest_history.medals_earned`列(`migrations/0009_add_quest_history_medals_earned.sql`)にも記録される(555〜561行目)。 | 直接ソース確認: `MY_HOME_SYSTEM/game_logic.py:64-79`, `MY_HOME_SYSTEM/services/quest/quest_service.py:522-575` |
...
| `viewer_user_id`クエリパラメータのバックエンド側の扱い | `MY_HOME_SYSTEM/routers/quest_router.py`37〜39行目の`def get_all_data(viewer_user_id: Optional[str] = None)`が受け取り、`game_system.get_all_view_data(viewer_user_id)`へそのまま渡す(**省略可能で、既定は`None`**)。`get_all_view_data`(`services/quest/game_system.py:168-318`)での用途は**連続達成ボーナスの計算対象ユーザーの決定**に限られる: `quest_master.target_user`は実在の`user_id`だけでなく`'siblings'`のようなグループ指定も取りうるため、これをそのまま`user_id`として履歴照合に使うとボーナスが常に0固定になる。そこで`target_user`が実在ユーザーでない場合に、`viewer_user_id`(閲覧中のユーザー)の履歴を代表として使う。**（F-L6/#412で改善）** ただし`target_user='siblings'`(兄妹連携)については、`viewer_user_id`が兄妹のどちらでもない場合(親が閲覧中、または横画面4分割ビューで`users[0]`固定になるEcho Show等)に履歴が1件も無くボーナスが0固定になっていたため、現在は`role_child`のユーザーのうち最初の1人のidを使う(204〜212行目)。したがって`viewer_user_id`は**認可にも絞り込みにも使われず、返るデータの範囲は指定の有無で変わらない**(変わるのは一部クエストのボーナス表示値のみ)。 | 直接ソース確認: `MY_HOME_SYSTEM/routers/quest_router.py:37-39`, `MY_HOME_SYSTEM/services/quest/game_system.py:168,196-212`（参考: [quest_router.md](../../../MY_HOME_SYSTEM/quest_router.md)・[quest_game_system.md](../../../MY_HOME_SYSTEM/quest_game_system.md)） |
## 10. 自己検証結果
