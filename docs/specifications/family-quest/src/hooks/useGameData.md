...
* **（2026-09-06 品質監査で修正）** `chronicle` クエリに `refetchInterval: 1000 * 60`(60秒ポーリング)を追加した。この `useQuery` は `App` に常駐してアンマウントされないため、`staleTime`(5分)だけでは再取得の契機が一切無く(このデバイス自身の mutation による `invalidateQueries` 時のみ更新)、横画面キオスクのように他デバイスで完了・承認された記録が反映されなかった。**（Issue #803で追記）** `queryClient` の `refetchOnWindowFocus` は既定値の `true` に戻されたが、常に前面に出したままのキオスク端末ではフォーカス復帰イベントそのものが発生しないため、この60秒ポーリングは引き続き必要である。
* 根拠: (行番号: 127〜136 / 抜粋: "staleTime: 1000 * 60 * 5,", "refetchInterval: 1000 * 60,")
