...
| 解析対象 | 提供されたコードのみ |
| 推測・補完 | 一切なし |
| 解析基準コミット | `0d67384` (+同一ブランチ内で朝の準備の順不同チェックリスト化(`is_checklist`/`preview_bonus_gold`/`bonus_full_gold`フィールド追加)・**大人用フロー分離に伴うステップ個別報酬フィールド(`gold`/`exp`/`granted_gold`/`granted_exp`)の追加**を追加修正) |
...
* 根拠: `leveled_up`/`new_level`フィールドと`routineActiveFlowSchema` (行番号: 36〜38, 55 / 抜粋: "// チェックポイント通過ボーナスでレベルアップした「その1回のレスポンス」でのみtrue。\n    leveled_up: z.boolean(),\n    new_level: z.number().nullable(),", "export const routineActiveFlowSchema = startedFlowSchema;")
* 根拠: `is_checklist`/`preview_bonus_gold`/`bonus_full_gold`フィールド (行番号: 13〜14, 32〜35 / 抜粋: "// 順不同でチェック/チェック解除できる項目(例: 朝の準備5項目)かどうか。\n    is_checklist: z.boolean(),", "// チェックリストを1つチェックするたびに増える、出発ボーナスの見込みgold額。\n    // チェックポイント通過前はライブプレビュー、通過後はbonus_goldと同じ値になる。\n    preview_bonus_gold: z.number(),\n    bonus_full_gold: z.number(),")
* 根拠: `export const routineTodayResponseSchema` (行番号: 57〜63 / 抜粋: "export const routineTodayResponseSchema = z.object({\n    date: z.string(),\n    flows: z.object({\n        am: routineFlowSchema,\n        pm: routineFlowSchema,\n    }),\n});")
* 根拠: 2つの型ガード関数と`selectRoutineFlow`のエクスポート (行番号: 72〜110 / 抜粋: "export const isRoutineFlowBlocking = (flow: RoutineFlowState | undefined): flow is RoutineActiveFlow =>\n    !!flow && flow.started && !flow.is_complete && !flow.in_free_time;", "export const selectRoutineFlow = (flows: RoutineFlowsMap | undefined): SelectedRoutineFlow => {")
...
* **（大人用フロー分離で追加）** `gold: z.number()`/`exp: z.number()`が追加された。コメントによれば、バックエンドの大人用フロー(`routine_data.py`の`DAD_ROUTINE_FLOWS`/`MOM_ROUTINE_FLOWS`)で「生活動線そのものだったデイリークエストをすごろくへ寄せた分」のステップのみ非0で、子ども用フローのステップは常に0(報酬はチェックポイント通過ボーナスのみ)。いずれも必須フィールドとして定義されている(バックエンドが全ステップに対して常に返すため)。
* 根拠: [定数定義] (行番号: 8〜21 / 抜粋: "const routineStepSchema = z.object({\n    key: z.string(),\n    label: z.string(),\n    icon_key: z.string(),\n    is_checkpoint: z.boolean(),\n    // 順不同でチェック/チェック解除できる項目(例: 朝の準備5項目)かどうか。\n    is_checklist: z.boolean(),\n    status: z.enum(['locked', 'current', 'done', 'remind']),\n    // ステップ個別の即時報酬。大人用フロー(routine_data.py DAD/MOM_ROUTINE_FLOWS)で、\n    // 生活動線そのものだったデイリークエストをすごろくへ寄せた分の gold/exp が入る。\n    // 子ども用フローのステップは常に0(報酬はチェックポイント通過ボーナスのみ)。\n    gold: z.number(),\n    exp: z.number(),\n});")
...
* 根拠: [定数定義] (行番号: 57〜63 / 抜粋: "export const routineTodayResponseSchema = z.object({\n    date: z.string(),\n    flows: z.object({\n        am: routineFlowSchema,\n        pm: routineFlowSchema,\n    }),\n});")
...
* 根拠: [関数定義] (行番号: 75〜76 / 抜粋: "export const isRoutineFlowFreeTime = (flow: RoutineFlowState | undefined): flow is RoutineActiveFlow =>\n    !!flow && flow.started && flow.in_free_time;")
...
* 根拠: (行番号: 24, 47 / 抜粋: "started: z.literal(true),", "started: z.literal(false),")
...
* 根拠: (行番号: 32〜35 / 抜粋: "// チェックリストを1つチェックするたびに増える、出発ボーナスの見込みgold額。\n    // チェックポイント通過前はライブプレビュー、通過後はbonus_goldと同じ値になる。\n    preview_bonus_gold: z.number(),\n    bonus_full_gold: z.number(),")
...
* **（大人用フロー分離で新規追加）** `routineStepSchema`の`gold`/`exp`と`startedFlowSchema`の`granted_gold`/`granted_exp`は意味が異なる: 前者は「そのステップを完了すると(いくら)もらえるか」という定義上の額で毎レスポンスに含まれる表示用の値、後者は「このレスポンスで実際に付与された額」で、付与を行った`POST /api/routine/complete`の1回のみ非0になる`leveled_up`と同じ性質の一度きりの通知である。`useRoutineData.ts`はこの違いを前提に、後者が非0のときだけトースト表示と`gameData`キャッシュの無効化を行う(`useRoutineData.md`参照)。
* 根拠: (行番号: 16〜20, 39〜42 / 抜粋: "// ステップ個別の即時報酬。大人用フロー(routine_data.py DAD/MOM_ROUTINE_FLOWS)で、\n    // 生活動線そのものだったデイリークエストをすごろくへ寄せた分の gold/exp が入る。\n    // 子ども用フローのステップは常に0(報酬はチェックポイント通過ボーナスのみ)。\n    gold: z.number(),\n    exp: z.number(),", "// leveled_upと同様、ステップ個別報酬を付与した「その1回のレスポンス」でのみ非0。\n    // GET /today は付与を行わないため常に0になる。\n    granted_gold: z.number(),\n    granted_exp: z.number(),")
* **（大人用フロー分離で新規追加）** `routineTodayResponseSchema`の`flows`は`am`/`pm`の2キーを必須とする形のままであり、バックエンドがユーザーごとに異なるフロー定義(子ども用/大人用)を返すようになっても、フロントエンドのスキーマ側に変更は不要だった。逆に言えば、本スキーマは「どのユーザーのどのフローか」を区別せず、ステップの`key`や`label`の中身にも一切制約を置いていないため、ユーザーによってステップ構成が変わることはこの型定義からは読み取れない(9章参照)。
...
| ユーザーによってフローのステップ構成が異なること | `routineTodayResponseSchema`は`flows.am`/`flows.pm`の2キーと各ステップの形しか規定しておらず、同じ`am`/`pm`でもユーザー(子ども/保護者)によってステップの`key`・`label`・`gold`/`exp`が異なることは本ファイルからは読み取れない。 | `MY_HOME_SYSTEM/routine_data.py`、`MY_HOME_SYSTEM/services/routine_service.py` |
...
| --- | --- | --- |
| `GET /api/routine/today`のバックエンド実装との整合性 | バックエンドを直接確認したところ、**本スキーマは現行実装と完全に整合している**。`RoutineService.get_today_state`(`services/routine_service.py:463-481`)は`{"date": "YYYY-MM-DD", "flows": {...}}`を返し、`flows`の各要素は未開始なら`{"started": False, "title": ...}`の2キー、開始済みなら`_serialize_flow`(419〜461行目)が返す`started` / `title` / `checkpoint_time` / `current_step_index` / `in_free_time` / `is_complete` / `bonus_gold` / `bonus_exp` / `preview_bonus_gold` / `bonus_full_gold` / `leveled_up` / `new_level` / `steps` の**13キー**である。本ファイルの`startedFlowSchema`(18〜35行目)と`notStartedFlowSchema`(37〜40行目)はこれと1対1で対応し、`started: z.literal(true/false)`による判別共用体という形も、バックエンドが`started`で明確に分岐する実装と合っている。`steps`の各要素も`key`/`label`/`icon_key`/`is_checkpoint`/`is_checklist`/`status`の6キーで一致し、`status: z.enum(['locked','current','done','remind'])`もサービス側が`steps_status`へ入れる値の集合と一致する。`POST /api/routine/complete`は`_serialize_flow`の結果をそのまま返すため`routineActiveFlowSchema`(=`startedFlowSchema`)で受けられる。リクエスト側の制約は`models/routine.py`の`RoutineCompleteAction`(`user_id`/`step_key`は1〜64文字、`flow_key`は`Literal['am','pm']`)。 | 直接ソース確認: `MY_HOME_SYSTEM/services/routine_service.py:419-481`, `MY_HOME_SYSTEM/models/routine.py`（全体）, `MY_HOME_SYSTEM/routers/routine_router.py`（全体）（参考: [routine_service.md](../../../MY_HOME_SYSTEM/routine_service.md)・[routine.md](../../../MY_HOME_SYSTEM/routine.md)・[routine_router.md](../../../MY_HOME_SYSTEM/routine_router.md)） |
| `is_complete`と`in_free_time`が同時に`true`になりうるか | **現行実装では同時に`true`にはならない**。`is_complete`は`_serialize_flow`(419〜461行目)で`progress['current_step_index'] >= len(flow['steps'])`として算出される。フローが完了状態へ至る経路は2つあり、どちらでも`in_free_time`は`False`になる: (1) 締切超過による強制遷移`_apply_forced_transition`は、進めた先が末尾を超える(`end >= len(steps)`)場合に**明示的に`progress['in_free_time'] = False`を代入する**(406〜415行目)。(2) `complete_step`の逐次ステップ分岐(524〜530行目)は`next_index < len(flow['steps'])`のときだけ`in_free_time`を更新するため、末尾に到達した場合は直前の値を引き継ぐ — ただしこの分岐に入れるのは現在ステップがチェックリストでもチェックポイント(自由時間)でもない逐次ステップのときだけで(自由時間のステップは`HTTPException(400, "自由時間は時間になると自動的に次へ進みます")`で拒否される)、その逐次ステップへ進入した時点で`in_free_time`は`False`が入っているため、引き継がれる値も`False`になる。**ただしこれは不変条件としてアサートされているわけではなく**、(2)の「末尾では更新しない」という書き方に依存しているため、将来フロー末尾にチェックポイント付きステップを置くなどの変更を加える際は、この前提が崩れないか確認が必要である。 | 直接ソース確認: `MY_HOME_SYSTEM/services/routine_service.py:406-415,443,517-530` （参考: [routine_service.md](../../../MY_HOME_SYSTEM/routine_service.md)） |
| `icon_key`が取りうる値の全一覧 | `MY_HOME_SYSTEM/routine_data.py`を直接確認した。`ROUTINE_FLOWS`が使用する`icon_key`は**`meal` / `clothes` / `wash` / `teeth` / `toilet` / `free` / `handwash` / `snack` / `homework` / `tomorrow_prep` / `bath` / `sleep` の12種**(`am`フローが64〜78行目、`pm`フローが90〜107行目)。本スキーマが`icon_key: z.string()`と緩く定義しているのは、この一覧を列挙型で固定していないためで、描画側の`family-quest/src/features/routine/components/RoutineFlow.tsx`も`const ICONS: Record<string, LucideIcon>`(16〜33行目)に同じ12キーを持ち、`ICONS[step.icon_key] \|\| Star`(207・250行目)で**未知キーは`Star`へフォールバック**する。つまりバックエンドに新しい`icon_key`を追加してもフロントは壊れず、代わりに無言で汎用の星アイコンになる(追加時は`RoutineFlow.tsx`の`ICONS`への追記が必要だが、型でもCIでも検出されない点が保守上の注意点)。 | 直接ソース確認: `MY_HOME_SYSTEM/routine_data.py:64-107`, `family-quest/src/features/routine/components/RoutineFlow.tsx:16-33,207,250`（参考: [routine_data.md](../../../MY_HOME_SYSTEM/routine_data.md)・[RoutineFlow.md](../features/routine/components/RoutineFlow.md)） |
