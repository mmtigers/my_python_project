...
* **役割**: `gameData.users`配列の1要素（`quest_users`テーブルの行 + `get_all_view_data`が付与する`nextLevelExp`）を検証するZodスキーマ。`user_id`/`name`/`level`/`exp`/`gold`は必須、`nextLevelExp`は任意。**（Issue #390で修正）** `avatar`/`medal_count`/`job_class`/`role`は`quest_users`のNULL可カラム（`migrations/0000_baseline_schema.sql`、`0001_add_quest_users_role.sql`）であるため`.nullable().optional()`とし、`null`を受理する。以前は`.optional()`のみで`null`を拒否していたため、`quest_data.py`に`role`無しでメンバーを追加すると`"role": null`で届いてZod検証が失敗し、全端末が「サーバーに繋がりません」のフォールバック表示になっていた。**（Issue #470で追加）** `nextLevelExp: z.number().optional()`が新たに定義された。`get_all_view_data`は以前から`nextLevelExp`を実際に付与していたが、`.strict()`を使わない設計のためスキーマに定義されていないこのフィールドはこれまで`.parse()`後に無音でstripされていた（バックエンドが実際に返しているのにスキーマ側の定義漏れで検証対象から抜け落ちる、この設計の既知の穴の一例）。定義に加わったことで、`nextLevelExp`は`.parse()`後の戻り値オブジェクトにも保持され、消費側（`types/index.ts`の`User.nextLevelExp`）が参照できるようになった。**（Issue #327で削除）** 以前ここにあった`hp: z.number().optional()`/`maxHp: z.number().optional()`は削除された。バックエンドは`get_all_view_data`で`hp`/`maxHp`を引き続き送出するが、対応する表示UI(`UserStatusCard.tsx`)が既に存在せずフロント側では未使用であり、オーナー判断によりHP表示は廃止で確定したため。`.strict()`を使わない設計のため、これらのフィールドは今後も未知フィールドとして無視されるだけで`.parse()`が失敗することはない。
* 根拠: [定数定義] (行番号: 17〜39 / 抜粋: "// #390: quest_users の avatar / job_class / role は NULL 可のカラム\n// (migrations/0000_baseline_schema.sql, 0001_add_quest_users_role.sql)。", "const userSchema = z.object({\n    user_id: z.string(),\n    name: z.string(),\n    level: z.number(),\n    exp: z.number(),\n    avatar: z.string().nullable().optional(),\n    medal_count: z.number().nullable().optional(),\n    job_class: z.string().nullable().optional(),\n    gold: z.number(),\n    role: z.string().nullable().optional(),\n    // #327: hp/maxHpはバックエンドが送出し続けているが、対応する表示UI\n    // (UserStatusCard.tsx)が既に存在せずフロント側では未使用のため、\n    // オーナー判断(HP表示は廃止で確定)によりスキーマから除いた。.strict()を\n    // 使わないため、未知フィールドとして無視されるだけでparseエラーにはならない。\n    // #470: get_all_view_dataが実際に付与しているフィールドだが、.strict()を\n    // 使わないためこれまでスキーマに含まれておらず、parse後は無音で消えていた\n    // (バックエンドの新フィールド追加を検知できないこの設計の既知の穴の一例)。\n    nextLevelExp: z.number().optional(),\n});")
...
flowchart TD
    ...
    UsersField --> UserSchemaDef["userSchema:\nuser_id/name/level/exp/gold は必須\navatar/medal_count/job_class/role/nextLevelExp は任意\n(#470でnextLevelExpを追加、#327でhp/maxHpを削除)"]
...
* 根拠: (行番号: 34, 54, 76, 89 / 抜粋: "const userSchema = z.object({", "const questSchema = z.object({", "const rewardSchema = z.object({", "const questHistorySchema = z.object({")
...
| --- | --- | --- |
| バックエンドレスポンスとの整合性（`userSchema`の`nextLevelExp`欠落、Issue #470で解消済み） | `MY_HOME_SYSTEM/services/quest_service.py`の`GameSystem.get_all_view_data`を直接確認した。`users`の各要素に対し`u['nextLevelExp'] = game_logic.GameLogic.calculate_next_level_exp(u['level'])`、`u['maxHp'] = game_logic.GameLogic.calculate_max_hp(u['level'])`、`u['hp'] = u['maxHp']`の3フィールドが付与されることを確認した。以前は`maxHp`/`hp`のみ`userSchema`に定義があり`nextLevelExp`は定義に含まれていなかった（`.strict()`を使わない設計のため`.parse()`自体は成功していたが、`userSchema`はバックエンドが実際に返す全フィールドの完全な一覧ではなかった）が、Issue #470で`nextLevelExp: z.number().optional()`が追加され、この差異は解消された。**（Issue #327で追記）** その後`maxHp`/`hp`は、対応する表示UI(`UserStatusCard.tsx`)が存在せず未使用と判明したため`userSchema`から削除された。バックエンド側は変更されていないため、`get_all_view_data`は引き続き`hp`/`maxHp`を送出し続けるが、`.strict()`を使わない`userSchema`はこれらを未知フィールドとして無視する。 | 直接ソース確認: `MY_HOME_SYSTEM/services/quest_service.py`（`GameSystem.get_all_view_data`内、`nextLevelExp`/`maxHp`/`hp`付与箇所）、`family-quest/src/lib/gameDataSchema.ts:17-39` |
...
| `.parse()`失敗時の`useQuery`エラー状態の扱い | `family-quest/src/hooks/useGameData.ts`の`queryFn`(86〜96行目)が`gameDataResponseSchema.parse(raw)`を**try/catchせずに直接呼ぶ**ため、Zodの`ZodError`はそのまま`queryFn`の reject となり、React Queryの通常のエラー扱いになる。`src/lib/queryClient.ts`の`defaultOptions.queries.retry: 1`により**1回だけ再試行**され、それでも失敗すれば`isError`/`error`が立つ。**（Issue #390で修正）** 以前はフックが`isError`/`error`を捨てており、取得失敗やZod検証失敗が起きても「最後に成功したデータ」を表示し続けて無言だったが、現在は`error: gameDataError`として呼び出し元(`App.tsx`)へ返し、バナー表示させる(76〜82行目)。検証を`queryFn`内で行うのは意図的な設計で、形状不一致をコンポーネント側の`undefined`参照(幽霊フィールドバグ)にせず即座に検知させるため(#291のコメント)。 | 直接ソース確認: `family-quest/src/hooks/useGameData.ts:76-98`, `family-quest/src/lib/queryClient.ts:4-6`（参考: [useGameData.md](../hooks/useGameData.md)） |
| バックエンドレスポンスとの完全な整合性 | **（Issue #550で実体が`services/quest/game_system.py`へ移設）** `GameSystem.get_all_view_data`(168〜318行目)の戻り値は`users` / `quests` / `rewards` / `completedQuests` / `logs` / `pendingQuests` の**6キー**(314〜318行目)である。一方`gameDataResponseSchema`(89〜95行目)が定義しているのは`users`/`quests`/`rewards`/`completedQuests`/`pendingQuests`の**5キー**で、**`logs`はスキーマに含まれていない**。Zodの`z.object()`は既定で未知キーを検証せず取り除くため、`logs`は`.parse()`を通った時点で欠落し、型(`GameDataResponse`)にも現れない — 検証エラーにはならない。`logs`は`_fetch_recent_logs`(320〜344行目)が返す`id`/`text`/`dateStr`/`timestamp`の4キーを持つ最大20件の配列で、Streamlitダッシュボード(`views/dashboard/quest_tab.py`)側では使われているが、family-quest側は`getFamilyChronicle`(`/api/quest/chronicle`)を別に取得しているため本レスポンスの`logs`に依存していない。**したがって現状は「バックエンドが送っているがフロントが意図的に捨てている1キー」があるだけで、スキーマが要求するフィールドはすべてバックエンドが送出しており、整合している**。同様に`hp`/`maxHp`もバックエンドは送出し続けているが、Issue #327でHP表示が廃止されたためフロントの型・スキーマからは除かれている。 | 直接ソース確認: `family-quest/src/lib/gameDataSchema.ts:21-95`, `MY_HOME_SYSTEM/services/quest/game_system.py:168-192,251-257,314-344`（参考: [quest_game_system.md](../../../MY_HOME_SYSTEM/quest_game_system.md)） |
