...
- [InventoryList.md](../features/shop/components/InventoryList.md) — `InventoryItem.is_youtube_reward`/`InventoryItem.youtube_duration_minutes`/`InventoryResponse`の利用元。
...
* **（2026-09-23 要件追加）** `required?: boolean`が追加された。「毎日の必須クエスト」（常時表示）か「ボーナスクエスト」（折りたたみ表示）かの区分で、バックエンド(`ViewQuest.required`)は常に`true`/`false`のいずれかを返すが、`masterData.js`のフォールバック疑似クエストが`required`キー自体を持たないため型としてはオプショナルにしてある。欠けている場合は`QuestList.tsx`側の`q.required !== false`判定により必須側として扱われる。
* 根拠: [該当要素] (行番号: 44〜78 / 抜粋: "// #390: difficulty はバックエンドが送出しない幽霊フィールドだったため削除。\nexport interface Quest {")
* 根拠: `required?: boolean;` (行番号: 77)
...
* **役割**: インベントリアイテムのデータ構造の定義（`MY_HOME_SYSTEM/models/quest.py`の`InventoryItem`に対応）。**（Issue #390で修正）** `desc`はサーバー側が`Optional[str] = None`のため`string | null`を許容する任意フィールドに、また`used_at?: string | null`を追加した。**（YouTubeごほうび券クールダウン機能で追加）** `is_youtube_reward: boolean`フィールドを追加した。このアイテムがYouTubeの視聴制限(クールダウン・日次上限)の対象かどうかを表し、判定ロジック自体はバックエンド(`config.YOUTUBE_REWARD_IDS`)側に一本化されているため、フロントエンド(`InventoryList.tsx`)はこのフラグを見るだけでよい。**（日次上限機能で追加）** `youtube_duration_minutes?: number | null`を追加した。この券1枚で視聴できる分数で、YouTube系でない券では`null`。コメントによれば「今日の残り分数に収まらない券をタップ前に使えない表示にするために使う」。
* 根拠: [該当要素] (行番号: 111〜127 / 抜粋: "export interface InventoryItem {", "desc?: string | null;", "used_at?: string | null;", "is_youtube_reward: boolean;", "youtube_duration_minutes?: number | null;")
...
* **役割**: YouTubeの視聴制限の猶予期間中(バックエンドが実際の使用制限をまだ強制していない期間)に、`InventoryList.tsx`が「この日から変わるよ」という予告バナーを表示するための情報のデータ構造の定義。施行済み、または対象のごほうび券(`config.YOUTUBE_REWARD_IDS`)が設定されていない場合は該当フィールドが`null`になるため、この型が実際に使われるのは猶予期間中のみ。**（日次上限機能で変更）** クールダウンと日次上限がそれぞれ別の施行日を持つようになったため、この型は`youtube_cooldown_announcement`と`youtube_daily_limit_announcement`の2フィールドで共用される。
* 根拠: [該当要素] (行番号: 132〜135 / 抜粋: "export interface YoutubeCooldownAnnouncement {\n    starts_on: string; // ISO日付(YYYY-MM-DD)\n    days_remaining: number;\n}")
