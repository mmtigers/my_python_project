...
* **（eslint-plugin-react-hooks 7 への更新で変更）カメラごとの `onVideoRef` は ID の並びをキーにメモ化する**: `HlsPlayer` は `onVideoRef` を `useEffect` の依存配列に含めており、関数が変わるたびに HLS.js のセットアップをやり直す。以前は `videoRefSetters`（ref）に**描画中に書き込んで**遅延生成していたが、react-hooks 7 の `refs` ルールが描画中の ref への書き込みを禁じる。現在は `cameras.map(c => c.id).join('\n')` をキーに `useMemo` で設定関数の表を作る。`cameras` 配列そのものを依存にしないのは、親の再描画で配列が作り直されるたびに関数が変わってしまうため。`RecordView.test.tsx`（新規）が、配列を作り直して再描画しても同じ関数が渡り続けることと、カメラごとに別の関数であることを検査する
