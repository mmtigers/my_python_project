...
| `document.title`のデフォルト値`"Family Quest"`が他画面と一致しているか | `family-quest/index.html`12行目が`<title>Family Quest</title>`であり、**一致している**ことを確認した。`document.title`を書き換えているのはリポジトリ全体で本ファイルだけ(`CameraDashboard.tsx:43-44`)で、マウント時に`"ホーム監視カメラ"`へ変更し、クリーンアップ関数で`"Family Quest"`へ戻す。したがって復帰値はHTMLの初期タイトルと同一で、カメラ画面から離れると元のタイトルに戻る。ただし**両者は別ファイルのリテラルとして二重管理**されており、`index.html`のタイトルを変えるとここだけ古い値に戻す不整合が起きうる(定数の共有はされていない)。 | 直接ソース確認: `family-quest/index.html:12`, `family-quest/src/features/camera/components/CameraDashboard.tsx:43-44` |
| `CameraSettingsModal`が有効/無効切り替えをどのAPIエンドポイントで永続化しているか | `PUT /api/cameras/settings/{camera_id}` にボディ`{"enabled": bool}`を送る。バックエンドは`MY_HOME_SYSTEM/routers/camera_router.py`58〜64行目の`update_camera_settings`で、`camera_service.set_camera_enabled(camera_id, payload.enabled)`が`False`(=該当カメラなし)を返した場合のみ404、成功時は`{"id": camera_id, "enabled": ...}`を返す。永続化の実体は`services/camera_service.py`494〜521行目で、`devices.json`を読み込んで該当カメラの`enabled`を書き換え、**`.tmp`へ書き出してから`os.replace`でアトミックに差し替える**(直接上書きだと書き込み途中のクラッシュ・電源断で全カメラ設定を失うため)。あわせてメモリ上の`config.CAMERAS`にも反映するので、次回の`GET /api/cameras/settings`に即座に効く。 | 直接ソース確認: `MY_HOME_SYSTEM/routers/camera_router.py:14-15,44-64`, `MY_HOME_SYSTEM/services/camera_service.py:494-521`（参考: [CameraSettingsModal.md](./CameraSettingsModal.md)・[camera_router.md](../../../../../MY_HOME_SYSTEM/camera_router.md)） |
