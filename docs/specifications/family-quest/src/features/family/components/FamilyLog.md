...
* **（Issue #660 で変更）** ユーザー切替スワイプの閾値は `src/lib/uiConstants.ts` の `VIEW_SWIPE_THRESHOLD_PX`(60px)を参照する(`App.tsx` と共通)。
...
| `users`が空配列かつ`initialUserId`未指定の場合の`selectedUserId` | `useState`の初期化子は`() => initialUserId ?? users[0]?.user_id`(106〜107行目)なので、`users`が空かつ`initialUserId`未指定なら**`undefined`になる**(オプショナルチェーンのため例外にはならない)。その場合`selectedIndex`は`users.findIndex(...)`で`-1`、ユーザー切り替えタブは`users.map`が空配列を返すため何も描画されない。実際の呼び出し元は`family-quest/src/App.tsx`620行目の`<FamilyLog chronicle={chronicle} users={users} initialUserId={currentUser.user_id} />`**1箇所のみ**で、**常に`initialUserId`が渡される**ため、この経路では`undefined`にはならない(`currentUser`は`users`から解決されるため、`users`が空ならそもそもこの画面に到達しない)。したがって当該分岐は現状のアプリ内では到達せず、コンポーネントを他所から再利用した場合にのみ問題になりうる。 | 直接ソース確認: `family-quest/src/features/family/components/FamilyLog.tsx:13,105-112`, `family-quest/src/App.tsx:620` |
## 10. 自己検証結果
