...
| --- | --- | --- |
| `Quest` オブジェクトの完全なスキーマ | `family-quest/src/types/index.ts`を直接確認した。`Quest`インターフェース(29〜59行目)は`id?: ID`(30行目)と`quest_id?: ID`(31行目)、`type?: string`(45行目)と`quest_type?: 'daily' \| 'weekly' \| 'infinite' \| 'challenge' \| string`(44行目)をいずれもオプショナルかつ併存する形で定義しており、本ファイルが`quest.quest_id || quest.id`（36行目）や`quest.type === 'infinite' || quest.quest_type === 'infinite'`（39行目）とフォールバックしている実態と一致する。ただし、なぜ2つの類似フィールドが併存するのか（バックエンドAPIの移行過程等）を示すコメントは`types/index.ts`自体には存在せず、この点は特定できなかった。また共有クエスト判定用の`is_shared_completed_by`等(54〜58行目)には「バックエンドの`get_available_quests`が付与するフィールド」というコメントがある。 | 直接ソース確認: `family-quest/src/types/index.ts:29-59` |
