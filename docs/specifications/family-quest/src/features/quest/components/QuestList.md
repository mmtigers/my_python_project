...
* 根拠: `export default function QuestList` (行番号: 417 / 抜粋: "export default function QuestList({ quests, completedQuests, pendingQuests, currentUser, onQuestClick, completedSignal, processingQuestKeys, panelMode, iconFirst }: QuestListProps) {")
...
* 根拠: `isQuestVisibleToUser`によるターゲット判定への集約 (行番号: 7, 422 / 抜粋: "import { isQuestVisibleToUser } from '@/lib/questTargeting';", "if (!isQuestVisibleToUser(q, currentUser)) return false;")
...
* 根拠: `QuestItem`への`completedSignal`転送 (行番号: 524 / 抜粋: "completedSignal={completedSignal}")
...
* 根拠: (行番号: 112 / 抜粋: "const questId = quest.quest_id;") と (行番号: 461〜462 / 抜粋: "const idA = Number(a.quest_id ?? 0);\n            const idB = Number(b.quest_id ?? 0);")
...
* 根拠: (行番号: 484 / 抜粋: "if (isPending) continue; // 申請中は常にカード表示のままにする(件数上限の対象外)")
