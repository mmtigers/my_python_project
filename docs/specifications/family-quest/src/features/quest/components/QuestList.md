...
* 根拠: `export default function QuestList` (行番号: 412 / 抜粋: "export default function QuestList({ quests, completedQuests, pendingQuests, currentUser, onQuestClick, completedSignal, processingQuestKeys, panelMode, iconFirst }: QuestListProps) {")
...
* 根拠: `isQuestVisibleToUser`によるターゲット判定への集約 (行番号: 7, 417 / 抜粋: "import { isQuestVisibleToUser } from '@/lib/questTargeting';", "if (!isQuestVisibleToUser(q, currentUser)) return false;")
...
* 根拠: `renderQuestCards`での`isLast`/`forceSlim`算出 (行番号: 521〜522 / 抜粋: "isLast={index === list.length - 1}\n                        forceSlim={!showAllActionable && forceSlimIds.has(q.quest_id ?? -1)}")
...
* **（実行可能クエスト過多への対応で追加）** `activeCount`を算出する同じ`useMemo`が、`forceSlimIds`（強制的に縮小1行にする`quest_id`の`Set`）・`overflowCount`（その件数）・`cutoffQuestId`（上限ちょうどに達したクエストの`quest_id`、「もっと見る」の差し込み位置）も合わせて算出するようになった。ループは`sortedQuests`を1回だけ走査し、ロック中・完了済みは`continue`、申請中は`activeCount`にだけ加算して上限カウント(`countedForLimit`)には加算せず、それ以外を`countedForLimit`としてインクリメントし`ACTIONABLE_CARD_LIMIT`（3）を超えた分の`quest_id`を`forceSlim`に追加する。`showAllActionable`という新しい`useState`（既定`false`）が真になると、`renderQuestCards`は`forceSlim`propを渡さなくなり（`!showAllActionable && forceSlimIds.has(...)`）、全件がカード表示に戻る。折りたたみ直しの操作（`showAllActionable`を`false`に戻すUI）は用意されていない。`renderQuestCards`自体も、以前の`list.map`から`Array.prototype.forEach`でノード配列を組み立てる方式に変わり、`cutoffQuestId`に一致するクエストを描画した直後（かつ`overflowCount > 0`かつ`!showAllActionable`のとき）に、ノード列（`⋯`アイコン）とコンテンツ列（「もっと見る (N件)」ボタン）を持つ疑似アイテムを`key="more-toggle"`で配列に追加する。これにより、隠れたクエストがあってもレールの接続線自体は途切れない（ノードを持たない要素を挟まない）設計を保っている。
* 根拠: `export default function QuestList` (行番号: 412 / 抜粋: "export default function QuestList({ quests, completedQuests, pendingQuests, currentUser, onQuestClick, completedSignal, processingQuestKeys, panelMode, iconFirst }: QuestListProps) {")
* 根拠: `activeCount`/`forceSlimIds`/`overflowCount`/`cutoffQuestId`の統合`useMemo` (行番号: 470〜488 / 抜粋: "const { activeCount, forceSlimIds, overflowCount, cutoffQuestId } = useMemo(() => {\n        let activeCount = 0;\n        let countedForLimit = 0;\n        const forceSlim = new Set<number>();\n        let cutoffId: number | undefined;\n        for (const q of sortedQuests) {\n            const { isLocked, isDone, isPending } = getQuestLockState(q, currentUser, completedQuests, pendingQuests);\n            if (isLocked || isDone) continue;\n            activeCount++;\n            if (isPending) continue;")
* 根拠: `showAllActionable`ステート (行番号: 490〜492 / 抜粋: "// 一度「もっと見る」を開いたら、その画面を見ている間は展開したままにする\n    // (折りたたみ直しのボタンは持たない。UI側の複雑さを避けるための単純化)。\n    const [showAllActionable, setShowAllActionable] = useState(false);")
* 根拠: `listContainerClass`/`headerClass`（単一カラム化） (行番号: 494〜499 / 抜粋: "const listContainerClass = panelMode\n        ? 'flex flex-col animate-in fade-in duration-300'\n        : 'flex flex-col animate-in fade-in slide-in-from-bottom-2 duration-300 pb-20';")
* 根拠: `renderQuestCards`（`forEach`によるノード配列組み立てと「もっと見る」の差し込み） (行番号: 501〜555 / 抜粋: "const renderQuestCards = (list: Quest[]) => {\n        const nodes: React.ReactNode[] = [];\n        list.forEach((q, index) => {")
* 根拠: `QuestItem`への`completedSignal`転送 (行番号: 519 / 抜粋: "completedSignal={completedSignal}")
...
* 根拠: インターフェース定義および引数 (行番号: 11〜30, 412 / 抜粋: "interface QuestListProps {")

* **戻り値/レスポンス**: ReactElement（JSX）
* 根拠: `return` 文 (行番号: 557〜579 / 抜粋: "return (\n        <div className={listContainerClass}>")

* **副作用**: なし（`useMemo`による`sortedQuests`/`activeCount`等のメモ化と、`useState`による`showAllActionable`（「もっと見る」展開状態）の管理のみで、外部API呼び出しやDOM直接操作は存在しない）
* 根拠: `useMemo` ブロック (行番号: 413〜460, 470〜488 / 抜粋: "const sortedQuests = useMemo(() => {", "const { activeCount, forceSlimIds, overflowCount, cutoffQuestId } = useMemo(() => {")
...
flowchart TD
    ...
    subgraph "QuestItem のコンテンツ列分岐 (すごろく風UI化 + 実行可能クエスト過多への対応)"
        ...
        I_Check -- No --> I_Slim["縮小1行のミュート表示(完了済み=取消ヒント、未開放=鍵ノード+「未開放」、forceSlim=通常の縮小1行と同じ見た目)"]
    end
    MapItem --> I_Check
...
* 根拠: (行番号: 112 / 抜粋: "const questId = quest.quest_id;") と (行番号: 456〜457 / 抜粋: "const idA = Number(a.quest_id ?? 0);\n            const idB = Number(b.quest_id ?? 0);")
...
* 根拠: (行番号: 479 / 抜粋: "if (isPending) continue; // 申請中は常にカード表示のままにする(件数上限の対象外)")
