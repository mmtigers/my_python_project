...
| Propsとして渡される `onApprove`, `onReject` の具体的な処理 | `family-quest/src/App.tsx`と`family-quest/src/features/family/components/FamilyDashboard.tsx`を直接確認した。縦画面(`App.tsx`422〜429行目)では`onApprove={handleApprove}`, `onReject={handleReject}`が直接渡される。`handleApprove(history)`(299〜311行目)は`await approveQuest(getRepresentativeParent(users), history)`を即座に実行し、成功時は`play('approve')`、失敗時はリトライ付きのエラーモーダルを表示する。`handleReject(history)`(338〜344行目)はAPIを呼ばず、`setConfirmTarget(history); setConfirmMode('reject'); setConfirmUser(null); setRejectReason(null); play('select');`により却下理由選択の確認モーダルを開くのみで、実際の却下API呼び出しはモーダルの確認後、`confirmMode === 'reject'`分岐(277行目)の`await rejectQuest(getRepresentativeParent(users), confirmTarget as QuestHistory, rejectReason || undefined)`で行われる。横画面では`FamilyDashboard.tsx`(81〜89行目)が`onApprove={onApprove}`, `onReject={onReject}`としてPropsをそのまま`ApprovalList`へ中継しており、`FamilyDashboard`自体は`App.tsx`から渡された同名の`handleApprove`/`handleReject`をProps経由で受け取っているだけで、実体は縦画面と同一の`App.tsx`側の関数であることを確認した。 | 直接ソース確認: `family-quest/src/App.tsx:299-344, 422-429`, `family-quest/src/features/family/components/FamilyDashboard.tsx:81-89` |
