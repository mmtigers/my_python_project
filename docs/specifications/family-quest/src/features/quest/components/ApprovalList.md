...
| `CheckCircle`, `XCircle`, `ChevronDown`, `ChevronUp`, `CheckCheck` | ライブラリ (`lucide-react`) | UI上のアイコン表示（承認、却下、折りたたみ開閉、一括承認） | 根拠: [import文] (行番号: 3 / 抜粋: "import { CheckCircle, XCircle, ChevronDown, ChevronUp, CheckCheck } from 'lucide-react';") |
...
| `Button` | コンポーネント (`../../../components/ui/Button`) | 承認・却下・一括承認ボタンのUI構築 | 根拠: [import文] (行番号: 6 / 抜粋: "import { Button } from '../../../components/ui/Button';") |
...
| `Button` | デザインや振る舞い（`variant`, `size`などのPropsの処理）の実装詳細が不明なため。 | 根拠: [import文] (行番号: 6 / 抜粋: "import { Button } from '../../../components/ui/Button';") |
...
* 根拠: (行番号: 43 / 抜粋: "const draggable = !!(onSwipeApprove || onSwipeReject);")
...
* 根拠: (行番号: 64 / 抜粋: "return users.find(u => u.user_id === userId)?.name || userId;")
...
* 根拠: (行番号: 117 / 抜粋: "担当: {getUserName(quest.user_id)} / 報酬: {quest.gold_earned ?? 0}G")
...
* 根拠: (行番号: 67 / 抜粋: "if (pendingQuests.length === 0) return null;")
...
| 低 | `family-quest/src/features/shop/components/InventoryList.tsx` | 「8. 保守上の注意点」で触れる、かつて本コンポーネントに存在したと見られるアイテム使用承認UIが、廃止後の現行の即時使用フローでどのように実装されているかを確認するため。 | 根拠: [コメント] (行番号: 97 / 抜粋: "{/* --- クエスト承認リスト (既存) --- */}") |
...
* 根拠: (行番号: 24〜25 / 抜粋: "// スワイプで承認/却下できる行ラッパー。右スワイプ=承認、左スワイプ=却下。\n// ボタンは廃止せず併存させ、スワイプに気づかない人でも従来通り操作できるようにする。")
...
| --- | --- | --- |
| Propsとして渡される `onApprove`, `onReject`, `onApproveAll` の具体的な処理 | 唯一の実装元は`family-quest/src/App.tsx`で、`handleApprove`(319行目〜)・`handleReject`・`handleApproveAll`(360行目〜)が`FamilyDashboard`経由(521〜523行目)と直接(553〜555行目)の2経路で渡される。`handleApprove(history)`は**多重送信ガードが中心**で、`approvingHistoryIdsRef`に`history.id`が既に入っていれば静かに無視し(#119: 2回目のタップ・スワイプを1回分として扱いエラー表示を出さない)、そうでなければidを登録してから`approveQuest(getRepresentativeParent(users), history)`を呼ぶ。成功時は`play('approve')`で効果音を鳴らし、レスポンスの`earnedMedals`を見てクエスト完了フローと同じ演出を出す(M-6-1)。`handleApproveAll()`は`isApprovingAllRef`で連打を防いだうえで、**送信中の履歴を対象から除外**し、残り全idを先に`approvingHistoryIdsRef`へ入れてから順に承認する(#391/F-L8: 一括承認中の個別タップは`handleApprove`側で無視され、以前のように競合して400のエラーモーダルにならない)。承認者は個別に選択するのではなく`getRepresentativeParent(users)`で代表の親を解決する。 | 直接ソース確認: `family-quest/src/App.tsx:319-333,360-372,521-523,553-555`, `family-quest/src/features/family/components/FamilyDashboard.tsx:27-28`（参考: [App.md](../../../../App.md)） |
| `pendingQuests`, `users` の取得元・ポーリング間隔 | どちらも`family-quest/src/hooks/useGameData.ts`の`useQuery<GameDataResponse>`(84行目〜)が返す`GET /api/quest/data`のレスポンス由来で、`App.tsx`が`ApprovalList`へ流し込む。**ポーリング間隔は`refetchInterval: 1000 * 10`＝10秒**、`staleTime`は30秒(97〜98行目)。リトライは`src/lib/queryClient.ts`の`retry: 1`により1回のみ。バックエンド側では`GameSystem.get_all_view_data`が`pendingQuests`を`SELECT * FROM quest_history WHERE status='pending' ORDER BY completed_at DESC`で、`users`を`quest_users`全行(+`nextLevelExp`/`maxHp`/`hp`を付与し`quest_data.USERS`の宣言順へ再ソート)として返す。取得値は`gameDataResponseSchema.parse`(95行目)でZod検証を通るため、形状不一致は即エラーになる(#291)。 | 直接ソース確認: `family-quest/src/hooks/useGameData.ts:84-98`, `family-quest/src/lib/queryClient.ts:4-6`, `MY_HOME_SYSTEM/services/quest/game_system.py:168-192,271-273`（参考: [useGameData.md](../../../hooks/useGameData.md)・[quest_game_system.md](../../../../../MY_HOME_SYSTEM/quest_game_system.md)） |
| かつて存在したと見られるアイテム使用承認UIの現行の実装場所 | **現在このUIは存在せず、承認フロー自体が廃止されている**ことを確認した。`family-quest/src/features/shop/components/InventoryList.tsx`97行目のコメントが「アイテム使用は即座に消費が確定する(親の承認は不要)」と明記しており、同ファイルにも`pendingInventory`相当の承認待ちリストや承認ボタンは存在しない。バックエンド側も`InventoryService.use_item`(`services/quest/inventory_service.py:64-82`)がその場で`user_inventory.status`を`consumed`へ更新する実装で、承認待ち状態を経由しない。したがって本ファイル(`ApprovalList`)が扱うのはクエストの承認待ち(`quest_history.status='pending'`)のみである。 | 直接ソース確認: `family-quest/src/features/shop/components/InventoryList.tsx:97`, `MY_HOME_SYSTEM/services/quest/inventory_service.py:64-82`（参考: [InventoryList.md](../../shop/components/InventoryList.md)・[quest_inventory_service.md](../../../../../MY_HOME_SYSTEM/quest_inventory_service.md)） |
