...
* **（大人用フロー分離で新規追加）** `RoutineStepRewardChip`は`step.gold`または`step.exp`のいずれかが非0なら描画されるが、チップに表示するのは`step.gold`のみで`step.exp`は画面に出ない。したがって「goldは0だがexpのみ非0」というステップをバックエンドが定義した場合、チップは`0`という数字だけを表示することになる(現状の`routine_data.py`の大人用フローには、両方とも非0のステップしか存在しない)。
* 根拠: (行番号: 250〜255 / 抜粋: "    if (!step.gold && !step.exp) return null;\n    return (\n        <span className={`flex items-center gap-0.5 rounded-full border px-2 py-0.5 text-[10px] font-bold whitespace-nowrap flex-none ${THEME[flowKey].chip}`}>\n            <Coins size={10} />{step.gold}\n        </span>")
* **（大人用フロー分離で新規追加）** ステップ個別報酬のチップは`RoutineStepRow`(一本道のステップ)からのみ描画され、`RoutineChecklistBlock`/`RoutineChecklistItem`(チェックリスト項目)には追加されていない。これは現状の大人用フローでチェックリスト項目(朝の準備・寝る準備)が`gold`/`exp`を持たないためであり、将来チェックリスト項目にも個別報酬を持たせる場合は`RoutineChecklistItem`側にもチップの描画を追加する必要がある。
