...
* 根拠: (行番号: 250〜255 / 抜粋: "    if (!step.gold && !step.exp) return null;\n    return (\n        <span className={`flex items-center gap-0.5 rounded-full border px-2 py-0.5 text-[10px] font-bold whitespace-nowrap flex-none ${THEME[flowKey].chip}`}>\n            <Coins size={10} />{step.gold}\n        </span>")
...
* 根拠: (行番号: 300〜320 / 抜粋: "                            <RoutineStepRewardChip flowKey={flowKey} step={step} />", "                        <RoutineStepRewardChip flowKey={flowKey} step={step} />")
...
* 根拠: (行番号: 60〜66 / 抜粋: "interface RoutineFlowProps {\n    flowKey: 'am' | 'pm';\n    flow: RoutineFlowState;\n    onCompleteStep: (stepKey: string) => void;\n    isCompleting?: boolean;\n    compact?: boolean;\n}")
