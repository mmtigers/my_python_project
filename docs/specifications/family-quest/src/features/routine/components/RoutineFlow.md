...
| 解析対象 | 提供されたコードのみ |
| 推測・補完 | 一切なし |
| 解析基準コミット | `3ac46ca` (+同一ブランチ内で`THEME[flowKey].chip`の冗長参照をコードレビューで修正、朝の準備の順不同チェックリスト化(`RoutineChecklistBlock`/`RoutineChecklistItem`を新規追加)、夜の切り替え時刻変更/寝る準備チェックリスト化/明日の準備追加(チェックリストブロックの描画位置をamの先頭固定からflow.steps上の実際の位置に沿って挿入する一般化、pm向けにチェックポイント後チェックリストではボーナスチップを出さない分岐を追加)・**大人用フロー分離(大人用ステップのアイコン3種追加、ステップ個別報酬チップ`RoutineStepRewardChip`の新規追加)**を追加修正) |
...
| `Droplet`, `UtensilsCrossed`, `Shirt`, `Sparkles`, `Star`, `Waves`, `Cookie`, `Pencil`, `BedDouble`, `Bath`, `ShowerHead`, `Backpack`, `CookingPot`, `Briefcase`, `Sofa` | 外部ライブラリ(`lucide-react`) | `ICONS`テーブルの各`icon_key`に対応するアイコンコンポーネント（`Star`は未知の`icon_key`に対するフォールバックにも使われる）。**（朝の準備チェックリスト化で追加）**`Bath`は`toilet`キー用。**（夜の切り替え/寝る準備チェックリスト化で変更）**単独ステップ`nightprep`が廃止されたため`Moon`の利用箇所が無くなりimportから削除され、代わりに新設の`tomorrow_prep`キー用`Backpack`、`bath`キー用`ShowerHead`(`toilet`が既に`Bath`を使っているため、実際の入浴と見分けが付くよう別アイコンにした)が追加された。**（画面非表示化で変更）**`am`フローの末尾ステップ`leave`が廃止され`ICONS`テーブルから対応エントリ`leave: DoorOpen`が削除されたことに伴い、他の場所で使われなくなった`DoorOpen`もimportから削除された。**（大人用フロー分離で追加、パパのpm再構成で`Sofa`を追加）**大人用フローのタスクステップ用に`CookingPot`(`kitchen`キー)・`Briefcase`(`work`キー)・`Sofa`(`living`キー)が追加された。 | 根拠: [インポート宣言] (行番号: 10〜14 / 抜粋: "import {\n    Droplet, UtensilsCrossed, Shirt, Sparkles, Star,\n    Waves, Cookie, Pencil, BedDouble, Bath, ShowerHead, Backpack,\n    CookingPot, Briefcase, Sofa,\n} from 'lucide-react';") |
| `RoutineFlowState`, `RoutineStep` | 外部モジュール(型定義) | `RoutineFlowState`は`RoutineFlow`/`RoutineFreeTimeBanner`propsの`flow`の型、`RoutineStep`は`RoutineStepRow`/**（朝の準備チェックリスト化で追加）**`RoutineChecklistBlock`/`RoutineChecklistItem`/**（大人用フロー分離で追加）**`RoutineStepRewardChip`の`step`(または`steps`)propの型 | 根拠: [インポート宣言] (行番号: 15 / 抜粋: "import { RoutineFlowState, RoutineStep } from '@/lib/routineDataSchema';") |
...
| `RoutineFlowState`/`RoutineStep`の詳細なフィールド定義 | `@/lib/routineDataSchema.ts`に実装があり、本ファイルからは各フィールドの参照箇所のみが分かる（詳細は`routineDataSchema.md`を参照）。 | 根拠: [インポート宣言] (行番号: 15 / 抜粋: "import { RoutineFlowState, RoutineStep } from '@/lib/routineDataSchema';") |
...
* **役割**: `RoutineFlow`コンポーネントが受け取るPropsのインターフェース。`flowKey`（`'am' | 'pm'`、テーマ色の切り替えに使用）、`flow`（`RoutineFlowState`、当該フローの現在の状態）、`onCompleteStep`（現在のステップの完了報告コールバック。`RoutineChecklistBlock`/`RoutineChecklistItem`のトグル操作もこの同じコールバックを経由する）、`isCompleting`（送信中フラグ、任意）、`compact`（横画面パネル向けの縮小表示フラグ、任意）を持つ。**（Issue #718 で追加）** さらに `isAdult`（大人向け表示フラグ、任意）を持つ。本型定義自体は夜の切り替え/寝る準備チェックリスト化による変更を受けていない。

  `isAdult` は「大人(`role_adult`)には『まだだよ』を出さない」ための**表示専用**のフラグである。ソース上のコメントは、サーバー側の状態を `'remind'` のままにする必要がある理由として、追いつき完了の可否がフロント側のボタン表示条件とサーバー側 `routine_service.complete_step` の `is_catch_up` 判定の**両方が `status === 'remind'` を見ている**ことを挙げている。
  * 根拠: [プロパティ定義] (行番号: 85 / 抜粋: "    isAdult?: boolean;")
* 根拠: `interface RoutineFlowProps {` (行番号: 66)
...
* 根拠: (行番号: 190〜199 / 抜粋: "{steps.map((step) => (\n                    <RoutineChecklistItem\n                        key={step.key}\n                        flowKey={flowKey}\n                        step={step}\n                        onToggle={() => onToggleStep(step.key)}\n                        isCompleting={isCompleting}\n                        compact={compact}\n                    />\n                ))}")

* **エラーハンドリング**: なし

### `RoutineChecklistItem` (内部コンポーネント、非export、朝の準備チェックリスト化で新規追加)

* **役割**: `RoutineChecklistBlock`から1項目ぶんマップされる、タップでチェック/チェック解除できる行。行全体が`<button>`であり、`step.status`に関わらず常にタップ可能（`RoutineStepRow`の「`current`のときだけボタンが出る」設計とは異なり、チェックリスト項目は`'current'`(未チェック)/`'done'`(チェック済み)のどちらでも同じ`onToggle`ハンドラを呼ぶ）。左側の丸いバッジに`step.status === 'done'`ならチェックマーク、`'remind'`ならベルアイコン、それ以外は`ICONS[step.icon_key] || Star`のアイコンを表示し、行の背景色・バッジ色・ラベル色を`checked`/`remind`の状態に応じてテーマカラー(`theme.node`/`theme.nodeDone`)またはオレンジ系(リマインド)に変える。`remind`状態のときのみ行末に「まだだよ」バッジを追加で表示する。**（Issue #718 で変更）** この`remind`は`step.status === 'remind'`そのものではなく`step.status === 'remind' && !isAdult`であり、`isAdult`が真のときはベルアイコン・オレンジ系の配色・「まだだよ」バッジのいずれも出さず、未完了の通常ステップと同じ見た目になる（`step.status`自体は`'remind'`のまま扱う）。`compact`propに応じてバッジサイズ・アイコンサイズ・フォントサイズを縮小する。本コンポーネント自体は夜の切り替え/寝る準備チェックリスト化による変更を受けていない。
* 根拠: `const RoutineChecklistItem: React.FC<{` (行番号: 225)
* 根拠: [`remind`の算出] (行番号: 239 / 抜粋: "    const remind = step.status === 'remind' && !isAdult;")
* 根拠: ボタン本体とトグル呼び出し (行番号: 226〜232 / 抜粋: "return (\n        <button\n            type=\"button\"\n            onClick={onToggle}\n            disabled={isCompleting}\n            className={`flex items-center gap-3 w-full rounded-xl border px-3 py-2.5 text-left transition-colors disabled:opacity-50 ${rowClass}`}\n        >")
...
* 根拠: (行番号: 227〜232 / 抜粋: "<button\n            type=\"button\"\n            onClick={onToggle}\n            disabled={isCompleting}\n            className={`flex items-center gap-3 w-full rounded-xl border px-3 py-2.5 text-left transition-colors disabled:opacity-50 ${rowClass}`}\n        >")

* **エラーハンドリング**: なし（`onToggle`の呼び出し結果自体はこのコンポーネントの関知するところではない。`isCompleting`が真の間はボタンを`disabled`にするのみ）

### `RoutineStepRewardChip` (内部コンポーネント、非export、大人用フロー分離で新規追加)

* **役割**: ステップ個別の即時報酬(`step.gold`/`step.exp`)を`Coins`アイコン付きの小さなチップとして描画する内部コンポーネント。`step.gold`・`step.exp`がともに偽値(0)なら`null`を返して何も描画しない — コメントによれば、子ども用フローのステップは両方とも0のためこの経路になる。チップに表示するのは`step.gold`のみで、`step.exp`は表示条件の判定にのみ使われる。配色は`THEME[flowKey].chip`(チェックポイント時刻チップ・出発ボーナスチップと共通)を使う。
* 根拠: [コンポーネント定義] (行番号: 246〜256 / 抜粋: "// ステップ個別の即時報酬(大人用フローに寄せたデイリークエスト相当)の表示。\n// 子ども用フローのステップは gold/exp とも0のため、何も描画しない。\nconst RoutineStepRewardChip: React.FC<{ flowKey: 'am' | 'pm'; step: RoutineStep }> = ({ flowKey, step }) => {\n    if (!step.gold && !step.exp) return null;\n    return (\n        <span className={`flex items-center gap-0.5 rounded-full border px-2 py-0.5 text-[10px] font-bold whitespace-nowrap flex-none ${THEME[flowKey].chip}`}>\n            <Coins size={10} />{step.gold}\n        </span>\n    );\n};")

* **引数/リクエスト**: `{ flowKey: 'am' | 'pm'; step: RoutineStep }`
* 根拠: (行番号: 249)

* **戻り値/レスポンス**: 報酬を持つステップならJSX要素(チップ)、持たないステップなら`null`
* 根拠: (行番号: 249〜255 / 抜粋: "    if (!step.gold && !step.exp) return null;")

* **副作用**: なし(表示専用でイベントハンドラを持たない)
* 根拠: (行番号: 249〜257)
...
* **役割**: `RoutineFlow`の誘導中描画から`is_checklist`が偽なステップについてマップされる、1ステップぶんの行を描画する内部コンポーネント。左側に円形のステップノード（`step.status`に応じて`done`＝チェックマーク、`remind`＝ベルアイコン、それ以外＝`ICONS[step.icon_key] || Star`のアイコン）と、その下に接続線・チェックポイントチップを描画する。**（画面非表示化で変更）** 以前は最終ステップ（`isLast`）なら接続線・チェックポイントチップの描画ブロック自体を`{!isLast && (...)}`でスキップしていたため、`step.is_checkpoint`が真かつ`isLast`も真という組み合わせ(現に`am`フローの`free`で発生するようになった)ではチェックポイント時刻チップが一切表示されなくなってしまっていた。チェックポイントの時刻チップは「次のステップへの接続線」ではなく「この時刻に自動で進む/完了する」という告知であり最終ステップでも意味を持つため、`step.is_checkpoint`を先に判定し真であれば`isLast`に関わらずチップを描画し、偽の場合のみ`isLast`でない時に接続線(縦棒)を描画するよう分岐順序を入れ替えた。右側には、`step.status === 'current'`の場合は強調されたカード（見出し「🌟 今ここ」＋ステップ名＋「完了！」ボタン、`isCompleting`中は`disabled`）を、それ以外の場合は単純なラベル表示（「まだだよ」バッジを先頭に付与）を描画する。**（自由時間の時間固定をやめる変更で追加）** さらに`!step.is_checkpoint`の場合は、行の右端（`ml-auto`）に「いまやった！」ボタンを描画する。**（Issue #718 で変更）** これらの条件は`step.status === 'remind'`を直接見るのをやめ、`catchUp`（`step.status === 'remind'`そのもの）と`showRemind`（`catchUp && !isAdult`）の2つに分かれた。ノードのベルアイコン・オレンジ系の配色・「まだだよ」バッジは`showRemind`を見るため大人では出ないが、**「いまやった！」ボタンは`catchUp`を見るため大人でも出る**（配色だけが`showRemind`で分岐し、大人にはオレンジ系ではなくグレー系が当たる）。押すと`current`のカードと同じ`onComplete`（＝`onCompleteStep(step.key)`）を呼び、バックエンドの追いつき完了（締切超過で「まだだよ」になった一本道のステップを後から完了報告する経路）に繋がる。チェックポイントステップ自身（自由時間）は`'remind'`になってもタップ対象ではないためボタンを出さない。`compact`propに応じてノード・アイコン・フォントサイズ・余白を縮小する。**（大人用フロー分離で変更）** 右側の描画の両分岐(`current`のスポットライトカード内・それ以外の通常ラベル行)に`<RoutineStepRewardChip flowKey={flowKey} step={step} />`が追加され、そのステップが即時報酬を持つ場合のみ報酬額のチップがラベルの隣に並ぶ。これに伴い、通常ラベル行は`gap-1.5`のflexコンテナになり、「まだだよ」バッジの右マージン(`mr-1.5`)は`gap`に置き換えられ、ラベル自体は`<span className="min-w-0">`で包まれた。**（夜の切り替え/寝る準備チェックリスト化で確認済み）** `isLast`propの意味が「`flow.steps`全体の最後」から「チェックリストを除いた非チェックリストステップの中での最後」に変わった(`RoutineFlow`側の`lastPathKey`算出方法の変更)。
* 根拠: [コンポーネント本体] (行番号: 257〜265 / 抜粋: "const RoutineStepRow: React.FC<{\n    flowKey: 'am' | 'pm';\n    step: RoutineStep;\n    checkpointTime: string | null;\n    isLast: boolean;\n    onComplete: () => void;\n    isCompleting: boolean;\n    compact: boolean;\n}> = ({ flowKey, step, checkpointTime, isLast, onComplete, isCompleting, compact }) => {")
* 根拠: `const RoutineStepRow: React.FC<{` (行番号: 279)
* 根拠: [`catchUp`/`showRemind`の算出] (行番号: 295 / 抜粋: "    const catchUp = step.status === 'remind';")
* 根拠: [同上] (行番号: 296 / 抜粋: "    const showRemind = catchUp && !isAdult;")
* 根拠: チェックポイントチップの分岐（画面非表示化で`isLast`に関わらず描画するよう変更） (行番号: 282〜289 / 抜粋: "{step.is_checkpoint ? (\n                    // チェックポイントの時刻表示は「次のステップへの接続線」ではなく\n                    // 「この時刻に自動で進む/完了する」という告知のため、そのステップが\n                    // フロー最後(isLast)でも出し続ける(例: amは'free'がチェックポイント\n                    // 兼フロー最後のステップ、画面非表示化で'leave'廃止のため)。\n                    <span className={`my-1 flex items-center gap-1 rounded-full border border-dashed px-2 py-0.5 text-[10px] font-bold whitespace-nowrap ${theme.chip}`}>\n                        <Clock size={10} />{checkpointTime}\n                    </span>")、[接続線の分岐（`isLast`でなければ描画）] (行番号: 290〜294 / 抜粋: ") : (\n                    !isLast && (\n                        <div className={`w-[3px] flex-1 min-h-[16px] rounded-full ${step.status === 'done' ? theme.seg : 'bg-gray-700'}`} />\n                    )\n                )}")
* 根拠: ステップ個別報酬チップの描画箇所（大人用フロー分離で追加） (行番号: 301〜321 / 抜粋: "                        <div className=\"flex items-center gap-2 mb-3\">\n                            <div className=\"text-xl font-black text-white min-w-0\">{step.label}</div>\n                            <RoutineStepRewardChip flowKey={flowKey} step={step} />\n                        </div>", "                        <span className=\"min-w-0\">{step.label}</span>\n                        <RoutineStepRewardChip flowKey={flowKey} step={step} />")、`current`ステップの完了ボタン (行番号: 298〜310 / 抜粋: "{step.status === 'current' ? (\n                    <div className={`rounded-2xl border p-4 ${theme.spotlight}`}>\n                        <div className={`flex items-center gap-1 text-xs font-bold mb-1 ${theme.text}`}>🌟 今ここ</div>\n                        <div className=\"text-xl font-black text-white mb-3\">{step.label}</div>\n                        <button\n                            type=\"button\"\n                            onClick={onComplete}\n                            disabled={isCompleting}\n                            className={`w-full rounded-full py-3 font-black shadow-lg disabled:opacity-50 ${theme.btn}`}\n                        >\n                            完了！\n                        </button>\n                    </div>\n                ) : (")
...
flowchart TD
    ...
    StepStatus -- "remind かつ isAdult でない(showRemind)" --> RemindRow["ベルアイコン + 「まだだよ」バッジ + ラベル\n(is_checkpointでなければ右端に「いまやった！」ボタン、\nonClickで onCompleteStep(step.key) を実行=追いつき完了)"]
    StepStatus -- "remind かつ isAdult(catchUp のみ)" --> RemindAdultRow["通常アイコン + ラベル(「まだだよ」なし)\n(is_checkpointでなければ右端にグレー系の「いまやった！」ボタン、\n追いつき完了は大人でも可能)"]
    ...
    subgraph "RoutineChecklistItem(step, onToggle, ...)"
        ...
        CIRemind -- "Yes かつ isAdult でない" --> CIRemindRow["ベルアイコン + 「まだだよ」バッジ + オレンジ系行(タップでonToggle)"]
        CIRemind -- "Yes だが isAdult" --> CIRemindAdultRow["通常アイコン + 通常配色の行(「まだだよ」なし、タップでonToggle)"]
...
* **（eslint-plugin-react-hooks 7 への更新で変更）チェックリストのブロックは `checklistStartIndex` と比べて1回だけ差し込む**: 以前は外側の `let checklistRendered` を `reduce` の中で書き換えていたが、react-hooks 7 の `immutability` ルールが描画後の再代入として禁じる。`reduce` のコールバックに `index` を受け取り、`index === checklistStartIndex`（最初のチェックリスト項目の位置。既に算出済み）のときだけブロックを積む。描画結果は従来と同じ
...
* 根拠: (行番号: 249, 316 / 抜粋: "const theme = THEME[flowKey];", "<span className={`my-1 flex items-center gap-1 rounded-full border border-dashed px-2 py-0.5 text-[10px] font-bold whitespace-nowrap ${theme.chip}`}>")
...
* **（Issue #718 で新規追加）`isAdult`は「表示」だけを変えるフラグであり、`step.status`の扱いを変えてはならない**: `RoutineStepRow`が`catchUp`（`step.status === 'remind'`）と`showRemind`（`catchUp && !isAdult`）を分けているのは、「いまやった！」ボタンの表示条件を`step.status === 'remind'`のまま保つためである。追いつき完了はフロント側のこの表示条件と、サーバー側`MY_HOME_SYSTEM/services/routine_service.py`の`complete_step`が持つ`is_catch_up`判定（同じく`progress['steps_status'].get(step_key) == 'remind'`を見る）の**両方が揃って初めて成立する**。したがって「大人には『まだだよ』を出さない」を、サーバー側で`'remind'`を付けない形や、フロント側で`catchUp`まで`!isAdult`で絞る形に変更すると、**帰宅後に「いまやった！」を押す経路ごと失われ**、大人用フローに移設された即時報酬（ママの「夕食を作る」の150G/150EXP等）も受け取れなくなる。`RoutineFlow.test.tsx`にこの退行を防ぐテストがある。
* 根拠: (行番号: 295〜296 / 抜粋: "const catchUp = step.status === 'remind';\n    const showRemind = catchUp && !isAdult;")
* 根拠: (行番号: 226〜231 / 抜粋: "return (\n        <button\n            type=\"button\"\n            onClick={onToggle}\n            disabled={isCompleting}\n            className={`flex items-center gap-3 w-full rounded-xl border px-3 py-2.5 text-left transition-colors disabled:opacity-50 ${rowClass}`}\n        >")
...
* 根拠: (行番号: 137〜139 / 抜粋: "const checkpointIndex = flow.steps.findIndex((step) => step.is_checkpoint);\n    const checklistStartIndex = flow.steps.findIndex((step) => step.is_checklist);\n    const checklistAffectsBonus = checkpointIndex === -1 || checklistStartIndex < checkpointIndex;")
* **（大人用フロー分離で新規追加）** `RoutineStepRewardChip`は`step.gold`または`step.exp`のいずれかが非0なら描画されるが、チップに表示するのは`step.gold`のみで`step.exp`は画面に出ない。したがって「goldは0だがexpのみ非0」というステップをバックエンドが定義した場合、チップは`0`という数字だけを表示することになる(現状の`routine_data.py`の大人用フローで報酬を持つのはママの`cook_dinner`とパパの`kitchen_reset`/`living_reset`で、いずれも`gold`/`exp`とも非0である)。
* 根拠: (行番号: 249〜254 / 抜粋: "    if (!step.gold && !step.exp) return null;\n    return (\n        <span className={`flex items-center gap-0.5 rounded-full border px-2 py-0.5 text-[10px] font-bold whitespace-nowrap flex-none ${THEME[flowKey].chip}`}>\n            <Coins size={10} />{step.gold}\n        </span>")
* **（大人用フロー分離で新規追加）** ステップ個別報酬のチップは`RoutineStepRow`(一本道のステップ)からのみ描画され、`RoutineChecklistBlock`/`RoutineChecklistItem`(チェックリスト項目)には追加されていない。これは現状の大人用フローで報酬を持つステップ(ママの`cook_dinner`、パパの`kitchen_reset`/`living_reset`)がいずれも一本道側にあり、チェックリスト項目(朝の準備・寝る準備)は`gold`/`exp`を持たないためであり、将来チェックリスト項目にも個別報酬を持たせる場合は`RoutineChecklistItem`側にもチップの描画を追加する必要がある。
* 根拠: (行番号: 303〜323 / 抜粋: "                            <RoutineStepRewardChip flowKey={flowKey} step={step} />", "                        <RoutineStepRewardChip flowKey={flowKey} step={step} />")
* **（大人用フロー分離で新規追加）** 本ファイルは`flow`propの中身だけを見て描画しており、そのフローが子ども用か大人用かを区別するロジックを一切持たない。ユーザーごとのフローの出し分けは完全にバックエンド(`routine_data.get_flow_set`)の責務であり、フロントエンドは`role`等のユーザー属性を参照しない。
* 根拠: `interface RoutineFlowProps {` (行番号: 66)
