...
* 根拠: `export default RoutineFlow` (行番号: 326 / 抜粋: "export default RoutineFlow;")
* 根拠: `export const RoutineFreeTimeBanner` (行番号: 73 / 抜粋: "export const RoutineFreeTimeBanner: React.FC<{ flowKey: 'am' | 'pm'; flow: RoutineFlowState }> = ({ flowKey, flow }) => {")
...
* 根拠: 誘導中の最終的な戻り値 (行番号: 160 / 抜粋: "return <div className=\"flex flex-col\">{rows}</div>;")
...
* 根拠: ファイル冒頭のコメント（自由時間中は別画面に任せる設計意図） (行番号: 72 / 抜粋: "// 自由時間中は別画面(既存のクエスト選択UI)に任せ、ここでは現在地バナーだけを表示する。")

* **引数/リクエスト**: `{ flowKey: 'am' | 'pm'; flow: RoutineFlowState }`
* 根拠: (行番号: 73 / 抜粋: "export const RoutineFreeTimeBanner: React.FC<{ flowKey: 'am' | 'pm'; flow: RoutineFlowState }> = ({ flowKey, flow }) => {")

* **戻り値/レスポンス**: `null`（`flow.started`が偽、または`flow.in_free_time`が偽の場合)、またはバナーのJSX要素
* 根拠: (行番号: 74 / 抜粋: "if (!flow.started || !flow.in_free_time) return null;")
...
* 根拠: [`remind`の算出] (行番号: 237 / 抜粋: "    const remind = step.status === 'remind' && !isAdult;")
...
* 根拠: [`catchUp`/`showRemind`の算出] (行番号: 293 / 抜粋: "    const catchUp = step.status === 'remind';")
* 根拠: [同上] (行番号: 294 / 抜粋: "    const showRemind = catchUp && !isAdult;")
...
* 根拠: (行番号: 293〜294 / 抜粋: "const catchUp = step.status === 'remind';\n    const showRemind = catchUp && !isAdult;")
...
* 根拠: (行番号: 121〜123 / 抜粋: "const checkpointIndex = flow.steps.findIndex((step) => step.is_checkpoint);\n    const checklistStartIndex = flow.steps.findIndex((step) => step.is_checklist);\n    const checklistAffectsBonus = checkpointIndex === -1 || checklistStartIndex < checkpointIndex;")
