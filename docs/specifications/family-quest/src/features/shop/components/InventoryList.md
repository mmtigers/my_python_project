...
- [quest_service.md](../../../../../MY_HOME_SYSTEM/quest_service.md) — `GET /api/quest/inventory/{user_id}`のレスポンス形状(`items`/`youtube_cooldown_remaining_seconds`/`is_youtube_reward`/`youtube_duration_minutes`/`youtube_daily_limit_minutes`/`youtube_daily_used_minutes`/`youtube_daily_limit_announcement`/`youtube_extension`)を決定するバックエンド実装。
...
* 根拠: (行番号: 22〜26, 231〜232 / 抜粋: "// PC横画面の4人並びパネルなど、実際の表示幅が狭い枠内に埋め込む場合に指定する。", "const gridClass = panelMode ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2';\n    const iconBoxClass = panelMode ? 'text-xl w-9 h-9' : 'text-2xl w-11 h-11';")
...
* 根拠: (行番号: 52, 73〜75, 140〜147 / 抜粋: "const isUsingItemRef = useRef(false);", "onSettled: () => {\n            isUsingItemRef.current = false;\n        }", "if (itemToUse && !isUsingItemRef.current) {\n                                    isUsingItemRef.current = true;\n                                    useMutationAction.mutate(itemToUse.id);\n                                }")
* **新機能(YouTubeごほうび券クールダウン)**: `apiClient.fetchInventory`の戻り値が配列から`InventoryResponse`（`{items, youtube_cooldown_remaining_seconds}`）に変更されたのに合わせ、`data?.items`を一覧として使う。バックエンドが返す`youtube_cooldown_remaining_seconds`(サーバー側の残り秒数、5秒間隔のポーリングで再同期)を起点に、1秒間隔の`setInterval`でローカルに`youtubeCooldownSeconds`をカウントダウンする`useEffect`を追加した。`item.is_youtube_reward`が真かつ`youtubeCooldownSeconds > 0`のアイテムは`isCoolingDown`となり、`isLocked`としてカードのクリック(使用確認モーダルを開く操作)を無効化したうえで、説明文の代わりに`formatCooldown`(mm:ss形式)を使った「目を休めよう。あと{mm:ss}で使えます」という文言と`Lock`アイコンを表示する。使用成功時のキャッシュ更新（`onSuccess`内の`setQueryData`）も、配列を直接フィルタする形から`InventoryResponse`の`items`フィールドのみを`filter`する形に変更した。
* 根拠: `const items = data?.items;` (行番号: 60)、`useEffect(() => {\n        const serverValue = data?.youtube_cooldown_remaining_seconds ?? 0;` (行番号: 76〜85)、`const isCoolingDown =\n                    !exceedsDailyLimit && item.is_youtube_reward && youtubeCooldownSeconds > 0;` (行番号: 229〜230)、`onClick={() => { if (!isLocked) setItemToUse(item); }}` (行番号: 244)、`queryClient.setQueryData<InventoryResponse>(queryKey, (old) => {\n                if (!old) return old;\n                return { ...old, items: old.items.filter(item => item.id !== usedInventoryId) };\n            });` (行番号: 100〜103)
* **新機能(YouTubeごほうび券クールダウンの予告バナー)**: バックエンドがクールダウンの猶予期間中(実際の制限開始前)に返す`data?.youtube_cooldown_announcement`を`cooldownAnnouncement`として取り出し、真であれば`formatAnnouncementDate`(ISO日付を「9/12(土)」のような表示用文字列に変換)を使った「{開始日}から、YouTubeのごほうび券は見おわってから15分間、次の1枚が使えなくなります(目を休めるため)。」という予告バナー(`announcementBanner`)を表示する。**（日次上限機能で変更）** `announcementBanner`はクールダウンと日次上限の2つの予告を束ねる入れ物になり、`dailyLimitAnnouncement`が真なら「{開始日}から、YouTubeのごほうび券は1日ぜんぶで{上限}分までになります(目を休めるため)。」というバナーも併せて表示する(2つは別々の施行日を持つため独立に出る)。いきなり制限がかかると子どもが困惑するため、施行日を迎えるまでの猶予期間中に事前告知する目的。このバナーは、もちものが空の状態(`items.length === 0`)でも、通常のアイテム一覧表示時でも(グリッドの`col-span-full`要素として)表示され、個別のアイテムの`is_youtube_reward`/所持状況とは無関係に、`cooldownAnnouncement`の有無だけで出し分けられる。
* 根拠: `const cooldownAnnouncement = data?.youtube_cooldown_announcement ?? null;` (行番号: 61)、`function formatAnnouncementDate(isoDate: string): string {` (行番号: 35〜38)、`const announcementBanner = (cooldownAnnouncement || dailyLimitAnnouncement) && (` (行番号: 142〜164)、空状態での表示 (行番号: 190)、一覧表示時の`col-span-full`表示 (行番号: 213〜215)
* **新機能(YouTubeごほうび券の1日の合計分数上限)**: `data?.youtube_daily_limit_minutes`(上限分数。上限なし設定なら`null`)と`data?.youtube_daily_used_minutes`(今日すでに使った分数)から`dailyRemainingMinutes`(= `max(0, 上限 - 使用済み)`、上限が`null`なら`null`)を算出し、上限が有効なときだけ「きょうのYouTube: あと{残り}分 ({使用済み}/{上限}分)」という常設バナー(`dailyBudgetBanner`)を表示する。残りが0なら「きょうはおしまい」という文言とグレーの配色に切り替わる。このバナーはもちものが空の状態でも一覧表示時でも表示される。各アイテムについては、YouTube系かつ日次上限が施行済みかつ`item.youtube_duration_minutes`が`dailyRemainingMinutes`を超える場合に`exceedsDailyLimit`となり、`isCoolingDown`とあわせて`isLocked`としてカードのクリックを無効化する。理由の文言(`lockedReason`)は日次上限を優先し、残り0なら「きょうのYouTubeはおしまい。また明日つかおうね」、残りがあるなら「きょうはあと{残り}分。この券は使えません」を表示する(この優先順位はバックエンドの`InventoryService._use_item_locked`の判定順と揃えてある)。「施行済みかどうか」は専用フィールドではなく`dailyLimitAnnouncement === null`(予告情報はまだ施行されていない間だけサーバーが返す)から導出している。
* 根拠: `const dailyLimitMinutes = data?.youtube_daily_limit_minutes ?? null;` (行番号: 65)、`const dailyRemainingMinutes =\n        dailyLimitMinutes === null ? null : Math.max(0, dailyLimitMinutes - dailyUsedMinutes);` (行番号: 67〜68)、`const isDailyLimitEnforced = dailyLimitAnnouncement === null;` (行番号: 71)、`const dailyBudgetBanner = dailyRemainingMinutes !== null && dailyLimitMinutes !== null && (` (行番号: 166〜188)、`const exceedsDailyLimit =\n                    item.is_youtube_reward &&\n                    isDailyLimitEnforced &&\n                    dailyRemainingMinutes !== null &&\n                    durationMinutes > dailyRemainingMinutes;` (行番号: 224〜228)、`const isLocked = exceedsDailyLimit || isCoolingDown;` (行番号: 231)、`const lockedReason = exceedsDailyLimit` (行番号: 233〜237)
* **新機能(プリントによる上限の延長)**: サーバーが返す`data?.youtube_extension`を`extension`として取り出し、`can_extend_now`(=いまプリントを1枚やれば上限が延びる状態か)を`canExtendNow`として使う。真のときは、残り分数バナーに「プリントを1枚やると、あと{minutes_per_quest}分ふえるよ📝」の一行を足し、使えない券の理由も「きょうのぶんはおしまい。プリントを1枚やると{minutes_per_quest}分ふえるよ」に差し替える(延長が尽きている場合のみ従来の「きょうのYouTubeはおしまい。また明日つかおうね」になる)。コメントにあるとおり「いつ・何回まで延びるか」の判定は全てサーバー側(`services/quest/locks.py`)に置き、本コンポーネントは案内の出し分けにだけ使う。`youtube_daily_limit_minutes`自体もサーバー側で延長を反映済みの実効上限であるため、フロント側で足し込む処理は無い。
* 根拠: `const extension = data?.youtube_extension ?? null;` (行番号: 76)、`const canExtendNow = extension?.can_extend_now ?? false;` (行番号: 77)、`プリントを1枚やると、あと{extension.minutes_per_quest}分ふえるよ📝` (行番号: 193)、`const outOfMinutesReason = canExtendNow && extension` (行番号: 249〜251)
...
| `useQuery`, `useMutation`, `useQueryClient` | フック | データ取得、データ更新、キャッシュ操作 | 根拠: [`@tanstack/react-query`] (行番号: 3 / 抜粋: "import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';") |
| `apiClient` | オブジェクト | サーバーサイドとのAPI通信 | 根拠: [`apiClient`] (行番号: 4 / 抜粋: "import { apiClient } from '../../../lib/apiClient';") |
| `Card` | コンポーネント | アイテムごとのUIカードレイアウト表示 | 根拠: [`Card`] (行番号: 5 / 抜粋: "import { Card } from '../../../components/ui/Card';") |
| `Button` | コンポーネント | 使用確認モーダル内の「キャンセル」「はい」ボタン | 根拠: [`Button`] (行番号: 6 / 抜粋: "import { Button } from '../../../components/ui/Button';") |
| `Modal` | コンポーネント | 「つかう」実行前の確認ダイアログ表示 | 根拠: [`Modal`] (行番号: 7 / 抜粋: "import { Modal } from '../../../components/ui/Modal';") |
| `useSound` | フック | アクション時の効果音再生 | 根拠: [`useSound`] (行番号: 8 / 抜粋: "import { useSound } from '../../../hooks/useSound';") |
| `useToast` | フック | 使用失敗時のエラートースト表示 | 根拠: [`useToast`] (行番号: 9 / 抜粋: "import { useToast } from '../../../context/useToast';") |
| `Loader2`, `PackageOpen`, `Lock` | コンポーネント(アイコン) | UI上の状態表示アイコン（ローディング中/所持アイテム/YouTubeごほうび券が使えない状態=クールダウン中または日次上限超過） | 根拠: [`lucide-react`] (行番号: 10 / 抜粋: "import { Loader2, PackageOpen, Lock } from 'lucide-react';") |
| `InventoryItem`, `InventoryResponse` | 型定義 | アイテムデータおよびインベントリ取得APIレスポンス全体（`items`+`youtube_cooldown_remaining_seconds`+`youtube_cooldown_announcement`+`youtube_daily_limit_minutes`+`youtube_daily_used_minutes`+`youtube_daily_limit_announcement`+`youtube_extension`）の型チェックと補完 | 根拠: [`InventoryItem`, `InventoryResponse`] (行番号: 11 / 抜粋: "import { InventoryItem, InventoryResponse } from '../../../types';") |

### ブラックボックスとなる外部要素

| 名称 | 理由 | 根拠 |
| --- | --- | --- |
| `apiClient`の各メソッド (`fetchInventory`, `useItem`) | 具体的なエンドポイント、リクエスト/レスポンス形式、エラーハンドリングの実装が不明（`../../../lib/apiClient`に依存のため要確認）。 | 根拠: [`apiClient`の呼び出し] (行番号: 57, 121 / 抜粋: "queryFn: () => apiClient.fetchInventory(userId),", "mutationFn: (inventoryId: number) => apiClient.useItem(userId, inventoryId),") |
| `Card`, `Button`, `Modal`の内部実装 | `../../../components/ui/`配下の実装が提供されていないため、propsの全容やレンダリング内容が不明。 | 根拠: [`Card`, `Button`, `Modal`] (行番号: 4〜6) |
| `useSound`の挙動 | 音声再生時のエラー処理や、再生可能な音声キー（`'clear'`, `'cancel'`）の定義が不明（`../../../hooks/useSound`に依存のため要確認）。 | 根拠: [`useSound`] (行番号: 42 / 抜粋: "const { play } = useSound();") |
| `useToast`/`showToast`の内部実装 | トーストの表示時間・スタック方法・スタイルなど、`../../../context/useToast`（および`ToastContext`）に依存する具体的な描画内容が不明。 | 根拠: [`useToast`] (行番号: 9, 43 / 抜粋: "import { useToast } from '../../../context/useToast';", "const { showToast } = useToast();") |
...
* 根拠: **（Issue #441）** 一覧取得エラーの初回トースト (行番号: 108〜118 / 抜粋: "const hasShownFetchErrorRef = useRef(false);\n    useEffect(() => {\n        if (isError) {\n            if (!hasShownFetchErrorRef.current) {\n                hasShownFetchErrorRef.current = true;\n                showToast({ title: \"エラー\", text: extractErrorDetail(error, 'アイテム一覧の取得に失敗しました'), icon: \"⚠️\" });\n            }\n        } else {\n            hasShownFetchErrorRef.current = false;\n        }\n    }, [isError, error, showToast]);")
...
| 優先度 | ファイル名(推測可) | 理由 | 根拠 |
| --- | --- | --- | --- |
| 高 | `../../../lib/apiClient.ts` | APIの実際のエンドポイント、パラメータ仕様、レスポンス構造、およびAPI側で発生しうるエラーの詳細を把握するため。 | 根拠: [`apiClient`への依存] (行番号: 4 / 抜粋: "import { apiClient }") |
...
| 中 | `../../../components/ui/Modal.tsx` | `isOpen`/`onClose`/`footer`以外に受け付けるprops、およびアクセシビリティ対応（フォーカストラップ等）の実装状況を確認するため。 | 根拠: [`Modal`への依存] (行番号: 7 / 抜粋: "import { Modal }") |
| 低 | `../../../hooks/useSound.ts` | 再生可能な音声キーの全容や、音声ファイルのロード状況による動作への影響を確認するため。 | 根拠: [`useSound`への依存] (行番号: 8 / 抜粋: "import { useSound }") |
| 低 | `../../../context/useToast.ts` / `ToastContext.tsx` | エラートーストの表示時間・スタック方法など、UI上の具体的な挙動を確認するため。 | 根拠: [`useToast`への依存] (行番号: 9 / 抜粋: "import { useToast }") |

## 8. 保守上の注意点

* **（eslint-plugin-react-hooks 7 への更新で追加）YouTube クールダウンの effect では `set-state-in-effect` ルールを1行だけ外している**: ポーリングで届いたサーバー値を起点にローカルのカウントダウンを始め直す処理で、`setInterval`（外部のタイマー）との同期が本来の役割である。描画中に算出する形へ移すと経過時間のために描画中に `Date.now()` を呼ぶことになり `purity` ルールに触れるため、**挙動は変えずに** `eslint-disable-next-line react-hooks/set-state-in-effect` で `setYoutubeCooldownSeconds(serverValue)` の行だけを対象外にした（理由をコード中に明記）
...
* 根拠: (行番号: 47, 139〜149 / 抜粋: "const [itemToUse, setItemToUse] = useState<InventoryItem | null>(null);", "onClick={() => {\n                                // #119: ...\n                                if (itemToUse && !isUsingItemRef.current) {\n                                    isUsingItemRef.current = true;\n                                    useMutationAction.mutate(itemToUse.id);\n                                }\n                                setItemToUse(null);\n                            }}")
...
* 根拠: (行番号: 231〜232 / 抜粋: "const gridClass = panelMode ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2';\n    const iconBoxClass = panelMode ? 'text-xl w-9 h-9' : 'text-2xl w-11 h-11';")
...
* **YouTubeごほうび券クールダウンの表示は「サーバー値5秒同期＋ローカル1秒カウントダウン」のハイブリッド**: `youtubeCooldownSeconds`は`data?.youtube_cooldown_remaining_seconds`が変化する(=5秒間隔のポーリングでサーバー値が更新される)たびに`useEffect`内で丸ごと上書きされ、そこから`setInterval`(1000ms)でローカルに1ずつ減算していく。そのため、次のポーリング(最大5秒後)までの間はローカルカウントダウンが唯一の情報源であり、ページ内の別タブ操作等でサーバー側の状態が変わっても最大5秒のズレが生じ得る。クールダウン判定自体（使用可否）はサーバー側(`InventoryService._use_item_locked`)が最終的に強制するため、表示のズレが実際の誤使用を許すことはない。日次上限のほうはローカルカウントダウンを持たず、ポーリングで取得したサーバー値をそのまま表示する。
* 根拠: `useEffect(() => {\n        const serverValue = data?.youtube_cooldown_remaining_seconds ?? 0;\n        setYoutubeCooldownSeconds(serverValue);\n        if (serverValue <= 0) return;\n\n        const id = window.setInterval(() => {\n            setYoutubeCooldownSeconds((s) => Math.max(0, s - 1));\n        }, 1000);\n        return () => window.clearInterval(id);\n    }, [data?.youtube_cooldown_remaining_seconds]);` (行番号: 76〜85)
* **「きょうの上限」はサーバーが返す実効上限をそのまま表示している**: `youtube_daily_limit_minutes`にはプリントによる延長が既に反映されているため、本コンポーネントは`youtube_extension.minutes_per_quest`を足し込んだりしない(足すと二重計上になる)。`youtube_extension`は案内文の出し分けだけに使う。
* 根拠: `const dailyLimitMinutes = data?.youtube_daily_limit_minutes ?? null;` (行番号: 65)、`const extension = data?.youtube_extension ?? null;` (行番号: 76)
* **日次上限が「施行済みか」はレスポンスの`youtube_daily_limit_announcement === null`から導出している**: サーバーは施行前だけこの予告情報を返し、施行後は`null`にする。専用の真偽値フィールドは存在しないため、将来サーバーが「上限は無効だが予告は出す」といった状態を返すようになると、この導出は破綻する。上限そのもの(`youtube_daily_limit_minutes`)は施行前でも返る点(表示は猶予期間中から行い、ブロックだけ施行日から始める設計)と混同しないこと。
* 根拠: `const isDailyLimitEnforced = dailyLimitAnnouncement === null;` (行番号: 71)、コメント (行番号: 69〜70)
* **（2026-09-23 要件追加）YouTube等の時間消費型ごほうびは自由時間中のみ使える。判定はサーバーの`is_in_free_time`をそのまま信頼する**: `isInFreeTime = data?.is_in_free_time ?? true`（データ未取得時はロック表示のちらつきを避けるため既定`true`）。`item.is_youtube_reward`かつ`!isInFreeTime`のとき`isOutsideFreeTime`となり、他の2つの制限(日次上限・クールダウン)より**先に**判定される（`exceedsDailyLimit`/`isCoolingDown`の算出式はいずれも`!isOutsideFreeTime &&`から始まり、自由時間外なら他の理由を計算するまでもなく最優先でロックする）。ロック理由の文言は「自由時間になったら見られるよ」で、日次上限・クールダウンの文言よりバックエンド(`InventoryService._use_item_locked`のチェック順)と揃えて先頭に判定される。施行猶予期間(`ENFORCE_FROM`によるバナー予告)の仕組みはこの制限には存在せず、`is_in_free_time`が`false`ならバナー無しで即座にロックされる。YouTube以外のごほうび(おやつ・ガチャガチャ等)は`item.is_youtube_reward`が`false`のため、自由時間外でも通常通り使用できる。
* 根拠: `const isInFreeTime = data?.is_in_free_time ?? true;` (行番号: 80)、`const isOutsideFreeTime = item.is_youtube_reward && !isInFreeTime;` (行番号: 247)、`const isLocked = isOutsideFreeTime || exceedsDailyLimit || isCoolingDown;` (行番号: 257)、`const lockedReason = isOutsideFreeTime\n                    ? '自由時間になったら見られるよ'` (行番号: 264〜265)
