...
- [quest_service.md](../../../../../MY_HOME_SYSTEM/quest_service.md) — `GET /api/quest/inventory/{user_id}`のレスポンス形状(`items`/`youtube_cooldown_remaining_seconds`/`is_youtube_reward`/`youtube_duration_minutes`/`youtube_daily_limit_minutes`/`youtube_daily_used_minutes`/`youtube_daily_limit_announcement`)を決定するバックエンド実装。
...
| `InventoryItem`, `InventoryResponse` | 型定義 | アイテムデータおよびインベントリ取得APIレスポンス全体（`items`+`youtube_cooldown_remaining_seconds`+`youtube_cooldown_announcement`+`youtube_daily_limit_minutes`+`youtube_daily_used_minutes`+`youtube_daily_limit_announcement`）の型チェックと補完 | 根拠: [`InventoryItem`, `InventoryResponse`] (行番号: 10 / 抜粋: "import { InventoryItem, InventoryResponse } from '../../../types';") |
