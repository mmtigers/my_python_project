...
* [../../../App.md](../../../App.md) - 呼び出し元。`avatarUser`が設定されたときに本コンポーネントを描画し、`onUploadComplete`で`refreshData()`とメッセージ表示を行う
* [../../lib/apiClient.md](../../lib/apiClient.md) - `postForm`メソッドの実装元
* [../../types/index.md](../../types/index.md) - `User`型の定義元
* [./Modal.md](./Modal.md) - 利用するモーダルコンポーネント
* [./Button.md](./Button.md) - 利用するボタンコンポーネント
* [../../../../MY_HOME_SYSTEM/quest_router.md](../../../../MY_HOME_SYSTEM/quest_router.md) - Family Quest系バックエンドのルーター定義（アップロード系エンドポイントを提供）
...
| --- | --- | --- |
| `apiClient.postForm` の詳細仕様 | `apiClient.md`の解析によれば、`postForm`は`Content-Type`ヘッダーを明示的に指定せず、ブラウザに`boundary`付きヘッダーを自動生成させる実装であり（手動指定するとboundaryが欠落しリクエストが壊れるためとコメントされている）、共通の`_request`メソッドがエラー時に`errorData.detail`またはステータスコードを用いた`Error`を送出するとされている。ただしこれは`apiClient.md`側の解析結果からの補足であり、`apiClient.ts`のソースコード自体は本ファイルの解析時点では確認していない。 | `../../lib/apiClient.md` |
| 画像のトリミング責務／サーバー側のファイルサイズ・形式検証の有無 | `quest_router.md`の解析によれば、Family Quest系バックエンド（`MY_HOME_SYSTEM/quest_router.py`）には`POST /upload`（拡張子チェック＋マジックナンバー検証付きの汎用画像アップロード）と`POST /user/update`（`avatar_url`によるユーザー情報更新）というエンドポイントは存在するが、本コンポーネントが呼び出す`POST /api/quest/upload_avatar`という名称・パスに一致するエンドポイントの記載は見当たらない。トリミング処理やファイルサイズ検証がサーバー側のどこで行われているかは、`quest_router.md`の解析結果からは確認できなかった。 | `../../../../MY_HOME_SYSTEM/quest_router.md` |
