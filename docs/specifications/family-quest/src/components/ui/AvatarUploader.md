...
* **（eslint 10 / eslint-plugin-react-hooks 7 への更新で変更）保存ボタンの有効/無効は `selectedFile`（state）で決める**: 以前は `disabled={!preview && !fileInputRef.current?.files?.[0]}` と**描画中に ref を読んで**いた。ref は変わっても再描画を起こさないため値が古くなりうる（react-hooks 7 の `refs` ルールも禁じる）。実際は FileReader の完了で `preview` が入ることで辻褄が合っていただけだった。現在は検証を通ったファイルを `setSelectedFile(file)` で保持し（検証に落ちたら `null`）、ボタンは `disabled={!selectedFile}`、アップロードも `selectedFile` を送る。副次的に、**プレビューの読み込みを待たずに**保存できるようになった。`AvatarUploader.test.tsx` がプレビューが来ない状況でも有効になること・不正なファイルで無効に戻ること・選んだファイルそのものを送ることを検査する
...
| ロールバック用`DELETE /api/quest/upload/{filename}`エンドポイントの実装詳細 | `MY_HOME_SYSTEM/routers/quest_router.py`93〜96行目を直接確認した。`@router.delete("/upload/{filename}")` / `def delete_uploaded_image(filename: str):` は`user_service.delete_unlinked_avatar(filename)`へ委譲し、**常に200で`{"status": "deleted"}`または`{"status": "skipped"}`を返す**(削除できなくてもエラーにしない、ベストエフォートの後始末という設計。87〜92行目のコメントに明記)。実体の`delete_unlinked_avatar`(`services/quest/user_service.py:256-292`)は、(1) パスを解決して`config.UPLOAD_DIR`配下から出ていないことを確認し、(2) `SELECT 1 FROM quest_users WHERE avatar = ? LIMIT 1`で**どのユーザーにも紐付いていないこと**を確認し、(3) 条件を満たしたときだけ`os.remove`して`True`を返す。いずれかの条件を満たさない場合・例外時は`False`(=`skipped`)。つまり本コンポーネントの2段階アップロードで2段目が失敗したときに「まだ紐付いていない自分の直前のアップロード画像」だけが消え、既に他ユーザーが使っている画像は消せない。 | 直接ソース確認: `MY_HOME_SYSTEM/routers/quest_router.py:87-96`, `MY_HOME_SYSTEM/services/quest/user_service.py:256-292`（参考: [quest_router.md](../../../../MY_HOME_SYSTEM/quest_router.md)・[quest_user_service.md](../../../../MY_HOME_SYSTEM/quest_user_service.md)） |
| `/api/quest/upload`が返す`url`の正確なフォーマット | `MY_HOME_SYSTEM/services/quest/user_service.py`の`save_avatar_image`は最終行(254行目)で`return f"/uploads/{new_filename}"`を返し、ルーター(`quest_router.py:98-102`)がこれを`{"url": url}`として返す。`new_filename`は`f"{uuid.uuid4()}{file_ext}"`(217行目)なので、**形式は`/uploads/<UUID4><元の拡張子(小文字)>`**(例: `/uploads/3f2b1c4e-....png`)。拡張子は`{.jpg, .jpeg, .png, .gif, .webp}`のいずれかに限定される(18行目)。**ホストを含まない同一オリジンの絶対パス**であり、これがそのまま`quest_users.avatar`へ保存され、フロント側の`isSameOriginAvatarPath`(`src/lib/utils.ts:19-23`)の`url.startsWith('/')`かつ`/^\/[\\/]/`でない、という判定を通る形式になっている。配信は`unified_server.py:366`の`app.mount("/uploads", StaticFiles(...))`が担う。 | 直接ソース確認: `MY_HOME_SYSTEM/services/quest/user_service.py:18,217,254`, `MY_HOME_SYSTEM/routers/quest_router.py:98-102`, `MY_HOME_SYSTEM/unified_server.py:366`, `family-quest/src/lib/utils.ts:19-23` |
