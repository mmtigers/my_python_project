...
| --- | --- | --- |
| 実際に`durationMs`へ渡される値（呼び出し元での具体的なミリ秒数・定数名） | `family-quest/src/features/quest/components/QuestList.tsx`を直接確認した。49行目で`const COOLDOWN_MS = 60000;`（コメント上の「60秒」と一致）が定義されており、84行目の`setTimeout(() => setIsCooldown(false), COOLDOWN_MS)`でクールダウン状態の解除タイマーに使われるのと同じ定数が、190行目の`<CooldownRing durationMs={COOLDOWN_MS} size={24} />`にそのまま渡されている。`QuestList.tsx`全体を検索した限り、`CooldownRing`の呼び出し箇所はこの1箇所のみであり、`durationMs`に固定値`60000`以外が渡されるケースは存在しないことを確認した。 | 直接ソース確認: `family-quest/src/features/quest/components/QuestList.tsx:49, 84, 190` |
