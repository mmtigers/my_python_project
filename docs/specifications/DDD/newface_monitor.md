...
* **副作用**: `write_and_verify()`の呼び出し。失敗のたびに`tmp_path.unlink(missing_ok=True)`（次の試行前のクリーンアップ）、最終試行以外では警告ログ出力と`time.sleep(_SAVE_VERIFY_RETRY_DELAY_SECONDS)`。
...
* 根拠: [リトライループ] (行番号: 1009〜1024 / 抜粋: "for attempt in range(1, DataManager._SAVE_VERIFY_MAX_ATTEMPTS + 1):\n            try:\n                write_and_verify()\n                return\n            except (OSError, ValueError, TypeError) as e:" / "raise last_error")
