...
* [../MY_HOME_SYSTEM/discord.md](../MY_HOME_SYSTEM/discord.md) — Issue #661 で Discord Webhook への POST を集約した `core.discord.post_webhook`。本ファイルの `DiscordNotifier` の3つの送信箇所はすべてこれを経由する（インポートできない環境向けにファイル内フォールバックあり）。
...
* **（Issue #661で追加）** Discord Webhookへの送信も`core.discord.post_webhook`へ委譲する。こちらは上記とは**独立した`try`/`except ImportError`ブロック**にしてある（`core.logger`等がインポートできる環境で`core.discord`だけが欠けていたときに、ロガーまでフォールバックへ巻き込まないため）。フォールバック実装は本ファイルが使う引数だけを満たす最小版。
...
* 根拠: [WebMonitor._parse_htmlとCastMember] (行番号: 1955〜2019, 415〜457 / 抜粋: "def _parse_html(self, soup: BeautifulSoup, site: SiteConfig) -> Set[CastMember]:")
...
* 根拠: [DataFileUnavailableError定義とコメント] (行番号: 847〜858 / 抜粋: "class DataFileUnavailableError(Exception):\n    \"\"\"load_daily_summary/load_site_failuresが、ファイルは存在するのにI/Oエラーで\n    読めなかったことを示す例外(#578)。")
* 根拠: [DataManager.save_known_castsのコメント] (行番号: 1233〜1235 / 抜粋: "# アトミック書き込み: 一時ファイルに書き出してから置き換えることで、\n            # 書き込み中断時に既存データが破損/空になるのを防ぐ\n            # (batch_download_discord.py の _purge_skipped_tasks と同じパターン)")
...
| `core.logger.get_logger` | 内部モジュール（オプショナル、try節） | ロガーインスタンスの取得。インポート失敗時はファイル内フォールバック実装を使用 | 根拠: [import文] (行番号: 63 / 抜粋: "from core.logger import get_logger") |
| `core.nas_utils.get_managed_target_directory` | 内部モジュール（オプショナル、try節） | NAS/ローカルのデータ保存ディレクトリの解決・管理。インポート失敗時はファイル内フォールバック実装を使用 | 根拠: [import文] (行番号: 64 / 抜粋: "from core.nas_utils import get_managed_target_directory") |
| `core.discord.post_webhook` (`post_discord_webhook`として別名) | 内部モジュール（オプショナル、独立したtry節） | **（Issue #661で追加）** Discord Webhookへの1回のPOST（分割・リトライ・URLマスクの集約先）。インポート失敗時はファイル内フォールバック実装を使用 | 根拠: [import文] (行番号: 129 / 抜粋: "from core.discord import post_webhook as post_discord_webhook") |
| `core.utils.wait_for_storage_warmup` | 内部モジュール（オプショナル、try節） | ストレージ（NAS等）が書き込み可能になるまでの待機処理。インポート失敗時はファイル内フォールバック実装を使用 | 根拠: [import文] (行番号: 65 / 抜粋: "from core.utils import wait_for_storage_warmup") |
...
| `MonitorConfig.SITES`に登録された79件の対象Webサイト | 各サイトのHTML構造（CSSセレクタが依拠する実際のマークアップ）は本ファイルのコードからは分からず、外部Webサイトの実物に依存する。 | 根拠: [MonitorConfig.SITESの初期化] (行番号: 345 / 抜粋: "SITES: List[SiteConfig] = _load_sites(SITES_JSON_PATH)") |
...
* 根拠: [関数定義] (行番号: 73〜74 / 抜粋: "def get_logger(name: str) -> logging.Logger: \n        return logging.getLogger(name)")


* **引数/リクエスト**: `name: str`
* 根拠: [引数定義] (行番号: 73 / 抜粋: "def get_logger(name: str) -> logging.Logger: ")


* **戻り値/レスポンス**: `logging.Logger`
* 根拠: [戻り値ヒント] (行番号: 73 / 抜粋: "-> logging.Logger: ")
...
* 根拠: [関数定義とDocstring] (行番号: 86〜123 / 抜粋: "def wait_for_storage_warmup(target_dir: Path, max_retries: int = 5, base_delay: float = 1.0) -> bool:\n        """\n        NAS等のストレージがマウントされ、書き込み可能になるまで待機する。")
...
* 根拠: [メソッド定義とDocstring] (行番号: 245〜252 / 抜粋: "def get_data_filename(self) -> str:\n        """既知キャストの保存先ファイル名を返す。")
...
* 根拠: [定数定義とコメント] (行番号: 215〜221 / 抜粋: "# Issue #413: 監視対象サイト定義（旧: 本ファイル内の約970行のPythonリテラル、\n# 79サイト分）を sites.json へ外出しした。" / "SITES_JSON_PATH: Path = CURRENT_DIR / 'sites.json'")、[関数定義とDocstring] (行番号: 283〜336 / 抜粋: "def _load_sites(json_path: Path) -> List[SiteConfig]:\n    """sites.json を読み込み、SiteConfigのリストとして返す。")


* **引数/リクエスト**: `json_path: Path`（`sites.json`のパス）
* 根拠: [引数定義] (行番号: 283, 258 / 抜粋: "def _load_sites(json_path: Path) -> List[SiteConfig]:" / "json_path (Path): sites.json のパス。")
...
* 根拠: [クラス定義とDocstring] (行番号: 339〜450 / 抜粋: "class MonitorConfig:\n    """モニタリング設定および定数管理クラス。"""")
...
* **戻り値/レスポンス**: 該当なし
* **副作用**: **（Issue #663で追加）** `NAS_DIR_STR`・`MOUNT_POINT`のクラス変数定義時に`file_utils.resolve_nas_data_dir('newface_monitor')`・`resolve_nas_mount_point()`を呼び出し、環境変数`NAS_MOUNT_POINT`(未設定なら`/mnt/nas`)から組み立てる。`SITES`のクラス変数定義時に`_load_sites(SITES_JSON_PATH)`を呼び出し`sites.json`を読み込む（不正があれば`RuntimeError`でモジュールimport自体が失敗する）。`DISCORD_WEBHOOK_URL`のクラス変数定義時にモジュールレベル関数`_resolve_discord_webhook_url()`（**Issue #586で追加**）を呼び出し、環境変数`DISCORD_WEBHOOK_NOTIFY`→`DISCORD_WEBHOOK_URL`の優先順位で解決する（後述）。
...
* 根拠: [メソッド定義] (行番号: 480〜481 / 抜粋: "def __hash__(self) -> int:\n        return hash(self.id)")


* **引数/リクエスト**: なし（`self`のみ）
* **戻り値/レスポンス**: `int`
* 根拠: [戻り値ヒント] (行番号: 480 / 抜粋: "def __hash__(self) -> int:")
...
* 根拠: [メソッド定義] (行番号: 483〜486 / 抜粋: "def __eq__(self, other: object) -> bool:\n        if not isinstance(other, CastMember):\n            return False\n        return self.id == other.id")


* **引数/リクエスト**: `other: object`
* **戻り値/レスポンス**: `bool`
* 根拠: [戻り値ヒント] (行番号: 483 / 抜粋: "def __eq__(self, other: object) -> bool:")
...
* 根拠: [メソッド定義とDocstring] (行番号: 488〜494 / 抜粋: "def to_dict(self) -> Dict[str, str]:\n        """辞書形式に変換する。")


* **引数/リクエスト**: なし（`self`のみ）
* **戻り値/レスポンス**: `Dict[str, str]`（`asdict(self)`の結果）
* 根拠: [戻り値] (行番号: 494 / 抜粋: "return asdict(self)")
...
* 根拠: [戻り値ヒントと処理] (行番号: 521, 490 / 抜粋: "def _to_well_formed_url(url: str) -> str:" / "return requests.utils.requote_uri(url)")
...
* 根拠: [クラス定義とDocstringおよび__init__] (行番号: 501〜823 / 抜粋: "class DiscordNotifier:\n    """Discordへの通知を担当するサービスクラス。"""\n\n    def __init__(self, webhook_url: Optional[str]):")
...
* 根拠: [メソッド定義とDocstring] (行番号: 547〜569 / 抜粋: "def _create_rate_limited_session(self) -> requests.Session:\n        """Discordのレート制限(429)に自動追従するHTTPセッションを作成する。")


* **引数/リクエスト**: なし（`self`のみ）
* 根拠: [引数定義] (行番号: 547 / 抜粋: "def _create_rate_limited_session(self) -> requests.Session:")
...
* 根拠: [メソッド定義とDocstring] (行番号: 571〜574 / 抜粋: "def close(self) -> None:\n        """保持しているHTTPセッションのリソースを明示的に解放する。"""\n        if self.session:\n            self.session.close()")


* **引数/リクエスト**: なし（`self`のみ）
* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 571 / 抜粋: "def close(self) -> None:")
...
* 根拠: (行番号: 519〜522, 538〜661 / 抜粋: "sent_count, _unsent = self.notify_casts(new_casts, site_name=site_name)", "def notify_casts(self, new_casts: List[CastMember], site_name: str = \"\") -> Tuple[int, List[CastMember]]:", "return sent_count, unsent")
...
* 根拠: (行番号: 772 / 抜粋: "logger.error(f\"Failed to send daily summary notification: {type(e).__name__}: {redact_discord_webhook_url(e)}\")")
...
* 根拠: [メソッド定義とDocstring] (行番号: 712〜774 / 抜粋: "def notify_daily_summary(self, counts: Dict[str, int], site_names: Dict[str, str], date_str: str) -> bool:\n        """その日に新規検知したサイト別件数を、テキスト形式でDiscordに通知する。")
...
* 根拠: [戻り値ヒントとDocstring] (行番号: 712, 648〜652 / 抜粋: "def notify_daily_summary(self, counts: Dict[str, int], site_names: Dict[str, str], date_str: str) -> bool:" / "Returns:\n            bool: 送信に成功した場合True。Webhook未設定または送信失敗の場合False。")


* **副作用**: サーキットブレーカーの開放チェック（開いていれば警告ログを出力し早期return）、件数降順でのサマリ文字列組み立て、2000文字制限に対する安全な切り詰め（1900文字超過分。切り詰め済みなので`core.discord`側の分割は働かず、「...(以下省略)」で1通に収める従来の文面が保たれる）、Webhookへの送信（`post_discord_webhook(..., username="New Face Monitor", session=self.session, raise_for_status=True)`）、成功/失敗ログ出力と`self._circuit_breaker`の状態更新(`record_success`/`record_failure`)。
...
* 根拠: [メソッド定義とDocstring] (行番号: 776〜823 / 抜粋: "def notify_site_failure_alert(self, site: SiteConfig, failure_count: int) -> bool:\n        """連続巡回失敗中のサイトについて「閉鎖・移転の疑い」をDiscordへテキスト通知する。")
...
* **副作用**: Webhookへの送信（`post_discord_webhook(..., username="New Face Monitor", session=self.session, raise_for_status=True)`）、成功/失敗ログ出力と`self._circuit_breaker`の状態更新(`record_success`/`record_failure`)。
* 根拠: [送信処理] (行番号: 803〜811 / 抜粋: "post_discord_webhook(\n                self.webhook_url,\n                content=content,\n                username=\"New Face Monitor\",\n                timeout=10,\n                session=self.session,\n                raise_for_status=True,\n            )")
...
* 根拠: [戻り値] (行番号: 953 / 抜粋: "return self.data_dir / site.get_data_filename()")
...
* 根拠: [クラス定数定義とコメント] (行番号: 887〜891 / 抜粋: "# #461: load_known_casts/load_daily_summaryが破損検知のたびに作成する\n    # .corrupted-*隔離ファイルは、.bak(常に最新の1世代のみ保持され上書きされる)\n    # と異なり削除処理を持たず、破損が繰り返されるたびに増え続けディスクを\n    # 圧迫し得た。この日数より古い隔離ファイルは巡回のたびに削除する。\n    _QUARANTINE_RETENTION_DAYS = 30")、[メソッド定義とDocstring] (行番号: 955〜983 / 抜粋: "def cleanup_old_quarantine_files(self, retention_days: int = _QUARANTINE_RETENTION_DAYS) -> int:\n        """`_QUARANTINE_RETENTION_DAYS`日より古い`.corrupted-*`隔離ファイルを削除する。")


* **引数/リクエスト**: `retention_days: int = _QUARANTINE_RETENTION_DAYS`（省略時30日）
* 根拠: [引数定義] (行番号: 955 / 抜粋: "def cleanup_old_quarantine_files(self, retention_days: int = _QUARANTINE_RETENTION_DAYS) -> int:")


* **戻り値/レスポンス**: `int`（削除できたファイル数。`self.data_dir`のファイル一覧取得自体に失敗した場合は`0`）
* 根拠: [戻り値ヒントとDocstring・各return] (行番号: 832〜833, 841, 983 / 抜粋: "Returns:\n            int: 削除できたファイル数。", "return 0", "return deleted")
...
* 根拠: [メソッド定義とDocstring] (行番号: 986〜993 / 抜粋: "def _read_casts_file(data_file: Path) -> Set[CastMember]:\n        """JSONファイルを読み込み、CastMemberの集合に変換する。\n\n        パース失敗時は例外をそのまま送出する（呼び出し側でハンドリングする前提）。\n        """")


* **引数/リクエスト**: `data_file: Path`（読み込み対象のJSONファイルパス）
* 根拠: [引数定義] (行番号: 986 / 抜粋: "def _read_casts_file(data_file: Path) -> Set[CastMember]:")
...
* 根拠: [処理内容] (行番号: 991〜993 / 抜粋: "with open(data_file, 'r', encoding='utf-8') as f:\n            data = json.load(f)\n            return {CastMember(**item) for item in data}")
...
* 根拠: [Docstring] (行番号: 989 / 抜粋: "パース失敗時は例外をそのまま送出する（呼び出し側でハンドリングする前提）。")
...
* 根拠: [クラス定数の定義とコメント] (行番号: 893〜901 / 抜粋: "# 2026-09-09 運用障害対応: save_known_castsの一時ファイル書き込み→読み戻し\n    # 検証(D-L8)が、NAS等の一時的な書き込み不良(例: nas_monitor.pyの日次\n    # 保持期間超過ファイル自動削除とのNAS I/O競合)により空ファイル/破損内容と\n    # なって失敗することがある。" / "_SAVE_VERIFY_MAX_ATTEMPTS = 3\n    _SAVE_VERIFY_RETRY_DELAY_SECONDS = 2.0")、[`_retry_transient_write`定義とDocstring] (行番号: 1077〜1128 / 抜粋: "def _retry_transient_write(tmp_path: Path, write_and_verify: Callable[[], None]) -> None:\n        \"\"\"一時ファイルへの書き込み+検証処理を、NAS等の一時的な書き込み不良に\n        備えて最大`_SAVE_VERIFY_MAX_ATTEMPTS`回リトライする共通ヘルパー。")
...
* **戻り値/レスポンス**: `None`。全`_SAVE_VERIFY_MAX_ATTEMPTS`回とも失敗した場合は最後の例外(`OSError`/`ValueError`/`TypeError`)をそのまま`raise`する。
* 根拠: [戻り値ヒントとRaises] (行番号: 1077, 1002〜1007 / 抜粋: "def _retry_transient_write(tmp_path: Path, write_and_verify: Callable[[], None]) -> None:" / "Raises:\n            OSError: 全リトライを使い切ってもファイルI/Oが失敗し続けた場合。")


* **副作用**: `write_and_verify()`の呼び出し。失敗のたびに`tmp_path.unlink(missing_ok=True)`（次の試行前のクリーンアップ）、最終試行以外では警告ログ出力と`time.sleep(_SAVE_VERIFY_RETRY_DELAY_SECONDS)`。**（2026-09-10で変更）** このクリーンアップ用`unlink`自体もNAS I/O不良で`OSError`を送出しうる（コードレビュー指摘）。以前はここが無保護で、`unlink`が失敗すると1回目の試行だけでリトライループ自体が異常終了し、リトライ機構が無効化されていた。他の3箇所（各`save_*`メソッド外側の`except`節の`tmp_path.unlink`）と同じ`try: ... except OSError: pass`で囲み、削除自体の失敗も無視して次の試行へ進めるようにした。
* **エラーハンドリング**: `(OSError, ValueError, TypeError)`を捕捉し、リトライ回数が残っていれば警告ログを出力して再試行、残っていなければループを抜けて最後の例外を`raise`する（呼び出し元の`try`/`except`が最終的なハンドリングを担う）。
* 根拠: [リトライループ] (行番号: 1020〜1035 / 抜粋: "for attempt in range(1, DataManager._SAVE_VERIFY_MAX_ATTEMPTS + 1):\n            try:\n                write_and_verify()\n                return\n            except (OSError, ValueError, TypeError) as e:\n                last_error = e\n                try:\n                    tmp_path.unlink(missing_ok=True)\n                except OSError:\n                    pass" / "raise last_error")


### `DataManager._nas_file_create_lock`（クラス属性）/ `DataManager._write_bytes_and_compare` / `DataManager._backup_and_replace`（2026-09-19 運用障害対応で追加）

* **役割**: 2026-09-18〜19の運用ログで、`save_*`の読み戻し検証が`Expecting value: line 1 column 1 (char 0)`（空）や`Extra data`（書いた内容より長い別の内容）で失敗し（毎回2回目で成功）、さらに`load_known_casts`が破損として`.corrupted-*`へ隔離したファイルの多くはNAS上の実体が正常なJSONだった。実機データを調べると79ファイル中33ファイルに**他サイトのキャスト**が混入していた。実機NAS上の専用ディレクトリでの再現実験で、NASは新規作成ファイルのinode番号(SMBのUniqueId)を作成時刻由来で払い出すため、スレッド並列（Issue #458）で別々のファイルをほぼ同時に作成すると同じinode番号が割り当てられ、Linuxのcifsクライアント(`serverino`)がそれらを1つのinode（ページキャッシュ・ファイルサイズ）として扱うことを確認した（8スレッド並列の作成で約2割の読み戻しが不一致、一時ファイル名の一意化・`fsync`では改善せず、作成の直列化で0件）。読み戻しが「別サイトの正しいJSON」を返すと以前のJSONパース検証をすり抜け、次回の読み込みでもそれが既知キャストとして返りunion保存で混入が定着していた。対策として、`_nas_file_create_lock`（`threading.Lock`。全`DataManager`インスタンス・全ファイル共通）で、`_retry_transient_write`の各試行（書き込み+検証。リトライ待機の`sleep`はロック外）と`_backup_and_replace`（`.bak.tmp`作成→`.bak`へreplace→一時ファイルを本番ファイルへreplace）を直列化する。取得順は常に「`_daily_summary_lock`/`_site_failures_lock`→本ロック」で、逆順の取得は無い。`_write_bytes_and_compare(tmp_path, payload: bytes)`は`payload`を`'wb'`で書き込み、`'rb'`で読み戻したバイト列が完全一致しなければ`ValueError`を送出する。`_backup_and_replace(tmp_path, target_file)`は、以前`save_known_casts`/`save_daily_summary`/`save_site_failures`の3箇所に複製されていた「既存ファイルがあれば`.bak`をアトミック更新（失敗は警告のみ）→一時ファイルで本番ファイルを置き換え」を集約したもの。
* 根拠: [クラス属性の定義とコメント] (行番号: 922 / 抜粋: "_nas_file_create_lock = threading.Lock()")、[`_backup_and_replace`定義] (行番号: 1131 / 抜粋: "def _backup_and_replace(tmp_path: Path, target_file: Path) -> None:")、[`_write_bytes_and_compare`定義] (行番号: 1172 / 抜粋: "def _write_bytes_and_compare(tmp_path: Path, payload: bytes) -> None:")、[`_retry_transient_write`定義] (行番号: 1077 / 抜粋: "def _retry_transient_write(tmp_path: Path, write_and_verify: Callable[[], None]) -> None:")


* **引数/リクエスト**: `_write_bytes_and_compare(tmp_path: Path, payload: bytes)`、`_backup_and_replace(tmp_path: Path, target_file: Path)`
* **戻り値/レスポンス**: いずれも`None`。
* **副作用**: `_write_bytes_and_compare`は一時ファイルの書き込みと読み戻し。`_backup_and_replace`は`.bak.tmp`の作成・`.bak`へのreplace・本番ファイルへのreplace、`.bak`更新失敗時の警告ログと`.bak.tmp`削除。
* **エラーハンドリング**: `_write_bytes_and_compare`は不一致で`ValueError`、I/O失敗で`OSError`を送出する（`_retry_transient_write`がリトライ判断に使う）。`_backup_and_replace`は`.bak`更新の`OSError`を警告ログのみで握りつぶし、本番ファイルへのreplaceの`OSError`は呼び出し元へ送出する。
* **運用上の補足**: 本対策はこのプロセス内の同時作成のみを防ぐ。マウントオプションに`noserverino`を加える（クライアント側でinode番号を採番させる）ことがより根本的な対策候補だが、本PRの範囲外（未検証）。既に混入した他サイトのキャストは`_merge_known_casts`の剪定（`KNOWN_CAST_PRUNE_AFTER_MISSES`回連続で一覧に無いものを除去）で順次消える。

### `DataManager._write_and_verify_tmp` / `_write_and_verify_json_tmp`（2026-09-09 運用障害対応で追加）

* **役割**: `_retry_transient_write`へ渡す実際の「書き込み+読み戻し検証」処理を担う2つの静的メソッド。`_write_and_verify_tmp(tmp_path, data: list)`は`save_known_casts`専用で、`CastMember.to_dict()`のリストを書き込み、`_read_casts_file`で`CastMember`集合として読み戻せることまで検証する（型不整合は`TypeError`/`KeyError`として現れうる）。`_write_and_verify_json_tmp(tmp_path, data: Dict)`は`save_daily_summary`/`save_site_failures`向けの汎用版で、`CastMember`への再構築が不要な単純な辞書を保存するため`json.load`のみで検証する。
* 根拠: [`_write_and_verify_tmp`定義とDocstring] (行番号: 1199〜1218 / 抜粋: "def _write_and_verify_tmp(tmp_path: Path, data: list) -> None:\n        \"\"\"一時ファイルへJSONを書き込み、直後に読み戻して内容を検証する。")、[`_write_and_verify_json_tmp`定義とDocstring] (行番号: 1375〜1397 / 抜粋: "def _write_and_verify_json_tmp(tmp_path: Path, data: Dict) -> None:\n        \"\"\"一時ファイルへJSON(Dict)を書き込み、直後に読み戻して検証する。\n\n        `_write_and_verify_tmp`のDict版。")


* **引数/リクエスト**: いずれも`(tmp_path: Path, data)`（`data`の型はメソッドにより`list`/`Dict`）
* **戻り値/レスポンス**: `None`。失敗時は`OSError`/`ValueError`（`_write_and_verify_tmp`はさらに`TypeError`）をそのまま送出する（`_retry_transient_write`がリトライ判断に使う）。
* 根拠: [戻り値ヒントとRaises] (行番号: 1199, 1034〜1038 / 抜粋: "def _write_and_verify_tmp(tmp_path: Path, data: list) -> None:" / "Raises:\n            OSError: ファイルI/Oに失敗した場合。")、(行番号: 1375, 1230〜1233 / 抜粋: "def _write_and_verify_json_tmp(tmp_path: Path, data: Dict) -> None:")


* **副作用**: **（2026-09-19で変更）** JSONを`json.dumps(..., ensure_ascii=False, indent=2)`でUTF-8バイト列にし、`_write_bytes_and_compare`で書き込み+バイト列完全一致の読み戻し検証を行う（以前は`open(tmp_path, 'w')`+`json.dump`で書き、JSONとしてパースできるかだけを検証していた）。続けて`_write_and_verify_tmp`は`DataManager._read_casts_file(tmp_path)`、`_write_and_verify_json_tmp`は`open(tmp_path, 'r')`+`json.load`による読み戻し検証も従来どおり行う。
* **エラーハンドリング**: いずれも例外を捕捉せずそのまま送出する（呼び出し元の`_retry_transient_write`がリトライ判断とログ出力を担う）。
* 根拠: [処理内容] (行番号: 1206〜1208 / 抜粋: "DataManager._write_bytes_and_compare(\n            tmp_path, json.dumps(data, ensure_ascii=False, indent=2).encode('utf-8')\n        )" / "DataManager._read_casts_file(tmp_path)")、(行番号: 1385〜1391 / 抜粋: "DataManager._write_bytes_and_compare(" / "with open(tmp_path, 'r', encoding='utf-8') as f:\n            json.load(f)")


### `DataManager.save_known_casts`（D-L7・D-L8・2026-09-09 運用障害対応で変更）

* **役割**: 指定サイトのキャスト集合をJSONファイルへアトミックに保存するインスタンスメソッド。一時ファイルへ書き出したのち`replace`で置き換えることで書き込み中断時の既存データ破損/消失を防ぐ従来のアトミック書き込みパターンに加え、(1)一時ファイルを本番ファイルへ置き換える前に`_read_casts_file`で一時ファイルを読み戻して正しくパースできることを検証し、(2)本番ファイルへの置換前に現在の本番ファイルの内容を`.bak`としてバックアップする、という2つの安全策を追加している。**（D-L7で変更）** `.bak`の更新自体も、以前は`backup_path.write_bytes(data_file.read_bytes())`という非アトミックな直接上書きだったが、他の永続化と同じ「`.bak.tmp`へ書き込み→`replace`」パターンに揃えた。書き込み中に中断しても`.bak`本体は直前の内容のまま無傷で残る。**（D-L8で変更）** `tmp_path.replace(data_file)`に到達する前に例外（読み戻し検証失敗等）が発生すると、以前は書き込み済みの`.tmp`ファイルが削除されずディレクトリに残り続けていた。`tmp_path`が生成済みであれば、外側の`except`節でbest-effortに削除する。**（2026-09-09 運用障害対応で変更）** (1)の書き込み+検証は`DataManager._write_and_verify_tmp`へ切り出したうえで、`DataManager._retry_transient_write`（前項参照。`save_daily_summary`/`save_site_failures`と共通）へ委譲し、NAS等の一過性の書き込み不良に備えて`_SAVE_VERIFY_MAX_ATTEMPTS`（既定3回）までリトライする。全リトライ失敗時のみ最後の例外が呼び出し元の`except`節へ伝播し、それ以外の挙動（D-L7の`.bak`更新・D-L8のtmp削除）は変更していない。
* 根拠: [メソッド定義とDocstring] (行番号: 1220〜1264 / 抜粋: "def save_known_casts(self, site: SiteConfig, casts: Set[CastMember]) -> None:\n        """指定サイトのキャストデータをJSONファイルに保存する。")、[`_retry_transient_write`呼び出し] (行番号: 1071〜1073 / 抜粋: "DataManager._retry_transient_write(\n                tmp_path, lambda: DataManager._write_and_verify_tmp(tmp_path, data)\n            )")


* **引数/リクエスト**: `site: SiteConfig`, `casts: Set[CastMember]`（保存対象のキャスト集合）
* 根拠: [引数定義とDocstring] (行番号: 1006〜1008 / 抜粋: "site (SiteConfig): 対象サイトの設定。\n            casts (Set[CastMember]): 保存対象のキャスト集合。")


* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 1220 / 抜粋: "def save_known_casts(self, site: SiteConfig, casts: Set[CastMember]) -> None:")


* **副作用**: 保存先ディレクトリの作成(`mkdir`)、**（2026-09-09で変更）** `DataManager._retry_transient_write(tmp_path, lambda: DataManager._write_and_verify_tmp(tmp_path, data))`の呼び出し（一時ファイル(`.tmp`)への書き込み＋`_read_casts_file`による読み戻し検証を、最大`_SAVE_VERIFY_MAX_ATTEMPTS`回までリトライ制御ごと`_retry_transient_write`側に委譲）、本番ファイルが存在する場合はその内容を`.bak`ファイルへ**（D-L7で変更）** `.bak.tmp`経由のアトミックな`replace`でコピー、`tmp_path.replace(data_file)`によるアトミックな置換、デバッグログ出力。バックアップの更新は最後の`replace`より前に行われるが、これはコメントに明記の通り「万一この途中でプロセスが中断しても本番ファイル(`data_file`)は無傷のまま残る」ようにするための意図的な順序である。 **（2026-09-19で変更）** `.bak`のアトミック更新と本番ファイルへの置き換えは、共通化した`DataManager._backup_and_replace`で`_nas_file_create_lock`を保持したまま行う（処理内容・順序は従来と同じ）。
* 根拠: [処理順序とコメント] (行番号: 1057, 1060〜1063, 1071〜1073, 1075〜1078, 1086〜1089 / 抜粋: "data_file.parent.mkdir(parents=True, exist_ok=True)" / "# アトミック書き込み: 一時ファイルに書き出してから置き換えることで、\n            # 書き込み中断時に既存データが破損/空になるのを防ぐ" / "DataManager._retry_transient_write(\n                tmp_path, lambda: DataManager._write_and_verify_tmp(tmp_path, data)\n            )" / "# コピー元(data_file)は最後のreplaceまで保持したままにすることで、\n            # 万一この途中でプロセスが中断しても本番ファイルは無傷のまま残る。" / "bak_tmp_path = backup_path.with_suffix(backup_path.suffix + '.tmp')\n                try:\n                    bak_tmp_path.write_bytes(data_file.read_bytes())\n                    bak_tmp_path.replace(backup_path)")


* **エラーハンドリング**: **（2026-09-09で変更）** `_retry_transient_write`が全`_SAVE_VERIFY_MAX_ATTEMPTS`回とも失敗した場合にのみ、最後の例外を`raise`して`save_known_casts`側の`except (OSError, ValueError, TypeError) as e:`へ伝播させる（リトライ制御自体の詳細は前々項の`_retry_transient_write`を参照）。この`except`節は`exc_info=True`付きでエラーログを出力する（例外の再送出はしない。D-L8時点から変更なし）。**（D-L8で追加、変更なし）** `tmp_path`が生成済み（`None`でない）であれば`tmp_path.unlink(missing_ok=True)`をbest-effortで実行する（`_retry_transient_write`の各試行内で既に削除済みのため通常はno-op）。`.bak`バックアップファイルへの書き込み（**D-L7で変更**。`bak_tmp_path.write_bytes`＋`replace`）のみが失敗した場合は、内側の`try`/`except OSError`で警告ログを出力し、失敗した`bak_tmp_path`をbest-effortで削除したうえで、後続のアトミック置換自体は中断せず継続する。
* 根拠: [`_retry_transient_write`呼び出しとouter except] (行番号: 1071〜1073, 1098〜1099 / 抜粋: "DataManager._retry_transient_write(\n                tmp_path, lambda: DataManager._write_and_verify_tmp(tmp_path, data)\n            )" / "except (OSError, ValueError, TypeError) as e:\n            logger.error(f"Failed to save data: {e}", exc_info=True)")、[.bak書き込み失敗時の内側try-except] (行番号: 1087〜1093 / 抜粋: "except OSError as e:\n                    logger.warning(f"Failed to update backup file {backup_path}: {e}")\n                    # 中断された.bak用一時ファイルを残さない(best-effort)。\n                    bak_tmp_path.unlink(missing_ok=True)")
...
* 根拠: [メソッド定義とDocstring] (行番号: 1266〜1272 / 抜粋: "def _daily_summary_file(self) -> Path:\n        """日次サマリの集計状態を保存するファイルのパスを返す。")


* **引数/リクエスト**: なし（`self`のみ）
* **戻り値/レスポンス**: `Path`
* 根拠: [戻り値] (行番号: 1272 / 抜粋: "return self.data_dir / 'daily_summary.json'")
...
### `DataManager.save_daily_summary`（Issue #462・2026-09-09 運用障害対応で変更）

* **役割**: 日次サマリの集計状態を、`save_known_casts`と同じ一時ファイル経由のアトミックパターンでJSONファイルに保存するインスタンスメソッド。**（Issue #462で追加）** `save_known_casts`と同様、(1)一時ファイル(`.tmp`)へ書き出した内容を`json.load`で読み戻して正しくパースできることを検証し、(2)本番ファイルへの置換前に、既存の本番ファイルが存在すればその内容を`.bak.tmp`経由のアトミックな`replace`で`.bak`バックアップへ複製・更新する（`load_daily_summary`が破損時の復旧に使う）、という2つの安全策が追加された。読み戻し検証・`replace`到達前に例外が発生した場合、生成済みの`tmp_path`は外側の`except`節でbest-effortに削除される（`save_known_casts`のD-L8修正と同じパターン）。**（2026-09-09 運用障害対応で変更）** (1)の書き込み+検証は`DataManager._write_and_verify_json_tmp`へ切り出したうえで、`save_known_casts`と共通の`DataManager._retry_transient_write`（前々項参照）へ委譲し、NAS等の一過性の書き込み不良に備えて`_SAVE_VERIFY_MAX_ATTEMPTS`（既定3回）までリトライするようになった。以前は`save_known_casts`と同じ検証を持ちながらリトライを持たなかったため、NAS I/O競合等による一過性の空振りが起きるたびに、その時点までに累積していた新規検知件数の保存を諦めていた（次回21時台のサマリ送信での過少報告に繋がる）。
* 根拠: [メソッド定義とDocstring] (行番号: 1399〜1426 / 抜粋: "def save_daily_summary(self, data: Dict) -> None:\n        """日次サマリの集計状態をJSONファイルに保存する。")、[`_retry_transient_write`呼び出し] (行番号: 1256〜1259 / 抜粋: "# 2026-09-09 運用障害対応: save_known_castsと同じ理由でリトライする。\n            DataManager._retry_transient_write(\n                tmp_path, lambda: DataManager._write_and_verify_json_tmp(tmp_path, data)\n            )")


* **引数/リクエスト**: `data: Dict`（保存対象の集計状態）
* 根拠: [引数定義とDocstring] (行番号: 1245〜1246 / 抜粋: "data (Dict): 保存対象の集計状態。")


* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 1399 / 抜粋: "def save_daily_summary(self, data: Dict) -> None:")


* **副作用**: 保存先ディレクトリの作成(`mkdir`)、**（2026-09-09で変更）** `DataManager._retry_transient_write`経由での一時ファイル(`.tmp`)への書き込み＋読み戻し検証(`_write_and_verify_json_tmp`、リトライ込み)、本番ファイルが存在する場合はその内容を`.bak.tmp`経由のアトミックな`replace`で`.bak`ファイルへコピー、`tmp_path.replace(summary_file)`によるアトミックな置換。 **（2026-09-19で変更）** `.bak`のアトミック更新と本番ファイルへの置き換えは、共通化した`DataManager._backup_and_replace`で`_nas_file_create_lock`を保持したまま行う（処理内容・順序は従来と同じ）。
* 根拠: [処理内容] (行番号: 1253〜1259, 1263〜1271, 1273 / 抜粋: "# アトミック書き込み: save_known_castsと同じパターン\n            tmp_path = summary_file.with_suffix(summary_file.suffix + '.tmp')" / "bak_tmp_path.write_bytes(summary_file.read_bytes())\n                    bak_tmp_path.replace(backup_path)")


* **エラーハンドリング**: **（Issue #462で変更）** 例外捕捉が`IOError`単独から`(OSError, ValueError, TypeError)`へ拡張された（一時ファイルの読み戻し検証が送出しうる`ValueError`を含めて捕捉するため。`save_known_casts`と同じタプル）。**（2026-09-09で変更）** `_retry_transient_write`が全`_SAVE_VERIFY_MAX_ATTEMPTS`回とも失敗した場合にのみ、最後の例外がこの`except`節へ伝播する。捕捉時は`exc_info=True`付きでエラーログを出力し（例外の再送出はしない）、`tmp_path`が生成済み（`None`でない）であれば`tmp_path.unlink(missing_ok=True)`をbest-effortで実行して残置を防ぐ（削除自体の失敗も無視する。`_retry_transient_write`の各試行内で既に削除済みのため通常はno-op）。`.bak`バックアップファイルへの書き込みのみが失敗した場合は、内側の`try`/`except OSError`で警告ログを出力し、失敗した`bak_tmp_path`をbest-effortで削除したうえで、後続のアトミック置換自体は中断せず継続する。
* 根拠: [`_retry_transient_write`呼び出しと外側try-except・内側try-except] (行番号: 1256〜1259, 1266〜1271, 1274〜1280 / 抜粋: "DataManager._retry_transient_write(\n                tmp_path, lambda: DataManager._write_and_verify_json_tmp(tmp_path, data)\n            )" / "except OSError as e:\n                    logger.warning(f\"Failed to update backup file {backup_path}: {e}\")\n                    bak_tmp_path.unlink(missing_ok=True)" / "except (OSError, ValueError, TypeError) as e:\n            logger.error(f\"Failed to save daily summary: {e}\", exc_info=True)\n            if tmp_path is not None:\n                try:\n                    tmp_path.unlink(missing_ok=True)\n                except OSError:\n                    pass")


### `DataManager.record_daily_new_casts`

* **役割**: サイト単位で検知した新規キャスト件数を、直近の送信以降の累積集計に加算するインスタンスメソッド（`self._daily_summary_lock`で直列化。#458。**2026-09-10で変更**: 以前は`site_failures.json`用の更新とも共有する単一の`self._shared_file_lock`だったが、`daily_summary.json`専用のロックへ分割された。詳細は8章参照）。cron等により1時間毎に別プロセスとして実行される前提のため、実行毎にファイルを読み書きして状態を永続化する。**（Issue #183で修正）** 以前は集計中の日付が当日と異なる場合（日付が変わった後の最初の検知）に集計をリセットしてから加算していたが、この無条件のカレンダー日付リセットには2つの過少報告経路があった: (1) 21時台のサマリ送信後(22時〜24時)に検知した件数が、送信済みにもかかわらず加算され続けた挙げ句、翌日最初の検知時のリセットでどのサマリにも計上されないまま消える、(2) 21時台に実行自体が無かった日(cron欠落・ロック競合)は日付リセットにより追い付き送信もできずその日の集計が丸ごと失われる。日付によるリセットを廃止し、`_maybe_send_daily_summary`が実際に送信した直後にのみ集計をクリアすることで、未送信の件数が日付をまたいでも必ず次回送信に引き継がれるようにした。**（Issue #578で追加）** `self.load_daily_summary()`の呼び出しを`try/except DataFileUnavailableError`で囲み、送出された場合は`logger.warning`のみ出力して`count`の加算・`save_daily_summary`の呼び出しをスキップし早期`return`する。以前は読み込みに失敗すると（空辞書へフォールバックしたうえで）無条件に`save_daily_summary`まで進んでいたため、たまたま読み込みが一時的なI/Oエラーで失敗しただけでも、累積中だった他サイト分のカウントが空辞書での上書きで丸ごと消えていた。
* 根拠: [メソッド定義とDocstring] (行番号: 1428〜1462 / 抜粋: "def record_daily_new_casts(self, site_id: str, count: int) -> None:\n        \"\"\"サイト単位で検知した新規キャスト件数を、直近の送信以降の累積集計に加算する。")、except節 (行番号: 1179〜1186 / 抜粋: "try:\n                data = self.load_daily_summary()\n            except DataFileUnavailableError as e:\n                # #578: 読み込めない状態のまま保存すると累積中の他サイト分の\n                # カウントを消してしまうため、今回のcount加算は諦めて既存の\n                # 永続化状態に触れない" / "logger.warning(f\"Skipping daily summary update for site '{site_id}': {e}\")\n                return")
...
* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 1428 / 抜粋: "def record_daily_new_casts(self, site_id: str, count: int) -> None:")
...
* 根拠: [メソッド定義] (行番号: 1464〜1469 / 抜粋: "def _site_failures_file(self) -> Path:\n        \"\"\"サイト別の連続巡回失敗状態を保存するファイルのパスを返す。" / "return self.data_dir / 'site_failures.json'")
* **`_is_valid_site_failures(data: object) -> bool`**（2026-09-09 運用障害対応で追加）: `load_daily_summary`の`_is_valid_daily_summary`と同じ役割の静的メソッド。`site_failures.json`の内容が最低限「site_idをキーとする辞書」という形状かどうかだけを判定する（`isinstance(data, dict)`）。各エントリの値がdictかどうかの検証・フィルタ（`{"site": 5}`のような混入エントリの除外。Issue #395）は、一部サイトのみ不正でも他サイト分の記録を活かすため本メソッドでは行わず、`_filter_valid_site_failure_entries`（次項）へ分離している。
* 根拠: [定義とDocstring] (行番号: 1472〜1478 / 抜粋: "def _is_valid_site_failures(data: object) -> bool:\n        \"\"\"site_failures.json の内容が最低限「site_idをキーとする辞書」という\n        形状かを判定する静的メソッド")
* **`load_site_failures() -> Dict`**（2026-09-09 運用障害対応で変更）: 状態を読み込む。ファイルが存在しない場合は空辞書を返す。**（変更前）** 以前は内容破損（トップレベルが辞書でない、`_CONTENT_ERRORS`）を検知しても直ちに空辞書を返すのみで、`load_known_casts`/`load_daily_summary`が持つ「破損ファイルを`.corrupted-*`へ隔離し`.bak`から復旧する」経路を持たなかった。このため同じ破損ファイルへの読み込み失敗が巡回のたびに繰り返され続け、かつ`save_site_failures`が書き込むようになった`.bak`（次々項参照）が一切読まれないままだった。**（変更後）** `load_daily_summary`と同じ隔離+バックアップ復旧を適用した: `_is_valid_site_failures`で検証し、不正であれば（`_CONTENT_ERRORS`発生時も同様に）`.corrupted-{タイムスタンプ}`へリネームして隔離したうえで、`.bak`が存在すればそちらも同じ検証を経て読み込みを試みる。一次ファイル・`.bak`いずれで有効と判定された場合も、`_filter_valid_site_failure_entries`（次項）でエントリ単位の妥当性検証・フィルタ（Issue #395）を適用してから返す。**（Issue #578で変更、維持）** `DataManager._CONTENT_ERRORS`（`ValueError`/`TypeError`/`KeyError`。非UTF-8破損の`UnicodeDecodeError`を含む）は監視処理本体を止めず上記の隔離+復旧経路へ進む一方、`OSError`（NAS/CIFSの瞬断等。一次ファイル・`.bak`いずれでも）は空辞書へフォールバックせず`DataFileUnavailableError`を送出し、呼び出し元に保存処理のスキップを求める（従来通り。以前はこの`OSError`も他の内容起因の破損と同列に扱い空辞書を返しており、呼び出し元がその空状態のまま`save_site_failures`を呼ぶと79サイト分の状態が丸ごと消え、既にアラート済みのサイトが再アラートされていた）。
* 根拠: [Docstringとメソッド定義] (行番号: 1334〜1358 / 抜粋: "2026-09-09 運用障害対応: 以前は内容破損時に空辞書を返すのみで、\n        load_known_casts/load_daily_summaryが持つ「破損ファイルを`.corrupted-*`へ\n        隔離し`.bak`バックアップから復旧する」経路を持たなかった。")、[隔離・復旧処理] (行番号: 1391〜1417 / 抜粋: "quarantine_path = failures_file.with_name(\n            f\"{failures_file.name}.corrupted-{datetime.now():%Y%m%d%H%M%S}\"\n        )" / "backup_file = failures_file.with_suffix(failures_file.suffix + '.bak')\n        if backup_file.exists():")
* **`_filter_valid_site_failure_entries(data: Dict, source_file: Path) -> Dict`**（2026-09-09 運用障害対応で追加。旧`load_site_failures`内のインライン処理を分離）: トップレベルは辞書だが個々のエントリの値がdictでない場合を弾く静的メソッド（Issue #395。`{"site": 5}`混入時の`AttributeError`対策）。不正なエントリは警告ログとともに読み飛ばし、正常なエントリのみを返す。一次ファイル・`.bak`のいずれから読み込んだ場合でも共通で呼ばれる（`source_file`はログ出力用）。
* 根拠: [定義とDocstring] (行番号: 1568〜1587 / 抜粋: "def _filter_valid_site_failure_entries(data: Dict, source_file: Path) -> Dict:\n        \"\"\"トップレベルは辞書だが個々のエントリの値がdictでない場合を弾く(#395)。")
* **`save_site_failures(data: Dict) -> None`**（2026-09-09 運用障害対応で変更）: **（変更前）** 以前は一時ファイルへの書き込み(`open`+`json.dump`)と`tmp_path.replace(failures_file)`のみで、書き込み後の読み戻し検証・`.bak`バックアップ・失敗時のtmp削除のいずれも持たず、`save_known_casts`/`save_daily_summary`より無防備だった（例外捕捉も`IOError`単独で`ValueError`/`TypeError`を捕捉しなかった）。**（変更後）** 他の2つと同水準の安全策一式を追加した: (1)`DataManager._retry_transient_write`経由で`_write_and_verify_json_tmp`（書き込み+`json.load`による読み戻し検証）を最大`_SAVE_VERIFY_MAX_ATTEMPTS`回リトライ、(2)本番ファイルが存在すればその内容を`.bak.tmp`経由のアトミックな`replace`で`.bak`へバックアップ（`save_daily_summary`と同じパターン。前項の通り`load_site_failures`もこの`.bak`から復旧できるようになった）、(3)例外捕捉を`(OSError, ValueError, TypeError)`へ拡張し、失敗時は`tmp_path`をbest-effortで削除する（D-L8と同じ）。 **（2026-09-19で変更）** `.bak`のアトミック更新と本番ファイルへの置き換えは、共通化した`DataManager._backup_and_replace`で`_nas_file_create_lock`を保持したまま行う（処理内容・順序は従来と同じ）。
* 根拠: [Docstringと処理内容] (行番号: 1443〜1487 / 抜粋: "2026-09-09 運用障害対応: 以前は書き込んだ内容の読み戻し検証・`.bak`\n        バックアップ・失敗時の一時ファイル削除のいずれも持たず、`save_known_casts`/\n        `save_daily_summary`より無防備だった" / "DataManager._retry_transient_write(\n                tmp_path, lambda: DataManager._write_and_verify_json_tmp(tmp_path, data)\n            )" / "except (OSError, ValueError, TypeError) as e:\n            logger.error(f\"Failed to save site failures: {e}\", exc_info=True)")
...
* 根拠: [メソッド定義とexcept節] (行番号: 1646〜1662 / 抜粋: "def mark_site_failure_alerted(self, site_id: str) -> None:" / "except DataFileUnavailableError as e:\n                # #578: 読めない状態のまま保存すると他サイト分の状態を消してしまうため\n                # スキップする" / "logger.warning(f\"Skipping alerted-flag update for '{site_id}': {e}\")\n                return")
...
* 根拠: [クラス定義とDocstringおよび__init__] (行番号: 1698〜2024 / 抜粋: "class WebMonitor:\n    """Webサイトの監視とスクレイピングを統括するクラス。"""\n\n    def __init__(self):\n        """HTTPセッションの初期化を行う。"""\n        self.session = self._create_robust_session()")


* **引数/リクエスト**: なし（`self`のみ）
* **戻り値/レスポンス**: 該当なし
* **副作用**: `self.session`への`_create_robust_session()`結果の代入。
* 根拠: [属性代入] (行番号: 1703 / 抜粋: "self.session = self._create_robust_session()")
...
* 根拠: [メソッド定義とDocstring] (行番号: 1705〜1722 / 抜粋: "def _create_robust_session(self) -> requests.Session:\n        """リトライロジックを組み込んだ堅牢なHTTPセッションを作成する。")


* **引数/リクエスト**: なし（`self`のみ）
* 根拠: [引数定義] (行番号: 1705 / 抜粋: "def _create_robust_session(self) -> requests.Session:")
...
* 根拠: [メソッド定義とDocstring・リダイレクト判定] (行番号: 1724〜1765 / 抜粋: "def fetch_current_casts(self, site: SiteConfig) -> Set[CastMember]:\n        """指定サイトのターゲットURLから現在のキャスト一覧を取得する。" / "SiteUnavailableError: 最終応答のドメインが target_url と異なる場合" / "if response.url and _normalized_netloc(response.url) != _normalized_netloc(site.target_url):\n                raise SiteUnavailableError(")、[_normalized_netloc定義] (行番号: 1688〜1695 / 抜粋: "def _normalized_netloc(url: str) -> str:\n    \"\"\"URLのドメイン部分を比較用に正規化する(小文字化し先頭の 'www.' を除去)。")
...
* 根拠: [各メソッド定義とDocstring] (行番号: 1768〜1800, 1845〜1923, 1926〜1953 / 抜粋: "def _extract_raw_name(div: Tag, site: SiteConfig) -> Tuple[str, Optional[Tag]]:", "def _extract_cast_link_and_id(div: Tag, site: SiteConfig, name: str) -> Tuple[str, str]:", "def _extract_cast_image_url(div: Tag, site: SiteConfig) -> str:")
...
* 根拠: [引数定義とDocstring] (行番号: 1803, 1459 / 抜粋: "def _extract_cast_age(name_elem: Optional[Tag]) -> str:" / "name_elem (Optional[Tag]): 名前要素（存在しない場合はNone）。")
...
* 根拠: [メソッド定義とDocstring] (行番号: 1955〜2019 / 抜粋: "def _parse_html(self, soup: BeautifulSoup, site: SiteConfig) -> Set[CastMember]:\n        """HTMLスープからキャスト情報を抽出する。")、[ヘルパー呼び出し] (行番号: 1977, 2000〜2002 / 抜粋: "name, name_elem = self._extract_raw_name(div, site)", "age = self._extract_cast_age(name_elem)\n                detail_url, cast_id = self._extract_cast_link_and_id(div, site, name)\n                image_url = self._extract_cast_image_url(div, site)")
...
* **役割**: 保持しているHTTPセッションのリソースを明示的に解放する。
* 根拠: [メソッド定義とDocstring] (行番号: 2021〜2024 / 抜粋: "def close(self):\n        """リソースを明示的に解放する。"""\n        if self.session:\n            self.session.close()")
...
* 根拠: (行番号: 2046〜2101 / 抜粋: "def _handle_site_network_failure(\n    site: SiteConfig,\n    exc: Exception,\n    data_manager: DataManager,")
...
* 根拠: [return] (行番号: 2101 / 抜粋: "return count if (threshold_reached and not alerted) else None")
...
* 根拠: (行番号: 2176 / 抜粋: "return merged, pruned")
...
* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 2295 / 抜粋: "def _maybe_send_daily_summary(notifier: DiscordNotifier, data_manager: DataManager) -> None:")
...
* 根拠: [関数定義とDocstring] (行番号: 2356〜2372 / 抜粋: "def run_monitor() -> None:\n    """モニタープロセスのエントリポイント。多重起動防止ロックを取得してから本処理を実行する。"""")


* **引数/リクエスト**: なし
* 根拠: [引数定義] (行番号: 2356 / 抜粋: "def run_monitor() -> None:")


* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 2356 / 抜粋: "def run_monitor() -> None:")
...
flowchart TD
    ...
    subgraph SKC["DataManager.save_known_casts"]
        ...
        SMkdir --> STmpWrite["_write_and_verify_tmp:<br>一時ファイル(.tmp)へJSON書き込み<br>+ _read_casts_file(tmp_path)で読み戻し検証<br>(2026-09-09対応: 最大_SAVE_VERIFY_MAX_ATTEMPTS=3回試行)"]
        STmpWrite -- "検証失敗<br>(OSError/ValueError/TypeError)<br>かつ 再試行回数が残っている" --> SRetryWarn["警告ログ出力<br>tmp_path削除<br>_SAVE_VERIFY_RETRY_DELAY_SECONDS(2秒)待機"] --> STmpWrite
        STmpWrite -- "検証失敗<br>かつ 最終試行だった" --> SErrLog["エラーログ出力(exc_info=True)<br>tmp_path削除<br>replaceせず終了"]
        STmpWrite -- 成功(いずれかの試行で) --> SBakCheck{"data_file.exists()?"}
...
| 優先度 | ファイル名(推測可) | 理由 | 根拠 |
| --- | --- | --- | --- |
| 高 | `core/nas_utils.py` | `get_managed_target_directory`の実際の実装（NASマウント確認・自動修復ロジック）が、フォールバック実装（`fallback_dir_str`をそのまま返すのみ）とどう異なるかを確認する必要があるため。 | 根拠: [import文] (行番号: 64 / 抜粋: "from core.nas_utils import get_managed_target_directory") |
| 中 | `core/utils.py` | `wait_for_storage_warmup`の実際の実装が、フォールバック実装（Exponential Backoffでのテストファイル書き込み確認）と同等かどうかを確認するため。 | 根拠: [import文] (行番号: 65 / 抜粋: "from core.utils import wait_for_storage_warmup") |
| 中 | `core/logger.py` | `get_logger`の実際の実装（出力フォーマット、ログレベル、出力先）を確認するため。 | 根拠: [import文] (行番号: 63 / 抜粋: "from core.logger import get_logger") |
...
* **[修正済み・Issue #458] 79サイトを1プロセスで逐次処理する構成**: 以前は`run_monitor`が`MonitorConfig.SITES`の全79件を単一プロセス内で順次処理しており、1回の実行時間がサイト数に比例して増大していた。現在は`_run_monitor_locked`が`ThreadPoolExecutor`（`MonitorConfig.SITE_CHECK_MAX_WORKERS`、既定8）でサイト処理を並列化している。79並列で一斉アクセスするとWAF誤検知や対象サイトへの負荷増大を招くため、同時実行数は小さく制限している。並列化に伴い、`DataManager`が読み書きするサイト横断の共有ファイル(`daily_summary.json`/`site_failures.json`)への読み込み→更新→書き込みが複数スレッドから同時に走りうるようになったため、`DataManager.__init__`で生成する`threading.Lock`で`record_daily_new_casts`/`record_site_failure`/`mark_site_failure_alerted`/`clear_site_failure`の各メソッド全体を直列化している（サイト単位の`known_casts`ファイルはサイトごとに別ファイルのためこのロックの対象外）。**（2026-09-10で変更）** 当初は`daily_summary.json`/`site_failures.json`両方を単一の`_shared_file_lock`一本で直列化していたが、`_retry_transient_write`の追加により、NAS I/O不良時は最大`_SAVE_VERIFY_MAX_ATTEMPTS`回×`_SAVE_VERIFY_RETRY_DELAY_SECONDS`秒のリトライ待機をロック保持したまま行うようになったため、単一ロックのままだと例えば`site_failures.json`側のリトライ中、無関係な`daily_summary.json`の更新まで巻き添えでブロックされてしまう問題があった（コードレビュー指摘）。`daily_summary.json`用の`_daily_summary_lock`と`site_failures.json`用の`_site_failures_lock`の2つに分割し、`record_daily_new_casts`は前者を、`record_site_failure`/`mark_site_failure_alerted`/`clear_site_failure`は後者を使うようにした。同一ファイルへの複数スレッドからの読み込み→更新→書き込み競合を防ぐという当初の目的は変わらず維持される（同じファイルを操作する呼び出し同士は引き続き同じロックで直列化される）。各サイト間の待機（`fetch_current_casts`内の`time.sleep(random.uniform(1.0, 3.0))`）自体は変更していない。
...
* **（2026-09-09 運用障害対応で追加）`_retry_transient_write`によるリトライは書き込み自体のみを再試行し、`_check_site`側の状態は再評価しない**: `save_known_casts`/`save_daily_summary`/`save_site_failures`が共通で使う`DataManager._retry_transient_write`は、`_SAVE_VERIFY_MAX_ATTEMPTS`回のリトライで「一時ファイルへの書き込み→読み戻し検証」だけを再試行し、全リトライが失敗した場合の挙動（既存ファイルを保持したまま当該保存を諦める。D-L8）自体はいずれのメソッドでも変更していない。したがって、`save_known_casts`でリトライを使い切ってなお失敗した場合、その回にDiscord通知済みの新規キャストは引き続き「既知」として保存されず、次回の巡回で重複通知される可能性がある（Issue #531の「送信できなかったキャストは既知に保存しない」仕様と同じ扱い）。`save_daily_summary`で使い切った場合はその時点までの累積件数の保存が見送られ、`save_site_failures`で使い切った場合は当該サイトの連続失敗カウント更新が失われる。リトライ回数・待機時間（既定3回・2秒間隔）は経験的な値であり、NAS障害が2秒間隔の3回を超えて継続する場合はこのリトライでは救えない。今後この保存経路に手を加える際は、`_write_and_verify_tmp`/`_write_and_verify_json_tmp`（書き込み+検証の単位）と`_retry_transient_write`（リトライ制御）、各`save_*`メソッド（バックアップ＋置換）という3層の責務分離を崩さないこと。回帰テストは`test_newface_monitor_datamanager.py::TestSaveKnownCastsBackup::test_verification_retries_and_recovers_from_transient_failure`/`test_verification_failure_does_not_leave_tmp_file_behind`（`save_known_casts`）、`TestSaveDailySummaryRetry`の2件（`save_daily_summary`）、`test_newface_monitor_site_failures.py::TestSaveSiteFailuresHardening`の3件（`save_site_failures`）。
* **（2026-09-09 運用障害対応で解消）`load_known_casts`/`load_daily_summary`/`load_site_failures`は全て同じ隔離+`.bak`復旧の仕組みを持つ**: 当初`save_site_failures`が`.bak`を書き込むようになった時点では、`load_site_failures`側にそれを読む「内容破損時に`.corrupted-*`へ隔離し`.bak`から復旧を試みる」経路（`load_daily_summary`が持つもの）が無く、「バックアップは作られるが使われない」非対称な状態だった。`load_daily_summary`の実装（`_is_valid_daily_summary`相当の形状検証を経て隔離+`.bak`復旧へ進む構造）をほぼそのまま移植し、`load_site_failures`にも同じ経路を追加した（`_is_valid_site_failures`で形状検証、`_filter_valid_site_failure_entries`でエントリ単位のフィルタ。Issue #395の「一部サイトのみ不正でも他サイト分は活かす」性質は維持）。これで3つの`load_*`メソッドは同水準の耐障害性を持つ。今後4つ目の永続化ファイルを追加する場合も、`_write_and_verify_*`/`_retry_transient_write`（書き込み側）とこの隔離+`.bak`復旧パターン（読み込み側）の両方を揃えること。回帰テストは`test_newface_monitor_site_failures.py::TestLoadSiteFailuresRecoversFromBackup`の3件。
...
* 根拠: [DataFileUnavailableError定義] (行番号: 758〜769)、[load_known_castsの.bak側OSError分岐] (行番号: 928〜940 / 抜粋: "# #578: .bak自体はCIFS/autofsの瞬断で開けなかっただけの可能性があり、" / "raise KnownCastsUnavailableError(")、[load_daily_summaryのOSError分岐] (行番号: 1048〜1059, 1090〜1094)、[load_site_failuresのOSError分岐(一次ファイル・.bak双方)] (行番号: 1371〜1384, 1411〜1415)、[5つの呼び出し元のexcept節] (行番号: 1179〜1186, 1968〜1974, 1283〜1287, 1300〜1306, 1321〜1328)
