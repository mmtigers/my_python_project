...
### `DataManager._SAVE_VERIFY_MAX_ATTEMPTS` / `_SAVE_VERIFY_RETRY_DELAY_SECONDS`（クラス定数）/ `DataManager._write_and_verify_tmp`（2026-09-09 運用障害対応で追加）

* **役割**: 運用ログで、`save_known_casts`の一時ファイル書き込み→読み戻し検証(D-L8)が`json.decoder.JSONDecodeError: Expecting value: line 1 column 1 (char 0)`（空ファイル相当の内容）で失敗する事象が発生した。NAS等の一時的な書き込み不良（例: `nas_monitor.py`の日次保持期間超過ファイル自動削除とのNAS I/O競合）による一過性の空振りと推測され、当時の実装では即座に諦めて既存の`known_casts`ファイルは保持されるため（安全機構自体は正しく機能）データ破損には至らないが、その回にDiscord通知済みの新規キャストが「既知」として保存されず、次回の巡回で重複通知される可能性があった。書き込み(`open`+`json.dump`)と読み戻し検証(`_read_casts_file`)を`_write_and_verify_tmp`という単一の静的メソッドへ切り出し、`save_known_casts`側でこれを`_SAVE_VERIFY_MAX_ATTEMPTS`（既定3回）まで、失敗のたびに`_SAVE_VERIFY_RETRY_DELAY_SECONDS`（既定2秒）待機してリトライするようにした。全リトライが失敗した場合のみ最後の例外を外側の`except`節へ伝播させ、従来通り（D-L8）既存データを保持したまま保存を諦める。
* 根拠: [クラス定数の定義とコメント] (行番号: 830〜838 / 抜粋: "# 2026-09-09 運用障害対応: save_known_castsの一時ファイル書き込み→読み戻し\n    # 検証(D-L8)が、NAS等の一時的な書き込み不良(例: nas_monitor.pyの日次\n    # 保持期間超過ファイル自動削除とのNAS I/O競合)により空ファイル/破損内容と\n    # なって失敗することがある。" / "_SAVE_VERIFY_MAX_ATTEMPTS = 3\n    _SAVE_VERIFY_RETRY_DELAY_SECONDS = 2.0")、[`_write_and_verify_tmp`定義とDocstring] (行番号: 982〜1001 / 抜粋: "def _write_and_verify_tmp(tmp_path: Path, data: list) -> None:\n        \"\"\"一時ファイルへJSONを書き込み、直後に読み戻して内容を検証する。")


* **引数/リクエスト**: `_write_and_verify_tmp(tmp_path: Path, data: list)`
* **戻り値/レスポンス**: `None`。失敗時は`OSError`/`ValueError`/`TypeError`をそのまま送出する（呼び出し元の`save_known_casts`がリトライ判断に使う）。
* 根拠: [戻り値ヒントとDocstringのRaises] (行番号: 983, 990〜994 / 抜粋: "def _write_and_verify_tmp(tmp_path: Path, data: list) -> None:" / "Raises:\n            OSError: ファイルI/Oに失敗した場合。\n            ValueError: 書き込んだ内容が正しいJSONとして読み戻せなかった場合")


* **副作用**: `open(tmp_path, 'w')`によるJSON書き込みと、`DataManager._read_casts_file(tmp_path)`による読み戻し検証（`_write_and_verify_tmp`自体）。呼び出し元`save_known_casts`側では、失敗のたびに`tmp_path.unlink(missing_ok=True)`、最終試行以外では警告ログ出力と`time.sleep(_SAVE_VERIFY_RETRY_DELAY_SECONDS)`。
* **エラーハンドリング**: `_write_and_verify_tmp`自体は例外を捕捉せずそのまま送出する（Docstring参照）。呼び出し元の`save_known_casts`が`(OSError, ValueError, TypeError)`を捕捉し、リトライ回数が残っていれば警告ログを出力して再試行、残っていなければ最後の例外を外側の`except`節（従来のD-L8ハンドリング）へ伝播させる。
* 根拠: [リトライループ] (行番号: 1027〜1044 / 抜粋: "for attempt in range(1, DataManager._SAVE_VERIFY_MAX_ATTEMPTS + 1):\n                try:\n                    DataManager._write_and_verify_tmp(tmp_path, data)\n                    last_error = None\n                    break\n                except (OSError, ValueError, TypeError) as e:" / "if last_error is not None:\n                raise last_error")


### `DataManager.save_known_casts`（D-L7・D-L8・2026-09-09 運用障害対応で変更）

* **役割**: 指定サイトのキャスト集合をJSONファイルへアトミックに保存するインスタンスメソッド。一時ファイルへ書き出したのち`replace`で置き換えることで書き込み中断時の既存データ破損/消失を防ぐ従来のアトミック書き込みパターンに加え、(1)一時ファイルを本番ファイルへ置き換える前に`_read_casts_file`で一時ファイルを読み戻して正しくパースできることを検証し、(2)本番ファイルへの置換前に現在の本番ファイルの内容を`.bak`としてバックアップする、という2つの安全策を追加している。**（D-L7で変更）** `.bak`の更新自体も、以前は`backup_path.write_bytes(data_file.read_bytes())`という非アトミックな直接上書きだったが、他の永続化と同じ「`.bak.tmp`へ書き込み→`replace`」パターンに揃えた。書き込み中に中断しても`.bak`本体は直前の内容のまま無傷で残る。**（D-L8で変更）** `tmp_path.replace(data_file)`に到達する前に例外（読み戻し検証失敗等）が発生すると、以前は書き込み済みの`.tmp`ファイルが削除されずディレクトリに残り続けていた。`tmp_path`が生成済みであれば、外側の`except`節でbest-effortに削除する。**（2026-09-09 運用障害対応で変更）** (1)の書き込み+検証は`DataManager._write_and_verify_tmp`へ切り出したうえで、NAS等の一過性の書き込み不良に備えて`_SAVE_VERIFY_MAX_ATTEMPTS`（既定3回）までリトライするループでラップされた（前項参照）。全リトライ失敗時のみ最後の例外を外側の`except`節へ伝播させ、それ以外の挙動（D-L7の`.bak`更新・D-L8のtmp削除）は変更していない。
* 根拠: [メソッド定義とDocstring] (行番号: 1003〜1009 / 抜粋: "def save_known_casts(self, site: SiteConfig, casts: Set[CastMember]) -> None:\n        """指定サイトのキャストデータをJSONファイルに保存する。")、[リトライループ] (行番号: 1027〜1044)
...
* **副作用**: 保存先ディレクトリの作成(`mkdir`)、**（2026-09-09で変更）** `DataManager._write_and_verify_tmp`（一時ファイル(`.tmp`)への書き込み＋`_read_casts_file`による読み戻し検証）の呼び出しを、失敗のたびの警告ログ・`tmp_path`削除・`time.sleep`を挟みつつ最大`_SAVE_VERIFY_MAX_ATTEMPTS`回まで実行、本番ファイルが存在する場合はその内容を`.bak`ファイルへ**（D-L7で変更）** `.bak.tmp`経由のアトミックな`replace`でコピー、`tmp_path.replace(data_file)`によるアトミックな置換、デバッグログ出力。バックアップの更新は最後の`replace`より前に行われるが、これはコメントに明記の通り「万一この途中でプロセスが中断しても本番ファイル(`data_file`)は無傷のまま残る」ようにするための意図的な順序である。
* 根拠: [処理順序とコメント] (行番号: 1013, 1016〜1019, 1027〜1044, 1046〜1048, 1057〜1060 / 抜粋: "data_file.parent.mkdir(parents=True, exist_ok=True)" / "# アトミック書き込み: 一時ファイルに書き出してから置き換えることで、\n            # 書き込み中断時に既存データが破損/空になるのを防ぐ" / "for attempt in range(1, DataManager._SAVE_VERIFY_MAX_ATTEMPTS + 1):" / "# コピー元(data_file)は最後のreplaceまで保持したままにすることで、\n            # 万一この途中でプロセスが中断しても本番ファイルは無傷のまま残る。" / "bak_tmp_path = backup_path.with_suffix(backup_path.suffix + '.tmp')\n                try:\n                    bak_tmp_path.write_bytes(data_file.read_bytes())\n                    bak_tmp_path.replace(backup_path)")


* **エラーハンドリング**: **（2026-09-09で変更）** `_write_and_verify_tmp`の呼び出しは内側の`for`ループ内で`(OSError, ValueError, TypeError)`を捕捉し、`tmp_path`をbest-effortで削除したうえで、最終試行でなければ警告ログを出力して`time.sleep(_SAVE_VERIFY_RETRY_DELAY_SECONDS)`後に再試行する。全`_SAVE_VERIFY_MAX_ATTEMPTS`回とも失敗した場合のみ、最後の例外(`last_error`)を`raise`して外側の`except`節へ伝播させる。外側の`except (OSError, ValueError, TypeError) as e:`は`exc_info=True`付きでエラーログを出力する（例外の再送出はしない。D-L8時点から変更なし）。**（D-L8で追加、変更なし）** この外側`except`節では、`tmp_path`が生成済み（`None`でない）であれば`tmp_path.unlink(missing_ok=True)`をbest-effortで再度実行する（内側ループで既に削除済みのため通常はno-op）。`.bak`バックアップファイルへの書き込み（**D-L7で変更**。`bak_tmp_path.write_bytes`＋`replace`）のみが失敗した場合は、内側の`try`/`except OSError`で警告ログを出力し、失敗した`bak_tmp_path`をbest-effortで削除したうえで、後続のアトミック置換自体は中断せず継続する。
* 根拠: [内側リトライループとouter except] (行番号: 1027〜1044, 1069〜1074 / 抜粋: "except (OSError, ValueError, TypeError) as e:\n                    last_error = e\n                    tmp_path.unlink(missing_ok=True)\n                    if attempt < DataManager._SAVE_VERIFY_MAX_ATTEMPTS:" / "if last_error is not None:\n                raise last_error" / "except (OSError, ValueError, TypeError) as e:\n            logger.error(f"Failed to save data: {e}", exc_info=True)")、[.bak書き込み失敗時の内側try-except] (行番号: 1058〜1064 / 抜粋: "except OSError as e:\n                    logger.warning(f"Failed to update backup file {backup_path}: {e}")\n                    # 中断された.bak用一時ファイルを残さない(best-effort)。\n                    bak_tmp_path.unlink(missing_ok=True)")
...
* **（2026-09-09 運用障害対応で追加）`save_known_casts`のリトライは書き込み自体のみを再試行し、`_check_site`側の状態は再評価しない**: `_SAVE_VERIFY_MAX_ATTEMPTS`回のリトライで対処するのは`DataManager._write_and_verify_tmp`の書き込み→読み戻し検証のみであり、全リトライが失敗した場合の挙動（既存の`known_casts`ファイルを保持したまま当該サイトの保存を諦める。D-L8）自体は変更していない。したがって、リトライを使い切ってなお失敗した場合、その回にDiscord通知済みの新規キャストは引き続き「既知」として保存されず、次回の巡回で重複通知される可能性がある（Issue #531の「送信できなかったキャストは既知に保存しない」仕様と同じ扱い）。リトライ回数・待機時間（既定3回・2秒間隔）は経験的な値であり、NAS障害が2秒間隔の3回を超えて継続する場合はこのリトライでは救えない。今後この保存経路に手を加える際は、`_write_and_verify_tmp`（書き込み+検証の単位）と`save_known_casts`（リトライ制御＋バックアップ＋置換）の責務分離を崩さないこと。回帰テストは`test_newface_monitor_datamanager.py::TestSaveKnownCastsBackup::test_verification_retries_and_recovers_from_transient_failure`（リトライで回復するケース）・`test_verification_failure_does_not_leave_tmp_file_behind`（リトライを使い切って諦めるケース）。
