...
* **（2026-09-09 運用障害対応で追加）`save_site_failures`が書く`.bak`は`load_site_failures`からまだ読まれない**: `save_site_failures`は`save_known_casts`/`save_daily_summary`と同水準まで安全策を揃えたが、`.bak`バックアップからの復旧経路（`load_daily_summary`が持つ「内容破損時に`.corrupted-*`へ隔離し`.bak`を試す」フロー）は`load_site_failures`側にまだ実装していない。現状`site_failures.json`が内容破損した場合、`load_site_failures`は（`.bak`が存在しても使わずに）空辞書を返すのみである（実害は連続失敗カウントのリセット程度で、既存の`_LOAD_ERRORS`系フォールバックと同じ安全側の挙動）。`load_site_failures`に隔離+`.bak`復旧を追加する場合は、`load_daily_summary`の実装（`_is_valid_daily_summary`相当の形状検証を伴う）をそのまま移植できるはずである。
