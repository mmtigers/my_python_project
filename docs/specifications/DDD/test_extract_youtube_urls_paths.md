...
* 根拠: [モジュールレベルのセットアップ] (行番号: 26〜29 / 抜粋: "DDD_DIR = Path(__file__).resolve().parent\nsys.path.insert(0, str(DDD_DIR))\n\nimport extract_youtube_urls as module  # noqa: E402")
* 2件の単体テスト関数（`PROJECT_ROOT`の解決先検証、`core.logger`のインポート可否検証）と、3件のテストクラス（フォールバックスタブの引数尊重を検証する`TestFallbackStubRespectsExplicitPath`、`_verify_environment`のフォールバック検知を検証する`TestVerifyEnvironmentDetectsFallback`、（Issue #123回帰テストとして追加された）`SubscriptionManager.process_subscriptions`のNAS状態再評価タイミングを検証する`TestProcessSubscriptionsUsesFreshDbPath`）で構成される。
* 根拠: [各テスト定義] (行番号: 32, 39, 46, 96, 120 / 抜粋: "def test_project_root_points_to_my_home_system_not_repo_root():")
...
* 根拠: [_reloaded_with_core_unavailable Docstring] (行番号: 55〜61 / 抜粋: "core.logger/core.nas_utilsのimportだけをImportErrorにしてモジュールを\n        再読込する。sys.pathからPROJECT_ROOTを除去するだけでは、モジュール自身の\n        トップレベルコードが `if str(PROJECT_ROOT) not in sys.path: sys.path.append(...)`\n        で毎回再追加してしまい、exceptブロックに到達できないため、\n        importそのものをブロックする。")
* **(Issue #123回帰テストとして追加)** `TestProcessSubscriptionsUsesFreshDbPath`は、`SubscriptionManager`構築時点ではNASがフォールバック中で、`process_subscriptions()`実行時までにNASが復帰する、という時間経過を`_RecoverableBaseDir`という状態を持つコーラブル（`recovered`フラグを持ち、`AppConfig.get_output_base_dir`のパッチの`side_effect`として渡される）でシミュレートし、NAS側に事前登録しておいたサブスクリプションが正しく読み込まれること（＝ローカルの空DBが使われないこと）を検証する。
* 根拠: [クラス定義とDocstring] (行番号: 120〜131 / 抜粋: "class TestProcessSubscriptionsUsesFreshDbPath:\n    """\n    Issue #123回帰テスト: アプリ起動(SubscriptionManager構築)時点ではNASが\n    フォールバック中でも、process_subscriptions()の実行時までにNASが復帰して\n    いれば(autofsの再マウント遅延はこのリポジトリで既知の事象)、NAS側の\n    home_system.dbを正しく参照してサブスクリプションを処理できること。")
* `if __name__ == "__main__":`ブロックにより、`pytest`未経由でも`python test_extract_youtube_urls_paths.py`のように直接実行可能である。
* 根拠: [エントリーポイント] (行番号: 195〜196 / 抜粋: "if __name__ == "__main__":\n    sys.exit(pytest.main([__file__, "-v"]))")
...
| `sqlite3` | 標準ライブラリ | `TestProcessSubscriptionsUsesFreshDbPath`でNAS側DBに事前にサブスクリプションを登録するための直接接続 | 根拠: [import文] (行番号: 18 / 抜粋: "import sqlite3") |
| `sys` | 標準ライブラリ | `sys.path`への`DDD_DIR`挿入、`sys.modules`の退避・復元、`sys.exit` | 根拠: [import文] (行番号: 19 / 抜粋: "import sys") |
| `contextlib.closing` | 標準ライブラリ | `TestProcessSubscriptionsUsesFreshDbPath`でのSQLite接続の確実なクローズ(`with closing(...)`) | 根拠: [import文] (行番号: 20 / 抜粋: "from contextlib import closing, contextmanager") |
| `contextlib.contextmanager` | 標準ライブラリ | `_reloaded_with_core_unavailable`をコンテキストマネージャとして定義するためのデコレータ | 根拠: [import文] (行番号: 20 / 抜粋: "from contextlib import closing, contextmanager") |
| `pathlib.Path` | 標準ライブラリ | ファイルパスの操作全般、テスト内でのパス構築・比較 | 根拠: [import文] (行番号: 21 / 抜粋: "from pathlib import Path") |
| `unittest.mock.MagicMock` | 標準ライブラリ | `TestProcessSubscriptionsUsesFreshDbPath`での`extractor`/`file_manager`のモック化 | 根拠: [import文] (行番号: 22 / 抜粋: "from unittest.mock import MagicMock, patch") |
| `unittest.mock.patch` | 標準ライブラリ | `builtins.__import__`のパッチ、`AppConfig.get_output_base_dir`/`AppConfig.LOCAL_DIR_STR`のパッチ | 根拠: [import文] (行番号: 22 / 抜粋: "from unittest.mock import MagicMock, patch") |
| `pytest` | サードパーティ | テストフレームワーク本体、`pytest.main`によるエントリーポイント実行 | 根拠: [import文] (行番号: 24 / 抜粋: "import pytest") |
| `extract_youtube_urls` (as `module`) | ローカルモジュール（テスト対象） | 本ファイルが検証する対象モジュール本体。`DDD_DIR`を`sys.path`に追加した上でインポートされる | 根拠: [import文] (行番号: 29 / 抜粋: "import extract_youtube_urls as module  # noqa: E402") |
| `builtins` | 標準ライブラリ | `_reloaded_with_core_unavailable`内で`__import__`をパッチするために元の`builtins.__import__`を保持 | 根拠: [import文] (行番号: 62 / 抜粋: "import builtins") |
| `core.logger` | 内部モジュール（テスト対象の間接依存） | `test_core_module_is_importable_with_fixed_project_root`が、修正後の`PROJECT_ROOT`で実際にインポート可能であることを直接検証する対象 | 根拠: [import文] (行番号: 43 / 抜粋: "import core.logger  # noqa: F401  (ImportErrorにならないこと自体が検証)") |

### ブラックボックスとなる外部要素

| 名称 | 理由 | 根拠 |
| --- | --- | --- |
| `extract_youtube_urls`モジュールの内部実装 | `PROJECT_ROOT`, `get_managed_target_directory`, `AppConfig`, `SubscriptionManager._verify_environment`/`process_subscriptions`の実装詳細は本ファイルからは分からず、テスト対象モジュール自体（`extract_youtube_urls.py`）に依存する。 | 根拠: [import文] (行番号: 29 / 抜粋: "import extract_youtube_urls as module  # noqa: E402") |
| `core.logger`の実際の実装 | `import core.logger`が成功すること自体は検証されるが、`core.logger`モジュール内部の実装（`get_logger`関数の有無等）は本ファイルからは分からない。 | 根拠: [import文] (行番号: 43 / 抜粋: "import core.logger  # noqa: F401  (ImportErrorにならないこと自体が検証)") |
| `pytest`の`tmp_path`フィクスチャ | 各テストメソッドの引数として使用されるが、フィクスチャ自体の実装は`pytest`本体に依存し、本ファイルのコードからは分からない。 | 根拠: [テストメソッドのシグネチャ] (行番号: 97, 102, 109, 133 / 抜粋: "def test_detects_fallback_when_base_dir_matches_local_dir_exactly(self):") |
...
* 根拠: [関数定義とDocstring] (行番号: 32〜34 / 抜粋: "def test_project_root_points_to_my_home_system_not_repo_root():\n    """H-12: PROJECT_ROOTはDDDの単なる親(repoルート)ではなく、\n    core/ が実在する develop/MY_HOME_SYSTEM を指すこと。"""")


* **引数/リクエスト**: なし
* 根拠: [関数定義] (行番号: 32 / 抜粋: "def test_project_root_points_to_my_home_system_not_repo_root():")
...
* 根拠: [assert文] (行番号: 35〜36 / 抜粋: "assert module.PROJECT_ROOT == module.CURRENT_DIR.parent / "MY_HOME_SYSTEM"\n    assert module.PROJECT_ROOT.name == "MY_HOME_SYSTEM"")
...
* 根拠: [関数定義とDocstring] (行番号: 39〜41 / 抜粋: "def test_core_module_is_importable_with_fixed_project_root():\n    """修正後のPROJECT_ROOTでは実際にcore.loggerがimportでき、\n    ローカルフォールバックスタブに落ちないこと。"""")


* **引数/リクエスト**: なし
* 根拠: [関数定義] (行番号: 39 / 抜粋: "def test_core_module_is_importable_with_fixed_project_root():")


* **戻り値/レスポンス**: 該当なし
* **副作用**: `core.logger`モジュールの実際のインポート実行（`sys.modules`へのキャッシュを含む）。
* 根拠: [import文] (行番号: 43 / 抜粋: "import core.logger  # noqa: F401  (ImportErrorにならないこと自体が検証)")


* **エラーハンドリング**: なし（`import core.logger`が`ImportError`を送出した場合、テスト自体が失敗する。すなわち「例外が発生しないこと」自体が検証内容である）
* 根拠: [assert文とコメント] (行番号: 42〜43 / 抜粋: "assert str(module.PROJECT_ROOT) in sys.path\n    import core.logger  # noqa: F401  (ImportErrorにならないこと自体が検証)")
...
* 根拠: [メソッド定義とDocstring] (行番号: 53〜61 / 抜粋: "def _reloaded_with_core_unavailable(self):\n        """\n        core.logger/core.nas_utilsのimportだけをImportErrorにしてモジュールを\n        再読込する。")


* **引数/リクエスト**: なし（`self`のみ）
* 根拠: [メソッド定義] (行番号: 54 / 抜粋: "def _reloaded_with_core_unavailable(self):")


* **戻り値/レスポンス**: `yield`により再読込後の`module`オブジェクトを呼び出し元の`with`文へ渡す（`@contextmanager`によるジェネレータベースのコンテキストマネージャ）。
* 根拠: [yield文] (行番号: 78 / 抜粋: "yield importlib.reload(module)")
...
* 根拠: [メソッド定義] (行番号: 90〜93 / 抜粋: "def test_stub_falls_back_to_relative_data_only_when_no_kwarg_given(self):\n        with self._reloaded_with_core_unavailable() as reloaded:\n            result = reloaded.get_managed_target_directory()\n        assert result == Path("./data")")
...
* **エラーハンドリング**: なし
* 根拠: [assert文] (行番号: 93 / 抜粋: "assert result == Path("./data")")
...
* **エラーハンドリング**: なし
* 根拠: [assert文] (行番号: 107 / 抜粋: "assert manager._verify_environment() is True")
...
* 根拠: [メソッド定義とDocstring] (行番号: 109〜117 / 抜粋: "def test_detects_fallback_even_with_non_normalized_path_representation(self):\n        """H-12回帰防止: 旧実装(部分文字列 in チェック)は、フォールバック関数が\n        バグって短い相対パス './data' を返した場合にフォールバック状態を\n        検知できなかった。")
...
* 根拠: [messy_pathの構築とpatch] (行番号: 114〜116 / 抜粋: "messy_path = Path(module.AppConfig.LOCAL_DIR_STR + "/../" + Path(module.AppConfig.LOCAL_DIR_STR).name)\n        with patch.object(module.AppConfig, "get_output_base_dir", return_value=messy_path):\n            manager = module.SubscriptionManager.__new__(module.SubscriptionManager)")


* **エラーハンドリング**: なし
* 根拠: [assert文] (行番号: 117 / 抜粋: "assert manager._verify_environment() is False")


### `TestProcessSubscriptionsUsesFreshDbPath._RecoverableBaseDir`（テスト内ヘルパークラス）

* **役割**: `test_recovers_and_reads_nas_db_when_nas_comes_back_before_processing`メソッド内で定義される、「NASフォールバック→復帰」という時間経過をシミュレートするコーラブルオブジェクト。`recovered`属性が`False`の間は`local_dir`を、`True`に切り替わった後は`nas_dir`を返す。`AppConfig.get_output_base_dir`のパッチの`side_effect`として渡され、`SubscriptionManager`構築時点(`recovered=False`)と`process_subscriptions()`実行時点(`recovered=True`)とで異なる戻り値を返させるために使われる。
* 根拠: [クラス定義] (行番号: 159〜168 / 抜粋: "class _RecoverableBaseDir:\n            """NASフォールバック→復帰、という時間経過をシミュレートするコールバック。"""\n\n            def __init__(self, local_dir, nas_dir):\n                self.local_dir = local_dir\n                self.nas_dir = nas_dir\n                self.recovered = False\n\n            def __call__(self):\n                return self.nas_dir if self.recovered else self.local_dir")


* **引数/リクエスト**: `local_dir`, `nas_dir`（`__init__`）。呼び出し時(`__call__`)は引数なし。
* 根拠: [__init__と__call__の定義] (行番号: 162, 167 / 抜粋: "def __init__(self, local_dir, nas_dir):", "def __call__(self):")


* **戻り値/レスポンス**: `__call__`は`self.recovered`が`True`なら`self.nas_dir`、`False`なら`self.local_dir`を返す。
* 根拠: [__call__の戻り値] (行番号: 168 / 抜粋: "return self.nas_dir if self.recovered else self.local_dir")


* **副作用**: なし（`self.recovered`の書き換えは呼び出し元テストコードが直接行う）
* **エラーハンドリング**: なし


### `TestProcessSubscriptionsUsesFreshDbPath.test_recovers_and_reads_nas_db_when_nas_comes_back_before_processing`

* **役割**: Issue #123の回帰テスト本体。`nas_base`/`local_base`という2つの`tmp_path`配下ディレクトリを用意し、NAS側の`home_system.db`相当のパスへ事前に`youtube_subscriptions`テーブルとアクティブなサブスクリプション1件を直接SQLiteで作成しておく。`AppConfig.get_output_base_dir`を`_RecoverableBaseDir`インスタンスでパッチした状態で`SubscriptionManager`を構築（この時点では`recovered=False`＝NASフォールバック中を模擬）し、その後`recovered = True`（NAS復帰を模擬）に切り替えてから`process_subscriptions()`を呼び出す。修正後の実装であれば、`process_subscriptions()`実行時点の（復帰済みの）NAS状態から`db_path`が導出され、事前登録したサブスクリプションが読み込まれて`extractor.extract_iter`が呼ばれることを検証する。
* 根拠: [メソッド定義とテスト本体] (行番号: 133〜185 / 抜粋: "def test_recovers_and_reads_nas_db_when_nas_comes_back_before_processing(self, tmp_path):\n        nas_base = tmp_path / "nas" / "youtube_extractor" / "data"\n        local_base = tmp_path / "local" / "data"")


* **引数/リクエスト**: `self`, `tmp_path`（`pytest`標準フィクスチャ、テストごとに一意な一時ディレクトリ）
* 根拠: [メソッドシグネチャ] (行番号: 133 / 抜粋: "def test_recovers_and_reads_nas_db_when_nas_comes_back_before_processing(self, tmp_path):")


* **戻り値/レスポンス**: 該当なし
* **副作用**: `tmp_path`配下への実ディレクトリ作成(`mkdir`)、NAS側`home_system.db`への実SQLite接続・テーブル作成・レコードINSERT、`AppConfig.get_output_base_dir`と`AppConfig.LOCAL_DIR_STR`の一時的なパッチ、`SubscriptionManager`の実インスタンス化と`process_subscriptions()`の実行（`extractor`/`file_manager`は`MagicMock`のため実際のネットワークアクセス・ファイル保存は発生しない）。
* 根拠: [DB作成とパッチ適用] (行番号: 142〜157, 176〜185 / 抜粋: "with closing(sqlite3.connect(nas_db_path)) as conn:\n            conn.execute(\n                """\n                CREATE TABLE youtube_subscriptions (")


* **エラーハンドリング**: なし（`assert`失敗時はテスト失敗として報告される）
* 根拠: [assert文] (行番号: 189, 192 / 抜粋: "extractor.extract_iter.assert_called_once_with("https://example.test/channel/known")", "assert not (local_base.parent / "home_system.db").exists()")


### `if __name__ == "__main__":` ブロック

* **役割**: 本ファイルが`pytest`経由ではなく直接実行された場合に、`pytest.main`を用いて自身のテストを実行するエントリーポイント。
* 根拠: [エントリーポイント定義] (行番号: 195〜196 / 抜粋: "if __name__ == "__main__":\n    sys.exit(pytest.main([__file__, "-v"]))")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: 該当なし
* **副作用**: `pytest.main([__file__, "-v"])`の実行（自身のテスト全件を詳細出力モードで実行）、その終了コードでの`sys.exit`。
* 根拠: [pytest.main呼び出し] (行番号: 196 / 抜粋: "sys.exit(pytest.main([__file__, "-v"]))")
...
flowchart TD
    ...
    ImportModule --> ProcessSubTests["TestProcessSubscriptionsUsesFreshDbPath<br>(Issue #123回帰テスト)"]
    ProcessSubTests --> SeedNasDb["NAS側home_system.dbへ事前にアクティブなサブスクリプションを1件登録"]
    SeedNasDb --> BuildManager["_RecoverableBaseDir(recovered=False)でパッチした状態で<br>SubscriptionManager(extractor, file_manager)を構築"]
    BuildManager --> FlipRecovered["base_dir_resolver.recovered = True<br>(NAS復帰をシミュレート)"]
    FlipRecovered --> CallProcessSub["manager.process_subscriptions()を実行"]
    CallProcessSub --> AssertExtractCalled["extractor.extract_iter が\nNAS側の登録URLで1回呼ばれたことを検証"]
    CallProcessSub --> AssertNoLocalDb["ローカル側にhome_system.dbが\n作成されていないことを検証"]
```
...
graph TD
    subgraph "test_extract_youtube_urls_paths.py"
        ...
        ProcessSubClass["TestProcessSubscriptionsUsesFreshDbPath<br>(Issue #123回帰テスト)"]
        RecoverableBaseDir["_RecoverableBaseDir<br>(テスト内ヘルパークラス)"]
    end

    subgraph "テスト対象"
        ExtractModule["extract_youtube_urls.py<br>(PROJECT_ROOT / get_managed_target_directory /<br>AppConfig / SubscriptionManager._verify_environment /<br>SubscriptionManager.process_subscriptions)"]
    end

    subgraph "外部依存（標準ライブラリ）"
        importlib_mod["importlib.reload"]
        unittest_mock["unittest.mock.patch / MagicMock"]
        builtins_mod["builtins.__import__"]
        contextlib_mod["contextlib.closing / contextmanager"]
        sqlite3_mod["sqlite3"]
    end
    ...
    ProcessSubClass --> ExtractModule
    ProcessSubClass --> RecoverableBaseDir
    ProcessSubClass --> unittest_mock
    ProcessSubClass --> contextlib_mod
    ProcessSubClass --> sqlite3_mod

    TestRootFn --> pytest_mod
    TestCoreImportFn --> pytest_mod
    FallbackStubClass --> pytest_mod
    VerifyEnvClass --> pytest_mod
    ProcessSubClass --> pytest_mod
```
...
| 優先度 | ファイル名(推測可) | 理由 | 根拠 |
| --- | --- | --- | --- |
| 高 | `DDD/extract_youtube_urls.py` | 本ファイルがテストする全ての挙動（`PROJECT_ROOT`解決、フォールバックスタブ、`_verify_environment`、`process_subscriptions`のIssue #123修正）の実装本体であり、既に`extract_youtube_urls.md`として解析済み。両ドキュメントを突き合わせて整合性を確認するとよい。 | 根拠: [import文] (行番号: 29 / 抜粋: "import extract_youtube_urls as module  # noqa: E402") |
| 中 | `MY_HOME_SYSTEM/core/logger.py` | `test_core_module_is_importable_with_fixed_project_root`が成功前提とする`core.logger`モジュールの実体を確認するため。 | 根拠: [import文] (行番号: 43 / 抜粋: "import core.logger  # noqa: F401  (ImportErrorにならないこと自体が検証)") |
...
* 根拠: [sys.modules操作] (行番号: 71〜75, 80〜81 / 抜粋: "removed_modules = {\n            name: sys.modules.pop(name)\n            for name in list(sys.modules)\n            if name == "core" or name.startswith("core.")\n        }")
...
* 根拠: [_blocking_import定義] (行番号: 66〜69 / 抜粋: "def _blocking_import(name, *args, **kwargs):\n            if name == "core" or name.startswith("core."):\n                raise ImportError(f"blocked for test: {name}")")
...
* **(Issue #123回帰テストの設計メモ)** `TestProcessSubscriptionsUsesFreshDbPath`は、`_verify_environment`の既存テスト群（`SubscriptionManager.__new__`による`__init__`を経由しないインスタンス生成＋単発の`get_output_base_dir`パッチ）とは異なり、`SubscriptionManager`を実際に`__init__`経由で構築し、`_RecoverableBaseDir`という状態を持つ`side_effect`コーラブルで「構築時はフォールバック中、実行時は復帰済み」という時間経過を再現している。これは、修正対象のバグが`__init__`時点と`process_subscriptions()`実行時点という2つの異なるタイミングでの`get_output_base_dir()`評価結果の食い違いによって生じるものであり、単発のパッチでは再現できないためである。今後同様に「2つの評価タイミングの整合性」を検証するテストを書く際は、この`_RecoverableBaseDir`パターン（`recovered`フラグの切り替えでシミュレートする）を参考にできる。
* 根拠: [_RecoverableBaseDirの定義とコメント] (行番号: 159〜168 / 抜粋: "class _RecoverableBaseDir:\n            """NASフォールバック→復帰、という時間経過をシミュレートするコールバック。"""")
