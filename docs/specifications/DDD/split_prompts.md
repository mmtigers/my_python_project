...
* 根拠: [PROMPT_PATTERN定義とsplit_prompts関数] (行番号: 27〜34, 44〜123 / 抜粋: "PROMPT_PATTERN = re.compile(r'(\\d+)\\.\\s+([^\\n]+)\\n+Prompt:[ \\t]*(.*)')")
...
* 根拠: [ファイルI/O処理] (行番号: 57, 81, 119 / 抜粋: "content = input_file.read_text(encoding='utf-8')", "output_dir.mkdir(parents=True, exist_ok=True)", "filepath.write_text(f"# {raw_title}\\n\\nPrompt: {prompt_text}\\n", encoding='utf-8')")、フォーマット外検知 (行番号: 64〜74)
...
* 根拠: [存在確認とexit] (行番号: 140〜142 / 抜粋: "if not input_path.exists():\n        logger.error(f"❌ 入力ファイルが見つかりません: {input_path}")\n        sys.exit(1)")
...
* 根拠: [PROMPT_PATTERN定義] (行番号: 34 / 抜粋: "PROMPT_PATTERN = re.compile(r'(\\d+)\\.\\s+([^\\n]+)\\n+Prompt:[ \\t]*(.*)')")、[_ITEM_START_PATTERNとフォーマット外検知] (行番号: 36〜41, 76〜89)
* **（Issue #581で修正）`Prompt:`直後の内容が空の場合に次項目を飲み込むバグ**: 以前は`Prompt:`直後を`\s+`（改行含む空白1文字以上）で受けていたため、内容が空（末尾空白のみ）の`Prompt:`行では、`\s+`が次の「番号. タイトル」行の直前までの改行をすべて飲み込んでしまい、次項目のタイトル行自体をこの項目のPrompt内容として誤って取り込んでいた（次項目は丸ごと抽出されず消失し、`written`件数も過少になっていた）。`Prompt:`直後を行内の空白のみを許容する`[ \t]*`＋改行を跨がない`(.*)`に変更し、内容が空の項目も1件として正しく抽出できるようにした。
* 根拠: [PROMPT_PATTERN定義とコメント] (行番号: 30〜34)、回帰テスト`test_split_prompts.py::test_empty_prompt_content_does_not_swallow_the_next_item`
* **タイトル・プロンプト内容とも単一行想定**: 正規表現のキャプチャグループがタイトル側`[^\n]+`・内容側`(.*)`（いずれも`re.DOTALL`指定が無いため改行を含まない）であるため、複数行にまたがるタイトルやプロンプト本文には対応していない。
* 根拠: [PROMPT_PATTERN定義] (行番号: 34 / 抜粋: "([^\\n]+)\\n+Prompt:[ \\t]*(.*)")
...
* 根拠: [pad_width計算] (行番号: 87 / 抜粋: "pad_width = max(2, max(len(num_str) for num_str, _, _ in matches))")
