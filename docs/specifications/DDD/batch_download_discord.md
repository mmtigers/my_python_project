...
| `file_utils.redact_discord_webhook_url` | ローカルモジュール | **（2026-09-06 品質監査で修正）** `_standalone_send_discord_webhook`の送信失敗ログに含まれる例外文字列中のDiscord Webhook URL（トークン部分）をマスクするために使用 | 根拠: [import文] (行番号: 44 / 抜粋: "from file_utils import redact_discord_webhook_url")、[利用箇所] (行番号: 120 / 抜粋: "logger.warning(f\"⚠️ Discord Webhook送信に失敗しました: {redact_discord_webhook_url(e)}\")") |
| `pathlib.Path` | 標準ライブラリ | パスオブジェクトの操作全般 | 根拠: [import文] (行番号: 47 / 抜粋: "from pathlib import Path") |
| `urllib.parse.urljoin` | 標準ライブラリ | m3u8マニフェスト内の相対URIを絶対URLへ書き換える処理で使用 | 根拠: [import文] (行番号: 48 / 抜粋: "from urllib.parse import urljoin") |
| `concurrent.futures.ThreadPoolExecutor`, `as_completed` | 標準ライブラリ | m3u8セグメントの並行ダウンロード（最大5ワーカー） | 根拠: [import文] (行番号: 49 / 抜粋: "from concurrent.futures import ThreadPoolExecutor, as_completed") |
| `requests.adapters.HTTPAdapter` | サードパーティ | セッションへのリトライ用アダプタのマウント | 根拠: [import文] (行番号: 52 / 抜粋: "from requests.adapters import HTTPAdapter") |
| `urllib3.util.retry.Retry` | サードパーティ | HTTPリクエストのリトライポリシー定義 | 根拠: [import文] (行番号: 53 / 抜粋: "from urllib3.util.retry import Retry") |
| `yt_dlp` | サードパーティ | 動画のメタデータ抽出およびダウンロード（Universal）、`playlist.m3u8`の結合（ScrapingStrategy）、バージョン鮮度チェック | 根拠: [import文] (行番号: 54 / 抜粋: "import yt_dlp") |
...
* 根拠: [引数定義とチャンネル分岐] (行番号: 99, 105〜108 / 抜粋: "def _standalone_send_discord_webhook(messages, image_data=None, channel="notify") -> bool:", "if channel == "error":\n        url = os.getenv("DISCORD_WEBHOOK_ERROR") or os.getenv("DISCORD_WEBHOOK_URL")\n    else:\n        url = os.getenv("DISCORD_WEBHOOK_NOTIFY") or os.getenv("DISCORD_WEBHOOK_URL")")


* **戻り値/レスポンス**: `bool`（送信成功時`True`、Webhook URL未設定時または送信失敗時`False`）
* 根拠: [戻り値ヒントと各return] (行番号: 99, 88, 95, 99 / 抜粋: "def _standalone_send_discord_webhook(messages, image_data=None, channel="notify") -> bool:", "if not url:\n        return False")


* **副作用**: `os.getenv`による環境変数(`DISCORD_WEBHOOK_ERROR`/`DISCORD_WEBHOOK_NOTIFY`/`DISCORD_WEBHOOK_URL`)の読み込み、`requests.post`によるDiscord Webhookへの実送信（本文は`text[:2000]`に切り詰め、タイムアウトは`CONFIG.REQUEST_TIMEOUT`）、送信失敗時の警告ログ出力。**（2026-09-06 品質監査で修正）** 警告ログに埋め込む例外文字列は`file_utils.redact_discord_webhook_url`を通して出力する（`requests`の例外文字列には送信先Webhook URL（トークン込み）が含まれるため、トークン部分をマスクしてからログへ出す）。
* 根拠: [requests.post呼び出しとマスク付き警告ログ] (行番号: 93, 96〜98 / 抜粋: "resp = requests.post(url, json={"content": text[:2000]}, timeout=CONFIG.REQUEST_TIMEOUT)" / "# 例外文字列には送信先の Webhook URL(トークン込み)が含まれるためマスクする\n        logger.warning(f\"⚠️ Discord Webhook送信に失敗しました: {redact_discord_webhook_url(e)}\")")


* **エラーハンドリング**: `channel`に応じたいずれの環境変数も未設定（URLが`None`）の場合は`requests.post`自体を呼ばずに`False`を返す。送信時の例外は`Exception`で捕捉して`logger.warning`で警告ログを出力したうえで`False`を返す（例外の再送出はしない）。**（2026-09-06 品質監査で修正）** 警告ログのメッセージには例外`e`をそのまま埋め込むのではなく`redact_discord_webhook_url(e)`の結果を埋め込む。
* 根拠: [try-exceptブロック] (行番号: 92〜99 / 抜粋: "try:\n        resp = requests.post(url, json={"content": text[:2000]}, timeout=CONFIG.REQUEST_TIMEOUT)\n        resp.raise_for_status()\n        return True\n    except Exception as e:\n        # 例外文字列には送信先の Webhook URL(トークン込み)が含まれるためマスクする\n        logger.warning(f"⚠️ Discord Webhook送信に失敗しました: {redact_discord_webhook_url(e)}")\n        return False")


### `_resolve_cookies_file`

* **役割**: 環境変数`YOUTUBE_COOKIES_FILE`が指すCookieファイルを解決する関数。環境変数が未設定、またはファイルが存在しない場合はCookie無し（`None`）にフォールバックする。
* 根拠: [関数定義とDocstring] (行番号: 133〜146 / 抜粋: "def _resolve_cookies_file() -> Optional[Path]:\n    """YouTube等のボット検知回避用Cookieファイルを解決する。")


* **引数/リクエスト**: なし
* 根拠: [関数定義] (行番号: 133 / 抜粋: "def _resolve_cookies_file() -> Optional[Path]:")


* **戻り値/レスポンス**: `Optional[Path]`（解決できたCookieファイルのパス、なければ`None`）
* 根拠: [戻り値ヒント] (行番号: 133 / 抜粋: "def _resolve_cookies_file() -> Optional[Path]:")


* **副作用**: `os.getenv`による環境変数読み込み、ファイル未存在時の警告ログ出力。
* 根拠: [処理内容] (行番号: 139, 144 / 抜粋: "cookies_env = os.getenv("YOUTUBE_COOKIES_FILE")", "logger.warning(f"⚠️ YOUTUBE_COOKIES_FILE で指定されたファイルが見つかりません: {cookies_path}")")
...
* **役割**: アプリケーション全体の設定値（時間制限、パス、リトライ回数、機能フラグ、ボット検知対策のスリープ範囲・閾値・マーカー文字列等）を保持するイミュータブル(`frozen=True`)なデータクラス。NAS関連では、`REQUIRE_NAS_MOUNT`（`false`でNAS未マウントでも起動を許可し`verify_nas_mount`自体をスキップする単独環境向けフラグ）、`LOCAL_TMP_DIR`（`ScrapingStrategy`がHLSフラグメント・結合を行うローカル一時ディレクトリ。既定は`tempfile.gettempdir()`ではなく`CURRENT_DIR/tmp_fragments`＝本スクリプトと同じ実ディスク上。理由はソースコメントによれば、`/tmp`がtmpfs運用の環境（一部Raspberry Pi OS構成含む）だと動画1本分(数GB)の書き込みでOOM・SSH切断を招きうるため）、`LOCAL_TMP_MIN_FREE_SPACE_GB`（`LOCAL_TMP_DIR`の空き容量下限）の3フィールドが、PR #72で追加されたローカル→NAS二段階転送パイプライン（`ScrapingStrategy._download_with_ytdlp`参照）向けに存在する。**（本PRで追加）** `DISCORD_CIRCUIT_BREAKER_THRESHOLD`（既定3）は、`_discord_circuit_breaker`（`DiscordNotifier.send`が参照するモジュールレベルの`file_utils.DiscordCircuitBreaker`）が連続何回の送信失敗でブレーカーを開くかの閾値。 **（Issue #663で変更）** `NAS_MOUNT_POINT`は以前`Path("/mnt/nas")`の直書きで環境変数による変更ができなかったが、`file_utils.resolve_nas_mount_point`を`default_factory`に指定する形になり、環境変数`NAS_MOUNT_POINT`（MY_HOME_SYSTEMと共用の`.env`のキー。DDD側は`config`をimportせず環境変数を直接読む）で上書きできる。未設定・空文字なら従来どおり`/mnt/nas`。
* 根拠: [AppConfigクラスとNAS関連フィールドのコメント] (行番号: 135〜136, 151〜153, 154〜166, 167〜169 / 抜粋: "@dataclass(frozen=True)\nclass AppConfig:", "REQUIRE_NAS_MOUNT: bool = os.getenv("DDD_REQUIRE_NAS_MOUNT", "true").lower() == "true"", "LOCAL_TMP_DIR: Path = Path(os.getenv("DDD_LOCAL_TMP_DIR") or str(CURRENT_DIR / "tmp_fragments"))", "LOCAL_TMP_MIN_FREE_SPACE_GB: int = _env_int("DDD_LOCAL_TMP_MIN_FREE_SPACE_GB", 10)")
...
* 根拠: [インスタンス生成] (行番号: 280 / 抜粋: "CONFIG = AppConfig()")


* **副作用**: `os.getenv` による環境変数(`ENABLE_YOUTUBE_DL`, `VIDEO_SAVE_DIR`, `DDD_REQUIRE_NAS_MOUNT`, `DDD_LOCAL_TMP_DIR`等)の読み込み、整数系の`DDD_LOCAL_TMP_MIN_FREE_SPACE_GB`・`DDD_REQUEST_TIMEOUT`はモジュールレベル関数`_env_int(name, default)`経由で読む（Issue #663: 空文字や整数として解釈できない値は警告ログを出して既定値にフォールバックする。以前は`int(os.getenv(...))`の直書きで、不正値だとimport時の`ValueError`でcron起動が黙って落ちていた。これらの変数は`MY_HOME_SYSTEM/.env.example`にも記載され、`MY_HOME_SYSTEM/tests/test_env_example_consistency.py`が`DDD/*.py`の`os.getenv`/`_env_int`呼び出しと照合する）、`field(default_factory=_resolve_cookies_file)`によるCookieファイル解決処理の実行。
...
* 根拠: [プロパティ定義] (行番号: 276〜278 / 抜粋: "@property\n    def nas_marker_path(self) -> Path:\n        return self.NAS_MOUNT_POINT / self.NAS_MARKER_FILE")


* **引数/リクエスト**: なし（`self`のみ）
* **戻り値/レスポンス**: `Path`
* 根拠: [戻り値] (行番号: 278 / 抜粋: "return self.NAS_MOUNT_POINT / self.NAS_MARKER_FILE")
...
* 根拠: [クラス定義とDocstring] (行番号: 283〜289 / 抜粋: "class BotDetectionError(Exception):\n    """YouTube等からボット検知/レート制限（429やSign-in要求等）を検知した際に送出する。")
...
* **役割**: 例外オブジェクトの文字列表現（小文字化し、Unicodeのアポストロフィ`’`をASCIIの`'`に正規化）に`CONFIG.BOT_DETECTION_MARKERS`のいずれかが一致するかを判定する関数。**（2026-09-06 品質監査で修正）** マーカーの種別で一致パターンを使い分ける: `marker.isdigit()`が真の数字マーカー（"429"/"403"/"503"）は`(?<![\w\-/.])NNN(?![\w\-/]|\.\w)`（前に英数字・`_`・`-`・`/`・`.`が無く、後ろに英数字・`_`・`-`・`/`が続かず、かつ「`.`＋英数字」も続かない位置）で、フレーズマーカーは従来どおり`\b...\b`（正規表現の単語境界）で`re.search`する。以前は数字マーカーも`\b`のみで判定していたが、正規表現の単語境界は`-`・`/`・`.`も境界とみなすため、品番・パス・一時ファイル名に含まれる数字列（ソースコメントの例: "ssis-403"、"ipx-403:"、"/dm18/ja/ssis-403"、"ssni-429.mp4.fragments"、"seg-403-v1"）にも一致し、単なる通信エラー1件で`BotDetectionError`→12時間クールダウンに入っていた。"HTTP Error 403: Forbidden"・"too many 503 error responses"・"error 429."のような本来の文言は引き続き一致する（ソースコメント記載の例）。**（Issue #467で修正）** 以前は数字のみのマーカー（"403"/"429"/"503"）だけを`\b`による厳密一致で判定し、フレーズマーカー（"sign in to confirm you're not a bot"等）は部分文字列一致(`in`)のままだった。数字マーカーを単純な部分文字列一致で判定すると、エラーメッセージに埋め込まれた動画ID等の英数字列（例: "AbC403XyZ"）に偶然含まれる数字列にまで誤爆し、`BOT_DETECTION_COOLDOWN_HOURS`（12時間）のセッション全停止を誤って引き起こし得たための修正だったが、フレーズマーカー側も無関係なログにたまたま同一フレーズが含まれる場合に誤検知しうる同種の問題を抱えていたため、マーカー種別を問わず`\b`による単語境界一致に統一した。**（Issue #396で修正）** 以前のフレーズマーカー`"sign in to confirm"`は、yt-dlpの年齢制限メッセージ「Sign in to confirm your age. This video may be inappropriate for some users.」にも部分一致し、年齢制限動画1本で`BotDetectionError`→セッション中断＋12時間クールダウンに入っていた。マーカーをボット検知に固有の`"sign in to confirm you're not a bot"`/`"confirm you're not a bot"`に絞るとともに、`CONFIG.BOT_DETECTION_EXCLUDED_MARKERS`（`"confirm your age"`）を含むメッセージはマーカー判定より優先して`False`を返す。
* 根拠: [数字マーカーの分岐] (行番号: 285〜300 / 抜粋: "for marker in CONFIG.BOT_DETECTION_MARKERS:\n        if marker.isdigit():\n            # 数字マーカー(403/429/503)は \\b だけでは不十分: 正規表現の単語境界は\n            # \"-\" \"/\" \".\" も境界とみなすため、品番・パス・一時ファイル名に含まれる\n            # 数字列(\"ssis-403\" \"ipx-403:\" \"/dm18/ja/ssis-403\" \"ssni-429.mp4.fragments\"\n            # \"seg-403-v1\")にも一致し、単なる通信エラー1件で BotDetectionError →\n            # 12時間クールダウンに入っていた。" / "pattern = rf\"(?<![\\w\\-/.]){re.escape(marker)}(?![\\w\\-/]|\\.\\w)\"\n        else:\n            pattern = rf\"\\b{re.escape(marker)}\\b\"\n        if re.search(pattern, message):\n            return True")
* 根拠: [関数定義とコメント] (行番号: 292〜324 / 抜粋: "def _is_bot_detection_error(exc: Exception) -> bool:\n    # M-7-2: "403"/"429"/"503" のような数字だけのマーカーを単純な部分文字列\n    # マッチ(in)で判定すると、エラーメッセージに埋め込まれた動画ID等の\n    # 英数字列(例: "...AbC403XyZ...")に偶然含まれる数字列にまで誤爆し" / "# #467: フレーズマーカーも従来は部分文字列一致(in)のままだったため、\n    # 無関係なログにたまたま同一フレーズが含まれる場合に誤検知しうる\n    # (可能性は低いが数字マーカーと同種の問題)。フレーズ・数字を問わず\n    # 単語境界(\\b)で厳密に判定する方式に統一する。" / "for marker in CONFIG.BOT_DETECTION_MARKERS:\n        if re.search(rf"\\b{re.escape(marker)}\\b", message):\n            return True")、[マーカー定義] (行番号: 207〜222 / 抜粋: "BOT_DETECTION_MARKERS: Tuple[str, ...] = (\n        \"sign in to confirm you're not a bot\",\n        \"confirm you're not a bot\"," / "BOT_DETECTION_EXCLUDED_MARKERS: Tuple[str, ...] = (\n        \"confirm your age\",\n    )")


* **引数/リクエスト**: `exc: Exception`
* 根拠: [引数定義] (行番号: 292 / 抜粋: "def _is_bot_detection_error(exc: Exception) -> bool:")


* **戻り値/レスポンス**: `bool`
* 根拠: [戻り値ヒントと各return] (行番号: 292, 284, 300, 301 / 抜粋: "def _is_bot_detection_error(exc: Exception) -> bool:")


* **副作用**: なし
* 根拠: [処理内容] (行番号: 280〜301 / 抜粋: "message = str(exc).lower().replace("’", "'")" / "for marker in CONFIG.BOT_DETECTION_MARKERS:\n        if marker.isdigit():" / "if re.search(pattern, message):\n            return True")
...
* 根拠: [関数定義とDocstring] (行番号: 327〜342 / 抜粋: "def _round_robin_flatten(groups: Iterable[List["DownloadTask"]]) -> List["DownloadTask"]:\n    """複数グループのリストを、グループ順ではなくラウンドロビンで1本のリストに平坦化する。")


* **引数/リクエスト**: `groups: Iterable[List["DownloadTask"]]`
* 根拠: [引数定義] (行番号: 327 / 抜粋: "def _round_robin_flatten(groups: Iterable[List["DownloadTask"]]) -> List["DownloadTask"]:")
...
* 根拠: [関数定義とDocstring] (行番号: 389〜396 / 抜粋: "def _looks_like_block_page(html: str) -> bool:\n    """取得したHTMLがCloudflare等のボット検知チャレンジページかを判定する。")


* **引数/リクエスト**: `html: str`
* 根拠: [引数定義] (行番号: 389 / 抜粋: "def _looks_like_block_page(html: str) -> bool:")


* **戻り値/レスポンス**: `bool`
* 根拠: [戻り値ヒント] (行番号: 389 / 抜粋: "def _looks_like_block_page(html: str) -> bool:")
...
* 根拠: [DownloadTaskクラス] (行番号: 407〜409 / 抜粋: "class DownloadTask(NamedTuple):\n    url: str\n    source_name: str")
...
* 根拠: [モジュールレベル変数定義] (行番号: 414 / 抜粋: "_discord_circuit_breaker = DiscordCircuitBreaker(failure_threshold=CONFIG.DISCORD_CIRCUIT_BREAKER_THRESHOLD)")
...
* 根拠: [DiscordNotifier.send] (行番号: 418〜434 / 抜粋: "def send(text: str, is_error: bool = False) -> None:")


* **引数/リクエスト**: `text: str` (通知内容), `is_error: bool = False` (エラー通知フラグ)
* 根拠: [引数定義] (行番号: 418 / 抜粋: "def send(text: str, is_error: bool = False) -> None:")
...
* 根拠: [関数定義] (行番号: 438〜440 / 抜粋: "def _read_history_file(path: Path) -> Set[str]:\n        with open(path, "r", encoding="utf-8") as f:\n            return {line.strip() for line in f if line.strip()}")


* **引数/リクエスト**: `path: Path`
* 根拠: [引数定義] (行番号: 438 / 抜粋: "def _read_history_file(path: Path) -> Set[str]:")


* **戻り値/レスポンス**: `Set[str]`
* 根拠: [戻り値ヒント] (行番号: 438 / 抜粋: "def _read_history_file(path: Path) -> Set[str]:")


* **副作用**: 指定パスのファイル読み込み（クローズは`with`文で保証）。
* 根拠: [ファイル読み込み] (行番号: 439〜440 / 抜粋: "with open(path, "r", encoding="utf-8") as f:\n            return {line.strip() for line in f if line.strip()}")
...
* 根拠: [メソッド定義とDocstring] (行番号: 502〜512 / 抜粋: "def is_in_cooldown() -> Optional[datetime.datetime]:\n        """クールダウン中であれば解除予定時刻を、そうでなければNoneを返す。"""")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `Optional[datetime.datetime]`
* 根拠: [戻り値ヒント] (行番号: 502 / 抜粋: "def is_in_cooldown() -> Optional[datetime.datetime]:")


* **副作用**: クールダウンファイルの読み込み。
* 根拠: [ファイル読み込み] (行番号: 508 / 抜粋: "until = datetime.datetime.fromisoformat(path.read_text(encoding="utf-8").strip())")
...
* 根拠: [メソッド定義] (行番号: 529〜533 / 抜粋: "def clear() -> None:\n        try:\n            CONFIG.BOT_DETECTION_COOLDOWN_FILE.unlink(missing_ok=True)")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 529 / 抜粋: "def clear() -> None:")


* **副作用**: クールダウンファイルの削除(`unlink`)。
* 根拠: [削除処理] (行番号: 531 / 抜粋: "CONFIG.BOT_DETECTION_COOLDOWN_FILE.unlink(missing_ok=True)")
...
* 根拠: [NetworkManager.create_session] (行番号: 537〜543 / 抜粋: "def create_session() -> requests.Session:")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `requests.Session`
* 根拠: [戻り値ヒント] (行番号: 537 / 抜粋: "def create_session() -> requests.Session:")
...
* 根拠: [FileSystemManager.sanitize_filename] (行番号: 547〜548 / 抜粋: "def sanitize_filename(filename: str) -> str:\n        return _shared_sanitize_filename(filename)")


* **引数/リクエスト**: `filename: str`
* **戻り値/レスポンス**: `str`
* 根拠: [関数定義] (行番号: 547 / 抜粋: "def sanitize_filename(filename: str) -> str:")
...
* 根拠: [FileSystemManager.ensure_dir] (行番号: 551〜565 / 抜粋: "def ensure_dir(path: Path) -> bool:")


* **引数/リクエスト**: `path: Path`
* **戻り値/レスポンス**: `bool`（成功時`True`、権限エラーおよびその他の`OSError`時`False`）
* 根拠: [戻り値ヒント] (行番号: 551 / 抜粋: "def ensure_dir(path: Path) -> bool:")


* **副作用**: ディレクトリ作成(`mkdir`)、エラー時のDiscord通知。
* 根拠: [mkdir呼び出し] (行番号: 553 / 抜粋: "path.mkdir(parents=True, exist_ok=True)")
...
* 根拠: [メソッド定義とDocstring] (行番号: 568〜586 / 抜粋: "def sweep_stale_fragment_dirs() -> None:
        """#398: クラッシュ・強制終了等で未クリーンアップのまま残った
        CONFIG.LOCAL_TMP_DIR配下の "*.fragments.tmp" ディレクトリを一掃する。")


* **引数/リクエスト**: なし
* 根拠: [引数定義] (行番号: 568 / 抜粋: "def sweep_stale_fragment_dirs() -> None:")


* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 568 / 抜粋: "def sweep_stale_fragment_dirs() -> None:")
...
* 根拠: [FileSystemManager.check_disk_space] (行番号: 589〜603 / 抜粋: "def check_disk_space(path: Path, min_free_gb: Optional[int] = None) -> bool:")
...
* 根拠: [SystemHealthChecker.is_within_time_window] (行番号: 607〜609 / 抜粋: "def is_within_time_window() -> bool:")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `bool`
* 根拠: [戻り値ヒント] (行番号: 607 / 抜粋: "def is_within_time_window() -> bool:")
...
* 根拠: [SystemHealthChecker.verify_nas_mountとREQUIRE_NAS_MOUNT分岐] (行番号: 612〜618 / 抜粋: "def verify_nas_mount() -> bool:\n        if not CONFIG.REQUIRE_NAS_MOUNT:\n            return True")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `bool`（`REQUIRE_NAS_MOUNT`無効時、またはマウント確認成功時は`True`。マウント確認失敗時は`False`）
* 根拠: [戻り値ヒント] (行番号: 612 / 抜粋: "def verify_nas_mount() -> bool:")


* **副作用**: `REQUIRE_NAS_MOUNT`有効時、未マウント検知時のDiscord通知(`is_error=True`)。
* 根拠: [通知送信] (行番号: 616 / 抜粋: "DiscordNotifier.send("⛔ CRITICAL: NASマウントエラー", is_error=True)")
...
* 根拠: [SystemHealthChecker.check_dependencies] (行番号: 621〜624 / 抜粋: "def check_dependencies() -> None:")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 621 / 抜粋: "def check_dependencies() -> None:")


* **副作用**: `logger.warning` によるログ出力、`check_yt_dlp_freshness`の呼び出し。
* 根拠: [ログ出力と呼び出し] (行番号: 623〜624 / 抜粋: "logger.warning("⚠️ ffmpeg not found.")\n        SystemHealthChecker.check_yt_dlp_freshness()")
...
* 根拠: [メソッド定義とDocstring] (行番号: 627〜646 / 抜粋: "def check_yt_dlp_freshness() -> None:\n        """yt-dlpのバージョン（YYYY.MM.DD形式）が古すぎないか警告する。")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 627 / 抜粋: "def check_yt_dlp_freshness() -> None:")
...
* 根拠: [DownloadStrategyクラス] (行番号: 651〜674 / 抜粋: "class DownloadStrategy(ABC):")


* **引数/リクエスト**: `__init__(self, save_base_dir: Path, session: requests.Session)`
* 根拠: [__init__定義] (行番号: 652〜654 / 抜粋: "def __init__(self, save_base_dir: Path, session: requests.Session):")
...
* 根拠: [_determine_save_dir内] (行番号: 666 / 抜粋: "if not FileSystemManager.ensure_dir(target_dir): return None")
...
* 根拠: [UniversalYtDlpStrategy.downloadとnoplaylistのコメント] (行番号: 678, 699〜703 / 抜粋: "def download(self, task: DownloadTask) -> bool:", "# M-7-3: リスト1行がプレイリストURL(またはチャンネルURL)だった場合、\n            # noplaylistが無いとyt-dlpがその1タスクの中で全件を無制限にダウンロード\n            # してしまい、MAX_TASKS_PER_RUNによる1回あたりの上限governanceが\n            # まるごと迂回されてしまう。単一動画のみを対象にする。\n            'noplaylist': True,")、trim_file_nameの修正 (行番号: 680〜689 / 抜粋: "#175: yt-dlpのtrim_file_nameは文字数ベース(no_ext[:trim_file_name]の\n            # 単純なスライス)であり、バイト数を保証しない。")、[D-L1: paths/outtmplのコメント] (行番号: 689〜696 / 抜粋: "# D-L1: 保存先ディレクトリ(target_dir、リストファイル名由来のsource_name\n            # を含みうる)をouttmpl文字列へf-stringで直接埋め込むと" / "'paths': {'home': str(target_dir)},\n            'outtmpl': '%(title)s.%(ext)s',")、[D-L2: process_ie_resultのコメント] (行番号: 722〜728 / 抜粋: "# D-L2: 以前はここで改めてydl.download([task.url])を呼んでおり、\n                # 直前のextract_info(download=False)と合わせてメタデータ取得の\n                # ネットワークリクエストが2回発生していた" / "ydl.process_ie_result(info, download=True)")、[Issue #566: '_type'チェックとコメント] (行番号: 703〜715 / 抜粋: "# #566: yt-dlpの'noplaylist'は「動画とプレイリストの両方を指すURL\n                # (watch?v=X&list=Y)で動画側だけを選ぶ」オプションであり、動画IDを\n                # 含まない純粋なプレイリスト/チャンネルURLには効果がない\n                # (yt-dlp内部のInfoExtractor._yes_playlistがvideo_id不在時は\n                # noplaylistを一切参照せず常にプレイリスト全体を返す実装のため)。" / "if info.get('_type') in ('playlist', 'multi_video'):\n                    logger.warning(\n                        f"⚠️ プレイリスト/チャンネルURLは対象外です(1動画のみ処理可能): {task.url}"\n                    )\n                    return False")


* **引数/リクエスト**: `task: DownloadTask`
* 根拠: [引数定義] (行番号: 678 / 抜粋: "def download(self, task: DownloadTask) -> bool:")


* **戻り値/レスポンス**: `bool`（成功・スキップ時`True`、失敗時`False`。**Issue #566で追加**: `info`の`_type`がプレイリスト/チャンネル相当と判定された場合も`False`）
* 根拠: [return文] (行番号: 661, 715, 719, 730, 737 / 抜粋: "if not target_dir: return False", "return False"（`_type`チェックによる早期return）, "if self._should_skip(filename): return True", "return True", "return False"（例外ハンドラ末尾）)


* **副作用**: 保存先ディレクトリの決定・作成、`yt_dlp`によるメタデータ取得とダウンロード（**D-L2で変更**。以前は`extract_info`＋`download`の2リクエストだったが、`extract_info`＋`process_ie_result`の1リクエストに統一）、成功時のDiscord通知。**（Issue #566で追加）** 抽出結果`info`の`_type`が`'playlist'`/`'multi_video'`だった場合は、ダウンロードやDiscord通知を一切行わず`logger.warning`による警告ログのみを出力する。
* 根拠: [ダウンロード実行と通知] (行番号: 728〜729 / 抜粋: "ydl.process_ie_result(info, download=True)\n                DiscordNotifier.send(f"✅ 動画保存完了\\nファイル: `{filename.name}`")")、[Issue #566: 警告ログ出力] (行番号: 711〜715 / 抜粋: "if info.get('_type') in ('playlist', 'multi_video'):\n                    logger.warning(\n                        f"⚠️ プレイリスト/チャンネルURLは対象外です(1動画のみ処理可能): {task.url}"\n                    )\n                    return False")


* **エラーハンドリング**: **（Issue #566で追加）** `extract_info`の戻り値`info`の`_type`が`'playlist'`または`'multi_video'`の場合、これは例外ではなく正常な抽出結果だが、`noplaylist`オプションでは防げない「プレイリスト/チャンネルURL全体の抽出」を意味するため、例外を送出せず`filename`算出前に早期`return False`する（1タスクの中での無制限ダウンロードを未然に防ぐガード節）。それ以外の`yt_dlp`実行時の例外は`except Exception`で捕捉してエラーログを出力し、ボット検知マーカーに一致する場合は`BotDetectionError`として再送出、それ以外は`False`を返す。
* 根拠: [_typeチェックによる早期return] (行番号: 711〜715 / 抜粋: "if info.get('_type') in ('playlist', 'multi_video'):\n                    logger.warning(\n                        f"⚠️ プレイリスト/チャンネルURLは対象外です(1動画のみ処理可能): {task.url}"\n                    )\n                    return False")、[try-exceptブロック] (行番号: 731〜737 / 抜粋: "except Exception as e:\n            logger.error(f"⚠️ Universal DL エラー: {e}", exc_info=True)\n            if _is_bot_detection_error(e):")
...
* 根拠: [ScrapingStrategy.downloadとクリーンアップ呼び出し] (行番号: 765〜791, 787 / 抜粋: "def download(self, task: DownloadTask) -> bool:", "self._cleanup_stale_ytdlp_artifacts(final_path)\n\n        if self._should_skip(final_path): return True")


* **引数/リクエスト**: `task: DownloadTask`
* 根拠: [引数定義] (行番号: 765 / 抜粋: "def download(self, task: DownloadTask) -> bool:")
...
* 根拠: [ダウンロード委譲] (行番号: 791 / 抜粋: "return self._download_with_ytdlp(m3u8_url, final_path, task.url, target_dir)")
...
* 根拠: [_cleanup_stale_ytdlp_artifactsとDocstring] (行番号: 794〜818 / 抜粋: "def _cleanup_stale_ytdlp_artifacts(final_path: Path) -> None:\n        """final_pathと同名で始まる中間生成物（NAS上に残った古い`.part`/\n        `.part-FragN.part`/`.ytdl`/旧版の`.fragments.tmp`ディレクトリ等）を削除する。", "以前の実装は結合(merge)処理をNAS上のfinal_pathへ直接outtmplさせていたため、\n        処理が中断すると数百〜数千個のフラグメント断片がNAS上に残留し続けていた\n        （実機で確認）。")


* **引数/リクエスト**: `final_path: Path`
* 根拠: [引数定義] (行番号: 794〜818 / 抜粋: "def _cleanup_stale_ytdlp_artifacts(final_path: Path) -> None:")


* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 794 / 抜粋: "def _cleanup_stale_ytdlp_artifacts(final_path: Path) -> None:")
...
* 根拠: [_fetch_html] (行番号: 820〜839 / 抜粋: "def _fetch_html(self, url: str) -> Optional[str]:")


* **引数/リクエスト**: `url: str`
* **戻り値/レスポンス**: `Optional[str]`（取得成功時はHTML文字列、失敗時`None`）
* 根拠: [戻り値ヒント] (行番号: 820 / 抜粋: "def _fetch_html(self, url: str) -> Optional[str]:")


* **副作用**: 対象URLへのHTTP GETリクエスト。
* 根拠: [HTTPリクエスト] (行番号: 823 / 抜粋: "res = self.session.get(url, timeout=CONFIG.REQUEST_TIMEOUT)")
...
* 根拠: [関数定義とDocstring] (行番号: 360〜386 / 抜粋: "def _packer_base_n_digits(num: int, radix: int) -> str:")


* **引数/リクエスト**: `num: int`（変換対象の索引値）, `radix: int`（パッカーの基数。正規表現でHTMLから捕捉した実際の値）
* 根拠: [引数定義] (行番号: 360 / 抜粋: "def _packer_base_n_digits(num: int, radix: int) -> str:")
...
* 根拠: [_extract_m3u8_urlとradix取得のコメント] (行番号: 841〜874, 847 / 抜粋: "def _extract_m3u8_url(self, html: str) -> Optional[str]:", "# D-L5: group(2)がpacker本来のradix('a')。以前はこれを無視してbase36固定\n        # (chars 36種のmod 36)で単語を復元していたため、radixが36以外(典型的には\n        # 62)のページでは誤った単語に置換され、m3u8抽出そのものに失敗しうった。\n        radix = int(match.group(2))")


* **引数/リクエスト**: `html: str`
* **戻り値/レスポンス**: `Optional[str]`（抽出できたm3u8 URL、失敗時`None`）
* 根拠: [戻り値ヒント と末尾return] (行番号: 841, 791 / 抜粋: "def _extract_m3u8_url(self, html: str) -> Optional[str]:", "return None")
...
* 根拠: [ガード節] (行番号: 844 / 抜粋: "if not match: return None")
...
* 根拠: [_fetch_m3u8_manifestとDocstring] (行番号: 895〜932 / 抜粋: "def _fetch_m3u8_manifest(self, m3u8_url: str, page_url: str) -> Optional[str]:\n        """m3u8マニフェスト本体を、ブラウザ偽装(impersonate)付きで直接取得する。")


* **引数/リクエスト**: `m3u8_url: str`, `page_url: str`（Refererヘッダー設定用）
* 根拠: [引数定義] (行番号: 895 / 抜粋: "def _fetch_m3u8_manifest(self, m3u8_url: str, page_url: str) -> Optional[str]:")


* **戻り値/レスポンス**: `Optional[str]`（マニフェスト本文、取得失敗時`None`）
* 根拠: [戻り値ヒント] (行番号: 895 / 抜粋: "def _fetch_m3u8_manifest(self, m3u8_url: str, page_url: str) -> Optional[str]:")
...
* 根拠: [引数定義] (行番号: 935 / 抜粋: "def _localize_m3u8_manifest(manifest_text: str, base_url: str) -> str:")


* **戻り値/レスポンス**: `str`（絶対URL化済みのマニフェスト本文）
* 根拠: [戻り値ヒントと末尾return] (行番号: 935, 639 / 抜粋: "def _localize_m3u8_manifest(manifest_text: str, base_url: str) -> str:", "return "\\n".join(lines)")
...
* 根拠: [_fetch_segment_once] (行番号: 958〜974 / 抜粋: "def _fetch_segment_once(self, url: str, page_url: str) -> bytes:\n        \"\"\"1個のHLSセグメントをcurl_cffi(ブラウザ偽装)で1回だけ取得する。\n\n        リトライは行わない(_download_segment側で行う)。")


* **引数/リクエスト**: `url: str`（セグメントの絶対URL）, `page_url: str`（Refererヘッダー設定用）
* 根拠: [引数定義] (行番号: 958 / 抜粋: "def _fetch_segment_once(self, url: str, page_url: str) -> bytes:")
...
* 根拠: [_download_segments_and_localize_manifestとDocstring] (行番号: 1006〜1078 / 抜粋: "def _download_segments_and_localize_manifest(\n        self, localized_manifest: str, page_url: str, tmp_dir: Path\n    ) -> str:")


* **引数/リクエスト**: `localized_manifest: str`（絶対URL化済みマニフェスト）, `page_url: str`（Refererヘッダー設定用）, `tmp_dir: Path`（セグメント保存先の一時ディレクトリ）
* 根拠: [引数定義] (行番号: 1006〜1078 / 抜粋: "def _download_segments_and_localize_manifest(\n        self, localized_manifest: str, page_url: str, tmp_dir: Path\n    ) -> str:")
...
* **副作用**: マニフェスト内の各セグメントURLを`_download_segment`で並行ダウンロードし`tmp_dir`配下へファイル書き込み（`ThreadPoolExecutor`, 最大5ワーカー）。いずれかのセグメントで例外が発生した場合（**（2026-09-06 品質監査で修正）** `as_completed`ループ中に`KeyboardInterrupt`等の`BaseException`が届いた場合も含む）は、`executor.shutdown(wait=True, cancel_futures=True)`により、まだ実行が始まっていないキュー済みのセグメント取得（＝HTTP GETリクエスト自体）をキャンセルする（Issue #104の修正、後述のエラーハンドリング参照）。
* 根拠: [並行ダウンロードとファイル書き込み] (行番号: 1035〜1045 / 抜粋: "def _fetch_one(idx: int, url: str) -> Tuple[int, str]:\n            suffix = Path(url.split('?')[0]).suffix or '.bin'\n            local_name = f"seg_{idx:06d}{suffix}"\n            local_path = tmp_dir / local_name\n            content = self._download_segment(url, page_url)\n            local_path.write_bytes(content)")


* **エラーハンドリング**: 個別セグメントのダウンロード失敗（`_download_segment`が送出する例外、`BotDetectionError`含む）は`future.result()`の呼び出し元でそのまま伝播する。**Issue #104の修正（`with ThreadPoolExecutor(...) as executor:`ブロック終了時の暗黙のshutdownがキュー済み残り全件の完走を待ってしまい、モジュールDocstring/仕様書が謳う「即時セッション中断」が事実上機能していなかった不具合の修正）**により、`as_completed`ループを`try`/`except`で囲み、例外捕捉時に`executor.shutdown(wait=True, cancel_futures=True)`を明示的に呼んで未着手のキュー済みセグメントをキャンセルしたうえで、同じ例外を`raise`により再送出する（実行中だった最大`_FRAGMENT_DOWNLOAD_WORKERS`件分の完了は待つ）。**（2026-09-06 品質監査で修正）** 捕捉するのは`Exception`ではなく`BaseException`である。2回目の停止シグナルで`BatchDownloader._signal_handler`（ソースコメント上は`_handle_signal`と表記されているが、本ファイルに存在するメソッド名は`_signal_handler`）が送出する`KeyboardInterrupt`は`Exception`の派生ではないため、以前の`except Exception:`ではここを素通りして`with`ブロック終了時の暗黙の`shutdown(wait=True)`（`cancel_futures`なし）に落ち、「即時強制中断」のはずが数千件のキュー済みセグメントを完走するまで止まらなかった、というのがソースコメントに記載された変更理由である。
* 根拠: [コメント] (行番号: 1052 / 抜粋: "idx, local_uri = future.result()  # 例外はそのまま呼び出し元へ伝播させる")
* 根拠: [Issue #104修正のtry/except] (行番号: 984〜1003 / 抜粋: "try:\n                for future in as_completed(futures):\n                    idx, local_uri = future.result()  # 例外はそのまま呼び出し元へ伝播させる\n                    resolved[idx] = local_uri\n            except BaseException:", "# Exception ではなく BaseException を捕捉する: 2回目の停止シグナルで\n                # _handle_signal が送出する KeyboardInterrupt は Exception の派生ではなく、\n                # 以前はここを素通りして with ブロック終了時の shutdown(wait=True)\n                # (cancel_futures なし)に落ち、「即時強制中断」のはずが数千件の\n                # キュー済みセグメントを完走するまで止まらなかった。", "executor.shutdown(wait=True, cancel_futures=True)\n                raise")
...
* 根拠: [各メソッド定義とDocstring] (行番号: 1081〜1109, 1111〜1215 / 抜粋: "def _prepare_fragment_tmp_dir(tmp_dir: Path) -> bool:", "def _merge_fragments_and_transfer_to_nas(")
...
* 根拠: (行番号: 877〜893, 1192〜1198 / 抜粋: "def _select_variant_from_master(manifest_text: str, manifest_url: str) -> Optional[str]:", "variant_url = self._select_variant_from_master(manifest_text, m3u8_url)")
...
* 根拠: [_download_with_ytdlpとヘルパー呼び出し] (行番号: 1217〜1281, 1133〜1140 / 抜粋: "def _download_with_ytdlp(self, m3u8_url: str, final_path: Path, page_url: str, save_dir: Path) -> bool:", "tmp_dir = CONFIG.LOCAL_TMP_DIR / (final_path.name + ".fragments.tmp")\n        if not self._prepare_fragment_tmp_dir(tmp_dir):\n            return False", "if not self._merge_fragments_and_transfer_to_nas(\n                localized_manifest, page_url, tmp_dir, local_merged_path, nas_tmp_path, final_path\n            ):\n                return False")


* **引数/リクエスト**: `m3u8_url: str`, `final_path: Path`（NAS上の最終保存先）, `page_url: str`, `save_dir: Path`
* 根拠: [引数定義] (行番号: 1217 / 抜粋: "def _download_with_ytdlp(self, m3u8_url: str, final_path: Path, page_url: str, save_dir: Path) -> bool:")
...
* 根拠: [_get_strategy] (行番号: 1311〜1320 / 抜粋: "def _get_strategy(self, url: str) -> DownloadStrategy:")


* **引数/リクエスト**: `url: str`
* **戻り値/レスポンス**: `DownloadStrategy`（`ScrapingStrategy` または `UniversalYtDlpStrategy`。`None` は返さない）
* 根拠: [戻り値ヒント] (行番号: 1311 / 抜粋: "def _get_strategy(self, url: str) -> DownloadStrategy:")
...
* 根拠: [_collect_tasks] (行番号: 1322〜1370 / 抜粋: "def _collect_tasks(self) -> List[DownloadTask]:")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `List[DownloadTask]`
* 根拠: [戻り値ヒント] (行番号: 1322 / 抜粋: "def _collect_tasks(self) -> List[DownloadTask]:")
...
* 根拠: [引数定義] (行番号: 1372 / 抜粋: "def _purge_skipped_tasks(self, skipped_tasks: List[DownloadTask]) -> None:")
...
* 根拠: [メソッド定義とDocstring] (行番号: 1436〜1451 / 抜粋: "def _sleep_between_tasks(self, url: str) -> None:\n        """次のタスクまで待機する。")


* **引数/リクエスト**: `url: str`
* 根拠: [引数定義とDocstring] (行番号: 1436 / 抜粋: "def _sleep_between_tasks(self, url: str) -> None:")
...
* 根拠: [run] (行番号: 1453〜1474 / 抜粋: "def run(self) -> None:")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 1453 / 抜粋: "def run(self) -> None:")
...
* 根拠: [各メソッド定義とDocstring] (行番号: 1476〜1509, 1511〜1554, 1556〜1606 / 抜粋: "def _preflight_checks(self) -> bool:", "def _prepare_tasks(self) -> List[DownloadTask]:", "def _process_tasks(self, tasks: List[DownloadTask]) -> None:")
...
* 根拠: [_run_locked全体] (行番号: 1608〜1622 / 抜粋: "def _run_locked(self) -> None:\n        if not self._preflight_checks():\n            return\n\n        tasks = self._prepare_tasks()\n        if not tasks:\n            return" / "self._process_tasks(tasks)")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 1608 / 抜粋: "def _run_locked(self) -> None:")
...
flowchart TD
    ...
    GetStrategy -->|"その他・有効なYouTube"| YTDlp["UniversalYtDlpStrategyを実行<br>(Issue #566: extract_info後、_typeが<br>playlist/multi_videoなら即False)"]
...
| 優先度 | ファイル名(推測可) | 理由 | 根拠 |
| --- | --- | --- | --- |
| 高 | `services/notification_service.py` | Discordへの実際のWebhook送信ロジック、接続先URL、引数の仕様（`image_data`など）がブラックボックスとなっているため。 | 根拠: [import文] (行番号: 124 / 抜粋: "from services.notification_service import _send_discord_webhook") |
...
| 低 | プロジェクトルート直下の `services/` ディレクトリ構成 | `resolve_my_home_system_root`（`file_utils.py`）の自動探索ロジックが依存する前提ディレクトリ構造を確認するため。 | 根拠: [PROJECT_ROOT解決処理] (行番号: 94 / 抜粋: "PROJECT_ROOT = resolve_my_home_system_root(CURRENT_DIR)") |
...
* 根拠: [_signal_handlerのD-L3コメント] (行番号: 1305〜1307 / 抜粋: "# D-L3: 1回目のシグナル後もタスクが終わらない(数GB規模のダウンロード中\n        # 等)場合、2回目のシグナルで即座に強制中断する。ロック解放は\n        # run()のtry/finallyが担保する。")
...
* 根拠: [_standalone_send_discord_webhookとimport/except] (行番号: 99〜121, 103, 131 / 抜粋: "def _standalone_send_discord_webhook(messages, image_data=None, channel="notify") -> bool:", "except ImportError:", "_send_discord_webhook = _standalone_send_discord_webhook")
...
* **`BOT_DETECTION_MARKERS`は数字・フレーズを問わず単語境界一致に統一済み（Issue #467）**: `_is_bot_detection_error`は当初、"429"/"403"/"503"のような数字のみのマーカーだけを正規表現の単語境界(`\b`)で厳密に判定し、"sign in to confirm you're not a bot"等のフレーズマーカーは部分文字列一致(`in`)のままだった。動画IDなどに埋め込まれた偶然の数字列（例:「AbC403XyZ」）への誤検知は数字マーカー側の`\b`化で解消済みだったが、フレーズマーカー側にも「無関係なログにたまたま同一フレーズが含まれる場合の誤検知」という同種のリスクが残っていたため、Issue #467でマーカー種別を問わず`\b`による単語境界一致へ統一した。**（Issue #396で修正）** 以前の`"sign in to confirm"`はyt-dlpの年齢制限メッセージ（"Sign in to confirm your age..."）にも一致し、年齢制限動画1本で12時間の全停止に入っていた（`ENABLE_YOUTUBE_DL=true`環境で顕在化）。マーカーを"not a bot"まで含む文言へ絞り、`BOT_DETECTION_EXCLUDED_MARKERS`（"confirm your age"）を含むメッセージは判定より優先して除外する。新たなマーカーを追加する際は、`\b`（正規表現の単語境界）による一致になる点を踏まえ、yt-dlpの他のサインイン系メッセージ（年齢制限・会員限定等）に意図せず一致しないか確認すること。回帰テストは`test_batch_download_discord_fixes.py`の`TestIsBotDetectionError`。**（2026-09-06 品質監査で修正）** 数字マーカー（`marker.isdigit()`が真のもの）は`\b`ではなく`(?<![\w\-/.])NNN(?![\w\-/]|\.\w)`で判定するようになった。`\b`は`-`・`/`・`.`も単語境界とみなすため、品番・URLパス・一時ファイル名に埋め込まれた数字列（"ssis-403"、"/dm18/ja/ssis-403"、"ssni-429.mp4.fragments"、"seg-403-v1"等）にも一致し、通信エラー1件で12時間クールダウンに入っていた。前後が英数字・`_`・`-`・`/`・`.`（後続については「`.`＋英数字」）のいずれかであれば識別子の一部とみなして除外し、"HTTP Error 403: Forbidden"・"too many 503 error responses"・"error 429."のような文言は引き続き一致する。フレーズマーカーは従来どおり`\b...\b`のままである。
...
* 根拠: [_is_bot_detection_errorのコメントと判定処理] (行番号: 274〜277, 285〜300 / 抜粋: "# #467: フレーズマーカーも従来は部分文字列一致(in)のままだったため、\n    # 無関係なログにたまたま同一フレーズが含まれる場合に誤検知しうる\n    # (可能性は低いが数字マーカーと同種の問題)。フレーズ・数字を問わず\n    # 単語境界(\\b)で厳密に判定する方式に統一する。" / "if marker.isdigit():\n            # 数字マーカー(403/429/503)は \\b だけでは不十分: 正規表現の単語境界は\n            # \"-\" \"/\" \".\" も境界とみなすため" / "pattern = rf\"(?<![\\w\\-/.]){re.escape(marker)}(?![\\w\\-/]|\\.\\w)\"\n        else:\n            pattern = rf\"\\b{re.escape(marker)}\\b\"\n        if re.search(pattern, message):\n            return True")
...
* **`noplaylist`によるプレイリスト一括ダウンロードの防止**: `UniversalYtDlpStrategy.download`の`ydl_opts`に`noplaylist: True`が追加され、リストの1行がプレイリスト/チャンネルURLだった場合に1タスクの中で無制限にダウンロードして`MAX_TASKS_PER_RUN`による1回あたりの上限が迂回される問題が修正されている。**[修正済み・Issue #566]** ただし`noplaylist: True`が実際に効くのは、動画とプレイリストの両方を指すURL（`watch?v=X&list=Y`のように動画IDとプレイリストIDが両方含まれる形式）で動画側を選ばせるケースのみである。動画IDを含まない純粋なプレイリスト/チャンネルURL（`list=...`のみ、または`/channel/...`等）に対しては、yt-dlp内部の`InfoExtractor._yes_playlist`がvideo_id不在時に`noplaylist`オプション自体を一切参照せず常にプレイリスト全体を返す実装になっており、`noplaylist: True`は無力だった。つまりM-7-3で導入された対策は「動画/プレイリスト混在URL」のケースしかカバーしておらず、`list/*.txt`に素のプレイリスト/チャンネルURLが1行紛れ込むだけで、`MAX_TASKS_PER_RUN`のガバナンスとボット検知回避のペーシングを迂回する無制限ダウンロードが起きうる欠落があった。Issue #566ではこれを補う形で、`extract_info(download=False)`の戻り値`info`の`_type`が`'playlist'`または`'multi_video'`の場合を明示的に検知し、`filename`算出・ダウンロード実行に進まず警告ログを出力して`False`を返すガード節を追加した（`noplaylist`の置き換えではなく、`noplaylist`が無力なケースを補完する追加防御）。
* 根拠: [ydl_optsのコメント(M-7-3)] (行番号: 675〜679 / 抜粋: "# M-7-3: リスト1行がプレイリストURL(またはチャンネルURL)だった場合、\n            # noplaylistが無いとyt-dlpがその1タスクの中で全件を無制限にダウンロード")、[Issue #566: '_type'チェックのコメントとコード] (行番号: 703〜715 / 抜粋: "# #566: yt-dlpの'noplaylist'は「動画とプレイリストの両方を指すURL\n                # (watch?v=X&list=Y)で動画側だけを選ぶ」オプションであり、動画IDを\n                # 含まない純粋なプレイリスト/チャンネルURLには効果がない\n                # (yt-dlp内部のInfoExtractor._yes_playlistがvideo_id不在時は\n                # noplaylistを一切参照せず常にプレイリスト全体を返す実装のため)。" / "if info.get('_type') in ('playlist', 'multi_video'):\n                    logger.warning(\n                        f"⚠️ プレイリスト/チャンネルURLは対象外です(1動画のみ処理可能): {task.url}"\n                    )\n                    return False")
* **回帰テスト（Issue #566）**: `DDD/test_batch_download_discord_fixes.py`の`TestUniversalYtDlpStrategyRejectsPlaylistTypeInfo`が、`_type`が`'playlist'`/`'multi_video'`の場合に`download`が`False`を返し`prepare_filename`/`process_ie_result`が一切呼ばれないこと（`test_playlist_type_info_is_rejected`）、および通常の`_type: 'video'`では既存の`_should_skip`短絡経路を通って`True`が返ること（`test_single_video_type_info_is_not_rejected`）を検証する。
...
| `curl_cffi`のブラウザ偽装(`impersonate="chrome"`)の忠実度・バージョン要件 | 忠実度そのもの(どのChromeバージョン相当のTLS/JA3フィンガープリントを再現するか)は`curl_cffi`本体の実装に依存するためリポジトリ内では確定できないが、**バージョン要件は`DDD/requirements.txt`で確認できる**。同ファイル7行目は`curl_cffi>=0.16.3`、すなわち**下限のみを指定した非ピン留め**であり、上限が無いため`pip install`のたびに新しいバージョンが入りうる(`pip-audit-weekly-audit.yml`が週次で監査する3ファイルのうちの1つでもある)。また`impersonate="chrome"`は特定バージョン番号を指定しない**エイリアス指定**であり、`curl_cffi`側が「その時点で最新とみなすChromeプロファイル」に解決するため、**`curl_cffi`を更新すると偽装対象のプロファイルが無言で変わりうる**点が保守上の注意点になる(WAF回避の挙動が更新前後で変化した場合、まずこの点を疑うこと)。リポジトリ内で確定できるのはここまでで、各プロファイルが再現するフィンガープリントの詳細は`curl_cffi`のドキュメント側の情報である。 | 直接ソース確認: `DDD/requirements.txt:7`（`curl_cffi>=0.16.3`）, `DDD/batch_download_discord.py:895,944`（`impersonate="chrome"` の2箇所）（参考: `.github/workflows/pip-audit-weekly-audit.yml`） |
## 10. 自己検証結果
