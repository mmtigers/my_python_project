...
| `file_utils.redact_discord_webhook_url` | ローカルモジュール | **（2026-09-06 品質監査で修正）** `_standalone_send_discord_webhook`の送信失敗ログに含まれる例外文字列中のDiscord Webhook URL（トークン部分）をマスクするために使用 | 根拠: [import文] (行番号: 44 / 抜粋: "from file_utils import redact_discord_webhook_url")、[利用箇所] (行番号: 116 / 抜粋: "logger.warning(f\"⚠️ Discord Webhook送信に失敗しました: {redact_discord_webhook_url(e)}\")") |
...
* 根拠: [戻り値ヒントと各return] (行番号: 95, 88, 95, 99 / 抜粋: "def _standalone_send_discord_webhook(messages, image_data=None, channel="notify") -> bool:", "if not url:\n        return False")
...
* 根拠: [関数定義とDocstring] (行番号: 129〜142 / 抜粋: "def _resolve_cookies_file() -> Optional[Path]:\n    """YouTube等のボット検知回避用Cookieファイルを解決する。")


* **引数/リクエスト**: なし
* 根拠: [関数定義] (行番号: 129 / 抜粋: "def _resolve_cookies_file() -> Optional[Path]:")


* **戻り値/レスポンス**: `Optional[Path]`（解決できたCookieファイルのパス、なければ`None`）
* 根拠: [戻り値ヒント] (行番号: 129 / 抜粋: "def _resolve_cookies_file() -> Optional[Path]:")


* **副作用**: `os.getenv`による環境変数読み込み、ファイル未存在時の警告ログ出力。
* 根拠: [処理内容] (行番号: 135, 140 / 抜粋: "cookies_env = os.getenv("YOUTUBE_COOKIES_FILE")", "logger.warning(f"⚠️ YOUTUBE_COOKIES_FILE で指定されたファイルが見つかりません: {cookies_path}")")
...
* 根拠: [インスタンス生成] (行番号: 276 / 抜粋: "CONFIG = AppConfig()")
...
* **引数/リクエスト**: なし（`self`のみ）
* **戻り値/レスポンス**: `Path`
* 根拠: [戻り値] (行番号: 274 / 抜粋: "return self.NAS_MOUNT_POINT / self.NAS_MARKER_FILE")
...
* 根拠: [クラス定義とDocstring] (行番号: 279〜285 / 抜粋: "class BotDetectionError(Exception):\n    """YouTube等からボット検知/レート制限（429やSign-in要求等）を検知した際に送出する。")
...
* 根拠: [関数定義とDocstring] (行番号: 323〜338 / 抜粋: "def _round_robin_flatten(groups: Iterable[List["DownloadTask"]]) -> List["DownloadTask"]:\n    """複数グループのリストを、グループ順ではなくラウンドロビンで1本のリストに平坦化する。")


* **引数/リクエスト**: `groups: Iterable[List["DownloadTask"]]`
* 根拠: [引数定義] (行番号: 323 / 抜粋: "def _round_robin_flatten(groups: Iterable[List["DownloadTask"]]) -> List["DownloadTask"]:")
...
* 根拠: [関数定義とDocstring] (行番号: 385〜392 / 抜粋: "def _looks_like_block_page(html: str) -> bool:\n    """取得したHTMLがCloudflare等のボット検知チャレンジページかを判定する。")


* **引数/リクエスト**: `html: str`
* 根拠: [引数定義] (行番号: 385 / 抜粋: "def _looks_like_block_page(html: str) -> bool:")


* **戻り値/レスポンス**: `bool`
* 根拠: [戻り値ヒント] (行番号: 385 / 抜粋: "def _looks_like_block_page(html: str) -> bool:")
...
* 根拠: [DownloadTaskクラス] (行番号: 403〜405 / 抜粋: "class DownloadTask(NamedTuple):\n    url: str\n    source_name: str")
...
* 根拠: [モジュールレベル変数定義] (行番号: 410 / 抜粋: "_discord_circuit_breaker = DiscordCircuitBreaker(failure_threshold=CONFIG.DISCORD_CIRCUIT_BREAKER_THRESHOLD)")
...
* 根拠: [DiscordNotifier.send] (行番号: 414〜430 / 抜粋: "def send(text: str, is_error: bool = False) -> None:")


* **引数/リクエスト**: `text: str` (通知内容), `is_error: bool = False` (エラー通知フラグ)
* 根拠: [引数定義] (行番号: 414 / 抜粋: "def send(text: str, is_error: bool = False) -> None:")
...
* 根拠: [関数定義] (行番号: 434〜436 / 抜粋: "def _read_history_file(path: Path) -> Set[str]:\n        with open(path, "r", encoding="utf-8") as f:\n            return {line.strip() for line in f if line.strip()}")


* **引数/リクエスト**: `path: Path`
* 根拠: [引数定義] (行番号: 434 / 抜粋: "def _read_history_file(path: Path) -> Set[str]:")


* **戻り値/レスポンス**: `Set[str]`
* 根拠: [戻り値ヒント] (行番号: 434 / 抜粋: "def _read_history_file(path: Path) -> Set[str]:")
...
* 根拠: [メソッド定義とDocstring] (行番号: 498〜508 / 抜粋: "def is_in_cooldown() -> Optional[datetime.datetime]:\n        """クールダウン中であれば解除予定時刻を、そうでなければNoneを返す。"""")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `Optional[datetime.datetime]`
* 根拠: [戻り値ヒント] (行番号: 498 / 抜粋: "def is_in_cooldown() -> Optional[datetime.datetime]:")


* **副作用**: クールダウンファイルの読み込み。
* 根拠: [ファイル読み込み] (行番号: 504 / 抜粋: "until = datetime.datetime.fromisoformat(path.read_text(encoding="utf-8").strip())")
...
* 根拠: [メソッド定義とコメント] (行番号: 511〜522 / 抜粋: "def trigger_cooldown() -> None:\n        until = datetime.datetime.now() + datetime.timedelta(hours=CONFIG.BOT_DETECTION_COOLDOWN_HOURS)")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 511 / 抜粋: "def trigger_cooldown() -> None:")
...
* 根拠: [メソッド定義] (行番号: 525〜529 / 抜粋: "def clear() -> None:\n        try:\n            CONFIG.BOT_DETECTION_COOLDOWN_FILE.unlink(missing_ok=True)")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 525 / 抜粋: "def clear() -> None:")


* **副作用**: クールダウンファイルの削除(`unlink`)。
* 根拠: [削除処理] (行番号: 527 / 抜粋: "CONFIG.BOT_DETECTION_COOLDOWN_FILE.unlink(missing_ok=True)")
...
* 根拠: [NetworkManager.create_session] (行番号: 533〜539 / 抜粋: "def create_session() -> requests.Session:")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `requests.Session`
* 根拠: [戻り値ヒント] (行番号: 533 / 抜粋: "def create_session() -> requests.Session:")
...
* 根拠: [FileSystemManager.sanitize_filename] (行番号: 543〜544 / 抜粋: "def sanitize_filename(filename: str) -> str:\n        return _shared_sanitize_filename(filename)")


* **引数/リクエスト**: `filename: str`
* **戻り値/レスポンス**: `str`
* 根拠: [関数定義] (行番号: 543 / 抜粋: "def sanitize_filename(filename: str) -> str:")
...
* 根拠: [FileSystemManager.ensure_dir] (行番号: 547〜561 / 抜粋: "def ensure_dir(path: Path) -> bool:")


* **引数/リクエスト**: `path: Path`
* **戻り値/レスポンス**: `bool`（成功時`True`、権限エラーおよびその他の`OSError`時`False`）
* 根拠: [戻り値ヒント] (行番号: 547 / 抜粋: "def ensure_dir(path: Path) -> bool:")


* **副作用**: ディレクトリ作成(`mkdir`)、エラー時のDiscord通知。
* 根拠: [mkdir呼び出し] (行番号: 549 / 抜粋: "path.mkdir(parents=True, exist_ok=True)")
...
* 根拠: [メソッド定義とDocstring] (行番号: 564〜582 / 抜粋: "def sweep_stale_fragment_dirs() -> None:
        """#398: クラッシュ・強制終了等で未クリーンアップのまま残った
        CONFIG.LOCAL_TMP_DIR配下の "*.fragments.tmp" ディレクトリを一掃する。")


* **引数/リクエスト**: なし
* 根拠: [引数定義] (行番号: 564 / 抜粋: "def sweep_stale_fragment_dirs() -> None:")


* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 564 / 抜粋: "def sweep_stale_fragment_dirs() -> None:")
...
* 根拠: [FileSystemManager.check_disk_space] (行番号: 585〜599 / 抜粋: "def check_disk_space(path: Path, min_free_gb: Optional[int] = None) -> bool:")
...
* 根拠: [SystemHealthChecker.is_within_time_window] (行番号: 603〜605 / 抜粋: "def is_within_time_window() -> bool:")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `bool`
* 根拠: [戻り値ヒント] (行番号: 603 / 抜粋: "def is_within_time_window() -> bool:")
...
* 根拠: [SystemHealthChecker.verify_nas_mountとREQUIRE_NAS_MOUNT分岐] (行番号: 608〜614 / 抜粋: "def verify_nas_mount() -> bool:\n        if not CONFIG.REQUIRE_NAS_MOUNT:\n            return True")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `bool`（`REQUIRE_NAS_MOUNT`無効時、またはマウント確認成功時は`True`。マウント確認失敗時は`False`）
* 根拠: [戻り値ヒント] (行番号: 608 / 抜粋: "def verify_nas_mount() -> bool:")


* **副作用**: `REQUIRE_NAS_MOUNT`有効時、未マウント検知時のDiscord通知(`is_error=True`)。
* 根拠: [通知送信] (行番号: 612 / 抜粋: "DiscordNotifier.send("⛔ CRITICAL: NASマウントエラー", is_error=True)")
...
* 根拠: [SystemHealthChecker.check_dependencies] (行番号: 617〜620 / 抜粋: "def check_dependencies() -> None:")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 617 / 抜粋: "def check_dependencies() -> None:")
...
* 根拠: [メソッド定義とDocstring] (行番号: 623〜642 / 抜粋: "def check_yt_dlp_freshness() -> None:\n        """yt-dlpのバージョン（YYYY.MM.DD形式）が古すぎないか警告する。")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 623 / 抜粋: "def check_yt_dlp_freshness() -> None:")
...
* 根拠: [DownloadStrategyクラス] (行番号: 647〜670 / 抜粋: "class DownloadStrategy(ABC):")


* **引数/リクエスト**: `__init__(self, save_base_dir: Path, session: requests.Session)`
* 根拠: [__init__定義] (行番号: 648〜650 / 抜粋: "def __init__(self, save_base_dir: Path, session: requests.Session):")
...
* 根拠: [_determine_save_dir内] (行番号: 662 / 抜粋: "if not FileSystemManager.ensure_dir(target_dir): return None")
...
* 根拠: [引数定義] (行番号: 674 / 抜粋: "def download(self, task: DownloadTask) -> bool:")
...
* 根拠: [ScrapingStrategy.downloadとクリーンアップ呼び出し] (行番号: 761〜787, 783 / 抜粋: "def download(self, task: DownloadTask) -> bool:", "self._cleanup_stale_ytdlp_artifacts(final_path)\n\n        if self._should_skip(final_path): return True")


* **引数/リクエスト**: `task: DownloadTask`
* 根拠: [引数定義] (行番号: 761 / 抜粋: "def download(self, task: DownloadTask) -> bool:")
...
* 根拠: [ダウンロード委譲] (行番号: 787 / 抜粋: "return self._download_with_ytdlp(m3u8_url, final_path, task.url, target_dir)")
...
* 根拠: [_cleanup_stale_ytdlp_artifactsとDocstring] (行番号: 790〜814 / 抜粋: "def _cleanup_stale_ytdlp_artifacts(final_path: Path) -> None:\n        """final_pathと同名で始まる中間生成物（NAS上に残った古い`.part`/\n        `.part-FragN.part`/`.ytdl`/旧版の`.fragments.tmp`ディレクトリ等）を削除する。", "以前の実装は結合(merge)処理をNAS上のfinal_pathへ直接outtmplさせていたため、\n        処理が中断すると数百〜数千個のフラグメント断片がNAS上に残留し続けていた\n        （実機で確認）。")


* **引数/リクエスト**: `final_path: Path`
* 根拠: [引数定義] (行番号: 790〜814 / 抜粋: "def _cleanup_stale_ytdlp_artifacts(final_path: Path) -> None:")


* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 790 / 抜粋: "def _cleanup_stale_ytdlp_artifacts(final_path: Path) -> None:")
...
* 根拠: [_fetch_html] (行番号: 816〜835 / 抜粋: "def _fetch_html(self, url: str) -> Optional[str]:")


* **引数/リクエスト**: `url: str`
* **戻り値/レスポンス**: `Optional[str]`（取得成功時はHTML文字列、失敗時`None`）
* 根拠: [戻り値ヒント] (行番号: 816 / 抜粋: "def _fetch_html(self, url: str) -> Optional[str]:")


* **副作用**: 対象URLへのHTTP GETリクエスト。
* 根拠: [HTTPリクエスト] (行番号: 819 / 抜粋: "res = self.session.get(url, timeout=CONFIG.REQUEST_TIMEOUT)")
...
* 根拠: [関数定義とDocstring] (行番号: 356〜382 / 抜粋: "def _packer_base_n_digits(num: int, radix: int) -> str:")


* **引数/リクエスト**: `num: int`（変換対象の索引値）, `radix: int`（パッカーの基数。正規表現でHTMLから捕捉した実際の値）
* 根拠: [引数定義] (行番号: 356 / 抜粋: "def _packer_base_n_digits(num: int, radix: int) -> str:")
...
* 根拠: [_extract_m3u8_urlとradix取得のコメント] (行番号: 837〜870, 843 / 抜粋: "def _extract_m3u8_url(self, html: str) -> Optional[str]:", "# D-L5: group(2)がpacker本来のradix('a')。以前はこれを無視してbase36固定\n        # (chars 36種のmod 36)で単語を復元していたため、radixが36以外(典型的には\n        # 62)のページでは誤った単語に置換され、m3u8抽出そのものに失敗しうった。\n        radix = int(match.group(2))")


* **引数/リクエスト**: `html: str`
* **戻り値/レスポンス**: `Optional[str]`（抽出できたm3u8 URL、失敗時`None`）
* 根拠: [戻り値ヒント と末尾return] (行番号: 837, 791 / 抜粋: "def _extract_m3u8_url(self, html: str) -> Optional[str]:", "return None")
...
* 根拠: [ガード節] (行番号: 840 / 抜粋: "if not match: return None")
...
* 根拠: [_fetch_m3u8_manifestとDocstring] (行番号: 891〜928 / 抜粋: "def _fetch_m3u8_manifest(self, m3u8_url: str, page_url: str) -> Optional[str]:\n        """m3u8マニフェスト本体を、ブラウザ偽装(impersonate)付きで直接取得する。")


* **引数/リクエスト**: `m3u8_url: str`, `page_url: str`（Refererヘッダー設定用）
* 根拠: [引数定義] (行番号: 891 / 抜粋: "def _fetch_m3u8_manifest(self, m3u8_url: str, page_url: str) -> Optional[str]:")


* **戻り値/レスポンス**: `Optional[str]`（マニフェスト本文、取得失敗時`None`）
* 根拠: [戻り値ヒント] (行番号: 891 / 抜粋: "def _fetch_m3u8_manifest(self, m3u8_url: str, page_url: str) -> Optional[str]:")
...
* 根拠: [引数定義] (行番号: 931 / 抜粋: "def _localize_m3u8_manifest(manifest_text: str, base_url: str) -> str:")


* **戻り値/レスポンス**: `str`（絶対URL化済みのマニフェスト本文）
* 根拠: [戻り値ヒントと末尾return] (行番号: 931, 639 / 抜粋: "def _localize_m3u8_manifest(manifest_text: str, base_url: str) -> str:", "return "\\n".join(lines)")
...
* 根拠: [_fetch_segment_once] (行番号: 954〜970 / 抜粋: "def _fetch_segment_once(self, url: str, page_url: str) -> bytes:\n        \"\"\"1個のHLSセグメントをcurl_cffi(ブラウザ偽装)で1回だけ取得する。\n\n        リトライは行わない(_download_segment側で行う)。")


* **引数/リクエスト**: `url: str`（セグメントの絶対URL）, `page_url: str`（Refererヘッダー設定用）
* 根拠: [引数定義] (行番号: 954 / 抜粋: "def _fetch_segment_once(self, url: str, page_url: str) -> bytes:")
...
* 根拠: [_download_segments_and_localize_manifestとDocstring] (行番号: 1002〜1074 / 抜粋: "def _download_segments_and_localize_manifest(\n        self, localized_manifest: str, page_url: str, tmp_dir: Path\n    ) -> str:")


* **引数/リクエスト**: `localized_manifest: str`（絶対URL化済みマニフェスト）, `page_url: str`（Refererヘッダー設定用）, `tmp_dir: Path`（セグメント保存先の一時ディレクトリ）
* 根拠: [引数定義] (行番号: 1002〜1074 / 抜粋: "def _download_segments_and_localize_manifest(\n        self, localized_manifest: str, page_url: str, tmp_dir: Path\n    ) -> str:")
...
* 根拠: [コメント] (行番号: 1048 / 抜粋: "idx, local_uri = future.result()  # 例外はそのまま呼び出し元へ伝播させる")
...
* 根拠: [各メソッド定義とDocstring] (行番号: 1077〜1105, 1107〜1211 / 抜粋: "def _prepare_fragment_tmp_dir(tmp_dir: Path) -> bool:", "def _merge_fragments_and_transfer_to_nas(")
...
* 根拠: (行番号: 873〜889, 1192〜1198 / 抜粋: "def _select_variant_from_master(manifest_text: str, manifest_url: str) -> Optional[str]:", "variant_url = self._select_variant_from_master(manifest_text, m3u8_url)")
...
* 根拠: [_download_with_ytdlpとヘルパー呼び出し] (行番号: 1213〜1277, 1133〜1140 / 抜粋: "def _download_with_ytdlp(self, m3u8_url: str, final_path: Path, page_url: str, save_dir: Path) -> bool:", "tmp_dir = CONFIG.LOCAL_TMP_DIR / (final_path.name + ".fragments.tmp")\n        if not self._prepare_fragment_tmp_dir(tmp_dir):\n            return False", "if not self._merge_fragments_and_transfer_to_nas(\n                localized_manifest, page_url, tmp_dir, local_merged_path, nas_tmp_path, final_path\n            ):\n                return False")


* **引数/リクエスト**: `m3u8_url: str`, `final_path: Path`（NAS上の最終保存先）, `page_url: str`, `save_dir: Path`
* 根拠: [引数定義] (行番号: 1213 / 抜粋: "def _download_with_ytdlp(self, m3u8_url: str, final_path: Path, page_url: str, save_dir: Path) -> bool:")
...
* 根拠: [_get_strategy] (行番号: 1307〜1316 / 抜粋: "def _get_strategy(self, url: str) -> DownloadStrategy:")


* **引数/リクエスト**: `url: str`
* **戻り値/レスポンス**: `DownloadStrategy`（`ScrapingStrategy` または `UniversalYtDlpStrategy`。`None` は返さない）
* 根拠: [戻り値ヒント] (行番号: 1307 / 抜粋: "def _get_strategy(self, url: str) -> DownloadStrategy:")
...
* 根拠: [_collect_tasks] (行番号: 1318〜1366 / 抜粋: "def _collect_tasks(self) -> List[DownloadTask]:")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `List[DownloadTask]`
* 根拠: [戻り値ヒント] (行番号: 1318 / 抜粋: "def _collect_tasks(self) -> List[DownloadTask]:")
...
* 根拠: [引数定義] (行番号: 1368 / 抜粋: "def _purge_skipped_tasks(self, skipped_tasks: List[DownloadTask]) -> None:")
...
* 根拠: [メソッド定義とDocstring] (行番号: 1432〜1447 / 抜粋: "def _sleep_between_tasks(self, url: str) -> None:\n        """次のタスクまで待機する。")


* **引数/リクエスト**: `url: str`
* 根拠: [引数定義とDocstring] (行番号: 1432 / 抜粋: "def _sleep_between_tasks(self, url: str) -> None:")
...
* 根拠: [run] (行番号: 1449〜1470 / 抜粋: "def run(self) -> None:")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 1449 / 抜粋: "def run(self) -> None:")
...
* 根拠: [各メソッド定義とDocstring] (行番号: 1472〜1505, 1507〜1550, 1552〜1602 / 抜粋: "def _preflight_checks(self) -> bool:", "def _prepare_tasks(self) -> List[DownloadTask]:", "def _process_tasks(self, tasks: List[DownloadTask]) -> None:")
...
* 根拠: [_run_locked全体] (行番号: 1604〜1618 / 抜粋: "def _run_locked(self) -> None:\n        if not self._preflight_checks():\n            return\n\n        tasks = self._prepare_tasks()\n        if not tasks:\n            return" / "self._process_tasks(tasks)")


* **引数/リクエスト**: なし
* **戻り値/レスポンス**: `None`
* 根拠: [戻り値ヒント] (行番号: 1604 / 抜粋: "def _run_locked(self) -> None:")
...
| 優先度 | ファイル名(推測可) | 理由 | 根拠 |
| --- | --- | --- | --- |
| 高 | `services/notification_service.py` | Discordへの実際のWebhook送信ロジック、接続先URL、引数の仕様（`image_data`など）がブラックボックスとなっているため。 | 根拠: [import文] (行番号: 120 / 抜粋: "from services.notification_service import _send_discord_webhook") |
...
| 低 | プロジェクトルート直下の `services/` ディレクトリ構成 | `resolve_my_home_system_root`（`file_utils.py`）の自動探索ロジックが依存する前提ディレクトリ構造を確認するため。 | 根拠: [PROJECT_ROOT解決処理] (行番号: 90 / 抜粋: "PROJECT_ROOT = resolve_my_home_system_root(CURRENT_DIR)") |
...
* 根拠: [_standalone_send_discord_webhookとimport/except] (行番号: 95〜117, 103, 127 / 抜粋: "def _standalone_send_discord_webhook(messages, image_data=None, channel="notify") -> bool:", "except ImportError:", "_send_discord_webhook = _standalone_send_discord_webhook")
