...
| --- | --- | --- |
| 対象システムの全体的なAPI依存先 | リポジトリ全体を`get_retry_session`/`create_resilient_session`/`retry_api_call`で検索し、実際の呼び出し元を直接確認した。`get_retry_session`は`MY_HOME_SYSTEM/old/news_service.py`23行目、`NewsService._fetch_feed(self, url)`(20〜32行目)内で`session = common.get_retry_session()`として使用されており、Google News RSS(`RSS_HYOGO_ITAMI`, `RSS_NARA`, `RSS_TOP`、9〜13行目で定義)の取得(`session.get(url, headers=self.HEADERS, timeout=10)`)にリトライ機能を適用する目的で使われていることを確認した（ただし本ファイルは`old/`ディレクトリ配下）。`create_resilient_session`と`retry_api_call`は、`MY_HOME_SYSTEM/tests/test_core_utils_and_network.py`(127〜168行目)でのユニットテスト以外に、本番コード側での呼び出し箇所は見つからなかった。特に`MY_HOME_SYSTEM/services/switchbot_service.py`11行目には`# from common import retry_api_call # 削除`というコメントアウトされたインポート文が残っており、SwitchBot API通信では過去に`retry_api_call`を使用していたが、明示的に「削除」された経緯があることを確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/old/news_service.py:1-32`, `MY_HOME_SYSTEM/services/switchbot_service.py:11`, `MY_HOME_SYSTEM/tests/test_core_utils_and_network.py:127-168`（`retry_api_call`/`create_resilient_session`の本番コードでの呼び出し箇所は不在を確認） |
| `core.network`ロガーの設定詳細 | `MY_HOME_SYSTEM/core/network.py`45行目の`logging.getLogger("core.network")`は、`core/logger.py`の`setup_logging`（`propagate=False`を設定しコンソール・ファイル・Discordの3ハンドラを付与する専用ロガー生成関数）を経由しない、標準の`logging.getLogger`呼び出しであることを直接確認した。リポジトリ全体を`logging.getLogger("core.network")`および`logging.basicConfig`で検索したが、`core.network`という名前（またはそのルートロガー）に対して明示的にハンドラ・フォーマット・レベルを設定している箇所は見つからず、主要な起動元である`MY_HOME_SYSTEM/unified_server.py`も`logging.basicConfig`を呼び出していないことを確認した。したがって、`retry_api_call`のリトライ発生時に`tenacity.before_sleep_log`が出力するWARNINGログは、Pythonの`logging`モジュールの既定動作（ルートロガーにハンドラが1つも設定されていない場合の`lastResort`ハンドラ、WARNING以上をフォーマットなしでstderrへ出力）に委ねられており、他の主要モジュールが用いる`setup_logging`ベースのログ（ファイル保存・Discord通知）とは独立した経路であることが判明した。 | 直接ソース確認: `MY_HOME_SYSTEM/core/network.py:37-46`, `MY_HOME_SYSTEM/core/logger.py:46-86`, `MY_HOME_SYSTEM/unified_server.py`（`logging.basicConfig`呼び出しなしを確認） |
