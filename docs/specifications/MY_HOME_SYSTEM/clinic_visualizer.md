...
| --- | --- | --- |
| `config.CLINIC_STATS_CSV` / `config.CLINIC_GRAPH_PATH`の実際の設定値 | `MY_HOME_SYSTEM/config.py`を直接確認した。`CLINIC_STATS_CSV`(504行目)は`os.path.join(ASSETS_DIR, "clinic_stats.csv")`、`CLINIC_GRAPH_PATH`(505行目)は`os.path.join(ASSETS_DIR, "clinic_trend.png")`であり、`ASSETS_DIR`(224〜227行目)は`ensure_safe_path_with_backoff(os.path.join(NAS_PROJECT_ROOT, "assets"), "assets")`（`NAS_PROJECT_ROOT`は既定`/mnt/nas/home_system`）であることを確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/config.py:224-227, 504-505` |
| 入力CSVの実際のスキーマ・データ内容 | `MY_HOME_SYSTEM/monitors/old/clinic_analyzer.py:32-36`を直接確認した。`self.headers`は`["timestamp", "am_reserved", "am_in_clinic", "pm_reserved", "pm_in_clinic"]`の5列であり、本ファイル(`clinic_visualizer.py`)が60行目で参照する`df['am_reserved']`, `df['pm_reserved']`等のカラム名と完全に一致することを確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/monitors/old/clinic_analyzer.py:32-36` |
| 生成されたグラフ画像の実際の利用先（表示箇所） | `MY_HOME_SYSTEM/dashboard.py`を`clinic`および`CLINIC_GRAPH_PATH`で検索したが該当箇所は見つからなかった。さらにリポジトリ全体（`MY_HOME_SYSTEM/views/`, `MY_HOME_SYSTEM/templates/`を含む）を`clinic`で検索しても、`config.py`（設定値定義）と`monitors/old/`配下の3ファイル自身以外に参照箇所は存在しないことを確認した。したがって`config.CLINIC_GRAPH_PATH`が指す画像ファイルを読み込み・表示する処理は、本リポジトリ内には存在しないことを確認した（解消不可）。 | 直接ソース確認: `MY_HOME_SYSTEM/dashboard.py`（`clinic`関連の記述なしを確認）、リポジトリ全体の`clinic`文字列検索結果 |
| 本ファイルの実行トリガー（cron/スケジューラ設定） | `MY_HOME_SYSTEM/scheduler_boot.py`を直接確認した。29〜43行目の`TASKS`リスト（定期実行対象スクリプトの一覧）に`clinic_visualizer.py`を含む`monitors/old/`配下のスクリプトは1件も含まれておらず、本ファイルは同スケジューラによる定期実行対象になっていないことを確認した。リポジトリ全体を`clinic_visualizer`で検索しても、本ファイルを呼び出す箇所は自分自身（`monitors/old/clinic_visualizer.py`）以外に見つからず、実際の実行トリガーは特定できなかった。 | 直接ソース確認: `MY_HOME_SYSTEM/scheduler_boot.py:29-43` |
