...
* 根拠: [クラス定義] (行番号: 26 / 抜粋: "class RoutineService:")、[シングルトン化] (行番号: 454 / 抜粋: "routine_service = RoutineService()")
...
* 根拠: [戻り値] (行番号: 451 / 抜粋: "return self._serialize_flow(flow, progress, now)")
...
* 根拠: [ロック・トランザクション] (行番号: 404-405)、[1回目の強制切替] (行番号: 416 / 抜粋: "progress = self._apply_forced_transition(cur, user_id, flow_key, flow, progress, now)")、[ステップ完了/トグル処理] (行番号: 426-444)、[2回目の強制切替とそのコメント] (行番号: 446-449 / 抜粋: "# 直後にチェックポイントへ到達し、かつ既に締切時刻を過ぎている場合\n                # (例: 出遅れて自由時間に入った瞬間には既に7:50だった)、この場で\n                # 通過処理まで済ませ、フロントが追加のポーリングを待たずに済むようにする。\n                progress = self._apply_forced_transition(cur, user_id, flow_key, flow, progress, now)")
...
* 根拠: [インスタンス化] (行番号: 454 / 抜粋: "routine_service = RoutineService()")
