...
| 解析対象 | 提供されたコードのみ |
| 推測・補完 | 一切なし |
| 解析基準コミット | `0d67384` (+同一ブランチ内で土日のチェックポイント時刻上書き対応・休日PMのステップスキップ/宿題引き継ぎ対応・朝の準備の順不同チェックリスト化を追加修正) |

## 関連ドキュメント

* [routine_data.md](./routine_data.md) - 本ファイルが`from routine_data import (FULL_BONUS_EXP, FULL_BONUS_GOLD, ROUTINE_FLOWS, WEEKEND_DAYS, RoutineFlow, get_checkpoint_index, get_effective_checkpoint_time)`でimportするフロー定義・定数の実体（`get_effective_checkpoint_time`は土日対応で新規追加、`WEEKEND_DAYS`は休日PM微修正で新規importに追加。`RoutineStep`の`checklist`フィールド自体は本ファイルではimportされる関数・定数ではなく`flow['steps']`の各要素の値として参照される）
...
| 名称 | 理由 | 根拠 |
| --- | --- | --- |
| `common.get_db_cursor(commit=True)`の内部実装 | SQLiteのロック再試行・WALモード設定・例外時ロールバック等の具体的な実装は`core/database.py`側にあり本ファイルからは不明。 | [with文] (行番号: 356, 382 / 抜粋: "with common.get_db_cursor(commit=True) as cur:") |
| `common.get_now_iso()`の出力形式 | 生成されるISO文字列の具体的なフォーマット(タイムゾーン表記等)が本ファイルからは不明。 | [関数呼び出し] (行番号: 155, 164, 181, 219 / 抜粋: "now_iso = common.get_now_iso()") |
| `game_logic.GameLogic.calc_level_progress`の内部計算式 | レベルアップに必要な経験値テーブル等の計算ロジックの詳細は不明。詳細は[game_logic.md](./game_logic.md)参照。 | [関数呼び出し] (行番号: 214-216 / 抜粋: "game_logic.GameLogic.calc_level_progress(\n            user['level'], user['exp'], exp\n        )") |
| `sound_manager.play`の実際の音声再生手段 | 音声ファイルの実体・再生失敗時の挙動が不明。詳細は[sound_manager.md](./sound_manager.md)参照。 | [関数呼び出し] (行番号: 222 / 抜粋: "sound_manager.play(\"level_up\")") |
| `_get_user_balance_lock`の内部実装(`RefCountedLockRegistry`) | 参照カウント付きロックレジストリの具体的な排他制御実装は`core/utils.py`側にあり不明。詳細は[quest_locks.md](./quest_locks.md)参照。 | [with文] (行番号: 355, 381 / 抜粋: "with _get_user_balance_lock(user_id):") |
...
* 根拠: [クラス定義] (行番号: 24 / 抜粋: "class RoutineService:")、[シングルトン化] (行番号: 431 / 抜粋: "routine_service = RoutineService()")


* **引数/リクエスト**: 該当なし(クラス定義自体はコンストラクタを持たず、`object`のデフォルト`__init__`を使用)
* 根拠: [クラス定義] (行番号: 24-431、`__init__`の明示的定義は存在しない)
...
### `_empty_statuses`（朝の準備チェックリスト化で再度シグネチャ・戻り値の組み立て方を変更）

* **役割**: フローの全ステップに対する初期`steps_status`辞書と、初期の`current_step_index`を組み立てて`(statuses, current_index)`のタプルで返す。**（休日PM微修正で変更）** `skip_keys`に含まれるステップは(位置に関わらず)`'done'`として扱われる。**（朝の準備チェックリスト化で再度変更）** 以前は「`skip_keys`に含まれない最初の1ステップだけを`'current'`にする」という単一の逐次ロジックだったが、`checklist=True`なステップを特別扱いするようになった。具体的には、まず全ステップを一度走査し、`skip_keys`に含まれるものは`'done'`、`checklist=True`(かつ`skip_keys`に含まれない)ものは無条件に`'current'`、それ以外は仮に`'locked'`とする。次に、`checklist=True`かつ未スキップのステップが1つでも残っていれば(`checklist_remaining_indices`が非空なら)、そのフローは「チェックリストフェーズ」にあるとみなし、`current_index`にはチェックリスト内の最初のステップのインデックスを設定して即座に返す(この場合`current_index`は「今チェックリスト中である」というフェーズの目印に過ぎず、個々のステップの状態は既に`'current'`/`'done'`として`statuses`に反映済み)。`checklist=True`のステップが1つも残っていない場合(フローに`checklist=True`のステップが元々無い、または`weekend_skip`/`weekend_carryover`で全て`skip_keys`に吸収された場合)は、従来通り先頭から走査して最初の非`'done'`ステップを`'current'`にする逐次ロジックにフォールバックする(`pm`フローは`checklist`が全ステップ`False`のため常にこちらの経路を通る)。全ステップが`'done'`扱いの場合、`current_index`は`len(flow['steps'])`(フロー完了扱い)になる。
* 根拠: [メソッド定義・docstring] (行番号: 79-91 / 抜粋: "def _empty_statuses(self, flow: RoutineFlow, skip_keys: Set[str]) -> Tuple[Dict[str, str], int]:\n        \"\"\"スキップ分を'done'扱いにしたうえで初期状態を組み立てる。\n\n        checklist=Trueなステップ(順不同でチェックできるグループ、例: 朝の準備5項目)が\n        残っている場合は、それら全てを同時に'current'(未チェック・チェック可能)にし、\n        current_step_indexはその先頭のインデックスにする(単なるフェーズの目印であり、\n        チェックリスト内の個々の状態はsteps_statusの方で管理する)。\n        checklistステップが無い、またはスキップで全て埋まっている場合は、従来通り\n        先頭から最初の未スキップステップだけを'current'にする逐次ロジックにフォールバックする。\n\n        戻り値は(steps_status, current_step_index)。全ステップがスキップ済みの場合の\n        current_step_indexはlen(flow['steps'])(=フロー完了扱い)になる。\n        \"\"\"")
...
* 根拠: [戻り値] (行番号: 123-126 / 抜粋: "idx = start_index\n        while idx < len(flow['steps']) and statuses.get(flow['steps'][idx]['key']) == 'done':\n            idx += 1\n        return idx")


* **副作用**: なし
* 根拠: [メソッド定義] (行番号: 116-126)


* **エラーハンドリング**: なし
* 根拠: [メソッド定義] (行番号: 116-126、`try`/`except`は存在しない)
...
* **副作用**: なし
* 根拠: [メソッド定義] (行番号: 128-136)


* **エラーハンドリング**: なし(`json.loads`が不正なJSONに対して送出する`json.JSONDecodeError`は捕捉されない)
* 根拠: [メソッド定義] (行番号: 128-136、`try`/`except`は存在しない)
...
* **戻り値/レスポンス**: `None`
* 根拠: [型ヒント] (行番号: 172 / 抜粋: "-> None:")


* **副作用**: `routine_progress`テーブルへの`UPDATE`実行。
* 根拠: [UPDATE文] (行番号: 173-182)


* **エラーハンドリング**: なし
* 根拠: [メソッド定義] (行番号: 172-182、`try`/`except`は存在しない)
...
* 根拠: [メソッド定義・docstring] (行番号: 184-197 / 抜粋: "def _eligible_done_ratio(self, flow: RoutineFlow, progress: Dict[str, Any]) -> float:\n        \"\"\"チェックポイントより前の全ステップのうち、'done'の割合(0.0〜1.0)を返す。\n\n        チェックポイントが無いフロー、またはチェックポイントより前にステップが\n        無いフローは常に1.0(満額)とみなす(ゼロ除算回避)。\n        \"\"\"")
...
* 根拠: [戻り値] (行番号: 207-208 / 抜粋: "ratio = self._eligible_done_ratio(flow, progress)\n        return round(FULL_BONUS_GOLD * ratio), round(FULL_BONUS_EXP * ratio)")


* **副作用**: なし
* 根拠: [メソッド定義] (行番号: 199-208)


* **エラーハンドリング**: なし
* 根拠: [メソッド定義] (行番号: 199-208、`try`/`except`は存在しない)
...
* 根拠: [メソッド定義] (行番号: 210-223 / 抜粋: "def _grant_bonus(self, cur, user_id: str, gold: int, exp: int) -> Dict[str, Any]:\n        user = cur.execute(\"SELECT * FROM quest_users WHERE user_id=?\", (user_id,)).fetchone()\n        if not user:\n            return {\"leveled_up\": False, \"new_level\": None}")、[戻り値] (行番号: 223 / 抜粋: "return {\"leveled_up\": leveled_up, \"new_level\": new_level}")
...
* 根拠: [早期リターン] (行番号: 235-237 / 抜粋: "checkpoint_idx = get_checkpoint_index(flow)\n        if checkpoint_idx is None or progress['current_step_index'] > checkpoint_idx:\n            return progress")、[時刻未到達の早期リターン] (行番号: 242-243 / 抜粋: "if now < deadline:\n            return progress")



### `_toggle_checklist_step`（朝の準備チェックリスト化で新規追加）

* **役割**: `checklist=True`なステップを順不同でチェック/チェック解除する。まずチェックポイントを既に通過している(`current_step_index`がチェックポイントのインデックスより大きい)かどうかをガードし、通過済みならボーナスが確定済みとして`HTTPException(400, "すでに出発済みのため変更できません")`を送出して変更を拒否する(要件確認済み: チェックポイント通過後は取り消し不可でロックする)。次に対象ステップの`steps_status`を`'current'`(未チェック)⇔`'done'`(チェック済み)でトグルする。チェックポイントを持つフローであれば、チェックポイントより前の`checklist=True`な全ステップが`'done'`になったかどうか(`all_done`)を毎回再計算し、全て`'done'`なら`current_step_index`をチェックポイントのインデックスへ進めて`in_free_time=True`にしチェックポイントステップ自体を`'current'`にする(通常の逐次フローが最後のステップを完了したときと同じ遷移)。1つでも未チェックが残っていれば、`current_step_index`をチェックリスト先頭のインデックスへ戻し(全チェック済みから1つ取り消した場合の巻き戻しに対応)、`in_free_time=False`、チェックポイントステップを`'locked'`に戻す。
* 根拠: [メソッド定義・docstring] (行番号: 276-285 / 抜粋: "def _toggle_checklist_step(self, flow: RoutineFlow, progress: Dict[str, Any], target_step: Dict[str, Any]) -> None:\n        \"\"\"checklist=Trueなステップを順不同でチェック/チェック解除する(要件: 朝の準備は\n        好きな順にチェックでき、間違えたら取り消せるようにしたい)。\n\n        チェックポイントを既に通過している(current_step_indexがチェックポイントの\n        インデックスより後)場合は、ボーナスが確定済みのため変更を拒否する。\n        チェックリスト全項目が'done'になった/でなくなったタイミングで、フローの\n        現在地(current_step_index)・in_free_timeをチェックポイントの前後に\n        まとめて進める/戻す。\n        \"\"\"")、[出発済みロックの拒否] (行番号: 286-288 / 抜粋: "checkpoint_idx = get_checkpoint_index(flow)\n        if checkpoint_idx is not None and progress['current_step_index'] > checkpoint_idx:\n            raise HTTPException(status_code=400, detail=\"すでに出発済みのため変更できません\")")、[トグル本体] (行番号: 290-292 / 抜粋: "key = target_step['key']\n        # 'current'=未チェック(いつでもチェック可能)、'done'=チェック済み、の2値をトグルする。\n        progress['steps_status'][key] = 'current' if progress['steps_status'].get(key) == 'done' else 'done'")、[全項目達成時の遷移/巻き戻し] (行番号: 297-308 / 抜粋: "checklist_keys = [s['key'] for s in flow['steps'][:checkpoint_idx] if s['checklist']]\n        all_done = bool(checklist_keys) and all(progress['steps_status'].get(k) == 'done' for k in checklist_keys)\n        checkpoint_key = flow['steps'][checkpoint_idx]['key']\n        if all_done:\n            progress['current_step_index'] = checkpoint_idx\n            progress['in_free_time'] = True\n            progress['steps_status'][checkpoint_key] = 'current'\n        else:\n            first_checklist_idx = next(i for i, s in enumerate(flow['steps']) if s['checklist'])\n            progress['current_step_index'] = first_checklist_idx\n            progress['in_free_time'] = False\n            progress['steps_status'][checkpoint_key] = 'locked'")
...
* **エラーハンドリング**: チェックポイント通過済みの場合`HTTPException(status_code=400, detail="すでに出発済みのため変更できません")`を送出する。それ以外の`try`/`except`は存在しない。
* 根拠: [例外送出] (行番号: 287-288 / 抜粋: "if checkpoint_idx is not None and progress['current_step_index'] > checkpoint_idx:\n            raise HTTPException(status_code=400, detail=\"すでに出発済みのため変更できません\")")
...
* **副作用**: なし
* 根拠: [メソッド定義] (行番号: 310-352)


* **エラーハンドリング**: なし
* 根拠: [メソッド定義] (行番号: 310-352、`try`/`except`は存在しない)
...
* 根拠: [メソッド定義] (行番号: 354-372 / 抜粋: "def get_today_state(self, user_id: str, now: Optional[datetime.datetime] = None) -> Dict[str, Any]:\n        with _get_user_balance_lock(user_id):\n            with common.get_db_cursor(commit=True) as cur:")、[休日PM微修正のnow伝播] (行番号: 368 / 抜粋: "progress = self._get_or_create_progress(cur, user_id, flow_key, flow, date_str, now)")


* **引数/リクエスト**: `user_id: str`、`now: Optional[datetime.datetime] = None`(省略時は`datetime.datetime.now(JST)`)
* 根拠: [メソッド定義] (行番号: 354 / 抜粋: "def get_today_state(self, user_id: str, now: Optional[datetime.datetime] = None) -> Dict[str, Any]:")、[デフォルト値解決] (行番号: 361 / 抜粋: "now = now or datetime.datetime.now(JST)")
...
* 根拠: [戻り値] (行番号: 372 / 抜粋: "return {\"date\": date_str, \"flows\": flows_out}")、[未開始時の値] (行番号: 366 / 抜粋: "flows_out[flow_key] = {\"started\": False, \"title\": flow['title']}")、[土日対応] (行番号: 370 / 抜粋: "flows_out[flow_key] = self._serialize_flow(flow, progress, now)")
...
* 根拠: [ロック・トランザクション] (行番号: 381-382)、[1回目の強制切替] (行番号: 393 / 抜粋: "progress = self._apply_forced_transition(cur, user_id, flow_key, flow, progress, now)")、[ステップ完了/トグル処理] (行番号: 403-421)、[2回目の強制切替とそのコメント] (行番号: 423-426 / 抜粋: "# 直後にチェックポイントへ到達し、かつ既に締切時刻を過ぎている場合\n                # (例: 出遅れて自由時間に入った瞬間には既に7:50だった)、この場で\n                # 通過処理まで済ませ、フロントが追加のポーリングを待たずに済むようにする。\n                progress = self._apply_forced_transition(cur, user_id, flow_key, flow, progress, now)")


* **エラーハンドリング**: (1) `flow_key`が`ROUTINE_FLOWS`に存在しなければ`HTTPException(404, "Unknown flow_key")`。(2) 対象ユーザーが存在しなければ`HTTPException(404, "User not found")`。(3) フローが本日まだ開始していなければ`HTTPException(400, "このフローはまだ開始していません")`。(4) 進捗の`current_step_index`が既に全ステップ数以上(完了済み)であれば`HTTPException(400, "本日のフローは完了しています")`。(5) **（朝の準備チェックリスト化で追加）** `step_key`が`flow['steps']`のどのキーとも一致しなければ`HTTPException(404, "Unknown step_key")`。(6) `checklist=False`の逐次ステップに限り、リクエストの`step_key`が現在のステップキーと一致しなければ`HTTPException(409, "表示が古いようです。再読み込みしてください")`。(7) 同じく逐次ステップに限り、現在のステップがチェックポイント(自由時間)であれば`HTTPException(400, "自由時間は時間になると自動的に次へ進みます")`(自由時間は時間経過による自動遷移のみで、明示的な完了操作は許可しない)。(8) **（朝の準備チェックリスト化で追加）** `checklist=True`のステップは、チェックポイントを既に通過していれば`_toggle_checklist_step`内で`HTTPException(400, "すでに出発済みのため変更できません")`。
* 根拠: [flow_key検証] (行番号: 377-378 / 抜粋: "if flow_key not in ROUTINE_FLOWS:\n            raise HTTPException(status_code=404, detail=\"Unknown flow_key\")")、[ユーザー検証] (行番号: 384-385 / 抜粋: "if not user:\n                    raise HTTPException(status_code=404, detail=\"User not found\")")、[開始判定] (行番号: 388-389 / 抜粋: "if not self._is_flow_started_today(flow, now):\n                    raise HTTPException(status_code=400, detail=\"このフローはまだ開始していません\")")、[完了済み判定] (行番号: 396-397 / 抜粋: "if idx >= len(flow['steps']):\n                    raise HTTPException(status_code=400, detail=\"本日のフローは完了しています\")")、[step_key未検出] (行番号: 400-401 / 抜粋: "if target_step is None:\n                    raise HTTPException(status_code=404, detail=\"Unknown step_key\")")、[ステップ不一致判定] (行番号: 409-410 / 抜粋: "if current_step['key'] != step_key:\n                        raise HTTPException(status_code=409, detail=\"表示が古いようです。再読み込みしてください\")")、[チェックポイント判定] (行番号: 411-412 / 抜粋: "if current_step['checkpoint_time']:\n                        raise HTTPException(status_code=400, detail=\"自由時間は時間になると自動的に次へ進みます\")")
...
* 根拠: [インスタンス化] (行番号: 431 / 抜粋: "routine_service = RoutineService()")
...
**（朝の準備チェックリスト化で新規追加）** 以下は`complete_step`から`checklist=True`なステップに対してのみ呼ばれる`_toggle_checklist_step`のフローチャートです。

```mermaid
flowchart TD
    TStart(["Start: _toggle_checklist_step(flow, progress, target_step)"]) --> TCheckpointIdx["checkpoint_idx = get_checkpoint_index(flow)"]
    TCheckpointIdx --> TLockedCheck{"checkpoint_idx is not None<br/>かつ<br/>current_step_index > checkpoint_idx?"}
    TLockedCheck -- Yes（既に出発済み） --> TRaise(["raise HTTPException(400, 'すでに出発済みのため変更できません')"])
    TLockedCheck -- No --> TToggle["steps_status[target_step.key] を\n'done'なら'current'へ、それ以外なら'done'へトグル"]
    TToggle --> TNoCheckpoint{"checkpoint_idx is None?"}
    TNoCheckpoint -- Yes --> TReturnVoid(["return（フローにチェックポイントが無い）"])
    TNoCheckpoint -- No --> TAllDoneCheck["checklist_keys = チェックポイントより前のchecklist=Trueな全キー\nall_done = 全てのchecklist_keysが'done'か?"]
    TAllDoneCheck -- Yes（全項目チェック済み） --> TAdvance["current_step_index = checkpoint_idx\nin_free_time = True\nチェックポイントステップを'current'に設定"]
    TAllDoneCheck -- No（未チェックが残っている） --> TRevert["current_step_index = チェックリスト先頭のインデックス\nin_free_time = False\nチェックポイントステップを'locked'に設定"]
...
| 優先度 | ファイル名(推測可) | 理由 | 根拠 |
| --- | --- | --- | --- |
| 高 | `routine_data.py` | `ROUTINE_FLOWS`の実際のフロー構成(ステップ数・チェックポイント位置・開始時刻)、**（土日対応で追加）**`get_effective_checkpoint_time`の解決ロジック、**（休日PM微修正で追加）**`weekend_skip`/`weekend_carryover`がどのステップに設定されているか、および**（朝の準備チェックリスト化で追加）**`checklist`がどのステップに設定されているかを把握しないと、`_resolve_skip_keys`/`_empty_statuses`/`_toggle_checklist_step`等のロジックの入力データを正しく理解できないため。 | [routine_data.md](./routine_data.md)(本バッチ内)、[インポート宣言] (行番号: 17-20) |
| 高 | `migrations/0010_add_routine_progress.sql` | `routine_progress`テーブルの正確なスキーマ(カラム制約・`UNIQUE`制約・インデックス)を確認し、本ファイルのSQL文(**休日PM微修正で追加**の`_was_done_on_any_date`による日付をまたいだ`IN (...)`検索を含む)が前提とする構造を検証するため(マイグレーション自体は仕様書ドリフト規約の対象外だが、コード理解のための直接参照は有用)。 | [SQL文] (行番号: 141-143, 156-165, 55-58) |
...
| 中 | `game_logic.py` | `calc_level_progress`のレベルアップ判定式の詳細を確認し、`_grant_bonus`が付与するボーナスがどうレベル/経験値に反映されるかを把握するため。 | [game_logic.md](./game_logic.md)(既存)、[関数呼び出し] (行番号: 214-216) |
...
* `complete_step`は`_apply_forced_transition`を2回呼び出す(処理前と処理後)。2回目の呼び出しについては「直後にチェックポイントへ到達し、かつ既に締切時刻を過ぎている場合、この場で通過処理まで済ませ、フロントが追加のポーリングを待たずに済むようにする」というコメントが付されている(行番号: 423-425)。`_apply_forced_transition`自体は冪等(`current_step_index > checkpoint_idx`なら何もしない)なため、2回呼んでも二重にボーナスが付与されることはない。
...
* **（朝の準備チェックリスト化で新規追加）** `_toggle_checklist_step`はチェックポイント通過後(`current_step_index > checkpoint_idx`)のトグルを`HTTPException(400)`で拒否する設計だが、この判定はメソッド呼び出し時点の`progress['current_step_index']`のみを見ており、`complete_step`内で直前に呼ばれる1回目の`_apply_forced_transition`(締切時刻を過ぎていれば強制的にチェックポイントを通過させる)の**後**に評価される。そのため、チェックリストが未完了のまま締切時刻を過ぎた状態で`complete_step`をチェックリスト項目に対して呼ぶと、1回目の`_apply_forced_transition`が先にチェックポイントを強制通過させてボーナスを確定させ、その直後に`_toggle_checklist_step`が「出発済み」としてリクエストを拒否する — 見た目には「締切を過ぎたらチェックできなくなる」という自然な挙動になるが、実装上は2つの独立したメソッドの呼び出し順序に依存している。
* **（朝の準備チェックリスト化で新規追加）** `_eligible_done_ratio`/`_compute_bonus_preview`という2つの独立したヘルパーへ計算ロジックを切り出したことで、`preview_bonus_gold`(表示専用、DBを変更しない)と`_apply_forced_transition`が実際に付与する`bonus_gold`(確定値、DBに永続化される)は常に同じ計算式を共有する。これにより、チェックポイント通過の前後でユーザーに見える数値が不連続に変わる(例えばプレビューでは90だったのに通過後に別の値になる)という不整合が構造的に起こらない設計になっている。逆に言えば、`preview_bonus_gold`は`_apply_forced_transition`が実際に呼ばれたかどうかに関わらず`_serialize_flow`のたびに毎回計算し直される値であり、`routine_progress`テーブルには一切保存されない。
