...
* 根拠: `def run_db_retention(now=None) -> RetentionOutcome:` (行番号: 364 / 抜粋: "def run_db_retention(")
...
| 各対象テーブルの実際の行数・最古の時刻 | 実機の DB にのみ存在する。本ファイルからは判断できない（これを測るための `build_report` である） | 根拠: `def build_report(now=None) -> dict[str, Any]:` (行番号: 319) |
| 書き込み側が実際に入れている時刻文字列の形式 | 本ファイルは `"%Y-%m-%d %H:%M:%S"` を前提に境界文字列を作るが、各テーブルへの書き込みは別モジュールが行う | 根拠: `def _cutoff_string(retention_days: int, now=None) -> str:` (行番号: 132) |
...
* 根拠: `class RetentionTarget:` (行番号: 59 / 抜粋: "class RetentionTarget:")

* **引数/リクエスト**: 該当なし（値オブジェクト）
* 根拠: `class RetentionTarget:` (行番号: 59)
...
* 根拠: `class TablePlan:` (行番号: 90 / 抜粋: "class TablePlan:"), `class TableResult:` (行番号: 102 / 抜粋: "class TableResult:"), `class RetentionOutcome:` (行番号: 110 / 抜粋: "class RetentionOutcome:")

### `plan_deletions`

* **役割**: 対象テーブルごとに「保持期間より古い行が何行あるか」を数える。**何も削除しない。** 読み取り専用接続を使うため書き込みロックを取らない。対象テーブルが DB に存在しない場合（マイグレーション未適用の古い DB）はスキップする。
* 根拠: `def plan_deletions(now=None) -> list[TablePlan]:` (行番号: 152 / 抜粋: "def plan_deletions(")

* **引数/リクエスト**: `now`（省略時は `get_now_jst()`）
* 根拠: `def plan_deletions(now=None) -> list[TablePlan]:` (行番号: 152)
...
* 根拠: `def _table_exists(conn: sqlite3.Connection, table: str) -> bool:` (行番号: 145 / 抜粋: "def _table_exists(")
...
* 根拠: `def apply_deletions(` (行番号: 196 / 抜粋: "def apply_deletions(")
...
* 根拠: `def apply_deletions(` (行番号: 196)
...
* 根拠: `def recent_backup(within_hours: int | None = None) -> tuple[bool, str]:` (行番号: 260 / 抜粋: "def recent_backup(")

* **引数/リクエスト**: `within_hours`（省略時 `config.DB_ROW_RETENTION_REQUIRE_BACKUP_WITHIN_HOURS`、既定26）
* 根拠: `def recent_backup(within_hours: int | None = None) -> tuple[bool, str]:` (行番号: 260)
...
* 根拠: `def reclaimable_bytes() -> int:` (行番号: 300 / 抜粋: "def reclaimable_bytes(")

* **引数/リクエスト**: なし
* 根拠: `def reclaimable_bytes() -> int:` (行番号: 300)
...
* 根拠: `def build_report(now=None) -> dict[str, Any]:` (行番号: 319 / 抜粋: "def build_report(")

* **引数/リクエスト**: `now`
* 根拠: `def build_report(now=None) -> dict[str, Any]:` (行番号: 319)
...
* **引数/リクエスト**: `now`
* 根拠: `def run_db_retention(now=None) -> RetentionOutcome:` (行番号: 364)
...
* 根拠: `def format_summary_lines(outcome: RetentionOutcome) -> list[str]:` (行番号: 395 / 抜粋: "def format_summary_lines(")

* **引数/リクエスト**: `outcome` (`RetentionOutcome`)
* 根拠: `def format_summary_lines(outcome: RetentionOutcome) -> list[str]:` (行番号: 395)
...
| 中 | `migrations/0014_add_timeseries_retention_indexes.sql` | 削除の `WHERE` 句が使う索引。無いと毎晩テーブル全体をスキャンしながら書き込みロックを保持する | `def apply_deletions(` (行番号: 196) |
