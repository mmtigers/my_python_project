...
| --- | --- | --- |
| DB接続・トランザクションの詳細 | `MY_HOME_SYSTEM/common.py:22-27`で`common.get_db_cursor`が`core.database.get_db_cursor`の再エクスポートであることを直接確認した上で、`MY_HOME_SYSTEM/core/database.py:11-50`の実装本体を直接確認した。`@contextmanager`が付与されたコンテキストマネージャで、`max_retries=5`回まで`sqlite3.connect(config.SQLITE_DB_PATH, timeout=30.0)`で接続し(`journal_mode=WAL`, `foreign_keys=ON`設定)、`yield conn.cursor()`後に`commit`引数が`True`なら`conn.commit()`する。`sqlite3.OperationalError`で"locked"を含む場合は`conn.close()`後`time.sleep(1.0)`してリトライし、それ以外の`sqlite3.OperationalError`または他の`Exception`発生時は`conn.rollback()`を実行してから例外を`raise`する設計である。リトライ上限到達時はエラーログを出力して`conn`を閉じる。本ファイル(`weather_service.py`)は50行目・113行目・351行目で`common.get_db_cursor(commit=True)`を`with`文で使用している。 | 直接ソース確認: `MY_HOME_SYSTEM/common.py:22-27`, `MY_HOME_SYSTEM/core/database.py:11-50`（本ファイル内利用箇所: `MY_HOME_SYSTEM/old/weather_service.py:50,113,351`） |
| 外部通知 (`send_push`) の仕様 | `MY_HOME_SYSTEM/common.py:31-37`で`common.send_push`が`services.notification_service.send_push`の再エクスポートであることを直接確認した上で、`MY_HOME_SYSTEM/services/notification_service.py:116-140`の実装本体を直接確認した。シグネチャは`send_push(user_id, messages, image_data=None, target="both", channel="notify", filename="snapshot.jpg") -> bool`。`target`は`"discord"`/`"line"`/`"both"`(既定)を受け付け、`"discord"`または`"both"`の場合は`_send_discord_webhook(messages, image_data, channel, filename)`を呼び出し、`"line"`または`"both"`の場合は`_send_line_push(user_id, line_msgs)`を呼び出す。LINE送信が失敗した場合はエラーログを出力した上でDiscordの`error`チャンネルへフォールバック通知を行う。戻り値は各送信経路の成否をANDした`bool`。本ファイルは152行目で`target="discord", channel="error"`を指定してエラー通知に、226行目で`target`(呼び出し元パラメータ、既定値未確認)を指定して天気レポート配信に使用している。 | 直接ソース確認: `MY_HOME_SYSTEM/common.py:31-37`, `MY_HOME_SYSTEM/services/notification_service.py:116-140`（本ファイル内利用箇所: `MY_HOME_SYSTEM/old/weather_service.py:152-156,226`） |
| 設定値の詳細と型 | `MY_HOME_SYSTEM/config.py:185`を直接確認した。`LINE_USER_ID: Optional[str] = os.getenv("LINE_USER_ID")`と定義されており、型は`Optional[str]`、値は環境変数`LINE_USER_ID`から取得され未設定時は`None`となる。ただし実際に設定される値そのものは`MY_HOME_SYSTEM/.env`(gitignore対象)にのみ存在し、`MY_HOME_SYSTEM/.env.example`を確認したが`LINE_USER_ID`というキー自体が記載されておらず、具体的な値は依然としてリポジトリ内からは確認できない。 | 直接ソース確認: `MY_HOME_SYSTEM/config.py:185`（`MY_HOME_SYSTEM/.env.example`にはキー記載なし、値自体は解消不可） |
