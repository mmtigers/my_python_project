...
| --- | --- | --- |
| `setup_logging` の詳細な挙動・副作用・戻り値 | `MY_HOME_SYSTEM/core/logger.py:46-85`を直接確認した。`setup_logging(name: str, webhook_url: str = None) -> logging.Logger`は、(1) `logging.getLogger(name)`を取得し既存ハンドラをクリア、(2) コンソール出力用`StreamHandler`(58〜60行目)、(3) `config.BASE_DIR/logs`配下に`home_system.log`固定名で出力する日次ローテーションの`TimedRotatingFileHandler`(`backupCount=7`、63〜74行目)、(4) `webhook_url`引数または`config.DISCORD_WEBHOOK_ERROR`が設定されていればERRORレベル以上を通知する`DiscordErrorHandler`(76〜84行目)、を登録した`logging.Logger`インスタンスを返す。`config.BASE_DIR`自体は`MY_HOME_SYSTEM/config.py:212`で`os.path.dirname(os.path.abspath(__file__))`（`config.py`の配置ディレクトリ）と定義されていることも確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/core/logger.py:46-85`, `MY_HOME_SYSTEM/config.py:212` |
