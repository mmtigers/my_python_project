...
| インポートされている全外部関数の詳細なシグネチャと挙動 | 各実装モジュールを直接確認した。`core/network.py`: `get_retry_session(retries=3, backoff_factor=1.0)`(8行目)と`create_resilient_session(retries=3, backoff_factor=2, status_forcelist=(500,502,504))`(22行目、旧API互換とコメントあり)はいずれもリトライ機能付き`requests.Session`を返し、`retry_api_call(func)`(37行目)は`tenacity`の`retry`(最大3回、指数バックオフ)でラップするデコレータ。`core/database.py`: `get_db_cursor(commit: bool = False)`(13行目)はリトライ機能付きDB接続のコンテキストマネージャ、`execute_read_query(query, params=())`(52行目)は`sqlite3.connect(f"file:{config.SQLITE_DB_PATH}?mode=ro", uri=True)`で読み取り専用接続しSELECTを実行、`save_log_generic(table, columns_list, values_list)`(67行目)は汎用INSERT、`save_log_async(...)`(81行目)はその`loop.run_in_executor`による非同期ラッパー。`services/notification_service.py`: `send_push(user_id, messages, image_data=None, target="both", channel="notify", filename="snapshot.jpg")`(116行目)はDiscordとLINEへの統合プッシュ通知、`send_reply(reply_token, messages)`(142行目)はLINE Reply API v3送信、`get_line_message_quota()`(167行目)はLINE送信数確認、`_send_discord_webhook(...)`(30行目)は`channel`引数に応じ`config.DISCORD_WEBHOOK_ERROR`等へPOST、`_send_line_push(user_id, messages)`(73行目)はLINE Push API v3送信。`core/utils.py`: `get_now_iso()`(12行目)/`get_today_date_str()`(15行目)/`get_display_date()`(18行目)はいずれも`pytz.timezone("Asia/Tokyo")`基準の現在日時をそれぞれISO8601、`"%Y-%m-%d"`、`"%m/%d"`形式の文字列で返す。いずれの関数も`common.py`自体（40行目の`setup_logging("common")`呼び出しを除く）からの呼び出しは確認できず、Facadeとして再エクスポートされているのみであることを確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/core/network.py:8-42`, `MY_HOME_SYSTEM/core/database.py:13-84`, `MY_HOME_SYSTEM/services/notification_service.py:30-171`, `MY_HOME_SYSTEM/core/utils.py:12-19` |
