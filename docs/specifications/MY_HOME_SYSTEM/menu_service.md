...
* [common.md](./common.md) - `get_db_cursor`を再エクスポートするFacade
* [database.md](./database.md) - `get_db_cursor`の実体
* [config.md](./config.md) - システム全体の設定値提供元(本ファイルでは未使用インポート)
...
| --- | --- | --- |
| DB接続・トランザクションの詳細仕様 | `MY_HOME_SYSTEM/core/database.py`を直接確認した。`get_db_cursor(commit: bool = False)`(12〜50行目)は`sqlite3.connect(config.SQLITE_DB_PATH, timeout=30.0)`(21行目)で接続し、`conn.row_factory = sqlite3.Row`(22行目)を明示的に設定しているため、本ファイル(`old/menu_service.py`)69行目の`r['date']`のような辞書ライクなキーアクセスは実際に成立する設計であることが確認できた。`PRAGMA journal_mode=WAL`/`PRAGMA foreign_keys=ON`(23〜24行目)を有効化し、`sqlite3.OperationalError`で`"locked"`を検知した場合は最大5回・1秒間隔でリトライ(19〜35行目)、それ以外のDBエラー・予期せぬ例外は`conn.rollback()`後に再送出する(36〜43行目)。`commit`引数が`True`の場合のみ`conn.commit()`を呼ぶ(28〜29行目)。`common.py`23行目で`from core.database import get_db_cursor`として再エクスポートされていることも確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/core/database.py:12-50`（参考: `MY_HOME_SYSTEM/common.py:23`） |
| `config` モジュールの役割 | `MY_HOME_SYSTEM/old/menu_service.py`を改めて直接確認したが、`import config`(10行目)以降、`config.`という参照は本ファイル中に一切出現しない（未使用インポート）ことを再確認した。本ファイルは`MY_HOME_SYSTEM/old/`ディレクトリ配下にあり、同ディレクトリの他ファイル(`old/update_schema.py`等)と同様に旧世代のスクリプトである可能性が高いが、`config`が未使用となった経緯（リファクタリング時の消し忘れ等）を示す記録は本ファイル・`config.py`のいずれにも存在せず、これ以上の解消はできなかった。 | 直接ソース確認: `MY_HOME_SYSTEM/old/menu_service.py:1-16`, `MY_HOME_SYSTEM/config.py`（該当参照なしを確認） |
| 履歴データの登録元 | リポジトリ内を`food_records`および`config.SQLITE_TABLE_FOOD`で検索し、実際の挿入処理を直接確認した。`MY_HOME_SYSTEM/services/line_service.py`の`log_food_record(user_id, user_name, category, item, is_manual=False)`(43〜50行目)と`MY_HOME_SYSTEM/handlers/line_logic.py`の`action == "food_record_direct"`分岐(350〜357行目)が、いずれも`core.database.save_log_async(config.SQLITE_TABLE_FOOD, ["user_id", "user_name", "meal_date", "meal_time_category", "menu_category", "timestamp"], (...))`を呼び出してLINE経由のユーザー入力を記録している。ただし、これらが挿入する列構成(`id, user_id, user_name, meal_date, meal_time_category, menu_category, timestamp` — `MY_HOME_SYSTEM/init_unified_db.py`193〜201行目のスキーマ定義と一致)は、本ファイル(`old/menu_service.py`)自身の`_init_db`(30〜44行目)が作成しようとする`food_records`テーブルの列構成(`id, date, menu, created_at`)と一致しない。したがって本ファイルは現行の記録経路(`line_service.py`/`line_logic.py`)とは別のスキーマ前提で書かれた旧世代のコードであり、実運用中のテーブルに対して`get_recent_menus`が期待通り`date`/`menu`列を取得できるかは疑わしいことが判明した。 | 直接ソース確認: `MY_HOME_SYSTEM/services/line_service.py:43-50`, `MY_HOME_SYSTEM/handlers/line_logic.py:350-357`, `MY_HOME_SYSTEM/init_unified_db.py:193-201`, `MY_HOME_SYSTEM/old/menu_service.py:30-44` |
