...
| --- | --- | --- |
| DB接続・トランザクションの詳細仕様 | `common.md`の解析によれば、`common.get_db_cursor`は`core.database.get_db_cursor`を再エクスポートするFacadeであり、`database.md`の解析によれば実体はリトライ機能付き（最大5回、1秒間隔）のコンテキストマネージャで、WALモードと外部キー制約(`PRAGMA foreign_keys=ON`)を有効化し、`commit`引数でコミット可否を制御する設計であることが判明した。ただし戻り値カーソルが`sqlite3.Row`ファクトリを使うかどうかは`database.py`側の解析結果にも明記がなく、本ファイルの`r['date']`のような辞書アクセスとの整合性は未確認のまま。 | `common.md`, `database.md` |
| `config` モジュールの役割 | `config.md`の解析によれば、`config.py`はシステム全体の環境変数・定数・ディレクトリパスを一元管理するモジュールであることが判明したが、本ファイルで未使用となっている理由自体は`config.md`側にも記載がなく未解決。 | `config.md` |
