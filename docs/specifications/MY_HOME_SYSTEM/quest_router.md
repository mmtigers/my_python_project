...
| 画像アップロード先のパス | `MY_HOME_SYSTEM/routers/quest_router.py`104行目の`file_path = os.path.join(config.UPLOAD_DIR, new_filename)`が参照する`config.UPLOAD_DIR`を`MY_HOME_SYSTEM/config.py`424行目で直接確認した。`UPLOAD_DIR: str = os.path.join(BASE_DIR, "uploads")`であり、保存先は`{BASE_DIR}/uploads`であることを確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/config.py:424`（参考: `MY_HOME_SYSTEM/routers/quest_router.py:104`） |
| 許可されている音声キー一覧 | `MY_HOME_SYSTEM/routers/quest_router.py`121〜122行目の`if req.sound_key not in config.SOUND_MAP:`が参照する`config.SOUND_MAP`を`MY_HOME_SYSTEM/config.py`455〜461行目で直接確認した。`{"level_up": "level_up.mp3", "quest_clear": "quest_clear.mp3", "medal_get": "medal_get.mp3", "submit": "submit.mp3", "approve": "approve.mp3"}`の5キーであることを確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/config.py:455-461`（参考: `MY_HOME_SYSTEM/routers/quest_router.py:121-122`） |
