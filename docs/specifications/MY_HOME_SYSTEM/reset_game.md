...
| --- | --- | --- |
| `quest_users` テーブルの完全なスキーマ | `MY_HOME_SYSTEM/current_schema.sql`223〜233行目の実際のDDLダンプにより、`quest_users`は`user_id TEXT PRIMARY KEY, name TEXT, job_class TEXT, level INTEGER DEFAULT 1, exp INTEGER DEFAULT 0, gold INTEGER DEFAULT 0, medal_count INTEGER DEFAULT 0, avatar TEXT DEFAULT '🙂', updated_at DATETIME, role TEXT`の10カラムを持つ現行スキーマであることを確認した（`role`は`ALTER TABLE`で後から追加されたため、ダンプ上は末尾にカンマ区切りで連結された形で現れる）。**（Issue #330でスキーマ定義元が移動）** 旧版が突き合わせ先としていた`init_unified_db.py`内の初期作成用`CREATE TABLE`定義は撤去済みで、現在のベースラインは`MY_HOME_SYSTEM/migrations/0000_baseline_schema.sql`にある(`role`列を含まない9カラム)。`role`列は`migrations/0001_add_quest_users_role.sql`（`ALTER TABLE quest_users ADD COLUMN role TEXT;`に続けて`dad`/`mom`へ`'role_adult'`、`daughter`/`son`/`child`へ`'role_child'`を設定する2本の`UPDATE`）で追加される設計と整合している。他テーブルとの外部キー等の直接的なリレーションは`quest_users`には定義されていない（したがって`reset_game.py`が関連テーブルを削除する順序はDB制約ではなくスクリプト側の責務である）。 | 直接ソース確認: `MY_HOME_SYSTEM/current_schema.sql:223-233`, `MY_HOME_SYSTEM/migrations/0000_baseline_schema.sql`, `MY_HOME_SYSTEM/migrations/0001_add_quest_users_role.sql:1-6`（参考: [init_unified_db.md](./init_unified_db.md)・[migrations.md](./migrations.md)） |
| リセット後の他システムへの影響 | **（Issue #550で`services/quest/quest_service.py`へ移設）** `_apply_quest_rewards`メソッド(522〜575行目)を直接確認した。クエスト完了時は`game_logic.GameLogic.calc_level_progress(user['level'], user['exp'], earned_exp)`(536行目)で新しい`level`/`exp`を算出し、`final_gold = user['gold'] + earned_gold`(540行目)で加算した`gold`とともに`UPDATE quest_users SET level = ?, exp = ?, gold = ?, medal_count = medal_count + ?, ...`で上書きする(543〜546行目)。すなわち`reset_game.py`によるリセット後、次回のクエスト完了処理はリセット後の`level=1, exp=0, gold=0, medal_count=0`を基準値として加算・レベル計算を行うことになる。`unified_server.py`や`quest_router.py`側に`reset_game.py`実行を検知して追加処理を行うようなフックは存在しない（`reset_game.py`はどこからも参照されていないスタンドアロンスクリプトであるため）。なお**（Issue #547で追加）** 同等のリセットはサーバーAPI`POST /api/quest/admin/reset_user`(`UserService.reset_user_data`、`services/quest/user_service.py:97-146`)からも実行できるようになっており、別プロセスから直接DBを書き換える`reset_game.py`だけが唯一の経路ではなくなっている。 | 直接ソース確認: `MY_HOME_SYSTEM/services/quest/quest_service.py:522-575`, `MY_HOME_SYSTEM/services/quest/user_service.py:97-146`（参考: [quest_quest_service.md](./quest_quest_service.md)・[quest_user_service.md](./quest_user_service.md)） |
| **（Issue #547で追加）** `POST /api/quest/admin/reset_user`のレスポンス形状・権限チェックの詳細 | `routers/quest_router.py`の`reset_user`エンドポイント（`ResetUserAction`/`ResetUserResponse`モデルを使用）と`services/quest/user_service.py`の`UserService.reset_user_data`/`_reset_user_data_locked`を直接確認した。`admin_id`に対応する`quest_users.role`が`'role_adult'`でない場合(存在しない`admin_id`を含む)は403、`target_user_id`が`quest_users`に存在しない場合は404を返す。成功時のレスポンスボディは`{"status": "reset", "deletedHistoryCount": <int>, "deletedInventoryCount": <int>}`の3フィールドのみで、`reset_game.py`が読んでいない追加フィールドは存在しない。処理は`_get_user_balance_lock(target_user_id)`の中で`core.database.get_db_cursor(commit=True)`の単一トランザクションとして実行され、`quest_users`のUPDATE・`quest_history`/`user_inventory`のDELETEを行う(`reward_history`は対象外)。 | 直接ソース確認: `MY_HOME_SYSTEM/routers/quest_router.py`(`reset_user`), `MY_HOME_SYSTEM/services/quest/user_service.py`(`reset_user_data`, `_reset_user_data_locked`) |
| **（Issue #547で追加）** `POST /api/quest/admin/reset_user`のレスポンスボディの正確な形状(`deletedHistoryCount`/`deletedInventoryCount`)と、本スクリプトによるリセットとの差異 | `MY_HOME_SYSTEM/services/quest/user_service.py`の`reset_user_data`(97〜112行目)と`_reset_user_data_locked`(114〜146行目)を直接確認した。`reset_user_data`は`_get_user_balance_lock(target_user_id)`を取ってから本体を呼ぶ(承認処理のUPDATEがリセット結果を上書きしうる問題への対応)。本体は同一トランザクション内で、(1) `SELECT role FROM quest_users WHERE user_id = ?`で`admin_id`を引き、行が無い/`role != ROLE_ADULT`なら`HTTPException(403, "リセット権限がありません")`、(2) 対象ユーザーが存在しなければ`HTTPException(404, "対象ユーザーが見つかりません")`、(3) `UPDATE quest_users SET level = 1, exp = 0, gold = 0, medal_count = 0 WHERE user_id = ?`、(4) `DELETE FROM quest_history WHERE user_id = ?`、(5) `DELETE FROM user_inventory WHERE user_id = ?` を実行する。**`reward_history`は削除されない**点に注意(購入履歴は残る)。戻り値は`{"status": "reset", "deletedHistoryCount": <削除した quest_history 件数>, "deletedInventoryCount": <削除した user_inventory 件数>}`(141〜145行目)で、`models/quest.py`156〜159行目の`ResetUserResponse(status: str, deletedHistoryCount: int, deletedInventoryCount: int)`と一致する(ルーターが`response_model=ResetUserResponse`を宣言している)。 本スクリプト(`reset_game.py`)との差異は、**API経由では`admin_id`の`role_adult`検証とユーザー単位のロックが入る**点と、**対象が`target_user_id`の1名に限定される**点である(本スクリプトは別プロセスから直接DBを書き換えるため、どちらの保護も受けない)。 | 直接ソース確認: `MY_HOME_SYSTEM/services/quest/user_service.py:97-146`, `MY_HOME_SYSTEM/models/quest.py:96-98,156-159`, `MY_HOME_SYSTEM/routers/quest_router.py:83-85`（参考: [quest_user_service.md](./quest_user_service.md)・[quest_router.md](./quest_router.md)） |
