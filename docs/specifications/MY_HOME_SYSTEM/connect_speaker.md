...
* 直接関連する仕様書は見つからなかった(本ファイルはPython製の他モジュールとは異なり、`.env`とOSコマンド[`bluetoothctl`, `pactl`等]のみに依存する独立したBashスクリプトであり、`config.py`や`common.py`等のPython共通基盤へのインポート・呼び出し関係を持たない。全体設計書.md内での言及以外に、他仕様書からの直接的な参照・呼び出し関係は確認できなかった)
