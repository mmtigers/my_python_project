...
| --- | --- | --- |
| 記念日設定のデータ構造 | `config.py`278〜286行目を直接確認した。`IMPORTANT_DATES: List[Dict[str, Any]] = []`がデフォルト値で、`BASE_DIR/family_events.json`（`MY_HOME_SYSTEM/family_events.json`）が存在すれば`json.load(f)`の結果でそのまま置き換わる(284行目)。実体の`family_events.json`はリポジトリ内に存在しない（ランタイム生成/gitignore対象と推測される）ため個々の記念日データ自体は未確認だが、消費側の`send_child_health_check.py`26〜37行目の`for event in config.IMPORTANT_DATES: ... event["date"]（"%Y-%m-%d"形式必須）, event.get('name', '???'), event["type"]（"birthday"を特別扱い）`という参照から、各要素は少なくとも`date`(必須, YYYY-MM-DD文字列), `type`(必須文字列), `name`(任意、省略時'???')の3キーを持つ辞書であることが判明した。 | 直接ソース確認: `MY_HOME_SYSTEM/config.py:278-286`, `MY_HOME_SYSTEM/old/send_child_health_check.py:26-37` |
| Discord向け通知の挙動 | `services/notification_service.py`30〜59行目の`_send_discord_webhook`を直接確認した。メッセージオブジェクトからのテキスト抽出は`hasattr(msg, "text")`を最優先し、次に`hasattr(msg, "alt_text")`（FlexMessageはこちらに該当）、いずれも無ければ辞書の`"text"`/`"altText"`キー、それも無ければ固定文字列「（画像またはスタンプ）」の順でフォールバックする(43〜53行目)。`send_child_health_check.py`51〜112行目の`create_start_check_flex`が実際に生成する`FlexMessage(alt_text="朝の体調確認", contents=container)`をこの経路に通すと、`msg.alt_text`である文字列`"朝の体調確認"`のみがDiscordのテキストコンテンツとして送信され、Flexの詳細なカードレイアウト（`bubble_json`の内容）自体はDiscord側では再現されない設計であることを確認した。`_send_line_push`側(73〜94行目)は`type: "flex"`の辞書メッセージへの変換処理こそ未実装(`pass`)だが、呼び出し元がv3 SDKの`FlexMessage`オブジェクトとして渡す限りは問題にならない。 | 直接ソース確認: `MY_HOME_SYSTEM/services/notification_service.py:30-59, 73-94`, `MY_HOME_SYSTEM/old/send_child_health_check.py:51-112` |
