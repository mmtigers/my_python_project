...
| SUUMOサイトの実際のHTML構造 | 外部サイトの内容は本ファイルからは確認できないため。（リポジトリ内をSUUMO関連のHTML固定ファイル・テストフィクスチャで検索したが該当ファイルは存在せず、解消不可。対象は動的な外部Webサイトのレスポンスであるため） | 対象Webサイトの実際のレスポンス（動的にしか取得不可） |
...
| --- | --- | --- |
| `config.SUUMO_SEARCH_URL` / `config.GEMINI_API_KEY` / `config.LINE_USER_ID`の実際の設定値 | `MY_HOME_SYSTEM/config.py`を直接確認した。`SUUMO_SEARCH_URL: Optional[str] = os.getenv("SUUMO_SEARCH_URL")`(495行目)、`GEMINI_API_KEY: Optional[str] = os.getenv("GEMINI_API_KEY")`(203行目)、`LINE_USER_ID: Optional[str] = os.getenv("LINE_USER_ID")`(185行目)のいずれもリテラルな既定値は設定されておらず、実行時の環境変数(`.env`)にのみ依存することを確認した。`.env`は`.gitignore`13行目の`.env`規則により追跡対象外であり、リポジトリ内に実体ファイルも存在しないため、実際の値そのものは確認できなかった。 | 直接ソース確認: `MY_HOME_SYSTEM/config.py:185, 203, 495`, `.gitignore:13` |
| `get_db_cursor` / `save_log_generic`の内部実装（`property_logs`テーブルのスキーマ） | 本ファイル(`monitors/old/suumo_monitor.py`)15行目で`from core.database import save_log_generic, get_db_cursor`と直接importしていることを確認した(`common.py`経由ではない)。実体の`MY_HOME_SYSTEM/core/database.py`を直接確認した。`get_db_cursor(commit: bool = False)`(13-43行目)はsqlite3接続のコンテキストマネージャで、`commit=True`時のみ`conn.commit()`し(28-29行目)、`"locked"`エラー時は最大5回リトライ(31-35行目)、それ以外の例外時は`conn.rollback()`後に再送出する(36-43行目)。`save_log_generic(table, columns_list, values_list)`(67-79行目)は`get_db_cursor(commit=True)`を用いて`INSERT INTO {table} ({columns}) VALUES ({placeholders})`を実行する汎用関数であることを確認した。一方、`property_logs`テーブルの`CREATE TABLE`定義はリポジトリ内(`init_unified_db.py`、`migrations/`配下の各`*.sql`)を検索したが見つからず、正確なスキーマは確認できなかった。ただし本ファイル自身のコードから、実際に使用されているカラムは`filter_new_properties`(93-106行目)の`SELECT id FROM {self.table_name} WHERE device_id = ?`、および`send_and_save`相当処理(158-162行目)の`save_log_generic(self.table_name, ["timestamp", "device_name", "device_id", "device_type", "contact_state"], (get_now_iso(), p['name'], p['id'], "Property", ai_comment[:100]))`から、少なくとも`id, timestamp, device_name, device_id, device_type, contact_state`の6カラムが存在することを確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/monitors/old/suumo_monitor.py:15, 93-106, 158-162`, `MY_HOME_SYSTEM/core/database.py:13-43, 67-79`（`property_logs`のCREATE TABLE定義は`init_unified_db.py`・`migrations/`配下に検索したが見つからず未解決） |
| `send_push`の内部実装（LINE/Discord通知の実際の送信方法） | 本ファイル17行目で`from services.notification_service import send_push`と直接importしていることを確認した。実体の`MY_HOME_SYSTEM/services/notification_service.py`の`send_push(user_id, messages, image_data=None, target="both", channel="notify", filename="snapshot.jpg")`(116-140行目)を直接確認した。本ファイル166行目は`send_push(self.line_user_id, [...], target="discord")`と`target="discord"`のみを指定しており、この場合`_send_discord_webhook`(30-71行目、`channel`引数に応じ`config.DISCORD_WEBHOOK_ERROR`/`DISCORD_WEBHOOK_REPORT`/`DISCORD_WEBHOOK_NOTIFY or DISCORD_WEBHOOK_URL`のいずれかへ`requests.post`を1回実行、リトライなし)のみが呼ばれ、127-138行目のLINE送信分岐は実行されないことを確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/monitors/old/suumo_monitor.py:17, 166`, `MY_HOME_SYSTEM/services/notification_service.py:30-140` |
| `traceback`/`datetime`インポートの実際の用途（未使用か、削除漏れか） | 本ファイル(`monitors/old/suumo_monitor.py`)全体を`grep`で確認した。`import traceback`(5行目)および`from datetime import datetime`(7行目)がそれぞれのimport文自体にしか出現せず、`traceback.`または`datetime`を呼び出す箇所は本ファイル中に他に存在しないことを確認した。したがって両インポートは現状のファイル内容では未使用であることが判明した。 | 直接ソース確認: `MY_HOME_SYSTEM/monitors/old/suumo_monitor.py:5, 7`（全文をgrep確認、他に呼び出し箇所なし） |
| `monitors/old/`ディレクトリの位置づけ（現行版との関係） | `MY_HOME_SYSTEM/monitors/`配下を`find`で検索したところ、`suumo_monitor.py`という名前のファイルは`monitors/old/suumo_monitor.py`の1件のみであり、`monitors/`直下等に非`old`版の同名ファイルは存在しないことを確認した。また`MY_HOME_SYSTEM`全体を`grep`で検索したが、`suumo_monitor`という文字列を含む他のPython/シェルファイル（呼び出し元）も見つからなかった。`monitors/old/`ディレクトリには他に`clinic_monitor.py`, `car_presence_checker.py`, `bicycle_parking_monitor.py`, `bluetooth_monitor.py`, `clinic_analyzer.py`, `clinic_visualizer.py`が同居しているが、位置づけを説明するREADME等は同ディレクトリ内に存在しない。以上より、本ファイルを現在アクティブに呼び出している箇所はリポジトリ内には見つからなかった。 | 直接ソース確認: `MY_HOME_SYSTEM/monitors/`ディレクトリ構成（`find`/`grep`による全文検索） |
