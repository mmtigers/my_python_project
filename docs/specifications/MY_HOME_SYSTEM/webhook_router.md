...
* `switchbot_webhook` は `config.SWITCHBOT_WEBHOOK_TOKEN` 設定時に `?token=...` クエリパラメータとの一致を `hmac.compare_digest` で検証し、不一致・未指定であれば HTTP 401 を返す。**（Issue #648で変更）** 未設定の場合は従来「検証せず通す」オプトイン設計だったが、現在は HTTP 503 で拒否する(フェイルクローズ)。無検証で受け付けるには `ALLOW_UNAUTHENTICATED_SWITCHBOT_WEBHOOK=true` の明示的なオプトインが必要(上記「認証・認可」節を参照)。
* 根拠: トークン検証ブロック (行番号: 51〜53)
* **（Issue #328で確認）** 実機ログでの調査の結果、`SwitchBotContext`(`models/switchbot.py`)が定義する全フィールド(`deviceMac`/`deviceType`/`detectionState`/`openState`/`brightness`/`timeOfSample`/`power`/`voltage`/`weight`/`watt`)の範囲内に、WoContact(開閉センサー)・WoPresence(人感センサー)・WoMeterPlus(温湿度計)・WoFan2の実ペイロードが収まることを確認した。未知のフィールドは検出されず、`switchbot_webhook`が受け取るWebhookペイロードの型定義に追加の厳密化は不要と判断した。なお`models/switchbot.py`に残る`DeviceStatusResponse.body: Dict[str, Any]`は、Webhookとは別の`GET /v1.1/devices/{id}/status`(デバイス状態取得API)のレスポンス用フィールドであり、本調査の対象外。
...
| `SWITCHBOT_WEBHOOK_TOKEN` の値・設定有無 | 環境変数として本番でどう設定されているかがファイル内からは不明なため。**（Issue #648以降）** 未設定=503で拒否(フェイルクローズ)、設定済み=検証あり。 | `config.py`, `.env` |
...
| `SWITCHBOT_WEBHOOK_TOKEN` の値・設定有無 | `MY_HOME_SYSTEM/config.py:191`に`SWITCHBOT_WEBHOOK_TOKEN: Optional[str] = os.getenv("SWITCHBOT_WEBHOOK_TOKEN")`と定義されていることを直接確認した。値は環境変数由来で未設定時は`None`となる。本ファイル自身の49〜52行目の`if config.SWITCHBOT_WEBHOOK_TOKEN: ... hmac.compare_digest(token, config.SWITCHBOT_WEBHOOK_TOKEN)`というロジックにより「設定されていれば検証、未設定ならスキップ」という後方互換設計であることは本ファイル単体からも確認できる。ただし実際の値自体は`.env`(gitignore対象)にのみ存在しうるものであり、具体的な値はリポジトリ内からは確認できなかった。**（この調査時点からの変化）** 当時は`MY_HOME_SYSTEM/.env.example`に`SWITCHBOT_WEBHOOK_TOKEN`というキー自体が記載されていなかったが、現在は記載済みである。また「設定されていれば検証、未設定ならスキップ」という上記の後方互換設計はIssue #648でフェイルクローズ(未設定時は503)へ変更されている。 | 直接ソース確認: `MY_HOME_SYSTEM/config.py:191`（値自体は解消不可） |
| `line_handler` の処理内容 | `MY_HOME_SYSTEM/handlers/line_handler.py`(全376行)を直接確認した。`line_handler`(29-30, 36行目)は`config.LINE_CHANNEL_ACCESS_TOKEN`と`LINE_CHANNEL_SECRET`の両方が設定されている場合のみ`WebhookHandler(config.LINE_CHANNEL_SECRET)`として初期化される`Optional[WebhookHandler]`。336〜338行目で`line_handler.add(MessageEvent, message=TextMessageContent)(handle_message)`と`line_handler.add(PostbackEvent)(handle_postback)`によりイベントディスパッチが登録される(後方互換の登録であり、本ファイルからの実際の呼び出し経路は`line_handler.dispatch_events`、341〜375行目)。`handle_message`(233〜263行目)はテキストメッセージ受信時に`asyncio.run(_process_message_async(...))`で非同期処理を同期的に実行し、内部の`_process_message_async`(265〜304行目)は「子供記録」/「体調」を含む健康記録コマンド、次いでAI応答フォールバック(`services.ai_service.analyze_text_and_execute`)の順に分岐する。**（Issue #358で撤去）** 以前あった、「ステータス」「クエスト」への完全一致および「承認」「却下」で始まる文字列に対するファミリークエストコマンドの分岐は、LINEの`event.source.user_id`とFamily Questの`quest_users.user_id`のマッピングが存在せず本番で機能しないデッドコードだったため削除された(オーナー判断)。`handle_postback`(306〜334行目)も同Issueで`"approve:"`/`"reject:"`プレフィックスの分岐が撤去されており、現在は無条件に`handlers.line_logic.handle_postback`へ処理を委譲する。副作用として、いずれの経路でも最終的にLINE Messaging APIへの`reply_message`呼び出し(146〜179行目)が発生しうる。 | 直接ソース確認: `MY_HOME_SYSTEM/handlers/line_handler.py:29-30,36,146-179,233-263,265-304,306-334,336-338,341-375` |
