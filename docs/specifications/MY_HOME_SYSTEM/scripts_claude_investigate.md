...
| 名称 | 種類 | 用途 | 根拠 |
| --- | --- | --- | --- |
| `flock` | 外部コマンド | ロックファイル(`logs/.claude_investigate.lock`)による多重起動防止 | 根拠: [ロック取得] (行番号: 146-147 / 抜粋: "exec 9>\"$LOCK_FILE\"\nif ! flock -n 9; then") |
...
| `WATCHDOG_NOTIFY_WEBHOOK_URL` | 環境変数(任意) | 調査結果の通知先Webhook | 根拠: [通知分岐] (行番号: 259 / 抜粋: "if [ -n \"${WATCHDOG_NOTIFY_WEBHOOK_URL:-}\" ]; then") |
...
| `gh issue create` / `gh pr create --draft` の認証・権限 | ghの認証状態は実機側セットアップに依存し本スクリプトからは不明 | 根拠: [前提コメント] (行番号: 71 / 抜粋: "gh CLI が Issue/PR作成の最小権限トークンで認証済み") |
...
* **[修正済み・Issue #339 2026-09-21] systemd 配下で `claude` が見つからず exit=127**: `claude`は公式インストーラが`~/.local/bin`に置くが、2026-09-20に`home_system`をsystemd管理へ移して以降、`health_watch`の子プロセスとして起動される本スクリプトの`PATH`(`/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin`)に含まれず、自動調査は毎回`exit=127`(command not found)で失敗していた(2026-09-20 09:24を最後に14回連続)。`resolve_claude_bin`で`CLAUDE_BIN`→`PATH`→`~/.local/bin/claude`の順に解決した実行ファイルを`"$CLAUDE_CMD" -p`で起動する。見つからない場合も起動を試み、従来どおり`exit=127`として通知経路に乗せる。
* **（Issue #339 2026-09-21・オーナー判断で起票モードを有効化）** リポジトリは一般公開のため、起票モード(`DRY_RUN`≠1)には次の歯止めがある: (1) 1日の起票数の上限(`CLAUDE_INVESTIGATE_MAX_ISSUES_PER_DAY`。確認できなければドライラン)、(2) 起票前に`gh issue list`で既存Issueを確認し、同じ原因なら`gh issue comment`で追記させる(許可ツールに`gh issue list/view/comment`を追加)、(3) 実機のpull遅れ・外部サービスとの一時的な通信失敗・テスト用の異常など改修不要なものは起票させない、(4) 起票・コメントに LINE等のメッセージ本文・人名・IP/MACアドレス・ホスト名・URL・認証情報を書かず抽象化させる、(5) 自動起票分に`auto-investigation`ラベルを付けさせる。(2)〜(5)はプロンプトの指示であり、モデルの遵守に依存する(Issue #379の多層防御と同じ位置づけ)。個人情報が載らないことの機械的な保証は無い。
...
* 根拠: (行番号: 247 / 抜粋: "--output-format json | jq -r '.result // .')")
...
* 根拠: (行番号: 12-19, 79, 246 / 抜粋: "実機検証済み(Issue #339, 2026-09-07,", "MAX_BUDGET_USD=\"${CLAUDE_INVESTIGATE_MAX_BUDGET_USD:-2.00}\"", "--max-budget-usd \"$MAX_BUDGET_USD\"")
