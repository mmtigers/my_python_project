...
* `fix_quest_reset_period.py`（本リポジトリに実体なし。実機デプロイ先にのみ存在すると見られる） - `quest_master.reset_period`列の値(`'weekly_monday'`→`'daily'`)を一括修正するワンショットスクリプト。本ファイルの`is_within_reset_period`が`'daily'`/`'weekly'`/`'monthly'`の3値しか扱わないこと（**（2026-09-06 品質監査で修正）** `'monthly'`分岐追加）と関連が疑われる
...
### `_get_item_use_lock` (モジュールレベル関数) と `_item_use_locks` (モジュールレベル変数)

* **役割**: `user_id`をキーとして`threading.Lock`を管理する簡易レジストリ（`_get_completion_lock`等と同一の構造）。`InventoryService.use_item`は「YouTube系ごほうび券の直近`used_at`を読む→クールダウン判定→`consumed`へ更新」というTOCTOUを持ち、同一ユーザーが異なるYouTube系ごほうび券(例: 10:00券と30:00券)をほぼ同時に使用しようとすると、両リクエストがクールダウンなし(0秒)を読んでしまい15分ロックをすり抜け得る。`InventoryService.use_item`が本ロックを取得したうえで`_use_item_locked`に処理を委譲することで、同一ユーザーの`use_item`呼び出しのDB更新部分をプロセス内で直列化しこのレースを防ぐ。**（Issue #544で変更）** ロックの保持範囲は`_use_item_locked`（DB更新・コミット）までで、その後のLINE通知・効果音はロック解放後に`use_item`が実行する。
* **（2026-09-06 品質監査で修正）** 以前の本仕様書は見出しに `_item_use_locks_guard` を、根拠に `_item_use_locks: Dict[str, threading.Lock] = {}` を記載していたが、現行コードにそのような辞書・ガードロック変数は存在せず、他の3レジストリと同様 `core.utils.RefCountedLockRegistry` のインスタンス1つ（`_item_use_locks`）で管理されている（`_get_item_use_lock`は`_item_use_locks.acquire(user_id)`を返すだけ）。
* 根拠: `_item_use_locks = RefCountedLockRegistry()` (行番号: 203), `def _get_item_use_lock(user_id: str):` (行番号: 206〜207 / 抜粋: "return _item_use_locks.acquire(user_id)")
* **引数/リクエスト**: `user_id: str`
* 根拠: (行番号: 205)
...
* **役割**: `quest_history`（`status='approved'`、最大100件）と`reward_history`（最大100件）を取得しマージ、`ts`降順で先頭100件に切り詰めたうえで、各ユーザーの名前・アバターを付与し、種別ごとの表示テキストと日付文字列を整形して返す。**（2026-09-06 品質監査で修正）** `quest_history`側の抽出条件に`AND quest_id != 0`が追加された。`InventoryService.use_item`が`quest_id=0`で挿入する「アイテム使用」行はクエスト達成ではないため、冒険ログから除外する（Q-L5。`get_family_chronicle`の総件数カウントは既に同条件だった）。
* 根拠: `def _fetch_full_adventure_logs(self, cur) -> List[dict]:` (行番号: 239)、`# Q-L5: use_item が quest_id=0 で挿入する「アイテム使用」行はクエスト達成ではないため除外する` (行番号: 240)、`... FROM quest_history WHERE status='approved' AND quest_id != 0 ORDER BY completed_at DESC LIMIT 100` (行番号: 241)
* **引数/リクエスト**: `cur`
* 根拠: (行番号: 105)
...
* 根拠: `if reset_period == 'daily': ... elif reset_period == 'weekly': ... elif reset_period == 'monthly': ... logger.warning(...) ... return False` (行番号: 373〜390)
* 根拠: [monthly分岐] (行番号: 379〜384 / 抜粋: "elif reset_period == 'monthly':\n            # models.quest.MasterQuest.reset_period は 'monthly' を受け付けるのに、ここに分岐が\n            # 無く常に False(=未完了扱い)になっていた。... JST の暦月で判定する。\n            return (completed_date.year, completed_date.month) == (today_jst.year, today_jst.month)")
...
* **（2026-09-06 品質監査で修正）** `_is_quest_currently_active`チェックの直後、スパムチェックの前に、前提クエスト(`quest_master.pre_requisite_quest_id`)の達成チェックがサーバー側にも追加された。フロントエンド(`useQuestStatus.getQuestLockState`)は「前提クエストの今周期の承認済み履歴」が無ければカードをロックするが、以前はサーバー側に対応する検査が無く、API直叩きでロックを素通りして報酬を得られた。`pre_requisite_quest_id`が設定されている場合、前提クエスト自身の`reset_period`（`quest_master`から取得。マスタに無い/`NULL`なら`'daily'`扱い）を用い、対象ユーザーの前提クエストの直近`status='approved'`履歴が`is_within_reset_period`で現在の周期内と判定されなければ`HTTPException(403, "前提クエストがまだ達成されていません")`を送出する。
* 根拠: (行番号: 543〜562 / 抜粋: "prereq_id = quest['pre_requisite_quest_id'] if 'pre_requisite_quest_id' in quest.keys() else None\n            if prereq_id:\n                prereq_quest = cur.execute(\n                    \"SELECT reset_period FROM quest_master WHERE quest_id = ?\", (prereq_id,)\n                ).fetchone()\n                prereq_period = (prereq_quest['reset_period'] if prereq_quest else None) or 'daily'\n                prereq_hist = cur.execute(... WHERE user_id = ? AND quest_id = ? AND status = 'approved' ORDER BY completed_at DESC LIMIT 1 ...)\n                if not (prereq_hist and prereq_hist['completed_at']\n                        and self.is_within_reset_period(prereq_hist['completed_at'], prereq_period)):\n                    raise HTTPException(status_code=403, detail=\"前提クエストがまだ達成されていません\")")
* **（2026-09-06 品質監査で修正）** 周期リセットガードのエラーメッセージ接頭辞`period_label`は、以前は`"今週" if reset_period == 'weekly' else "本日"`の二値だったが、`'monthly'`に対応する`"今月"`を含む辞書引き（`{"weekly": "今週", "monthly": "今月"}.get(reset_period, "本日")`）に変更された。
* 根拠: `# M-1-3: daily/weekly の周期リセットをサーバー側でも強制する。` (行番号: 581), `if quest['quest_type'] != 'infinite' and last_hist and last_hist['completed_at']:\n                reset_period = quest['reset_period'] or 'daily'\n                if self.is_within_reset_period(last_hist['completed_at'], reset_period):\n                    period_label = {"weekly": "今週", "monthly": "今月"}.get(reset_period, "本日")\n                    raise HTTPException(status_code=400, detail=f"{period_label}はこのクエストを完了済みです")` (行番号: 586〜590)
...
* **役割**: `ROLE_ADULT`のユーザーが子供のクエスト完了を承認する実処理（`process_approve_quest`が取得したユーザー単位ロック内で実行されることを前提とする）。`_apply_quest_rewards`で報酬を確定し、連結された相方履歴があれば`_approve_linked_history`でカスケード承認、TVロック解除対象クエストかつ子供のクエストであれば`_trigger_tv_unlock`を呼ぶ。**（2026-09-06 品質監査で修正）** この`_trigger_tv_unlock`呼び出しは`with common.get_db_cursor(commit=True)`ブロックの**外**（コミット完了後）に移動された。ブロック内では該当時に`tv_unlock_quest_id`（`Optional[int]`、初期値`None`）へ`quest['quest_id']`を記録するだけにし、ブロックを抜けた後`tv_unlock_quest_id is not None`なら`_trigger_tv_unlock(tv_unlock_quest_id)`を呼んで`result`を返す。以前はブロック内（コミット前）でスレッドを起動していたため、コミットが失敗（ディスクフル・ロック待ちタイムアウト等）して承認がロールバックされてもTVだけが点く可能性があった（Q-L7は`use_item`側だけを修正していた）。また`override_rewards`の`gold`/`exp`は`hist['gold_earned'] or 0`/`hist['exp_earned'] or 0`として`NULL`を0扱いにする。`quest_history.gold_earned`/`exp_earned`は`NULL`許容列のため、サービス外で挿入された行を承認すると`user['gold'] + None`の`TypeError`で500になっていた。TVロック判定の`quest`は、`sync_master_data`のマスタ削除(`DELETE ... NOT IN`)後も`quest_history`の`pending`行が残るケースで`None`になり得るため、`if quest and ...`で`None`ガードされている（M-1-1: 以前はこのガードが無く、マスタ削除済みクエストのpending履歴を承認しようとすると`quest['quest_id']`が無条件に評価され`TypeError`で500になり承認が恒久的に失敗していた）。**（Issue #238で修正）** `_approve_linked_history`が返す相方の報酬情報（`user_id`/`leveledUp`/`newLevel`/`earnedMedals`）を、`result`辞書へ`partnerUserId`/`partnerLeveledUp`/`partnerNewLevel`/`partnerEarnedMedals`として格納するようになった。以前は`_approve_linked_history`の戻り値が無く(`-> None`)、相方の報酬情報はDB上には正しく反映されるもののAPIレスポンスには一切含まれず、フロント側が相方（自分でタップしなかった方の子ども）のレベルアップ/メダル獲得演出を出す手段が無かった。
* 根拠: `def _process_approve_quest_locked(self, approver_id: str, history_id: int) -> Dict[str, Any]:` (行番号: 716〜772)
* 根拠: [コミット後のTV解錠] (行番号: 717〜721 / 抜粋: "# TV解錠(SwitchBot API 経由の副作用)はトランザクションのコミット後に起動する。\n        # 以前は with ブロック内(コミット前)でスレッドを起動していたため、...\n        tv_unlock_quest_id: Optional[int] = None")、(行番号: 764〜766 / 抜粋: "if user['role'] == ROLE_CHILD:\n                    tv_unlock_quest_id = quest['quest_id']")、(行番号: 770〜772 / 抜粋: "if tv_unlock_quest_id is not None:\n            self._trigger_tv_unlock(tv_unlock_quest_id)\n        return result")
* 根拠: [NULLガード] (行番号: 738〜743 / 抜粋: "# quest_history.gold_earned/exp_earned は NULL 許容列のため、サービス外で挿入された\n            # 行を承認すると user['gold'] + None の TypeError → 500 になっていた。0 扱いにする。\n            override_rewards = {\n                \"gold\": hist['gold_earned'] or 0,\n                \"exp\": hist['exp_earned'] or 0\n            }")
...
* 根拠: (行番号: 751〜757, 762〜766, 768, 770〜771)
...
* **（2026-09-06 品質監査で修正）** `override_rewards`の`gold`/`exp`は`linked_hist['gold_earned'] or 0`/`linked_hist['exp_earned'] or 0`として`NULL`を0扱いにする（`_process_approve_quest_locked`と同じ`NULL`許容列対策）。
* 根拠: `def _approve_linked_history(self, cur, linked_history_id: int) -> Optional[Dict[str, Any]]:` (行番号: 774〜794 / 抜粋: "相方側 quest_history 行を承認済みに確定する(冪等)")、`override_rewards = {"gold": linked_hist['gold_earned'] or 0, "exp": linked_hist['exp_earned'] or 0}` (行番号: 791)
...
* **（2026-09-06 品質監査で修正）** `calc_level_down`に渡す経験値は`hist['exp_earned'] or 0`として`NULL`を0扱いにする（ゴールド側の`gold_earned = hist['gold_earned'] or 0`と同様。`quest_history.exp_earned`は`NULL`許容列）。
* 根拠: `gold_earned = hist['gold_earned'] or 0` (行番号: 964)、`new_level, new_exp = game_logic.GameLogic.calc_level_down(\n            user['level'], user['exp'], hist['exp_earned'] or 0\n        )` (行番号: 972〜974)
...
* **役割**: アイテムを使用し、即座に消費を確定する（親の承認は不要）。`use_item`は`_get_item_use_lock(user_id)`でユーザー単位のプロセス内ロックを取得したうえで、実処理を`_use_item_locked`に委譲する薄いラッパー（**YouTubeごほうび券クールダウン機能で追加**。詳細は`_get_item_use_lock`を参照）。`_use_item_locked`は所有者(`user_id`)・所有状態(`'owned'`)を確認したうえで、対象アイテムの`reward_id`が`config.YOUTUBE_REWARD_IDS`に含まれ、**かつ**`_is_youtube_cooldown_enforced()`が`True`(猶予期間機能で追加。施行日を迎えている)の場合のみ`_get_youtube_cooldown_remaining_seconds`でクールダウン残り秒数を判定し、`0`より大きければ`HTTPException(429)`（分単位に切り上げたメッセージ付き）を送出して消費を拒否する。施行日より前(猶予期間中)は、対象アイテムがYouTube系であっても拒否せず素通しする(いきなり制限がかかると子どもが困惑するため)。クールダウン判定を通過（またはYouTube系対象外・猶予期間中）した場合、対象`user_inventory`行の`status`を直ちに`'consumed'`へ更新し（`used_at`に現在時刻を記録）、`quest_history`へ`quest_id=0`・`status='approved'`のログ行を挿入する。**（Issue #369で修正）** UPDATEは`WHERE id = ? AND status = 'owned'`の条件付きで行い、`cur.rowcount == 0`（SELECT後に別リクエストが先に消費していた）なら`HTTPException(400)`を送出する。以前はSELECT→Python判定→無条件UPDATEだったため、連打された2リクエストがWALで両方`'owned'`を読み、両方が消費・履歴INSERT・通知を実行する二重使用が起きていた。LINE通知（`config.LINE_USER_ID`宛）と`"quest_clear"`サウンド再生は、以前はトランザクション内で実行されLINE API往復中もSQLiteの書き込みロックを保持していたが、`get_db_cursor`ブロックを抜けたコミット後に実行する。**（Issue #544で修正）** さらに、その送信は`_use_item_locked`ではなく`use_item`側で`_get_item_use_lock`の`with`ブロックを**抜けた後**に行うようになった。`_use_item_locked`の戻り値は`Tuple[Dict[str, str], str]`（APIレスポンス辞書と通知メッセージ文字列）に変わり、自身では`send_push`/`sound_manager.play`を呼ばない。以前はロック保持中に同期の`send_push`（LINE、最大15秒）まで実行していたため、LINEが遅い/タイムアウトした場合に同一ユーザーの次の`use_item`がその往復の間直列化されていた。現在ユーザー単位ロックが保持されるのはDB更新(コミット)までである。`'pending'`状態を経由し`ROLE_ADULT`が承認する2段階の承認フローは存在しない（アイテム使用時の親承認フローはコミット`9d5edec`で廃止された）。
* 根拠: `def use_item(self, user_id: str, inventory_id: int) -> Dict[str, str]:` (行番号: 1184)、`with _get_item_use_lock(user_id):\n            result, msg = self._use_item_locked(user_id, inventory_id)` (行番号: 1189〜1190)、`notification_service.send_push(` (行番号: 1195)、`sound_manager.play("quest_clear")` (行番号: 1199)、`return result` (行番号: 1201)、`def _use_item_locked(self, user_id: str, inventory_id: int) -> Tuple[Dict[str, str], str]:` (行番号: 1203 / 抜粋: "アイテムを使用し、即座に消費を確定する(親の承認は不要)。")、`return {"status": "consumed", "message": "つかいました！"}, msg` (行番号: 1258)、`if item['reward_id'] in config.YOUTUBE_REWARD_IDS and _is_youtube_cooldown_enforced():\n                cooldown_remaining = _get_youtube_cooldown_remaining_seconds(cur, user_id)\n                if cooldown_remaining > 0:` (行番号: 1132〜1134)、`raise HTTPException(\n                        429,\n                        f"YouTubeのごほうび券は、目を休めるためあと{remaining_minutes}分ほど使えません",\n                    )` (行番号: 1136〜1139)、`WHERE id = ? AND status = 'owned'` (行番号: 1150)、`if cur.rowcount == 0:` (行番号: 1152〜1153)
...
* **（2026-09-06 品質監査で修正）** 以前の本仕様書は「JST算出を`try: now_jst = datetime.datetime.now(pytz.timezone("Asia/Tokyo"))` ... `except`で囲みローカル時刻へフォールバックする」「ボーナス計算は`target_user != 'all'`の場合のみ行う」と記述していたが、いずれも現行コードには存在しない。JSTの現在時刻はモジュールレベル定数`JST`を用いた`datetime.datetime.now(JST)`の1行で取得され（Issue #293でpytz廃止、#409で到達不能なtry/exceptを削除）、`filtered_quests`の全クエストについて`target_user`の値に応じた`boost_user_id`（`'all'`/空→`viewer_user_id`、`'siblings'`→`sibling_child_ids[0]`、それ以外→実在ユーザーならその`target_user`、さもなくば`viewer_user_id`）で連続達成ボーナスを算出する。`boost_user_id`が`None`（`viewer_user_id`未指定）の場合のみ`bonus_gold`/`bonus_exp`は0になる。
* 根拠: `# JST は固定オフセットの timezone なので now()/strftime が失敗することはない\n            # (#409: 到達不能な try/except フォールバックを削除)\n            now_jst = datetime.datetime.now(JST)` (行番号: 1473〜1475)、ボーナス算出ループ (行番号: 1445〜1462 / 抜粋: "if q['target_user'] == 'all' or not q['target_user']:\n                    boost_user_id = viewer_user_id\n                elif q['target_user'] == 'siblings' and sibling_child_ids:\n                    boost_user_id = sibling_child_ids[0]\n                else:\n                    boost_user_id = q['target_user'] if q['target_user'] in known_user_ids else viewer_user_id\n                if boost_user_id: ... else:\n                    q['bonus_gold'] = 0\n                    q['bonus_exp'] = 0")
* **引数/リクエスト**: `viewer_user_id: Optional[str] = None`（省略時は`None`。`target_user`が`'all'`/空、または実在ユーザーでない共有クエストのボーナス計算で、閲覧中ユーザーの代表IDとして使われる）
* 根拠: (行番号: 970)
...
* **エラーハンドリング**: なし。**（2026-09-06 品質監査で修正）** 以前記述していた「JST算出失敗時にローカル時刻へフォールバックする`try-except`」は#409で削除済みで、現行コードには本メソッド内に`try`/`except`が存在しない。
* 根拠: `now_jst = datetime.datetime.now(JST)` (行番号: 1475)、`def get_all_view_data` 本体 (行番号: 1384〜1530) に `try`/`except` が無いこと

### `GameSystem._fetch_recent_logs`

* **役割**: `quest_history`（`status='approved'`、`id`降順で20件）と`reward_history`（`id`降順で20件）を取得・マージし、`ts`降順に並べ替えて先頭20件に絞り、ユーザー名と表示テキスト・日付文字列を付与する。**（2026-09-06 品質監査で修正）** `quest_history`側の抽出条件に`AND quest_id != 0`が追加され、`use_item`が`quest_id=0`で挿入する「アイテム使用」行を直近ログから除外する（`UserService._fetch_full_adventure_logs`と同じQ-L5対応）。
* 根拠: `def _fetch_recent_logs(self, cur) -> List[dict]:` (行番号: 1532)、`FROM quest_history WHERE status='approved' AND quest_id != 0 ORDER BY id DESC LIMIT 20` (行番号: 1535)
* **引数/リクエスト**: `cur`
* 根拠: (行番号: 979)
* **戻り値/レスポンス**: `List[dict]`
* 根拠: (行番号: 979, 997)
...
以下は、アイテム使用処理（`use_item`）のフローです。親の承認は不要で、単一トランザクション内で即座に消費が確定します（アイテム使用時の親承認フローはコミット`9d5edec`で廃止されました）。LINE通知・効果音はコミット後かつユーザー単位ロック解放後に`use_item`側で実行されます（Issue #544）。`use_item`は`_get_item_use_lock(user_id)`取得後に`_use_item_locked`へ委譲し（YouTubeごほうび券クールダウン機能で追加）、所有者・状態チェックの後にYouTube系ごほうび券かつ施行済み(`_is_youtube_cooldown_enforced()`、猶予期間機能で追加)の場合のみクールダウン判定を行います。
...
flowchart TD
    ...
    UInsertHistory --> UUnlock["コミット後、_get_item_use_lock を解放<br>(_use_item_locked が (result, msg) を返す)"]
    UUnlock --> UNotify["外部: notification_service.send_push<br>「使用しました」"]
...
| 中 | `fix_quest_reset_period.py` | `quest_master.reset_period`が`'weekly_monday'`から`'daily'`へ一括変換されるワンショットスクリプトであり、`is_within_reset_period`が`'daily'`/`'weekly'`/`'monthly'`のみを扱う設計との整合性（未実行環境や`'boss_'`接頭辞クエストでの挙動）を確認するため。 | `is_within_reset_period`の`if reset_period == 'daily': ... elif reset_period == 'weekly': ... elif reset_period == 'monthly': ...` (行番号: 373〜384) |
...
* **`is_within_reset_period`が扱うリセット周期は`'daily'`・`'weekly'`・`'monthly'`の3値のみ**（**（2026-09-06 品質監査で修正）** `'monthly'`分岐を追加。`models.quest.MasterQuest.reset_period`の`Literal`と一致した）: `reset_period`列の供給はIssue #330以降migrations側に一本化されており（0002で追加・Issue #329対応の0008でテーブル再作成によりカラムDEFAULTも`'daily'`へ修正済み）、`sync_master_data`内のレガシー列追加分岐は退役した。デフォルト値の食い違い問題は解消済みだが、`reset_period`に`'daily'`/`'weekly'`/`'monthly'`以外の任意の値が入ったクエストは、`is_within_reset_period`のいずれの分岐にも一致せず`get_all_view_data`内での有効性判定で常に`False`を返し、`completedQuests`（および共有クエストの他者完了状況）へ反映されない可能性が残る。
* 根拠: `if reset_period == 'daily': ... elif reset_period == 'weekly': ... elif reset_period == 'monthly': ... return False` (行番号: 373〜390)
