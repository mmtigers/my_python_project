...
* [quest_locks.md](./quest_locks.md) - `services/quest/locks.py`。本ファイルが再エクスポートする定数群(`JST`/`ROLE_ADULT`/`ROLE_CHILD`/`SPAM_CHECK_INTERVAL_SECONDS`/`INFINITE_QUEST_COOLDOWN_SECONDS`/`YOUTUBE_REWARD_BREAK_SECONDS`)とロック関数群(`_get_completion_lock`/`_get_user_balance_lock`/`_get_purchase_lock`/`_get_item_use_lock`/`_acquire_user_balance_locks`)、`logger`の実体
...
| `services.quest.locks` (`INFINITE_QUEST_COOLDOWN_SECONDS`, `JST`, `ROLE_ADULT`, `ROLE_CHILD`, `SPAM_CHECK_INTERVAL_SECONDS`, `YOUTUBE_REWARD_BREAK_SECONDS`, `_acquire_user_balance_locks`, `_completion_locks`, `_get_completion_lock`, `_get_item_use_lock`, `_get_purchase_lock`, `_get_user_balance_lock`, `_get_youtube_cooldown_remaining_seconds`, `_is_youtube_cooldown_enforced`, `_is_youtube_daily_limit_enforced`, `_item_use_locks`, `_purchase_locks`, `_seconds_since_iso_timestamp`, `_user_balance_locks`, `can_extend_youtube_limit_now`, `get_youtube_daily_limit_minutes`, `get_youtube_daily_limit_with_extensions`, `get_youtube_reward_duration_minutes`, `get_youtube_used_minutes_today`, `logger`) | 内部モジュール | 分割後の実体を再エクスポート（詳細は[quest_locks.md](./quest_locks.md)） | `from services.quest.locks import (\n    INFINITE_QUEST_COOLDOWN_SECONDS,\n    ...\n    logger,\n)` (行番号: 27〜53) |
| `services.quest.user_service` (`UserService`, `InvalidImageError`, `ImageTooLargeError`) | 内部モジュール/例外クラス | 分割後の実体を再エクスポート。**（Issue #551で追加）** `InvalidImageError`/`ImageTooLargeError`は`routers/quest_router.py`の`upload_image`が捕捉する2つのドメイン例外（詳細は[quest_user_service.md](./quest_user_service.md)） | `from services.quest.user_service import ImageTooLargeError, InvalidImageError, UserService` (行番号: 48) |
...
| `quest_data.USERS` / `.QUESTS` / `.REWARDS` の構造 | 定義ファイルの実体は本ファイルからは確認できない | `import quest_data` (行番号: 118) |
...
* 根拠: `try:\n    import quest_data\nexcept ImportError:` (行番号: 117〜119)
...
| ルーター・テストの移行が完了する時期・条件 | リポジトリ内に移行完了の期限・条件を定めた文書は存在しないが、**現時点の移行状況は実測できる**。`routers/quest_router.py`は依然として`from services.quest_service import game_system, quest_service, shop_service, user_service, inventory_service`という形で本シム経由でサービスを取得しており(CLAUDE.mdの「依存性注入(DI)について」節にもこの形が推奨パターンとして明記されている)、**ルーター側の移行は行われていない**。加えて`services/quest/game_system.py`は`sync_master_data`(30行目)・`get_all_view_data`(171行目)の両方で、モジュールグローバルではなく`from services import quest_service as _quest_service_shim`として**実行のたびに本シム経由で`quest_data`を読む**設計になっている — これはテストが`monkeypatch.setattr(qs, "quest_data", fake)`でマスタデータを差し替える経路(Issue #529等)を維持するためで、同ファイル24〜29行目のコメントがその意図を明記している。したがって本シムは「移行途中の一時的な互換レイヤ」ではなく、**テストの差し替えポイントとして現役で依存されている**状態にあり、ルーター側のimportを書き換えるだけでは廃止できない(先に`quest_data`の差し替え経路を別の形で用意する必要がある)。 | 直接ソース確認: `MY_HOME_SYSTEM/routers/quest_router.py`（import部）, `MY_HOME_SYSTEM/services/quest/game_system.py:24-33,168-187`, `CLAUDE.md`「依存性注入(DI)について」（参考: [quest_router.md](./quest_router.md)・[quest_game_system.md](./quest_game_system.md)） |
