...
| `id` の一意性制約の仕様 | `MY_HOME_SYSTEM/quest_data.py`のQUESTS配列(53〜183行目)を直接確認したところ、コメントアウトされていない有効な行の中で`id: 15`(81行目`target: 'dad'`, 93行目`target: 'mom'`、いずれも「洗濯物を干す」)、`id: 16`(82行目`dad`, 94行目`mom`、「洗濯物を畳む」)、`id: 17`(83行目`dad`, 95行目`mom`、「洗濯物をしまう」)が、それぞれ異なる`target`で重複して存在することを直接確認した。DB側は`MY_HOME_SYSTEM/current_schema.sql`174行目で`quest_master.quest_id INTEGER PRIMARY KEY AUTOINCREMENT`であり、`sync_master_data`(`services/quest_service.py`744〜761行目)は`INSERT INTO quest_master (quest_id, ...) VALUES (...) ON CONFLICT(quest_id) DO UPDATE SET ...`という形でリスト順に処理するため、同一`id`の2件目(`target: 'mom'`側、ファイル内でより後方に定義)が1件目(`target: 'dad'`側)を上書きし、DB上には後勝ちで1行しか残らないことを確認した。したがって重複は意図した設計ではなく、`dad`向けの「洗濯物を干す/畳む/しまう」クエストが`mom`向けの同名クエストによって実質的に無効化されている不具合であることが判明した。 | 直接ソース確認: `MY_HOME_SYSTEM/quest_data.py:81-83, 93-95`, `MY_HOME_SYSTEM/services/quest_service.py:737-761`, `MY_HOME_SYSTEM/current_schema.sql:174` |
...
| `target: 'siblings'` の実処理 | `MY_HOME_SYSTEM/services/quest_service.py`を直接確認した。`_get_sibling_partner_id`(277〜279行目付近)は`rows = cur.execute("SELECT user_id FROM quest_users WHERE role = ?", (ROLE_CHILD,)).fetchall()`により`role = 'role_child'`のユーザーがちょうど2人であることを前提に相方のuser_idを解決する設計であることを確認した。承認処理側(319〜353行目付近)では`approver['role'] != ROLE_ADULT`のチェック後、`target: 'siblings'`のクエストに対して両者分の`quest_history`行(状態`pending`)を作成し`linked_history_id`で相互連結する処理、および承認・却下・取消それぞれの箇所(324, 353, 400行目付近)で連結先レコードもあわせて処理するカスケード処理が実装されていることを確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/services/quest_service.py:277-279, 319-406` |
