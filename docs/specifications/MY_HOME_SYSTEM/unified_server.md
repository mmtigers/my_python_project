...
* 根拠: `app = FastAPI(...)` (行番号: 154-159 / 抜粋: "app = FastAPI("), `uvicorn.run(...)` (行番号: 325 / 抜粋: "uvicorn.run(app, host="0.0.0.0"")
...
| 名称 | 理由 | 根拠 |
| --- | --- | --- |
| `config.QUEST_DIST_DIR` | 設定ファイル内の変数の有無・パス文字列が不明 | `getattr(config, "QUEST_DIST_DIR", None)` (行番号: 261 / 抜粋: "quest_dist_dir = getattr(config") |
| `setup_logging()` | ログ出力フォーマット等の詳細仕様が不明 | `logger = setup_logging("unifie")` (行番号: 39 / 抜粋: "logger = setup_logging("unifie") |
| `sensor_service.cancel_all_tasks()` | キャンセルされる具体的なタスク内容が不明 | `sensor_service.cancel_all_tasks()` (行番号: 151 / 抜粋: "sensor_service.cancel_all_tasks") |
| 各ルーター (`webhook`, `quest`, `system`, `bounty`, `camera`) | 各パス配下の具体的なルーティング定義が不明 | `app.include_router(...)` (行番号: 239-243 / 抜粋: "app.include_router(webhook_rout") |
| `monitors/camera_monitor.py` | 起動する外部スクリプトの処理内容が不明 | `subprocess.Popen([sys.executable, camera_script])` (行番号: 115 / 抜粋: "camera_process = subprocess.Po") |
| `scheduler_boot.py` | 起動する外部スクリプトの処理内容が不明 | `subprocess.Popen([sys.executable, scheduler_script])` (行番号: 122 / 抜粋: "scheduler_process = subprocess.") |
| `apply_pending_migrations()` | マイグレーション適用の具体的な内部処理は `core/migrations.py` にあるため不明 | `apply_pending_migrations(migration_conn)` (行番号: 107 / 抜粋: "apply_pending_migrations(migration_conn)") |
...
* 根拠: `class SilencePolicyFilter(logg` (行番号: 42-86 / 抜粋: "class SilencePolicyFilter(logg")
...
* 根拠: `except Exception: pass` (行番号: 82-84 / 抜粋: "except Exception: pass")
...
* 根拠: `async def lifespan(app: FastA` (行番号: 92-152 / 抜粋: "async def lifespan(app: FastA")
...
* 根拠: 該当関数内処理 (行番号: 104-111, 113-115, 118-127, 133-140, 142-149 / 抜粋: "apply_pending_migrations(migration_conn)", "scheduler_process.terminate()")
...
* 根拠: `except Exception as e: logger.error(f"⚠️ Migration check failed...")` (行番号: 110-111 / 抜粋: "Migration check failed"), `except Exception as e:` (行番号: 126-127 / 抜粋: "Failed to start scheduler"), `except subprocess.TimeoutExpired` (行番号: 138-139, 147-148 / 抜粋: "except subprocess.TimeoutExpired")
...
* 根拠: `async def ip_restriction_middle` (行番号: 177-228 / 抜粋: "async def ip_restriction_middle")


* **引数/リクエスト**: `request: Request`, `call_next: Callable[[Request], Awaitable[Response]]`
* 根拠: `async def ip_restriction_middle` (行番号: 177 / 抜粋: "async def ip_restriction_middle")


* **戻り値/レスポンス**: `Response` (後続の処理結果)
* 根拠: `-> Response:` (行番号: 177 / 抜粋: "-> Response:")


* **副作用**: 外部ネットワークからのアクセス判定時(`logger.debug`)のログ出力。
* 根拠: `logger.debug(f"Allowed extern` (行番号: 227 / 抜粋: "logger.debug(f"Allowed extern")


* **エラーハンドリング**: IPアドレス解析時(`ipaddress.ip_address`)の`ValueError`を補足し無視(`pass`)する。
* 根拠: `except ValueError: pass` (行番号: 219-220 / 抜粋: "except ValueError: pass")
...
* 根拠: `async def global_exception_hand` (行番号: 231-236 / 抜粋: "async def global_exception_hand")


* **引数/リクエスト**: `request: Request`, `exc: Exception`
* 根拠: `async def global_exception_hand` (行番号: 231 / 抜粋: "async def global_exception_hand")


* **戻り値/レスポンス**: `JSONResponse` (HTTP 500, `{"detail": "Internal Server Error"}`のみ)。例外の詳細文字列(`str(exc)`)はレスポンスボディに含めず、ログにのみ出力する。
* 根拠: `return JSONResponse(status_code=500, content={"detail": "Internal Server Error"})` (行番号: 233-236 / 抜粋: "content={"detail": "Internal Server Error"}")


* **副作用**: エラーログへのスタックトレース出力。
* 根拠: `logger.error(f"🔥 Global Exce` (行番号: 232 / 抜粋: "logger.error(f"🔥 Global Exce")


* **エラーハンドリング**: なし（本メソッド自体が最上位の例外ハンドラ）
* 根拠: `@app.exception_handler(Exceptio` (行番号: 230 / 抜粋: "@app.exception_handler(Exceptio")
...
* 根拠: `async def serve_quest_spa(full_` (行番号: 271-284 / 抜粋: "async def serve_quest_spa(full_")、`@app.get("/quest/{full_path:path}")` / `@app.get("/camera/{full_path:path}")` (行番号: 271-272)


* **引数/リクエスト**: `full_path: str`
* 根拠: `async def serve_quest_spa(full_` (行番号: 273 / 抜粋: "async def serve_quest_spa(full_")
...
* **副作用**: なし
* 根拠: 該当関数内処理 (行番号: 271-284 / 抜粋: "async def serve_quest_spa(full_")
...
* 根拠: `async def serve_quest_root():` (行番号: 288-296 / 抜粋: "async def serve_quest_root():")、`@app.get("/quest")` 等4つのデコレータ (行番号: 288-291)
...
graph TD
    ...
    subgraph "External Modules (Black Box)"
        ...
        Routers["routers (quest, webhook, system, bounty, camera)"]
    end
...
| 優先度 | ファイル名(推測可) | 理由 | 根拠 |
| --- | --- | --- | --- |
| 高 | `config.py` | システム全体の静的パス(`QUEST_DIST_DIR`等)や他の設定変数を決定しており、システムの配置構造を把握するため。 | `import config` (行番号: 27)、`getattr(config, "QUEST_DIST_DIR", None)` (行番号: 261) |
| 高 | `scheduler_boot.py` | APIサーバー起動と同時にサブプロセスとして起動・ライフサイクル共有されるため、非同期で動作する定期処理の仕様把握に必須であるため。 | `scheduler_script = os.path.join(PROJECT_ROOT, "scheduler_boot.py")` (行番号: 120) |
| 中 | `routers/quest_router.py` | `/api/quest`パス配下にマウントされる処理群であり、システム名である「Family Quest API」のコアドメイン処理を把握するため。 | `app.include_router(quest_router.router, prefix="/api/quest")` (行番号: 240) |
...
| 中 | `services/sensor_service.py` | 終了処理にタスクキャンセルが含まれており、起動後に常駐するセンサー処理の内容と影響範囲を特定するため。 | `sensor_service.cancel_all_tasks()` (行番号: 151) |
...
* シャットダウン時、スケジューラープロセス・カメラ監視プロセスの双方とも終了待ち（`wait`）が5秒でタイムアウトし、強制キル（`kill`）される同一パターンの処理となっている（行番号: 133-140, 142-149）。
* カメラ監視サブプロセス（`camera_process = subprocess.Popen(...)`、行番号: 115）の起動は、スケジューラー起動処理（行番号: 119-127）とは異なり `try-except` で囲まれていないため、起動に失敗した場合は `lifespan` 全体が例外で停止し、アプリケーションが起動できない可能性がある。
...
| APIルーティング詳細 | `/api/quest`、`/api/system`、`/api/bounty`、`/api/cameras`配下の実際のエンドポイント定義が不明 | `routers/quest_router.py`, `routers/system_router.py`, `routers/bounty_router.py`, `routers/webhook_router.py`, `routers/camera_router.py` |
