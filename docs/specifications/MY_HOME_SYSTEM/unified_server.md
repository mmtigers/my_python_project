...
* 根拠: `app = FastAPI(...)` (行番号: 142-147 / 抜粋: "app = FastAPI("), `uvicorn.run(...)` (行番号: 313 / 抜粋: "uvicorn.run(app, host="0.0.0.0"")
...
| `uvicorn` | 外部パッケージ | ASGIサーバーの起動と設定取得 | 根拠: `[uvicorn]` (行番号: 304 / 抜粋: "import uvicorn") |
...
| 名称 | 理由 | 根拠 |
| --- | --- | --- |
| `config.QUEST_DIST_DIR` | 設定ファイル内の変数の有無・パス文字列が不明 | `getattr(config, "QUEST_DIST_DIR", None)` (行番号: 249 / 抜粋: "quest_dist_dir = getattr(config") |
| `setup_logging()` | ログ出力フォーマット等の詳細仕様が不明 | `logger = setup_logging("unifie")` (行番号: 39 / 抜粋: "logger = setup_logging("unifie") |
| `sensor_service.cancel_all_tasks()` | キャンセルされる具体的なタスク内容が不明 | `sensor_service.cancel_all_tasks()` (行番号: 139 / 抜粋: "sensor_service.cancel_all_tasks") |
| 各ルーター (`webhook`, `quest`, `system`, `bounty`, `camera`) | 各パス配下の具体的なルーティング定義が不明 | `app.include_router(...)` (行番号: 227-231 / 抜粋: "app.include_router(webhook_rout") |
| `monitors/camera_monitor.py` | 起動する外部スクリプトの処理内容が不明 | `subprocess.Popen([sys.executable, camera_script])` (行番号: 112 / 抜粋: "camera_process = subprocess.Po") |
...
* 根拠: `async def ip_restriction_middle` (行番号: 164-216 / 抜粋: "async def ip_restriction_middle")


* **引数/リクエスト**: `request: Request`, `call_next: Callable[[Request], Awaitable[Response]]`
* 根拠: `async def ip_restriction_middle` (行番号: 165 / 抜粋: "async def ip_restriction_middle")


* **戻り値/レスポンス**: `Response` (後続の処理結果)
* 根拠: `-> Response:` (行番号: 165 / 抜粋: "-> Response:")


* **副作用**: 外部ネットワークからのアクセス判定時(`logger.debug`)のログ出力。
* 根拠: `logger.debug(f"Allowed extern` (行番号: 215 / 抜粋: "logger.debug(f"Allowed extern")


* **エラーハンドリング**: IPアドレス解析時(`ipaddress.ip_address`)の`ValueError`を補足し無視(`pass`)する。
* 根拠: `except ValueError: pass` (行番号: 207-208 / 抜粋: "except ValueError: pass")
...
* 根拠: `async def serve_quest_spa(full_` (行番号: 259-272 / 抜粋: "async def serve_quest_spa(full_")、`@app.get("/quest/{full_path:path}")` / `@app.get("/camera/{full_path:path}")` (行番号: 259-260)


* **引数/リクエスト**: `full_path: str`
* 根拠: `async def serve_quest_spa(full_` (行番号: 261 / 抜粋: "async def serve_quest_spa(full_")


* **戻り値/レスポンス**: `FileResponse` または `JSONResponse` (HTTP 404)
* 根拠: `return FileResponse(target_fil` (行番号: 266 / 抜粋: "return FileResponse(target_file"), `return JSONResponse(status_code` (行番号: 272 / 抜粋: "return JSONResponse(status_code")


* **副作用**: なし
* 根拠: 該当関数内処理 (行番号: 259-272 / 抜粋: "async def serve_quest_spa(full_")


* **エラーハンドリング**: `index.html`が存在しない場合は404エラーとしてJSONレスポンスを返す。
* 根拠: `if os.path.exists(index_path):` (行番号: 270-272 / 抜粋: "if os.path.exists(index_path):")
...
* 根拠: `async def serve_quest_root():` (行番号: 276-284 / 抜粋: "async def serve_quest_root():")、`@app.get("/quest")` 等4つのデコレータ (行番号: 276-279)


* **引数/リクエスト**: なし
* 根拠: `async def serve_quest_root():` (行番号: 280 / 抜粋: "async def serve_quest_root():")


* **戻り値/レスポンス**: `FileResponse` または `JSONResponse` (HTTP 404)
* 根拠: `return FileResponse(index_path)` (行番号: 283 / 抜粋: "return FileResponse(index_path)"), `return JSONResponse(status_code` (行番号: 284 / 抜粋: "return JSONResponse(status_code")


* **副作用**: なし
* 根拠: 該当関数内処理 (行番号: 280-284 / 抜粋: "async def serve_quest_root():")
...
* 根拠: `async def root():` (行番号: 291-297 / 抜粋: "async def root():")


* **引数/リクエスト**: なし
* 根拠: `async def root():` (行番号: 292 / 抜粋: "async def root():")


* **戻り値/レスポンス**: `dict` (status, system, timeキーを含む)
* 根拠: `return { "status": "ok", ... }` (行番号: 293-297 / 抜粋: "return { "status": "ok", "sy")


* **副作用**: なし
* 根拠: 該当関数内処理 (行番号: 291-297 / 抜粋: "async def root():")
...
* 根拠: `async def health_check():` (行番号: 299-301 / 抜粋: "async def health_check():")


* **引数/リクエスト**: なし
* 根拠: `async def health_check():` (行番号: 300 / 抜粋: "async def health_check():")


* **戻り値/レスポンス**: `dict` (statusキーを含む)
* 根拠: `return {"status": "healthy"}` (行番号: 301 / 抜粋: "return {"status": "healthy"}")


* **副作用**: なし
* 根拠: 該当関数内処理 (行番号: 299-301 / 抜粋: "async def health_check():")
...
| 優先度 | ファイル名(推測可) | 理由 | 根拠 |
| --- | --- | --- | --- |
| 高 | `config.py` | システム全体の静的パス(`QUEST_DIST_DIR`等)や他の設定変数を決定しており、システムの配置構造を把握するため。 | `import config` (行番号: 27)、`getattr(config, "QUEST_DIST_DIR", None)` (行番号: 249) |
| 高 | `scheduler_boot.py` | APIサーバー起動と同時にサブプロセスとして起動・ライフサイクル共有されるため、非同期で動作する定期処理の仕様把握に必須であるため。 | `scheduler_script = os.path.join(PROJECT_ROOT, "scheduler_boot.py")` (行番号: 117) |
| 中 | `routers/quest_router.py` | `/api/quest`パス配下にマウントされる処理群であり、システム名である「Family Quest API」のコアドメイン処理を把握するため。 | `app.include_router(quest_router.router, prefix="/api/quest")` (行番号: 228) |
| 中 | `routers/camera_router.py` | `/api/cameras`パス配下にマウントされ、SPAルーティング(`/camera/*`)とも連動するカメラ機能のAPI仕様を把握するため。 | `app.include_router(camera_router.router, prefix="/api/cameras")` (行番号: 231) |
| 中 | `services/sensor_service.py` | 終了処理にタスクキャンセルが含まれており、起動後に常駐するセンサー処理の内容と影響範囲を特定するため。 | `sensor_service.cancel_all_tasks()` (行番号: 139) |
