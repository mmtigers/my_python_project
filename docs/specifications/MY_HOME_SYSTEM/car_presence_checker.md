...
| --- | --- | --- |
| `config.CAMERA_IP`等の実際の設定値 | `config.py`を直接確認した。`CAMERA_IP`/`CAMERA_USER`/`CAMERA_PASS`は独立した環境変数ではなく、316〜321行目で`if CAMERAS: CAMERA_IP = CAMERAS[0].get("ip")`（`CAMERAS`が空なら`None`）というように、`devices.json`から読み込んだカメラ設定リスト`CAMERAS`の先頭要素(`CAMERAS[0]`)から派生する後方互換用変数であることが判明した。`config.SQLITE_TABLE_CAR`は244行目で`str = "car_records"`という固定値で定義されている。実際の`devices.json`はリポジトリ内に存在しない(`.gitignore`の`*.json`規則により追跡対象外)ため、`CAMERA_IP`の具体的な値そのものは確認できなかった。 | 直接ソース確認: `MY_HOME_SYSTEM/config.py:244, 297-298, 316-321` |
| `get_db_cursor` / `save_log_generic`の内部実装 | `core/database.py`を直接確認した。`get_db_cursor(commit=False)`(12〜13行目、`@contextmanager`)は`sqlite3.connect(config.SQLITE_DB_PATH, timeout=30.0)`のうえ`conn.row_factory = sqlite3.Row`を設定し、`sqlite3.OperationalError`で`"locked"`を含む場合は最大5回まで1秒間隔でリトライする(16〜35行目)。`save_log_generic(table, columns_list, values_list)`(67〜79行目)は`get_db_cursor(commit=True)`を使い`INSERT INTO {table} ({columns}) VALUES ({placeholders})`を動的構築・実行し、成功時`True`・例外時はログを出力し`False`を返す。本ファイル170行目の`save_log_generic(config.SQLITE_TABLE_CAR, cols, vals)`が呼び出す先の`car_records`テーブルは、`init_unified_db.py`232〜240行目の`CREATE TABLE IF NOT EXISTS {config.SQLITE_TABLE_CAR} (id INTEGER PRIMARY KEY AUTOINCREMENT, action TEXT, rule_name TEXT, timestamp DATETIME, score REAL)`で定義され、`current_schema.sql`42〜47行目にも同一構造の`CREATE TABLE car_records`が記録されており、本ファイル157行目の`cols = ["timestamp", "action", "rule_name", "score"]`と一致することを確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/core/database.py:12-13, 16-35, 67-79`, `MY_HOME_SYSTEM/init_unified_db.py:232-240`, `MY_HOME_SYSTEM/current_schema.sql:42-47` |
| `send_push`の内部実装（Discord通知の実際の送信方法） | `services/notification_service.py`を直接確認した。`send_push(user_id, messages, image_data=None, target="both", channel="notify", filename="snapshot.jpg")`(116〜140行目)は`target`に`"discord"`が含まれる場合、`_send_discord_webhook(messages, image_data, channel, filename)`(30〜71行目)を呼び出す。同関数は`channel`引数(既定`"notify"`→`config.DISCORD_WEBHOOK_NOTIFY`または`DISCORD_WEBHOOK_URL`、`"error"`→`config.DISCORD_WEBHOOK_ERROR`)に応じたWebhook URLへ、画像なしの場合は`requests.post(url, json={"content": text_content}, timeout=10)`でJSON送信し、ステータスコードが200/204以外または例外時はエラーログを出力し`False`を返す。本ファイル230〜234行目(状態変化通知)は`target="discord"`のみで`channel`省略(既定`"notify"`)、244行目(エラー通知)は`target="discord", channel="error"`を指定しており、それぞれ通知先チャンネルが異なることを確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/services/notification_service.py:30-71, 116-140`, `MY_HOME_SYSTEM/monitors/old/car_presence_checker.py:230-234, 244` |
| `with_exponential_backoff`が実際に使用されているか | `monitors/old/car_presence_checker.py`全体(253行)を直接確認した。19行目で`from core.utils import get_now_iso, with_exponential_backoff`とインポートされているものの、ファイル全体を検索しても`@with_exponential_backoff`というデコレータ適用箇所、および`with_exponential_backoff(...)`という直接呼び出し箇所はいずれも見つからなかった。`core/utils.py`21〜46行目を確認したところ、`with_exponential_backoff(base_delay=5, max_delay=300, alert_threshold=5)`は関数実行時の例外を捕捉し指数関数的バックオフで無限リトライを行うデコレータファクトリであり、本来は`get_camera_frame`等の関数に付与して使うことが想定される実装だが、本ファイルではインポートのみで実際には未使用（デッドインポート）であることを直接確認した。なお`get_camera_frame`関数自体は49〜106行目で独自の有限回数(`MAX_RETRIES=3`)のリトライ・指数バックオフ処理(98〜102行目)を内蔵しており、`with_exponential_backoff`とは別系統のリトライ機構が使われている。 | 直接ソース確認: `MY_HOME_SYSTEM/monitors/old/car_presence_checker.py:19, 49-106`（全253行を検索し他に使用箇所なしを確認）, `MY_HOME_SYSTEM/core/utils.py:21-46` |
| `monitors/old/`ディレクトリの位置づけ（現行版との関係） | `monitors/`ディレクトリを直接確認したところ、`car_presence_checker.py`という同名ファイルは`monitors/old/`配下にのみ存在し、`monitors/`直下（現行版が置かれる場所）には存在しなかった(`monitors/`直下の一覧: `camera_monitor.py`, `daily_timelapse_job.py`, `memory_monitor.py`, `nas_monitor.py`, `nature_remo_monitor.py`, `scheduled_timelapse.py`, `server_watchdog.py`, `smart_timelapse_generator.py`, `switchbot_power_monitor.py`, `timelapse_generator.py`, `timelapse_runner.py`, `tv_lock_monitor.py`のみ)。起動スクリプト`start_all.sh`（全75行）および`scheduler_boot.py`を直接確認したが、いずれも`car_presence_checker.py`を起動・参照する記述は見当たらず、現在の起動パイプラインからは呼び出されていないことを確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/monitors/`ディレクトリ一覧, `MY_HOME_SYSTEM/monitors/old/`ディレクトリ一覧, `MY_HOME_SYSTEM/start_all.sh`（全75行、該当箇所なしを確認）, `MY_HOME_SYSTEM/scheduler_boot.py`（`grep`による該当箇所なしを確認） |
