...
- [unified_server.md](./unified_server.md) — 呼び出し先(バックグラウンド起動)。起動後、内部で`scheduler_boot.py`と`monitors/camera_monitor.py`をさらにサブプロセス起動する。**（Issue #829で変更）** ダッシュボード(`routers/dashboard_router.py`)もこのプロセス自身が配信するようになったため、本スクリプトが別途起動するプロセスではなくなった
- [dashboard.md](./dashboard.md) / [dashboard_proxy_service.md](./dashboard_proxy_service.md) — **（廃止・Issue #829）** 以前は本スクリプトのPhase 4がStreamlit版ダッシュボード(`dashboard.py`)を`--server.baseUrlPath`付きでバックグラウンド起動し、`dashboard_proxy_service`が8000番側で中継していたが、Streamlit版廃止に伴い本スクリプトからの呼び出しは無くなった
- [dashboard_router.md](./dashboard_router.md) — **（Issue #829で変更）** 中継先ではなく、`unified_server.py`が直接配信するページの実体
...
| 名称 | 理由 | 根拠 |
| --- | --- | --- |
| `switchbot_webhook_fix.py` | スクリプト内で実行されているが、処理内容の実装が提供されていないため | `switchbot_webhook_fix.py` (行番号: 261 / 抜粋: "$PYTHON_EXEC switchbot_webhook_fix.py") |
| `unified_server.py` | スクリプト内で実行および停止対象となっているが、実装内容が不明なため | `unified_server.py` (行番号: 277 / 抜粋: "$PYTHON_EXEC unified_server.py") |
| `/mnt/nas` | マウント状況の確認先となっているが、システム上の具体的なNAS構成が不明なため | `MOUNT_POINT` (行番号: 137 / 抜粋: "MOUNT_POINT="/mnt/nas"") |
| `family-quest/deploy.sh` | Phase 2で`--if-stale`引数付きで実行されるが、冪等判定・ビルドの実装内容は本ファイル外のため | `bash "$QUEST_DIR/deploy.sh" --if-stale` (行番号: 240 / 抜粋: "bash "$QUEST_DIR/deploy.sh" --if-stale") |
| `sync_strict.py` | **（Issue #700 で追加）** Phase 2.5で`--if-stale`付きで実行されるが、鮮度判定・同期の実装内容は本ファイル外のため | `$PYTHON_EXEC sync_strict.py --if-stale` (行番号: 255 / 抜粋: "$PYTHON_EXEC sync_strict.py --if-stale > logs/quest_master_sync.log 2>&1") |
...
* 根拠: ディレクトリ移動処理 (行番号: 69 / 抜粋: "cd "$PROJECT_DIR" || exit 1")



### [要素名2：Phase 0: クリーンアップ処理]

* **役割**: 停止対象プロセス名の配列`CLEANUP_TARGETS`(`unified_server.py`, `camera_monitor.py`, `scheduler_boot.py`)を定義し、各対象へ`pkill`でSIGTERMを送って優しく停止させる。以前は`scheduler.py`という実在しないプロセス名を対象にしており実体の`scheduler_boot.py`にマッチしないため旧schedulerプロセスが再起動のたびに生き残っていた点と、存在しない`bluetooth_monitor.py`を対象にしていた点を修正し、実ファイル名の配列に置き換えている。**（Issue #829で変更）** 以前含まれていた`streamlit run`(Streamlit版ダッシュボード)は、その廃止に伴い配列から削除された。
...
* **[修正済み]（Issue #646で追加、Issue #829で削除）** 以前は`--prepare`モード(`PREPARE_ONLY=true`)で、Streamlit版ダッシュボード(`streamlit run`)を`CLEANUP_TARGETS`から除外する`filtered_targets`によるフィルタ処理があった(ダッシュボードは`home_dashboard.service`が別ユニットで管理しており、ここでSIGTERMするとsystemd側が予期しない停止と扱うため)。Streamlit版ダッシュボードの廃止に伴い、そもそも`CLEANUP_TARGETS`に`streamlit run`が含まれなくなったため、このフィルタ処理自体も不要になり削除された。`unified_server.py`・子プロセス・ffmpegの掃除(旧来の`nohup`起動が残っている場合の回収、および前世代の孤児の掃除)は変わらず行う。
...
* 根拠: `ls "$MOUNT_POINT" >/dev/null 2>&1` (行番号: 80 / 抜粋: "ls "$MOUNT_POINT" >/dev/null 2>&1")、`sleep "$MOUNT_WAIT"` (行番号: 153 / 抜粋: "sleep "$MOUNT_WAIT"")、`echo`コマンド (行番号: 85, 90, 92 / 抜粋: "echo "✅ NAS Mounted."")
...
* 根拠: Phase 1.5ブロック (行番号: 138〜185 / 抜粋: "# --- Phase 1.5: Python依存関係の鮮度チェック ---")、[対象ファイル定義] (行番号: 181 / 抜粋: "REQ_FILES=(\"requirements.txt\" \"$DEVELOP_ROOT/DDD/requirements.txt\")")
...
* 根拠: 実行・リダイレクト処理 (行番号: 240 / 抜粋: "bash "$QUEST_DIR/deploy.sh" --if-stale > logs/quest_deploy.log 2>&1")
...
* 根拠: 実行・リダイレクト処理 (行番号: 255 / 抜粋: "$PYTHON_EXEC sync_strict.py --if-stale > logs/quest_master_sync.log 2>&1")
...
* **役割**: Webhook修正スクリプト(`switchbot_webhook_fix.py`)を実行し、その後`unified_server.py`をバックグラウンドで起動する。標準出力・標準エラー出力は`logs/server_boot.log`にリダイレクトする。`--prepare`モードではPhase 3の直後に`exit 0`で終了し、Phase 4(サーバー起動)は実行しない。**（Issue #829で変更）** ダッシュボード(`routers/dashboard_router.py`)は`unified_server.py`自身が`${DASHBOARD_BASE_PATH}`配下で配信するようになったため、以前ここにあったStreamlit(`dashboard.py`)を`--server.baseUrlPath`付きで別プロセスとしてバックグラウンド起動する処理は削除された。
* 根拠: 起動処理ブロック (行番号: 271〜279 / 抜粋: "echo "--- Start Home System Server ---"")
...
* **役割**: 第1引数が`--prepare`のとき`PREPARE_ONLY=true`とし、Phase 3の直後に`exit 0`して Phase 4(サーバーの`nohup`起動)を行わない。実機の`deploy/systemd/home_system.service`はこのモードを`ExecStartPre`で呼び、`unified_server.py`本体を`ExecStart`(`Type=simple`)としてフォアグラウンド起動し、異常終了時は`Restart=on-failure`で自動復旧する。以前は`Type=oneshot`+`RemainAfterExit=yes`のもとで本スクリプトが`nohup ... & disown`でサーバーを起動しており、サーバー本体がsystemdの管理外にあった(落ちても通知のみ・人手復旧)。引数なしの従来経路(手動運用・開発用)は残している。**（Issue #829で変更）** 以前は`(1)`としてPhase 0の掃除対象から`streamlit run`を外す処理もあったが、Streamlit版ダッシュボード廃止に伴い`CLEANUP_TARGETS`自体に`streamlit run`が含まれなくなったため、この処理は不要になり削除された。

### [要素名7.5：手動起動ガード（Issue #824で追加）]

* **役割**: 引数解析の直後（`cd` と Phase 0 の掃除より前）で、`--prepare` なしの実行かつ `systemctl` が存在し、`home_system.service` が `systemctl is-enabled --quiet` で有効と判定された場合、`exit 1` で実行を拒否する。拒否メッセージは標準エラーへ出し、正しい手順（`sudo systemctl restart home_system.service`）と、上書き方法（`ALLOW_MANUAL_START=1`）を案内する。**（Issue #829で変更）** 以前は `home_system.service` / `home_dashboard.service` の対象2つを `for managed_unit in ...` でループ確認していたが、Streamlit版ダッシュボード(`home_dashboard.service`)の廃止に伴い対象が1つだけになったため、shellcheck(SC2043「このループは1回しか回らない」)の指摘を受けて単純な `if` に書き換えた。判定対象自体は `home_system.service` のみで変わっていない。
* 根拠: `if [ "$PREPARE_ONLY" != true ] && command -v systemctl >/dev/null 2>&1; then` から `# --- end 手動起動ガード ---` まで
* **なぜ必要か**: systemd がユニットを管理している実機でフルモードを実行すると、(1) Phase 0 の掃除が systemd 管理下のプロセスへ SIGTERM を送る、(2) SIGTERM による終了は systemd から見て正常終了なので `Restart=on-failure` は再起動せず `home_system.service` は inactive になる、(3) Phase 4 が `nohup` で起動し直し、プロセスは systemd の管理外（親 PID 1 の孤児）になる。サイトは孤児が応答するので普通に動いて見え、**#646 の自動復旧だけが黙って止まる**。2026-09-21 09:54 に実機で起き、約1時間半 `NRestarts=344` のまま放置された。`docs/runbooks/ラズパイデプロイ前動作検証手順.md` には以前から「引数なしで直接実行しないこと」と書かれていたが防げなかったため、コードで拒否する
...
flowchart TD
    ...
    WebhookFix --> ServerBoot["外部：unified_server.py() バックグラウンド起動<br/>(ダッシュボードもこのプロセス自身が配信。#829)"]
    ServerBoot --> End([End])
...
graph TD
    ...
    Server["unified_server.py<br/>(ダッシュボードもこのプロセス自身が配信。#829)"]
    NAS["/mnt/nas"]
    Logs["logs/"]
...
| 優先度 | ファイル名(推測可) | 理由 | 根拠 |
| --- | --- | --- | --- |
| 高 | `unified_server.py` | システム全体のコアとしてバックグラウンドで起動され、コメント上で`scheduler_boot.py`の起動も担うと記載されているため、全体ロジックの把握に必須。**（Issue #829）** ダッシュボード(`routers/dashboard_router.py`)もこのプロセス自身が配信するようになった。 | `unified_server.py` (行番号: 277 / 抜粋: "$PYTHON_EXEC unified_server.py") |
| 中 | `switchbot_webhook_fix.py` | 起動時に毎回実行されており、外部API(SwitchBot/Cloudflare Tunnel)との通信や設定更新を担っていると推測されるため。 | `switchbot_webhook_fix.py` (行番号: 261 / 抜粋: "$PYTHON_EXEC switchbot_webhook_fix.py") |
...
* **影響範囲の広いプロセス停止 (`pkill -f`)**: `CLEANUP_TARGETS`の各対象への`pkill -f`は部分一致でプロセスを終了させるため、無関係な別プロジェクトの同名プロセスが実行中の場合、巻き込んで終了させてしまう危険性がある。
* **[修正済み]（Issue #829で解消）** 以前はStreamlit版ダッシュボードの`--server.baseUrlPath`が本スクリプト(Phase 4)・`deploy/systemd/home_dashboard.service`・`config.DASHBOARD_BASE_PATH`の3箇所で一致している必要があり、どれか1つがずれると静的アセットが404になり画面が表示されない問題があった。Streamlit版廃止に伴い、ダッシュボードは`unified_server.py`自身が`config.DASHBOARD_BASE_PATH`を直接参照して配信するようになったため、この3箇所一致の要件自体が無くなった。
* **[修正済み] プロセスの起動監視漏れ (Issue #646)**: 以前は`unified_server.py`を`nohup`でバックグラウンド起動するだけで、即座にクラッシュしていないかの死活監視・エラー検知のロジックが存在しなかった。現在、実機経路では`deploy/systemd/home_system.service`(`Type=simple`+`Restart=on-failure`)が`unified_server.py`をフォアグラウンドで管理し、本スクリプトは`--prepare`で前処理のみ行う。引数なしの手動経路では従来どおり死活監視は無い。**（Issue #829で変更）** 以前は`home_dashboard.service`が`dashboard.py`(Streamlit)を同様にフォアグラウンドで管理していたが、Streamlit版廃止に伴い当該ユニット自体が削除された。
...
| `unified_server.py`の仕様 | `unified_server.md`の解析によれば、FastAPI製のAPIサーバーであり、`lifespan`内で`monitors/camera_monitor.py`と`scheduler_boot.py`をサブプロセスとして起動し、終了時にはそれらを停止させる構成になっているとされる。以前は`camera_monitor.py`の起動が`try-except`で保護されておらず起動失敗時にアプリ全体が起動できない可能性が`unified_server.md`の保守上の注意点として挙げられていたが、Issue #360で両子プロセスの起動が共通の`_spawn_child_process`内で保護され、Issue #646で30秒ごとの死活監視・自動再起動(`restart_dead_children`)も加わった。**（Issue #829）** ダッシュボード(`routers/dashboard_router.py`)も本スクリプトが別プロセスとして起動するのではなく、`unified_server.py`自身が配信するようになった(Streamlit版`dashboard.py`は廃止)。 | unified_server.md, scheduler_boot.md, dashboard_router.md |
