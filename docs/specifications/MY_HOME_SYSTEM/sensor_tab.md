> **廃止 (2026-09-25)**: このファイルはソース (`MY_HOME_SYSTEM/views/dashboard/sensor_tab.py`) が Issue #829(お家ダッシュボード改修:
...
| `df_sensor`の`device_type`列に実際に含まれる値の一覧 | `MY_HOME_SYSTEM/services/analysis_service.py`の`load_sensor_data(limit: int = 5000) -> pd.DataFrame`(167-223行目)を直接確認した。`device_type`列は3系統から構成される。(1) 旧`device_records`テーブル(172-180行目)は`device_type`列をそのままSELECTしており、値そのものはDB内の既存データに依存するため本関数のコードからは全列挙できない。(2) `config.SQLITE_TABLE_SWITCHBOT_LOGS`由来の`df_meter`は192行目で一律`"Meter"`に設定される。(3) `config.SQLITE_TABLE_POWER_USAGE`由来の`df_power`は203-205行目で`device_name`に`"Remo"`を含めば`"Nature Remo E Lite"`、含まなければ`"Plug"`を設定する。**(Issue #169で解消)** 以前はこの直後に`df_power["device_type"] = df_power["device_type"].replace("Plug", "Nature Remo E Lite")`という行があり、列内の`"Plug"`という値を全て`"Nature Remo E Lite"`に一括置換していたため、`df_power`由来の行の`device_type`は結果的に常に`"Nature Remo E Lite"`となり、`"Plug"`という値自体が最終的なDataFrameに一切残らなかった(本ドキュメント20行目が根拠として挙げる`str.contains("Plug", na=False)`フィルタが構造的に一致しない・全プラグの消費電力がスマートメーター全体消費のグラフへ混入する不具合)。この一括置換行は削除され、現在は`"Remo"`を含むかどうかの判定結果がそのまま最終的な`device_type`になる。旧`device_records`由来の値については、`MY_HOME_SYSTEM/views/dashboard/summary.py`(46-47行目)が`device_type`列に対し`"Motion"`(部分一致)および`"Webhook"`(完全一致)の判定を行っているコードを直接確認しており、これらの値がコード上で参照されている根拠として存在する。 | 直接ソース確認: `MY_HOME_SYSTEM/services/analysis_service.py:167-223`（参考: `MY_HOME_SYSTEM/views/dashboard/sensor_tab.py:46-47`, `MY_HOME_SYSTEM/views/dashboard/summary.py:46-47`） |
