> **⚠️ 廃止: このファイルは 2026-09-18 時点でソース (`MY_HOME_SYSTEM/tools/keep_alive_speaker.sh`) が削除されたため廃止されました。**
>
> Issue #664 で、同じ「Bluetoothスピーカーのキープアライブ」という目的を別方式で
> 実現していた2スクリプトを [keep_alive_anker.md](./keep_alive_anker.md)
> (`tools/keep_alive_anker.sh`) へ一本化し、本スクリプトは削除しました。
> `deploy/cron/crontab` の `2-59/5 * * * *` エントリも同時に撤去しています。
>
> 判断理由(要約。詳細は `deploy/cron/README.md`):
>
> 1. `keep_alive_anker.sh` は接続確認・自動再接続まで行い、機能的に本スクリプトを包含する
>    (本スクリプトは無音MP3を再生するだけで、切断状態からは復帰できない)
> 2. 本スクリプトの音源 `silent.mp3` はNASマウント配下にあり、NAS未マウント時に機能しない
> 3. `keep_alive_anker.sh` は Issue #663 でMACアドレスを `.env` へ外部化済み
> 4. 本スクリプトは成功時も毎回ログを書くため、5分毎の実行で1日約576行のノイズになっていた
>
