...
**（Issue #585で判明・部分解消）** 実行契機が本当にどこにも存在しないことを確認したうえで、`deploy/cron/crontab`に5分毎(`keep_alive_anker.sh`と1分ずらした`2-59/5 * * * *`)のエントリを追加した。ただしこの間隔は暫定値であり、`keep_alive_anker.sh`との使い分け基準(両方とも本当に必要か)も未解明のまま残っている(下表に残す)。
