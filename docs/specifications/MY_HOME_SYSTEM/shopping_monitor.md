...
| --- | --- | --- |
| `config.SHOPPING_TARGETS`の実際の内容 | `MY_HOME_SYSTEM/config.py`342-353行目を直接確認した。`SHOPPING_TARGETS`は`platform`/`sender`/`subject_keywords`キーを持つ辞書2件からなるリストで、Amazon(`sender: "auto-confirm@amazon.co.jp"`, `subject_keywords: ["Amazon.co.jpのご注文", "注文済み", "Amazon.co.jp order"]`)とRakuten(`sender: "order@rakuten.co.jp"`, `subject_keywords: ["注文内容ご確認", "ご注文内容の確認", "発送のご案内"]`)であることを確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/config.py:342-353` |
| `config.GMAIL_USER` / `config.GMAIL_APP_PASSWORD`の値 | `MY_HOME_SYSTEM/config.py`201-202行目を直接確認した。`GMAIL_USER: Optional[str] = os.getenv("GMAIL_USER")`、`GMAIL_APP_PASSWORD: Optional[str] = os.getenv("GMAIL_APP_PASSWORD")`のみで、いずれもリテラルな既定値は設定されておらず実行時の環境変数(`.env`)にのみ依存する。`.env`は`.gitignore`13行目の`.env`規則により追跡対象外であり、リポジトリ内に実体ファイルも存在しないため、実際の値は確認できなかった。 | 直接ソース確認: `MY_HOME_SYSTEM/config.py:201-202`, `.gitignore:13` |
| `config.SQLITE_TABLE_SHOPPING`の実テーブル名・スキーマ | `MY_HOME_SYSTEM/config.py`248行目で`SQLITE_TABLE_SHOPPING: str = "shopping_records"`と定義されていることを確認した。さらに`MY_HOME_SYSTEM/init_unified_db.py`277-287行目の`CREATE TABLE IF NOT EXISTS {config.SQLITE_TABLE_SHOPPING} (...)`文を直接確認したところ、`id INTEGER PRIMARY KEY AUTOINCREMENT, platform TEXT, order_date TEXT, item_name TEXT, price INTEGER, email_id TEXT UNIQUE, timestamp DATETIME NOT NULL`という7カラム構成であることが判明した。 | 直接ソース確認: `MY_HOME_SYSTEM/config.py:248`, `MY_HOME_SYSTEM/init_unified_db.py:277-287` |
| `common.get_db_cursor`のトランザクション制御詳細 | `MY_HOME_SYSTEM/common.py`22-27行目で`from core.database import get_db_cursor, ...`とあり、`common.get_db_cursor`は`core.database.get_db_cursor`をそのまま再エクスポートするfacadeであることを確認した。実体の`MY_HOME_SYSTEM/core/database.py`13-43行目の`get_db_cursor(commit: bool = False)`を直接確認した。`sqlite3.connect`後に`conn.row_factory = sqlite3.Row`、`PRAGMA journal_mode=WAL`、`PRAGMA foreign_keys=ON`を設定してカーソルを`yield`し(21-26行目)、`commit=True`が渡された場合のみ正常終了時に`conn.commit()`を実行する(28-29行目)。`sqlite3.OperationalError`で`"locked"`を含む場合は`conn.close()`後、1.0秒間隔(`retry_delay`)で最大5回(`max_retries`)リトライし(31-35行目)、それ以外のエラーおよびその他の例外時は`conn.rollback()`を行った上で例外を再送出する(36-43行目、接続自体を明示的に`close()`する記述はこの分岐にはない)ことを確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/common.py:22-27`, `MY_HOME_SYSTEM/core/database.py:13-43` |
| `common.send_push`の対応プラットフォームと失敗時挙動 | `MY_HOME_SYSTEM/common.py`31-35行目で`from services.notification_service import send_push, ...`とあり、`common.send_push`は`services.notification_service.send_push`をそのまま再エクスポートするfacadeであることを確認した。実体の`MY_HOME_SYSTEM/services/notification_service.py`116-140行目を直接確認した。`send_push(user_id, messages, image_data=None, target="both", channel="notify", filename="snapshot.jpg")`は`target`が`"discord"`または`"both"`の場合のみ`_send_discord_webhook`(30-71行目)を呼び出す(121-124行目)。`_send_discord_webhook`は`channel`引数に応じて`config.DISCORD_WEBHOOK_ERROR`/`DISCORD_WEBHOOK_REPORT`/`DISCORD_WEBHOOK_NOTIFY or DISCORD_WEBHOOK_URL`のいずれかへ`requests.post`を1回だけ実行する実装で(37-61行目)、明示的なリトライ処理は存在しない(画像添付時はタイムアウト60秒、テキストのみは10秒)。`target="discord"`単体を指定した場合、LINE送信分岐(127-138行目)およびLINE失敗時のDiscordフォールバック通知(134-137行目)は実行されない。 | 直接ソース確認: `MY_HOME_SYSTEM/common.py:31-35`, `MY_HOME_SYSTEM/services/notification_service.py:30-140` |
