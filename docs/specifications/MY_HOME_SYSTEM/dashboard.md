...
| `common.send_push` の仕様 | `MY_HOME_SYSTEM/common.py:31-37`が`services.notification_service`から`send_push`を再インポートしているFacadeであることを直接確認した上で、`MY_HOME_SYSTEM/services/notification_service.py:116-141`の実装を直接確認した。`send_push(user_id, messages, image_data=None, target="both", channel="notify", filename="snapshot.jpg")`は、`target`が`"both"`または`"discord"`を含む場合に`_send_discord_webhook`(30行目)、`"both"`または`"line"`を含む場合に`_send_line_push`(73行目)をそれぞれ呼び出す統合プッシュ通知関数であることを確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/common.py:31-37`, `MY_HOME_SYSTEM/services/notification_service.py:116-141` |
