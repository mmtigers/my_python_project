...
| DBのカラムの型定義 | `MY_HOME_SYSTEM/core/database.py`の`save_log_generic(table, columns_list, values_list)`(67〜79行目)を直接確認した。テーブル名・カラムリスト・値タプルから`INSERT INTO {table} ({columns}) VALUES ({placeholders})`を動的に構築する汎用関数であり、カラムの型自体は本関数には定義がない。本ファイル(`nas_monitor.py`)の`save_to_db`(199〜213行目)は`config.SQLITE_TABLE_SENSOR`（実体`"device_records"`、`config.py`235行目）へ`["timestamp", "device_name", "device_id", "device_type", "contact_state", "battery_level"]`列でINSERTしており、`mount_ok`は独立した列ではなく`contact_state`列に`"mounted"`/`"unmounted"`という文字列として、`percent`（NAS使用率）は`battery_level`列に格納する設計であることが判明した。ただし`MY_HOME_SYSTEM/init_unified_db.py`163〜178行目の`CREATE TABLE IF NOT EXISTS device_records`定義には`battery_level`列が存在しない（`id, timestamp, device_name, device_id, device_type, power_watts, temperature_celsius, humidity_percent, contact_state, movement_state, brightness_state, hub_onoff, cam_onoff, threshold_watts`の13列のみ）ことを確認した。一方`MY_HOME_SYSTEM/old/db_fix.py`14行目に`ALTER TABLE device_records ADD COLUMN battery_level INTEGER;`という一回限りの修正スクリプトが存在し、リポジトリ内の`MY_HOME_SYSTEM/current_schema.sql`16行目（実DBのスキーマスナップショットと見られる）にも`battery_level INTEGER`列が実在することを確認した。したがって`battery_level`列は`init_unified_db.py`の初期スキーマには含まれておらず、本番DBでは別途`db_fix.py`のような一回限りのALTER TABLEで後付けされたものと判断できる。 | 直接ソース確認: `MY_HOME_SYSTEM/core/database.py:67-79`, `MY_HOME_SYSTEM/monitors/nas_monitor.py:199-213`, `MY_HOME_SYSTEM/config.py:235`, `MY_HOME_SYSTEM/init_unified_db.py:161-178`, `MY_HOME_SYSTEM/old/db_fix.py:14`, `MY_HOME_SYSTEM/current_schema.sql:16` |
...
| 設定値の初期値と定義内容 | `MY_HOME_SYSTEM/monitors/nas_monitor.py`を直接確認したところ、本ファイルは`config`の値を`getattr(config, "属性名", デフォルト値)`で参照している(26〜29, 173〜178行目)。対応する`config.py`側の実体を直接確認した: `NAS_IP: str = os.getenv("NAS_IP", "192.168.1.20")`(408行目)、`NAS_CHECK_TIMEOUT: int = 5`(409行目、ハードコード)、`NVR_RECORD_DIR: str = os.path.join(NAS_MOUNT_POINT, "home_system", "nvr_recordings")`(436行目)、`RECORDING_RETENTION_DAYS: int = int(os.getenv("RECORDING_RETENTION_DAYS", "30"))`(442行目)、`DB_BACKUP_RETENTION_DAYS: int = int(os.getenv("DB_BACKUP_RETENTION_DAYS", "30"))`(444行目)、`DB_BACKUPS_DIR: str = os.path.join(NAS_PROJECT_ROOT, "db_backups")`(445行目)、`ASSETS_DIR`は224〜227行目で`ensure_safe_path_with_backoff(os.path.join(NAS_PROJECT_ROOT, "assets"), "assets")`(NAS到達不能時はローカルの`temp_fallback/assets`へフェイルソフト)。一方、`nas_monitor.py`28行目が参照する`getattr(config, "FALLBACK_DIR", "/tmp/temp_fallback")`については、`config.py`全体を検索したが`FALLBACK_DIR`という名前の属性は一切定義されておらず（類似名の`FALLBACK_ROOT`(213行目)のみ存在）、本ファイルの`self.fallback_dir`は常に`getattr`のデフォルト値`"/tmp/temp_fallback"`にフォールバックする設計になっていることが判明した。 | 直接ソース確認: `MY_HOME_SYSTEM/config.py:213, 408-409, 436, 442-445`, `MY_HOME_SYSTEM/monitors/nas_monitor.py:26-29, 173-178`（`FALLBACK_DIR`属性は`config.py`に不在を確認） |
