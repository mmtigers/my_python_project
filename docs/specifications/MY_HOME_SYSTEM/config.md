...
* 根拠: [HEALTH_WATCH_DEADMAN_PING_*定義] (行番号: 667〜668 / 抜粋: "HEALTH_WATCH_DEADMAN_PING_URL: str = os.getenv(\"HEALTH_WATCH_DEADMAN_PING_URL\", \"\")\nHEALTH_WATCH_DEADMAN_PING_TIMEOUT_SEC: int = _get_int_env(\"HEALTH_WATCH_DEADMAN_PING_TIMEOUT_SEC\", 10)")
...
* 「14. Family Quest: YouTubeごほうび券の視聴制限設定」セクション(Issue #488でモジュールdocstring目次の番号が旧21番から14番へ振り直された。見出しは当初「YouTubeごほうび券クールダウン設定」だったが、日次上限の追加に伴い「視聴制限設定」へ改称された)で、Family Questの子ども向けYouTube系ごほうび券について、連続視聴による目の負担を防ぐための制限対象reward_idを`YOUTUBE_REWARD_IDS`(環境変数`YOUTUBE_REWARD_IDS`、カンマ区切りの整数、未設定時は既定値`"10,11,12"`)として定義する。パース方式は「10. TVロック機能設定」の`TV_UNLOCK_QUEST_IDS`と同じ(`isdigit()`を満たす要素のみ`int`化してリスト化、パース例外は`logger.warning`のみで握りつぶす)。`services/quest_service.py`の`InventoryService.use_item`/`get_user_inventory`が参照する。
* 根拠: [Family Quest: YouTubeごほうび券の視聴制限設定セクション] (行番号: 726〜740 / 抜粋: "# 14. Family Quest: YouTubeごほうび券の視聴制限設定", "_youtube_reward_ids_str: str = os.getenv(\"YOUTUBE_REWARD_IDS\", \"10,11,12\")", "YOUTUBE_REWARD_IDS: List[int] = []")
...
* 同セクションに、日次上限を実際に強制し始める日`YOUTUBE_DAILY_LIMIT_ENFORCE_FROM`(環境変数`YOUTUBE_DAILY_LIMIT_ENFORCE_FROM`、`YYYY-MM-DD`形式、既定値`"2026-09-24"`、`datetime.date`型)を追加した。`YOUTUBE_REWARD_COOLDOWN_ENFORCE_FROM`と同じ猶予期間の仕組みだが、コメントによれば施行日は**クールダウンとは別管理**である(判定は`services/quest/locks.py`の`_is_youtube_daily_limit_enforced`)。パース失敗時は`logger.warning`を出したうえで`date(2000, 1, 1)`(=即時強制、安全側)にフォールバックする。捕捉する例外は`ValueError`のみで、コメントによれば「`_date.fromisoformat`が不正な日付文字列に対して送出するのはこれであり、それ以外の例外(実装の誤り)まで握り潰さないため」。すぐ上の`YOUTUBE_REWARD_COOLDOWN_ENFORCE_FROM`が`Exception`を捕捉しているのは先に書かれたコードの名残で、意図的な差ではない旨も同コメントに明記されている。既定値は当初(この機能を追加した日(2026-09-20)の1週間後=)`"2026-09-27"`だったが、2026-09-23の要件確認により翌日の`"2026-09-24"`へ前倒しされた。
* 根拠: [YOUTUBE_DAILY_LIMIT_ENFORCE_FROM定義] (行番号: 797〜813 / 抜粋: "_youtube_daily_limit_enforce_from_str: str = os.getenv(\"YOUTUBE_DAILY_LIMIT_ENFORCE_FROM\", \"2026-09-24\")", "    YOUTUBE_DAILY_LIMIT_ENFORCE_FROM: _date = _date.fromisoformat(_youtube_daily_limit_enforce_from_str)", "except ValueError as e:")
* 同セクションに、日次上限を「使い切ったあとに、追加でプリントをやれば延長できる」ようにするための設定3つを追加した。`YOUTUBE_EXTENSION_QUEST_IDS`(環境変数、カンマ区切りの整数、既定値`"31,307"`、`list[int]`型)は延長の対象になるクエストの`quest_master.quest_id`で、コメントによれば既定値は`quest_data.py`の「プリント」(31/智矢)と「なぞり書きプリント」(307/娘)、いずれも`type='infinite'`(1日に何度でも報告できる)である。`YOUTUBE_EXTENSION_MINUTES_PER_QUEST`(**2026-09-23 要件変更で既定30→既定10**)はプリント1枚あたりの延長分数、`YOUTUBE_EXTENSION_MAX_PER_DAY`(既定2)は1日に延長できる回数の上限で、コメントは回数に上限を設ける理由を「プリントを大量に出せば無制限に見られる状態にしないため」と記している。いずれも空文字・`0`以下にすると延長機能が無効になる。
* 根拠: [YOUTUBE_EXTENSION_*定義] (行番号: 814〜840 / 抜粋: "# 日次上限を「使い切ったあとに、追加でプリントをやれば延長できる」ようにするための設定。", "_youtube_extension_quest_ids_str: str = os.getenv(\"YOUTUBE_EXTENSION_QUEST_IDS\", \"31,307\")", "YOUTUBE_EXTENSION_MINUTES_PER_QUEST: int = _get_int_env(\"YOUTUBE_EXTENSION_MINUTES_PER_QUEST\", 10)", "YOUTUBE_EXTENSION_MAX_PER_DAY: int = _get_int_env(\"YOUTUBE_EXTENSION_MAX_PER_DAY\", 2)")
* **（Issue報告、2026-09-24で追加）** 同セクションに、涼花が平日8:50にYouTube系ごほうび券を使えなかった不具合の修正として2定数を追加した。1つは`YOUTUBE_HOME_CARE_USER_IDS`(`set[str]`型、既定`{"daughter"}`、環境変数なし固定値)で、コメントによれば涼花は今年度自宅保育(登校しない)のため、`is_user_currently_in_free_time`(登校/下校を前提とした`routine_data.py`のルーティンフローの自由時間判定)をこのユーザーIDに適用すると、平日07:50(登校の締切)〜14:00(下校の開始)の間ずっと「自由時間ではない」扱いに固定され券が一律使えなくなっていた(登校前提の設計と登校しない実態のミスマッチ)ため、ここに含まれるユーザーにはそのゲート自体を適用しない(`services/quest/inventory_service.py`の`_is_user_in_youtube_free_time`が判定)。もう1つは`YOUTUBE_NAP_BLOCK_START`/`YOUTUBE_NAP_BLOCK_END`(環境変数`YOUTUBE_NAP_BLOCK_START`/`YOUTUBE_NAP_BLOCK_END`、`"HH:MM"`形式、既定値`"13:30"`/`"15:00"`、`datetime.time`型)で、お昼寝の時間帯を表す。この間は涼花・智矢とも常にYouTube系ごほうび券を使えず、自由時間ゲート・自宅保育の適用除外とは別軸でルーティンフローの状態に関わらず常に適用される。環境変数の読み込みは`os.getenv(name, "").strip() or デフォルト値`という形で、未設定・空文字のいずれも`_get_int_env`と同じくデフォルト値へフォールバックする(**Claude Code Reviewの指摘、PR #855で追加**: 当初`os.getenv(name, default)`のみだったため、`.env.example`の空文字プレースホルダーをそのまま`.env`へコピーするとデフォルトへフォールバックせず`fromisoformat("")`が`ValueError`になり、お昼寝ブロックが意図せず無効化されていた)。それでもパースに失敗する値(空文字・未設定以外の不正な文字列)が渡された場合は`logger.warning`を出したうえで開始・終了とも`_time_of_day(0, 0)`にフォールバックし、範囲が空(常に偽)になって実質無効化される。捕捉する例外は`ValueError`のみ。
* 根拠: [YOUTUBE_HOME_CARE_USER_IDS定義とコメント] (行番号: 854〜862 / 抜粋: "# 涼花は今年度は自宅保育(登校しない)のため、is_user_currently_in_free_time", "YOUTUBE_HOME_CARE_USER_IDS: set[str] = {\"daughter\"}")、[YOUTUBE_NAP_BLOCK_START/END定義] (行番号: 864〜881 / 抜粋: "_youtube_nap_block_start_str: str = os.getenv(\"YOUTUBE_NAP_BLOCK_START\", \"\").strip() or \"13:30\"", "    YOUTUBE_NAP_BLOCK_START: _time_of_day = _time_of_day.fromisoformat(_youtube_nap_block_start_str)", "except ValueError as e:", "    YOUTUBE_NAP_BLOCK_START = _time_of_day(0, 0)")
...
* **（Issue #829で全面改訂）** 「16. ダッシュボード公開設定」セクション（見出しは当初「ダッシュボード(Streamlit)公開設定」だったが、Streamlit版廃止に伴い改称された）を持つ。以前はStreamlit製の「詳細表示」(`dashboard.py`)を`unified_server.py`がリバースプロキシする設計で、そのための4定数（`DASHBOARD_PROXY_ENABLED`/`DASHBOARD_INTERNAL_URL`/`DASHBOARD_BASE_PATH`/`DASHBOARD_PROXY_TIMEOUT_SEC`）を持っていたが、Streamlit版は廃止され、ダッシュボードは`routers/dashboard_router.py`がこのサーバー自身の中でHTMLを直接組み立てて返す(かんたん表示のみ。詳細表示・表示モード切替UIは廃止)ため、外部プロセスへの中継自体が無くなった。これに伴い定数も次の3つに置き換わった。
    ...
    * `ASA_NOTE_URL`（環境変数 `ASA_NOTE_URL`、既定 `"https://go-to-school-one.vercel.app/"`）— ホーム画面のリンクカードから開く外部サービス(あさノート)のURL。ダッシュボード内に機能・表示は持たず、リンクのみを置く方針で追加された。
...
* 根拠: [ダッシュボード公開設定セクション] (行番号: 901 / 抜粋: "# 16. ダッシュボード公開設定")、`DASHBOARD_ENABLED`/`DASHBOARD_BASE_PATH`/`ASA_NOTE_URL`定義 (行番号: 914, 916, 920 / 抜粋: "DASHBOARD_ENABLED: bool = os.getenv(\"DASHBOARD_ENABLED\", \"true\").strip().lower() != \"false\"", "DASHBOARD_BASE_PATH: str = \"/\" + os.getenv(\"DASHBOARD_BASE_PATH\", \"dashboard\").strip().strip(\"/\")", "ASA_NOTE_URL: str = os.getenv(\"ASA_NOTE_URL\", \"https://go-to-school-one.vercel.app/\")")、バイパス禁止の注意書き (行番号: 904〜907 / 抜粋: "# 重要: このパスは `unified_server.py` の `allowed_webhook_paths` とは逆で、")
...
* 根拠: `def _resolve_assets_dir() -> str:` (行番号: 692)、`def __getattr__(name: str) -> str:` (行番号: 706)、`def prewarm_nas_paths() -> None:` (行番号: 733)
...
* `YOUTUBE_REWARD_IDS`も同じパース方式(`isdigit()`を満たす要素のみ`int`化)のため同じ落とし穴を持つ。加えて`TV_UNLOCK_QUEST_IDS`(未設定時は空リスト=機能無効)と異なり、環境変数が未設定の場合は既定値`"10,11,12"`にフォールバックして視聴制限機能が有効な状態になる点に注意(明示的に`YOUTUBE_REWARD_IDS=`(空文字)を設定した場合のみ空リストとなり無効化される)。
* `YOUTUBE_EXTENSION_QUEST_IDS`に指定するのは「何度でも報告できるクエスト」であることが前提である。1日1回しか完了できない`type='daily'`のクエストを指定すると、上限を使い切った後にもう一度やって延長する、という運用自体が成立しない。
* `YOUTUBE_REWARD_IDS`と`YOUTUBE_REWARD_DURATION_MINUTES`は別々の環境変数のため、片方だけ更新すると静かに不整合になる。`YOUTUBE_REWARD_IDS`に券を足して`YOUTUBE_REWARD_DURATION_MINUTES`への追記を忘れると、その券は視聴分数0分として扱われ(クールダウンは休憩ぶんのみ、日次上限にも加算されない)、例外もログも出ない。
...
* 根拠: 退役コメント (行番号: 332 / 抜粋: "# Issue #701 (2026-09-19): 旧 SQLITE_TABLE_AI_REPORT")
...
| 各テーブルの詳細なスキーマ定義 | **（Issue #330でスキーマ定義元が移動）** `init_unified_db.py`から`CREATE TABLE`群は撤去され、スキーマの唯一の定義元は`MY_HOME_SYSTEM/migrations/`(空DBでは`0000_baseline_schema.sql`)になった。`config.SQLITE_TABLE_DAILY_LOGS`(実体は`"daily_logs"`)は`migrations/0000_baseline_schema.sql`59〜65行目の`CREATE TABLE IF NOT EXISTS`文で`id, user_id, category TEXT NOT NULL, detail, timestamp DATETIME NOT NULL`列を持つ。`config.SQLITE_TABLE_SWITCHBOT_LOGS`(`"switchbot_meter_logs"`)は同68〜75行目で`id, device_id, device_name, temperature REAL, humidity REAL, timestamp`列。`config.SQLITE_TABLE_POWER_USAGE`(`"power_usage"`)は同78〜84行目で`id, device_id, device_name, wattage REAL, timestamp`列。`init_unified_db.py`側に残るのは32〜75行目の`validate_schema_integrity(conn)`関数のみで、これが、これら`config.SQLITE_TABLE_*`定数を含む主要テーブル群について`PRAGMA table_info(table)`で必須カラムの存在を検証する仕組みを持つことを確認した。また`core/migrations.py`97行目〜の`apply_pending_migrations(conn)`が`migrations/`配下の`*.sql`ファイルをファイル名昇順で適用し、適用済みバージョンを`schema_migrations`テーブル(同44〜53行目で`CREATE TABLE IF NOT EXISTS`定義)で追跡する。`init_unified_db.init_db()`(77〜116行目)はWALモード設定→`apply_pending_migrations(cur.connection)`(103行目)→`validate_schema_integrity`(111行目)を行うだけの薄いラッパーであり、もはや独自のスキーマ定義を持たない(同関数のdocstring 85行目が「以前ここにあったCREATE TABLE群は migrations/0000_baseline_schema.sql へ移設済み」と明記)。 | 直接ソース確認: `MY_HOME_SYSTEM/init_unified_db.py:32-75, 77-116`, `MY_HOME_SYSTEM/core/migrations.py:44-53, 97`, `MY_HOME_SYSTEM/migrations/0000_baseline_schema.sql:59-84` |
...
| `TV_UNLOCK_QUEST_IDS` が参照するクエストの実体 | **（Issue #550の分割・その後の移設を反映）** 判定側は`MY_HOME_SYSTEM/services/quest/quest_service.py`の`_process_approve_quest_locked`へ移り、447行目の`if quest and quest['quest_id'] in config.TV_UNLOCK_QUEST_IDS and config.TV_PLUG_DEVICE_ID:`＋448行目の`if user['role'] == ROLE_CHILD:`で`tv_unlock_quest_id`を立て、DBカーソルのブロックを抜けた454行目で`switchbot_service.trigger_tv_unlock(f"quest_id={tv_unlock_quest_id}")`を呼ぶ(**発火点はクエスト「完了」ではなく親の「承認」時**)。解錠処理の実体は`MY_HOME_SYSTEM/services/switchbot_service.py`の`trigger_tv_unlock(context: str) -> None`(94〜125行目。`routine_service`からも共用するため`QuestService._trigger_tv_unlock`から切り出されたもの)で、`threading.Thread(target=unlock_task, daemon=True)`(124行目)でバックグラウンド実行し`send_device_command(config.TV_PLUG_DEVICE_ID, "turnOn")`(108行目)を呼ぶ。レスポンスの`statusCode`が100なら成功ログ、それ以外・例外時は`config.LINE_PARENTS_GROUP_ID`が設定されていれば`notification_service.send_push`で「⚠️ テレビの電源ON（自動ロック解除）に失敗しました。…」という親グループ向けFail-Soft通知を送る(113〜121行目)。なお`trigger_tv_unlock`自身は`TV_PLUG_DEVICE_ID`未設定のガードを持たない(docstring 101〜102行目が呼び出し元の責務と明記)。対応する具体的なクエストID・クエスト定義自体(`quest_master`テーブルの実データ)は依然としてこれらのファイルからは確認できない。 | 直接ソース確認: `MY_HOME_SYSTEM/services/quest/quest_service.py:447-454`, `MY_HOME_SYSTEM/services/switchbot_service.py:94-125`（参考: [quest_quest_service.md](./quest_quest_service.md)・[switchbot_service.md](./switchbot_service.md)） |
