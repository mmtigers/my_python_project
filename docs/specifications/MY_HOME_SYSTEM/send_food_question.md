...
| --- | --- | --- |
| DBの正確なスキーマ | `config.py`242行目で`SQLITE_TABLE_FOOD: str = "food_records"`と定義されており、`send_food_question.py`57〜60行目がこの`config.SQLITE_TABLE_FOOD`(=`food_records`)テーブルへ`SELECT menu_category, COUNT(*) as cnt FROM food_records ... GROUP BY menu_category`を発行していることを確認した。`current_schema.sql`94〜99行目の実際のDDLダンプにより、`food_records`テーブルは`id INTEGER PRIMARY KEY AUTOINCREMENT, date TEXT(YYYY-MM-DD), menu TEXT, created_at TEXT, menu_category TEXT, meal_date TEXT, meal_time_category TEXT, user_id TEXT, user_name TEXT, timestamp DATETIME`の10カラムを持つことを確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/config.py:242`, `MY_HOME_SYSTEM/current_schema.sql:94-99`, `MY_HOME_SYSTEM/old/send_food_question.py:57-60` |
| `MENU_OPTIONS`のデータ構造 | `config.py`272〜276行目に`MENU_OPTIONS: Dict[str, List[str]] = {"自炊": ["カレーライス", "豚しゃぶ", "焼き魚", "うどん", "味噌汁とご飯", "野菜炒め", "オムライス"], "外食": ["マクドナルド", "魚べえ", "サイゼリヤ", "丸亀製麺"], "その他": ["スーパーの惣菜", "コンビニ", "冷凍食品", "カップ麺"]}`が直接定義されていることを確認した（環境変数依存ではなくハードコードされたPython辞書）。`send_food_question.py`114行目の`getattr(config, "MENU_OPTIONS", {}).get(cat, [])`はこの3カテゴリ（`自炊`/`外食`/`その他`）いずれかの`cat`をキーにデフォルトメニュー候補を取得する。 | 直接ソース確認: `MY_HOME_SYSTEM/config.py:272-276`, `MY_HOME_SYSTEM/old/send_food_question.py:114` |
