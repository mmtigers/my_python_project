...
| `home_system.service` | OSのSystemdサービス。具体的な動作や内容が不明。 | `journalctl -u home_system.service` (行番号: 566 / 抜粋: "home_system.service") |
...
* 根拠: `def _classify_power_device_type(row) -> str:` (行番号: 247〜261)、`category = row.get("device_category")` (行番号: 255)、`load_sensor_data`内での呼び出し (行番号: 298〜306 / 抜粋: "df_power[\"device_type\"] = df_power.apply(_classify_power_device_type, axis=1)\n        df_power = df_power.drop(columns=[\"device_category\"])")


* **引数/リクエスト**: `limit` (`int`, デフォルト `5000`): 取得件数の上限。
* 根拠: `limit: int = 5000` (行番号: 264 / 抜粋: "def load_sensor_data(limit: int = 5000)")
...
* **副作用**: データベースの読み取り操作。
* 根拠: `load_data_from_db` を複数回呼び出し。 (行番号: 277 / 抜粋: "df_legacy = load_data_from_db(query_legacy)")


* **エラーハンドリング**: 内部で呼び出される `load_data_from_db` に依存。
* 根拠: 該当関数内に独自の `try-except` なし (行番号: 264 / 抜粋: "def load_sensor_data(")
...
* 根拠: `def calculate_monthly_cost_cumulative() -> int:` (行番号: 348〜351)、`return int(df["kwh"].sum() * ELECTRICITY_YEN_PER_KWH)` (行番号: 433)、`return moment.replace(day=1, hour=0, minute=0, second=0, microsecond=0)` (行番号: 376)、device_categoryによる絞り込み (行番号: 399〜403 / 抜粋: "WHERE timestamp >= '{start_of_month}' AND timestamp <= '{end_iso}'\n              AND (\n                device_category = 'smart_meter'\n                OR (device_category IS NULL AND device_name LIKE '%Remo%')\n              )")


* **引数/リクエスト**: なし
* 根拠: `def calculate_monthly_cost_cumulative() -> int:` (行番号: 348 / 抜粋: "def calculate_monthly_cost_cumulative()")
...
* 根拠: `def calculate_last_month_cost_same_point() -> int:` (行番号: 354 / 抜粋: "def calculate_last_month_cost_same_point() -> int:")

* **引数/リクエスト**: なし
* 根拠: `def calculate_last_month_cost_same_point() -> int:` (行番号: 354 / 抜粋: "def calculate_last_month_cost_same_point() -> int:")

* **戻り値/レスポンス**: `int`（先月の同じ時点までの電気代概算）
* 根拠: `return _calculate_cost_between(_start_of_month(same_point), same_point)` (行番号: 368 / 抜粋: "return _calculate_cost_between(_start_of_month(same_point), same_point)")
...
* 根拠: `day = min(now.day, end_of_last_month.day)` (行番号: 364 / 抜粋: "day = min(now.day, end_of_last_month.day)")
...
* 根拠: `def _calculate_cost_between(start: datetime, end: datetime) -> int:` (行番号: 379 / 抜粋: "def _calculate_cost_between(start: datetime, end: datetime) -> int:"), `ELECTRICITY_YEN_PER_KWH = 31` (行番号: 345 / 抜粋: "ELECTRICITY_YEN_PER_KWH = 31")

* **引数/リクエスト**: `_start_of_month(moment: datetime)`, `_calculate_cost_between(start: datetime, end: datetime)`
* 根拠: `def _start_of_month(moment: datetime) -> datetime:` (行番号: 371 / 抜粋: "def _start_of_month(moment: datetime) -> datetime:")
...
* 根拠: `except Exception as e:` (行番号: 435 / 抜粋: "logger.error(f\"Cost Calc Error: {e}\")")
...
* 根拠: `load_weather_history` (行番号: 291〜308 / 抜粋: "FROM weather_history")、JST明示化 (行番号: 442 / 抜粋: "start_date = (datetime.now(pytz.timezone(\"Asia/Tokyo\")) - timedelta(days=days)).strftime(\"%Y-%m-%d\")")


* **引数/リクエスト**: `days` (`int`, デフォルト `40`): 遡る日数。`location` (`str`, デフォルト `"伊丹"`): 取得対象の場所。
* 根拠: `days: int = 40, location: str = "伊丹"` (行番号: 438 / 抜粋: "days: int = 40, location: str = "伊丹"")
...
* 根拠: `load_yearly_temperature_stats` (行番号: 510 / 抜粋: "df_merged = pd.merge(df_weather, df_sensor, on="date"")


* **引数/リクエスト**: `year` (`int`): 対象年。`location` (`str`, デフォルト `"伊丹"`): 対象場所。
* 根拠: `year: int, location: str = "伊丹"` (行番号: 458 / 抜粋: "year: int, location: str = "伊丹"")
...
* **副作用**: データベースの読み取り操作。
* 根拠: `pd.read_sql_query` (行番号: 470 / 抜粋: "df_weather = pd.read_sql_query(q_weather, conn)")
...
* 根拠: `get_disk_usage` (行番号: 526 / 抜粋: "total, used, free = shutil.disk_usage("/")")


* **引数/リクエスト**: なし
* 根拠: `def get_disk_usage() -> Optional[Dict[str, float]]:` (行番号: 523 / 抜粋: "def get_disk_usage()")
...
* 根拠: `shutil.disk_usage("/")` (行番号: 526 / 抜粋: "shutil.disk_usage("/")")
...
* **引数/リクエスト**: なし
* 根拠: `def get_memory_usage() -> Optional[Dict[str, float]]:` (行番号: 542 / 抜粋: "def get_memory_usage()")
...
* 根拠: `lines: int = 50, priority: Optional[str] = None, target_date: Optional[date] = None` (行番号: 563 / 抜粋: "lines: int = 50, priority: Optional[str] = None")
...
* 根拠: `subprocess.run` (行番号: 576 / 抜粋: "subprocess.run(cmd")


* **エラーハンドリング**: 例外発生時はエラーメッセージを文字列として返す。
* 根拠: `except Exception as e:` (行番号: 579 / 抜粋: "return f"ログ取得エラー: {e}"")
