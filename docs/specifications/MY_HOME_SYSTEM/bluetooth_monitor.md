...
| --- | --- | --- |
| `config.ENABLE_BLUETOOTH` / `config.LINE_USER_ID`の実際の設定値 | `config.py`を直接確認した。172行目で`ENABLE_BLUETOOTH: bool = False`と、環境変数を経由しない固定値`False`で定義されている。本ファイル57行目は`if not getattr(config, "ENABLE_BLUETOOTH", True): ... return`（未定義時はTrueとみなす安全策）という実装だが、実際には`config.py`側で明示的に`False`が定義されているため、`run()`メソッドは呼び出されると即座に「🚫 Bluetooth Monitor is disabled by config. Exiting.」をログ出力して`while True`ループへは入らずに終了する仕様であることが判明した。`config.LINE_USER_ID`は185行目で`Optional[str] = os.getenv("LINE_USER_ID")`と環境変数由来であり、実値は`.env`が`.gitignore`13行目により追跡対象外のため確認できなかった。 | 直接ソース確認: `MY_HOME_SYSTEM/config.py:172, 185`, `MY_HOME_SYSTEM/monitors/old/bluetooth_monitor.py:57-59`（`.env`は`.gitignore:13`により追跡対象外） |
| `send_push`の内部実装（Discord通知の実際の送信方法） | `services/notification_service.py`を直接確認した。`send_push(user_id, messages, image_data=None, target="both", channel="notify", filename="snapshot.jpg")`(116〜140行目)は`target`に`"discord"`が含まれる場合、`_send_discord_webhook(messages, image_data, channel, filename)`(30〜71行目)を呼び出す。同関数は`channel`引数(`"error"`→`config.DISCORD_WEBHOOK_ERROR`、`"report"`→`config.DISCORD_WEBHOOK_REPORT`、既定`"notify"`→`config.DISCORD_WEBHOOK_NOTIFY`または`DISCORD_WEBHOOK_URL`)に応じたWebhook URLへ、`image_data`があれば`requests.post(url, files={'file': (filename, image_data)}, data={'content': text_content}, timeout=60)`、なければ`requests.post(url, json={"content": text_content}, timeout=10)`で送信し、ステータスコードが200/204以外またはリクエスト例外時はエラーログを出力し`False`を返す。本ファイル73〜77行目は`target="discord"`のみを指定して呼び出しており、`channel`は既定値`"notify"`が使われるため`config.DISCORD_WEBHOOK_NOTIFY`(未設定時は`DISCORD_WEBHOOK_URL`)へJSON形式でPOSTされる。 | 直接ソース確認: `MY_HOME_SYSTEM/services/notification_service.py:30-71, 116-140`, `MY_HOME_SYSTEM/monitors/old/bluetooth_monitor.py:73-77` |
| 本スクリプトの起動・プロセス管理方法 | 起動スクリプト`start_all.sh`を直接確認した。27〜31行目のクリーンアップフェーズに`pkill -f bluetooth_monitor.py`(29行目)という記述があり、起動処理の前に同名プロセスを停止させようとする意図が読み取れるが、本スクリプトを新規に起動する`python3 bluetooth_monitor.py`のような記述は`start_all.sh`（全75行）にも`scheduler_boot.py`にも見当たらなかった（実際に起動されているのは`unified_server.py`とStreamlitダッシュボードのみ、64〜73行目）。一方、`tools/keep_alive_speaker.sh`・`tools/keep_alive_anker.sh`・`tools/connect_speaker.sh`という別スクリプト群がログファイル`logs/bluetooth_monitor.log`を参照しており、これらが本ファイルの想定するログ出力先と関連することは確認できたが、これらのシェルスクリプト自体が本ファイルを起動しているかどうかまでは確認できなかった。 | 直接ソース確認: `MY_HOME_SYSTEM/start_all.sh:27-31, 64-73`, `MY_HOME_SYSTEM/tools/keep_alive_speaker.sh:7`, `MY_HOME_SYSTEM/tools/keep_alive_anker.sh:8`, `MY_HOME_SYSTEM/tools/connect_speaker.sh:9`（`scheduler_boot.py`は`grep`による該当箇所なしを確認） |
| `monitors/old/`ディレクトリの位置づけ（現行版との関係） | `monitors/`ディレクトリを直接確認したところ、`bluetooth_monitor.py`という同名ファイルは`monitors/old/`配下にのみ存在し、`monitors/`直下（現行版が置かれる場所）には存在しなかった(`monitors/`直下の一覧: `camera_monitor.py`, `daily_timelapse_job.py`, `memory_monitor.py`, `nas_monitor.py`, `nature_remo_monitor.py`, `scheduled_timelapse.py`, `server_watchdog.py`, `smart_timelapse_generator.py`, `switchbot_power_monitor.py`, `timelapse_generator.py`, `timelapse_runner.py`, `tv_lock_monitor.py`のみ)。前述の通り現在の起動パイプラインからも呼び出されておらず、`config.ENABLE_BLUETOOTH = False`により仮に起動されても即終了する仕様であることも踏まえると、実質的に無効化された旧版スクリプトである可能性が高い。ただし後継となる非`old`版が別途存在しないことも確認できたため、Bluetooth関連の現行の代替実装がリポジトリ内に存在するとまでは断定できない。 | 直接ソース確認: `MY_HOME_SYSTEM/monitors/`ディレクトリ一覧, `MY_HOME_SYSTEM/monitors/old/`ディレクトリ一覧 |
