...
* 根拠: `def build_status_cards(` (行番号: 492 / 抜粋: "def build_status_cards(")
...
* 判定関数（`get_takasago_status` 等6つ）はいずれも副作用を持たない純粋関数で、DataFrameや取得済みの値を受け取って `(表示文字列, テーマ名)` を返す。取得は呼び出し側の責務にしてある（Streamlit側はキャッシュ付きローダから、サーバー側は `collect_status_cards` から）。
* 根拠: `def get_takasago_status(df_sensor: pd.DataFrame, now: datetime) -> tuple[str, str]:` (行番号: 270 / 抜粋: "def get_takasago_status(df_sensor: pd.DataFrame, now: datetime) -> tuple[str, str]:")
* サーバー側用に、DB読み取りと各種取得をまとめて行う `collect_status_cards` と、TTL 60秒の自前キャッシュ（`unified_server` には `st.cache_data` が無いため）を持つ。
* 根拠: `def collect_status_cards(now: datetime | None = None) -> tuple[list[StatusCard], datetime]:` (行番号: 572 / 抜粋: "def collect_status_cards(now: datetime | None = None) -> tuple[list[StatusCard], datetime]:")
* 軽量ページのHTML全体（`<head>`・カードのグリッド・下部の導線リンク）を組み立てる `render_mobile_status_page_html` を持つ。配信層（ルーター）から独立させるため、リンク先のパス類は引数で受け取る。
* 根拠: `def render_mobile_status_page_html(` (行番号: 772 / 抜粋: "def render_mobile_status_page_html(")
...
* 根拠: `DASHBOARD_TABS: tuple[tuple[str, str], ...] = (` (行番号: 119 / 抜粋: "DASHBOARD_TABS: tuple[tuple[str, str], ...] = (")
* 赤・黄のカードだけを拾う要約（`summarize_alerts`）を持つ。軽量ページと Streamlit 側の両方がこれを呼ぶため、「気になること」の条件が2画面で食い違わない。
* 根拠: `def summarize_alerts(cards) -> list[StatusCard]:` (行番号: 210 / 抜粋: "def summarize_alerts(cards) -> list[StatusCard]:")
...
* 根拠: `def describe_takasago(df_sensor: pd.DataFrame, now: datetime) -> str | None:` (行番号: 439 / 抜粋: "def describe_takasago(df_sensor: pd.DataFrame, now: datetime) -> str | None:")
...
* 根拠: `class StatusCard(NamedTuple):` (行番号: 128 / 抜粋: "class StatusCard(NamedTuple):")

* **引数/リクエスト**: `title` (str), `value` (str), `theme` (str), `value_is_html` (bool, 既定 False), `tab` (str | None, 既定 None), `sub` (str | None, 既定 None)
* 根拠: `class StatusCard(NamedTuple):` (行番号: 128〜144 / 抜粋: "class StatusCard(NamedTuple):")
...
* 根拠: `def render_status_card_html(title: str, value: str, theme: str, *, value_is_html: bool = False) -> str:` (行番号: 147〜191 / 抜粋: "def render_status_card_html(title: str, value: str, theme: str, *, value_is_html: bool = False) -> str:")

* **引数/リクエスト**: `title` (str), `value` (str), `theme` (str), キーワード専用 `value_is_html` (bool, 既定 False), `href` (str | None, 既定 None), `sub` (str | None, 既定 None)
* 根拠: `def render_status_card_html(title: str, value: str, theme: str, *, value_is_html: bool = False) -> str:` (行番号: 147 / 抜粋: "def render_status_card_html(...)")
...
* **副作用**: なし（文字列を返すのみ）
* 根拠: `safe_title = html.escape(title)` (行番号: 172 / 抜粋: "safe_title = html.escape(title)")

* **エラーハンドリング**: なし
* 根拠: `safe_value = value if value_is_html else html.escape(value)` (行番号: 173 / 抜粋: "safe_value = value if value_is_html else html.escape(value)")


### `render_status_grid_html`

* **役割**: 複数のカードを `.status-grid` の1ブロックにまとめたHTMLを返す。`dashboard_path` を渡すと、`tab` を持つカードがそのタブへのリンクになる。
* 根拠: `def render_status_grid_html(cards, *, dashboard_path: str | None = None) -> str:` (行番号: 233〜250 / 抜粋: "def render_status_grid_html(cards, *, dashboard_path: str | None = None) -> str:")

* **引数/リクエスト**: `cards` (`StatusCard` のイテラブル), キーワード専用 `dashboard_path` (str | None, 既定 None)
* 根拠: `def render_status_grid_html(cards, *, dashboard_path: str | None = None) -> str:` (行番号: 233 / 抜粋: "def render_status_grid_html(cards, *, dashboard_path: str | None = None) -> str:")
...
* **副作用**: なし
* 根拠: `cards_html = "".join(` (行番号: 239 / 抜粋: "cards_html = \"\".join(")
...
* **引数/リクエスト**: なし（定数）
* 根拠: `DASHBOARD_TAB_KEYS: tuple[str, ...] = tuple(key for key, _ in DASHBOARD_TABS)` (行番号: 125 / 抜粋: "DASHBOARD_TAB_KEYS: tuple[str, ...] = tuple(key for key, _ in DASHBOARD_TABS)")
...
* 根拠: `def card_detail_href(card: StatusCard, dashboard_path: str) -> str | None:` (行番号: 194 / 抜粋: "def card_detail_href(card: StatusCard, dashboard_path: str) -> str | None:")

* **引数/リクエスト**: `card` (`StatusCard`), `dashboard_path` (str)
* 根拠: `def card_detail_href(card: StatusCard, dashboard_path: str) -> str | None:` (行番号: 194 / 抜粋: "def card_detail_href(card: StatusCard, dashboard_path: str) -> str | None:")

* **戻り値/レスポンス**: URL文字列、または `None`
* 根拠: `return f"{dashboard_path}?tab={card.tab}"` (行番号: 203 / 抜粋: "return f\"{dashboard_path}?tab={card.tab}\"")

* **副作用**: なし
* 根拠: `def card_detail_href(card: StatusCard, dashboard_path: str) -> str | None:` (行番号: 194 / 抜粋: "def card_detail_href(card: StatusCard, dashboard_path: str) -> str | None:")

* **エラーハンドリング**: なし
* 根拠: `if card.tab is None:` (行番号: 201 / 抜粋: "if card.tab is None:")
...
* 根拠: `def render_alerts_html(cards, *, dashboard_path: str | None = None) -> str:` (行番号: 219 / 抜粋: "def render_alerts_html(cards, *, dashboard_path: str | None = None) -> str:")
...
* **副作用**: なし
* 根拠: `ALERT_THEMES: tuple[str, ...] = ("theme-red", "theme-yellow")` (行番号: 207 / 抜粋: "ALERT_THEMES: tuple[str, ...] = (\"theme-red\", \"theme-yellow\")")

* **エラーハンドリング**: なし。要約行に出すタイトルは `html.escape` を通す。
* 根拠: `label = html.escape(card.title)` (行番号: 227 / 抜粋: "label = html.escape(card.title)")


### 判定関数（`get_takasago_status` / `get_itami_status` / `get_server_status` / `get_nas_status_simple` / `get_car_status` / `get_rice_status`）
...
* 根拠: `def get_takasago_status(df_sensor: pd.DataFrame, now: datetime) -> tuple[str, str]:` (行番号: 270 / 抜粋: "def get_takasago_status(df_sensor: pd.DataFrame, now: datetime) -> tuple[str, str]:"), `def get_itami_status(df_sensor: pd.DataFrame, now: datetime) -> tuple[str, str]:` (行番号: 329 / 抜粋: "def get_itami_status(df_sensor: pd.DataFrame, now: datetime) -> tuple[str, str]:"), `def get_server_status(memory: dict[str, float] | None) -> tuple[str, str]:` (行番号: 358 / 抜粋: "def get_server_status(memory: dict[str, float] | None) -> tuple[str, str]:"), `def get_nas_status_simple(nas_data: pd.Series | None) -> tuple[str, str]:` (行番号: 364 / 抜粋: "def get_nas_status_simple(nas_data: pd.Series | None) -> tuple[str, str]:"), `def get_car_status(df_car: pd.DataFrame) -> tuple[str, str]:` (行番号: 376 / 抜粋: "def get_car_status(df_car: pd.DataFrame) -> tuple[str, str]:"), `def get_rice_status(df_sensor: pd.DataFrame, now: datetime) -> tuple[str, str]:` (行番号: 400 / 抜粋: "def get_rice_status(df_sensor: pd.DataFrame, now: datetime) -> tuple[str, str]:")

* **引数/リクエスト**: DataFrame（センサー・車）、`now`（`datetime`）、取得済みの辞書（メモリ使用率）、`pd.Series | None`（NAS）
* 根拠: `def get_server_status(memory: dict[str, float] | None) -> tuple[str, str]:` (行番号: 358 / 抜粋: "def get_server_status(memory: dict[str, float] | None) -> tuple[str, str]:")
...
* 根拠: `if df_sensor.empty or "location" not in df_sensor.columns or "contact_state" not in df_sensor.columns:` (行番号: 263 / 抜粋: "if df_sensor.empty or \"location\" not in df_sensor.columns"), `except KeyError:` (行番号: 247 / 抜粋: "except KeyError:")


### `build_status_cards`

* **役割**: 渡された材料から、画面に並べる順で7枚の `StatusCard` を組み立てる（取得は行わない）。各カードには詳細が載っているタブ（`tab`）と、値の下に出す補足（`sub`、`describe_*` の戻り値）を併せて持たせる。
* 根拠: `def build_status_cards(` (行番号: 492〜531 / 抜粋: "def build_status_cards(")

* **引数/リクエスト**: `now`, `df_sensor`, `df_car`, `nas_data`, `memory`, `monthly_cost`、および補足表示にだけ使う省略可能な `last_month_cost` (int | None, 既定 None), `disk` (dict | None, 既定 None)。後者2つを渡さなくても補足が出ないだけで、カードの値と色は変わらない。
* 根拠: `def build_status_cards(` (行番号: 492〜531 / 抜粋: "def build_status_cards(")
...
* 根拠: `taka_val, taka_theme = get_takasago_status(df_sensor, now)` (行番号: 507 / 抜粋: "taka_val, taka_theme = get_takasago_status(df_sensor, now)")
...
* 根拠: `def _cached(key: str, loader: Callable[[], Any]) -> Any:` (行番号: 543〜563 / 抜粋: "def _cached(key: str, loader: Callable[[], Any]) -> Any:"), `def clear_status_cache() -> None:` (行番号: 566 / 抜粋: "def clear_status_cache() -> None:")

* **引数/リクエスト**: `key` (str), `loader` (引数なしの呼び出し可能オブジェクト)
* 根拠: `def _cached(key: str, loader: Callable[[], Any]) -> Any:` (行番号: 543 / 抜粋: "def _cached(key: str, loader: Callable[[], Any]) -> Any:")
...
* 根拠: `_cache[key] = (time.monotonic(), value)` (行番号: 562 / 抜粋: "_cache[key] = (time.monotonic(), value)")

* **エラーハンドリング**: `except Exception` で捕捉し、警告ログを出して `None` を返す（`# noqa: BLE001` 付きで意図を明示）。
* 根拠: `except Exception as e:  # noqa: BLE001 (1枚のカードの取得失敗でページ全体を落とさない)` (行番号: 557 / 抜粋: "except Exception as e:")


### `collect_status_cards`

* **役割**: DB読み取り（センサー・車・NAS）、`psutil`相当のメモリ使用率、今月の電気代をすべて `_cached` 経由で集め、`build_status_cards` に渡して `(カード一覧, 取得時刻)` を返す。取得に失敗した材料は空DataFrame・`None`・`0` で補う。
* 根拠: `def collect_status_cards(now: datetime | None = None) -> tuple[list[StatusCard], datetime]:` (行番号: 572〜599 / 抜粋: "def collect_status_cards(now: datetime | None = None) -> tuple[list[StatusCard], datetime]:")

* **引数/リクエスト**: `now` (`datetime | None`。省略時は `core.utils.get_now_jst()`)
* 根拠: `now = now or get_now_jst()` (行番号: 578 / 抜粋: "now = now or get_now_jst()")

* **戻り値/レスポンス**: `(list[StatusCard], datetime)`
* 根拠: `return cards, now` (行番号: 599 / 抜粋: "return cards, now")
...
* 根拠: `df_sensor if df_sensor is not None else empty,` (行番号: 591 / 抜粋: "df_sensor if df_sensor is not None else empty,")


### 補足表示（`describe_takasago` / `describe_itami` / `describe_car` / `describe_rice` / `describe_cost` / `describe_server` / `describe_nas`）
...
* 根拠: `def describe_takasago(df_sensor: pd.DataFrame, now: datetime) -> str | None:` (行番号: 439 / 抜粋: "def describe_takasago(df_sensor: pd.DataFrame, now: datetime) -> str | None:"), `def _format_moment(moment, now: datetime) -> str | None:` (行番号: 419 / 抜粋: "def _format_moment(moment, now: datetime) -> str | None:")

* **引数/リクエスト**: 判定関数と同じ材料（DataFrame・取得済みの辞書・`now`）
* 根拠: `def describe_cost(monthly_cost: int, last_month_cost: int | None) -> str | None:` (行番号: 465 / 抜粋: "def describe_cost(monthly_cost: int, last_month_cost: int | None) -> str | None:")
...
* **副作用**: なし（いずれもデータ取得を行わない）
* 根拠: `def describe_rice(df_sensor: pd.DataFrame, now: datetime) -> str | None:` (行番号: 460 / 抜粋: "def describe_rice(df_sensor: pd.DataFrame, now: datetime) -> str | None:")

* **エラーハンドリング**: 空DataFrame・列の欠落・`NaT`・`KeyError` はいずれも `None` を返す。
* 根拠: `def _latest_timestamp(df: pd.DataFrame):` (行番号: 432 / 抜粋: "def _latest_timestamp(df: pd.DataFrame):")
...
* 根拠: `MOBILE_PAGE_TITLE = "おうちの様子"` (行番号: 610 / 抜粋: "MOBILE_PAGE_TITLE = \"おうちの様子\""), `MOBILE_PAGE_REFRESH_SEC = STATUS_CACHE_TTL_SEC` (行番号: 613 / 抜粋: "MOBILE_PAGE_REFRESH_SEC = STATUS_CACHE_TTL_SEC")
...
* **副作用**: なし
* 根拠: (行番号: 615 / 抜粋: "_MOBILE_PAGE_BASE_CSS = \"\"\"")
...
* 根拠: `def render_status_section_html(` (行番号: 751 / 抜粋: "def render_status_section_html(")

* **引数/リクエスト**: `cards`, `fetched_at` (`datetime`)、キーワード専用 `dashboard_path` (str | None, 既定 None), `refresh_sec` (int, 既定 `MOBILE_PAGE_REFRESH_SEC`)
* 根拠: `def render_status_section_html(` (行番号: 751〜769 / 抜粋: "dashboard_path: str | None = None,")

* **戻り値/レスポンス**: `<div id="status">…</div>` の文字列（ページ全体ではない断片）
* 根拠: `f'<div id="{STATUS_SECTION_ID}">'` (行番号: 763 / 抜粋: "f'<div id=\"{STATUS_SECTION_ID}\">'")

* **副作用**: なし（文字列を返すのみ）
* 根拠: `STATUS_SECTION_ID = "status"` (行番号: 748 / 抜粋: "STATUS_SECTION_ID = \"status\"")
...
* 根拠: `def render_mobile_status_page_html(` (行番号: 772〜832 / 抜粋: "def render_mobile_status_page_html(")
...
* 根拠: `return (` (行番号: 808 / 抜粋: "\"<!DOCTYPE html>\"")
...
* 根拠: `f'<link rel="manifest" href="{html.escape(manifest_path)}" crossorigin="use-credentials">'` (行番号: 816 / 抜粋: "html.escape(manifest_path)")
...
| 優先度 | ファイル名(推測可) | 理由 | 根拠 |
| --- | --- | --- | --- |
| 高 | `routers/dashboard_router.py` | 軽量ページの配信（パス・マニフェスト・同期エンドポイントにしている理由）を把握するため。 | `def render_mobile_status_page_html(` (行番号: 772 / 抜粋: "def render_mobile_status_page_html(") |
...
* 根拠: `def _takasago_activity(df_sensor: pd.DataFrame) -> pd.DataFrame:` (行番号: 257 / 抜粋: "def _takasago_activity(df_sensor: pd.DataFrame) -> pd.DataFrame:")
...
* 根拠: `_MOBILE_PAGE_DARK_CSS = """` (行番号: 729 / 抜粋: "_MOBILE_PAGE_DARK_CSS = \"\"\"")
