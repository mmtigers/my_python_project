...
| --- | --- | --- |
| `config`の各変数の型と値 | `send_ai_report.py`が参照する`config`変数を`config.py`で直接確認した。`GEMINI_API_KEY`(203行目)は`Optional[str]`で環境変数`GEMINI_API_KEY`から取得。`LINE_USER_ID`(185行目)は`Optional[str]`で環境変数`LINE_USER_ID`から取得。`MONITOR_DEVICES`(298行目)は`List[Dict[str, Any]]`でデフォルト空リスト、`devices.json`が存在すれば`DeviceConfig`モデルでパースされた辞書のリストに置き換わる(307行目)。テーブル名定数は全て`str`型のリテラルで、`SQLITE_TABLE_SENSOR`(235行目)=`"device_records"`、`SQLITE_TABLE_CAR`(244行目)=`"car_records"`、`SQLITE_TABLE_CHILD`(245行目)=`"child_health_records"`、`SQLITE_TABLE_AI_REPORT`(247行目)=`"ai_report_records"`であることを確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/config.py:185, 203, 235, 244-245, 247, 298, 307` |
| 各種DBテーブルのスキーマ構造 | `current_schema.sql`の実際のDDLダンプを直接確認した。`device_records`(1〜13行目以降)は`id, timestamp, device_name, device_id, device_type, power_watts, temperature_celsius, humidity_percent, contact_state, movement_state, brightness_state, hub_onoff`等の列を持つ。`car_records`(42〜47行目)は`id, action, rule_name, timestamp, score`。`child_health_records`(48〜55行目)は`id, user_id, user_name, child_name, condition, timestamp`。`ai_report_records`(65〜69行目)は`id, message, timestamp`のみ。また`send_ai_report.py`186〜199行目のクエスト実績取得クエリ(`quest_status s JOIN quest_tasks t ON s.task_id = t.id JOIN quest_users u ON t.target_user_id = u.rowid`)で参照される`quest_tasks`(211〜217行目、`id, title, points, target_user_id`で`quest_users(id)`への外部キー)と`quest_status`(218〜224行目、`id, task_id, date, is_completed`で`quest_tasks(id)`への外部キー)の実体も確認したが、このクエリは`u.rowid`（SQLiteの暗黙列）で`quest_users`と結合しており、`quest_tasks.target_user_id`の外部キー定義先である`quest_users(id)`列（`user_id`ではない）とは異なる結合方法になっている。`quest_users`テーブル自体には`id`という名前のカラムは存在せず主キーは`user_id TEXT`であるため（`recover_mom.md`等で確認済み）、このJOINは現行の`quest_users`スキーマとは整合しない可能性が高く、実際のコードも`except sqlite3.OperationalError`で「Schema mismatch?」という警告を出しつつ空リストにフォールバックする設計になっている(197〜199行目)ことを確認した。 | 直接ソース確認: `MY_HOME_SYSTEM/current_schema.sql:1-13, 42-47, 48-55, 65-69, 211-224`, `MY_HOME_SYSTEM/old/send_ai_report.py:186-199` |
