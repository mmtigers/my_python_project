...
* 根拠: `def resolve_target_bluetooth_mac():` 〜 `return getattr(config, "SPEAKER_BLUETOOTH_MAC", None)` (行番号: 34〜44 / 抜粋: "if not getattr(config, "ENABLE_BLUETOOTH", False):\n        return None\n    return getattr(config, "SPEAKER_BLUETOOTH_MAC", None)")


* **引数/リクエスト**: なし
* 根拠: (行番号: 34 / 抜粋: "def resolve_target_bluetooth_mac():")
...
* 根拠: `@dataclass\nclass CheckResult:` (行番号: 58〜61 / 抜粋: "class CheckResult:\n    name: str\n    status: str\n    message: str")
...
* **副作用**: なし
* 根拠: (行番号: 58〜61 / 抜粋: "class CheckResult:")
...
* 根拠: `class PostBootHealthCheck:` (行番号: 63〜522 / 抜粋: "class PostBootHealthCheck:")


* **引数/リクエスト**: なし（コンストラクタは引数なし）
* 根拠: `def __init__(self):` (行番号: 64 / 抜粋: "def __init__(self):")


* **戻り値/レスポンス**: 該当なし（クラス定義）
* 根拠: (行番号: 63 / 抜粋: "class PostBootHealthCheck:")
...
* 根拠: `def __init__(self):` (行番号: 64〜70 / 抜粋: "def __init__(self):\n        self.max_retries = 12       \n        self.retry_interval = 10    \n        self.results: List[CheckResult] = []")


* **引数/リクエスト**: `self` のみ
* 根拠: (行番号: 64 / 抜粋: "def __init__(self):")
...
* 根拠: `def _check_port(self, host: str, port: int, timeout=3) -> bool:` (行番号: 73〜78 / 抜粋: "def _check_port(self, host: str, port: int, timeout=3) -> bool:")


* **引数/リクエスト**: `host: str`, `port: int`, `timeout=3`
* 根拠: (行番号: 73 / 抜粋: "def _check_port(self, host: str, port: int, timeout=3) -> bool:")
...
* 根拠: `def _check_http(self, url: str, timeout=5, headers=None) -> bool:` (行番号: 80〜85 / 抜粋: "def _check_http(self, url: str, timeout=5, headers=None) -> bool:")


* **引数/リクエスト**: `url: str`, `timeout=5`, `headers=None`
* 根拠: (行番号: 80 / 抜粋: "def _check_http(self, url: str, timeout=5, headers=None) -> bool:")
...
* 根拠: `def _get_uptime(self) -> str:` (行番号: 87〜98 / 抜粋: "def _get_uptime(self) -> str:")


* **引数/リクエスト**: `self` のみ
* 根拠: (行番号: 87 / 抜粋: "def _get_uptime(self) -> str:")
...
* 根拠: `def check_system_resources(self):` (行番号: 101〜143 / 抜粋: "def check_system_resources(self):")


* **引数/リクエスト**: `self` のみ
* 根拠: (行番号: 101 / 抜粋: "def check_system_resources(self):")
...
* 根拠: `def check_network_and_apis(self):` (行番号: 145〜175 / 抜粋: "def check_network_and_apis(self):")、NatureRemoのスキップ判定 (行番号: 153〜158 / 抜粋: "if config.NATURE_REMO_ACCESS_TOKEN:")


* **引数/リクエスト**: `self` のみ
* 根拠: (行番号: 145 / 抜粋: "def check_network_and_apis(self):")
...
* **引数/リクエスト**: `self` のみ
* 根拠: (行番号: 178 / 抜粋: "def check_database(self):")
...
* 根拠: `def check_services(self):` (行番号: 209〜226 / 抜粋: "def check_services(self):"), `with ThreadPoolExecutor(max_workers=len(targets)) as executor:\n            self.results.extend(executor.map(self._wait_for_service, targets))` (行番号: 200〜201 / 抜粋: "with ThreadPoolExecutor(max_workers=len(targets)) as executor:")


* **引数/リクエスト**: `self` のみ
* 根拠: (行番号: 209 / 抜粋: "def check_services(self):")
...
* 根拠: (行番号: 209〜226 / 抜粋: "def check_services(self):")
...
* 根拠: `def _wait_for_service(self, target: dict) -> CheckResult:` (行番号: 228〜251 / 抜粋: "def _wait_for_service(self, target: dict) -> CheckResult:")
...
* 根拠: `def check_peripherals(self) -> None:` (行番号: 254〜338 / 抜粋: "def check_peripherals(self) -> None:")


* **引数/リクエスト**: `self` のみ
* 根拠: (行番号: 254 / 抜粋: "def check_peripherals(self) -> None:")
...
* **引数/リクエスト**: `self` のみ
* **戻り値/レスポンス**: なし（`self.results` に追加する）
* **副作用**: `self.results` への `CheckResult` 追加。判定失敗時は `logger.error`
* **エラーハンドリング**: `check_security_posture(config)` 全体を `try/except Exception` で囲み、判定自体が失敗しても**起動レポートを落とさない**（他のチェック結果は届けたいため）。失敗時は `logger.error(..., exc_info=True)` を出したうえで `STATUS_WARN` の「判定に失敗」を結果に追加して `return` する
* **設定値を通知に載せない**: 明細に出すのは `f.key` / `f.ref` / `f.protection` のみで、設定の値（トークン等）は含めない。起動レポートは Discord に送られるため
...
* **（vcgencmd の代替監視で変更）起動時の CPU 温度はカーネルの thermal_zone から読む**: 以前は `vcgencmd measure_temp` を呼んでいたが、カーネルと userland の版ずれで使えなくなることがある（2026-09 の OS 更新後に "Unknown" 表示になった）。`CPU_TEMP_FILE`（`/sys/class/thermal/thermal_zone0/temp`、ミリ度）を直接読む。常時の温度監視は `monitors/server_watchdog.py` の `check_cpu_temperature` が担う
...
* **（Issue #290でデッドコード除去）`check_services` の未使用`backend_url`**: 以前は`backend_url = getattr(config, "BACKEND_URL", "http://localhost:8000")`という代入があったが、直後の`targets`リストでは"Backend Server"をポート8000への疎通確認(`{"type": "port", "val": 8000}`)としてハードコードしており、`backend_url`は一切参照されていなかった(未使用変数のためruffのF841で検出)。デッドコードとして削除した。なお、`config.BACKEND_URL`でポート番号を変更してもこのヘルスチェックには反映されない(常にポート8000固定)という潜在的な仕様上の制約自体は本Issueのスコープ外(デッドコード除去のみ)のため変更していない。
