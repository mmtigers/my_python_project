...
> ## ⚠️ 訂正（2026-09-20・Issue #745 クローズ）
>
> **本節の因果関係の記述は誤りです。** 「新しい `setuptools` ではビルドに失敗する」は
> 検証の結果**成立しませんでした**。Issue #745 は `not_planned` でクローズ済みです。
>
> | 条件 | 結果 |
> | --- | --- |
> | setuptools 84.0.0 + build isolation（通常の `pip install`） | **成功** |
> | setuptools 84.0.0 + `--no-build-isolation` | **成功** |
> | setuptools 80.9.0 + `--no-build-isolation` | **成功** |
> | Debian パッチ版 setuptools 68.1.2（`install_layout` を持つ）+ `--no-build-isolation` | **成功** |
>
> 下記のトレースバックは実在し、監査中に実際に踏んだものですが、問題の行
> `self.set_undefined_options('install',('install_layout','install_layout'))` は
> **Debian による setuptools へのパッチ**であり、上流の setuptools には存在しません。
> 監査環境では **pip とインタプリタが食い違っており**（`pip` が `/usr/lib/python3` 側、
> `python3` は `/usr/local/bin/python3`）、Debian パッチ版の `install_lib` が
> パッチの当たっていない `install` コマンドを参照したため `install_layout` が
> 見つからなかった、というのが妥当な説明です。
> **原因は「setuptools のバージョン」ではなく「壊れた混成環境」でした。**
>
> したがって下記の *最小修正*（`start_all.sh` と CI での `pip install "setuptools<81"`）は
> **効果がありません**。入れても何も直らない一方、他パッケージのビルド制約になりうる
> 実コストがあるため、**採用しません**。
>
> 「未保守・sdist のみの依存が `.venv` 再構築の単一障害点」という一般論としてのリスクは
> 残りますが、具体的な失敗が再現しない以上、それだけを根拠に課題として残すことはしません。
> 実際に `.venv` 再構築が `onvif-zeep` で止まったら、そのときの実際のエラーを根拠に
> 再起票してください。
>
> **本節の以下の記述はすべて、この訂正を前提に読んでください。**
...
> ## 対応結果（2026-09-20・Issue #752 クローズ）
>
> **最小修正は実施、推奨修正（生成パイプライン）は不採用。**
>
> - PR #777 が `get_all_view_data` に `response_model=GameDataResponse` を付け、
>   **OpenAPI にレスポンス形状が出るようになった**（本節が「どんな生成ツールを
>   入れてもこのエンドポイントだけは型が取れない」と指摘した点の解消）
> - 同 PR の `MY_HOME_SYSTEM/tests/test_quest_api_type_contract.py` が
>   **Pydantic ↔ SQLite のテーブル列 ↔ `gameDataSchema.ts` の Zod** を
>   突き合わせるブロッキングゲートになった。本節が挙げた
>   「`.strict()` を使わないため新フィールドの追加が無音で無視される」
>   （`nextLevelExp` の実例）は、**バックエンドだけが持つフィールドを
>   「フロントで使うなら Zod に、使わないならテストに明記」のどちらかに
>   強制する**形で塞がれた（どちらもせずに済ませられない）
> - **`openapi-typescript` 等の生成パイプラインは入れない。** 検知という目的は
>   上記で達成済みで、残るのは記述の重複だけである。一方コストは実在する
>   （`npm ci` の依存が1つ増え、Issue #757 で軽くしたばかりの実機ビルド時間に効く）。
>   また手書きの Zod は `gameDataSchema.ts` のモジュールコメントが述べる
>   「ここに無いフィールドを参照すれば実行時に必ず undefined になると分かるようにする」
>   という意図を明示する層であり、自動生成に置き換える対象ではない
>   （本節の *推奨修正* 自身も「Zod は手書きを維持する」ほうが設計意図に合うとしている）
> - 本 Issue は #470（CLOSED）の再提起であり、**一度「やらない」とした判断を維持する**
...
> ## 対応結果（2026-09-20・Issue #762 クローズ）
>
> **記録の保全は実施、論理削除への切り替えは不採用。**
>
> - **方針決定**: 取り消したクエストは**家族の年代記に出さない**。取消は
>   「間違って完了報告した」ときの機能であり、誤った達成が年代記に残り続ける
>   ほうが不都合であるため
> - PR #789 が `quest_cancellation_audit`（追記専用の監査テーブル）を追加し、
>   削除される `quest_history` 行の全フィールド・所有者・**取消要求者**
>   （`cancelled_by`。連携クエストのカスケード取消では所有者と食い違う）・
>   `cascaded` / `rewards_reverted` / `cancelled_at` を保全する。
>   本節が挙げた「監査証跡が無い」「AUDIT-003 の保持期間削除を実装する際に
>   『取消で消えた』と『保持期間で消えた』を区別できない」「`reward_history` を
>   監査用の記録として残す判断との非対称」はいずれも解消した
>   （本テーブルは保持期間削除の対象外）
> - **`status='cancelled'` の論理削除は実装しない。** 年代記を組み立てる
>   `_fetch_full_adventure_logs` は `WHERE status='approved'` で絞っているため、
>   **論理削除にしても年代記の見た目は物理削除と完全に同じ**であり、
>   得られるのは「行の内容が DB に残ること」だけ — それは上記の監査テーブルが
>   既に担っている。一方コストは本節の *副作用* が指摘するとおり実在し、
>   `status != 'rejected'` を条件にする3箇所（`quest_service.py:149` スパムチェック /
>   `quest_service.py:246` 周期リセット / `game_system.py:375` 連続達成ボーナス）を
>   **1箇所でも漏らすと「取り消したのに本日完了済みと判定されて再完了できない」
>   という回帰**になる。見た目が変わらない変更のためにこのリスクは負わない
> - 将来「取消済みも年代記に出したい」となった場合は、`quest_cancellation_audit` に
>   蓄積された実データ（頻度・`status_before`・誰が取り消したか）をそのまま
>   表示の原資にできる
...
| ~~AUDIT-016~~ | ~~AUDIT-006~~ | ~~`onvif-zeep` のビルド失敗を直さないと、Phase 1.5 の `pip install` 全体が中断する~~ **（2026-09-20 無効: AUDIT-016 の因果が再現せず、Issue #745 はクローズ。前提が崩れたためこの順序制約は存在しない）** |
