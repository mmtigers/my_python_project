...
## 0. 対応状況（最終更新: 2026-09-03・Issue #321）

指摘16件のうち**14件は対応完了**（2026-09-16棚卸しで 2.3「DBバックアップのGit混入」を対応済みへ訂正）。2.2は2026-09-03に意思決定（エッジ委譲を正式設計として確定）により方針確定。残る2.1は認証方式の意思決定を伴うもので、**意思決定によりスコープ外**として確定している。詳細は「10. 優先順位別まとめ」の各項目のステータスを参照。

> **2026-09-03決定（Issue #321）**: 2.2「IPアドレス制限ヘッダーの詐称可能性」は、`Cf-Access-Jwt-Assertion`検証の実装（再実装）ではなく、**オリジン直接接続をCloudflare IPレンジに限定するインフラ側設定に委譲する案（案B）**を正式設計として確定した。詳細は`docs/reports/CODE_REVIEW_REPORT_ALL.md`のCritical#2/#8、`CODE_REVIEW_REPORT_2026-08-22.md`のH-10を参照。

### 🔲 残件（未対応・要判断。2026-09-16棚卸しで 2.3 は「対応済み」へ訂正）

| # | 項目 | 状態 | 対応に必要なこと |
|---|---|---|---|
| 2.1 | APIの認可欠如（`user_id`/`approver_id`のクライアント信頼） | 未対応（2026-09-16棚卸し: **意思決定によりスコープ外**として確定） | 個人用IoTシステム・単一プロセス・LAN内を信頼境界とする前提のもとで対応スコープ外と合意済み（`CODE_REVIEW_REPORT_ALL.md` Critical#1と同一課題）。方針を覆す場合は、認証方式の決定（LINEログイン／共有APIキー等）＋フロントエンド側の変更に加え、LAN外からの到達経路とセットで再検討すること。現在この判断の正は`CLAUDE.md`（追跡用のIssue #614はクローズ済み） |
| 2.2 | IPアドレス制限ヘッダーの詐称可能性 | **対応方針確定（エッジ委譲、Issue #321・案B）** | 対応不要（JWT検証は再実装しない）。前提条件（オリジン直接接続をCloudflare IPレンジに限定するインフラ側設定）はオーナー側の運用事項として別途確認 |
| 2.3 | 個人情報を含むDBバックアップのGit混入 | ✅ 対応済み（2026-09-16棚卸しで訂正） | `.gitignore`修正・追跡解除に加え、**過去コミット履歴からの消去も完了済み**（Issue #296。`git log --all -- '*.db'` が0件であることを確認）。本表は長らく「判断待ち」のまま残っていたが、`CODE_REVIEW_REPORT_ALL.md` のCritical#4では既に「完全解消済み」と記録されており、2レポート間で記述が矛盾していた |
...
### 今すぐ修正すべきもの（🔴 Critical）
1. 2.3 個人情報を含むDBバックアップファイルのGit追跡（`.gitignore`修正＋履歴からの除去検討） — ✅ 対応済み（2026-09-16棚卸しで訂正: 追跡解除・.gitignore修正に加え、Issue #296で履歴からの消去も完了済み。`git log --all -- "*.db"` が0件であることを確認）
