...
> **2026-09-19 に実機で確認済み（結果は「実機確認の結果」列）。** 7件すべて解消・判断・実施済み
> （7 はサービス停止を含む通し演習のみ残）。詳細は各 AUDIT の節、同日の実機構成ドリフト解消
> （PR #699 / #702）、および `docs/runbooks/db_restore.md` の「実施記録」を参照。
...
| 7 | DB バックアップからの復元を実際に通したことがあるか | リストア手順書はあるがリハーサル記録が無い（AUDIT-024） | **2026-09-19 に非破壊版で実施・合格**（`integrity_check` ok、3点セットは直近5世代すべて揃い、復元DBへ未適用の `0011`/`0012` が自動適用されることと再実行の冪等性も確認）。記録は `docs/runbooks/db_restore.md` の「実施記録」。**サービス停止を含む通し演習は未実施** |
