...
| Critical#3 | Streamlitダッシュボードの無認証公開 | 部分対応（変わらず） | `--server.address 127.0.0.1`（`start_all.sh:176`）で外部露出は解消済み。`sudo systemctl restart`ボタンには誤操作防止の確認チェックボックスが追加された（`views/dashboard/log_tab.py:70-80`）が、**アプリ内認証は依然として無い**ためLAN内からは誰でも実行できる。LAN内を信頼境界とする方針（棚卸し課題4）の範囲内 |
