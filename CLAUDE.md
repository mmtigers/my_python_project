...
ダッシュボードは `routers/dashboard_router.py` が `config.DASHBOARD_BASE_PATH`（既定 `/dashboard`）配下でHTMLを直接返す（Issue #829でStreamlit版の「詳細表示」・表示モード切替UIは廃止し、かんたん表示のみの構成にした。外部プロセスへの中継は無い）。ホーム（ステータスカード＋見守り/くらし/システムへのナビカード＋ファミクエ・あさノートの外部リンクカード）、`/watch`（カメラのライブ選択・スナップショット・防犯ログ・実家センサーログ）、`/life`（電気・炊飯器カードの再掲）、`/sys`（全体サマリー・各機能の最終更新時刻・メンテナンス操作）の4ページ構成で、各サブページに「ホームへ戻る」リンクを置く。旧URL（`/dashboard/m`・Streamlit版の`?tab=`クエリ）は新しいページへリダイレクトする。ページ全体の組み立ては `services/dashboard_page_service.py`、ホーム画面追加用のマニフェスト・アイコンは `services/dashboard_pwa_service.py` が担う。カードの判定ロジック・HTML組み立ては `services/home_status_service.py` にあり、ホームページと自動更新フラグメント（`{DASHBOARD_BASE_PATH}/status`）の両方がここを呼ぶ — **判定をページ組み立て側へ書き戻すと2つの経路で内容が食い違う**ので、`tests/test_mobile_status_page.py` が固定している。
