...
QUESTS = [
    ...
    # A-1. 通常：共通 (All)
    # ------------------------------------------
    # id=1100「【朝】毎朝ミッション」は、朝の準備がroutine_data.py(「きょうのすごろく」am
    # フローの順不同チェックリスト)へ統合されたため廃止(要件確認済み)。全項目達成時のTV
    # 電源ON処理はservices/routine_service.py側へ移設した(services/switchbot_service.py
    # のtrigger_tv_unlock参照)。
    # id=1105「【夜】就寝ミッション」(exp50/gold70)も、内容が「きょうのすごろく」pmフローの
    # 寝る準備チェックリスト(晩ごはん・お風呂・着替え・歯磨き)と丸かぶりで、同じことを
    # クエストとすごろくで二重に報告させていたため廃止(要件確認済み)。報酬は
    # routine_data.py の「就寝」ステップへ移設した — 就寝は寝る準備を全項目終えないと
    # 着手できないため、元クエストの「全部できたらクリア！」と同じ意味になる。
    ...
    # ため廃止。パパの平日の昼は「お仕事」だけにしたことで土日のステップが
    # 無くなり、何もせずに満額ボーナスが入る状態になるのを避けるための移設で、
    # 報酬額・土日のみという条件はそのままステップ個別報酬として引き継いでいる。
    ...
    {'id': 1009, 'title': '習い事： ピアノ / あこーでぃおん', 'type': 'daily', 'required': False, 'target': 'son', 'category': 'study', 'difficulty': 'B', 'exp': 150, 'gold': 100, 'icon': '🏫', 'desc': '先生とのお勉強やレッスン'},  # 2026-09-23: requiredをFalseへ変更しボーナスクエスト(折りたたみ表示)化(要件確認済み)
    ...
    # (今日の九九タイム3件・九九チャレンジ1件、いずれも2026-09導入)ため、無限クエスト1本化で
    # 「毎日でも何度でも練習できる」意味を持たせる。報酬はdailyクエスト側の額(exp60/gold60)を
    # 引き継いだ(統合後も練習量が減らないようにするため)。
    ...
    # ------------------------------------------
    # id=1040「いっしょにおかたづけ」は、quest_historyで導入から一度も完了報告が無い
    # (実績データで完了0件を確認、要件確認済み)ため廃止した。画面上の「実行可能なクエストが
    # 多すぎる」という指摘を受けた整理の一環。
    ...
    # ------------------------------------------
    # id=1041「きょうだいでお手伝い」は、id=1040と同様にquest_historyで完了報告が一度も無い
    # ため廃止した(要件確認済み)。
]
