...
class TestThrottlingWithoutVcgencmd:
    def _run(self, monkeypatch, caplog, returncode=255):
        failed = subprocess.CompletedProcess(args=[], returncode=returncode, stdout="", stderr="")
        monkeypatch.setattr(server_watchdog.logger, "propagate", True)
        with caplog.at_level(logging.DEBUG, logger=server_watchdog.logger.name), \
                patch.object(server_watchdog.subprocess, "run", return_value=failed):
            server_watchdog.check_throttling_status()

    def test_vcgencmd_failure_is_reported_once_per_boot(self, tmp_path, monkeypatch, caplog):
    ...
    def test_reported_again_after_reboot(self, tmp_path, monkeypatch, caplog):
    ...
    def test_active_undervoltage_from_hwmon_is_error_every_time(self, tmp_path, monkeypatch, caplog):
    ...
    def test_missing_vcgencmd_binary_also_falls_back(self, tmp_path, monkeypatch, caplog):
        _setup(tmp_path, monkeypatch)
        monkeypatch.setattr(server_watchdog.logger, "propagate", True)
        with caplog.at_level(logging.DEBUG, logger=server_watchdog.logger.name), \
    ...
    def test_unreadable_hwmon_is_mentioned(self, tmp_path, monkeypatch, caplog):
        _setup(tmp_path, monkeypatch)
...
class TestCpuTemperature:
    ...
    def test_normal_temperature_is_silent(self, tmp_path, monkeypatch, caplog):
        self._setup(tmp_path, monkeypatch, "61250")
        with caplog.at_level(logging.DEBUG, logger=server_watchdog.logger.name):
            server_watchdog.check_cpu_temperature(now=1000.0)
        assert _errors(caplog) == []

    def test_high_temperature_alerts_then_suppresses_then_realerts(self, tmp_path, monkeypatch, caplog):
        self._setup(tmp_path, monkeypatch, "82000")
        with caplog.at_level(logging.DEBUG, logger=server_watchdog.logger.name):
            server_watchdog.check_cpu_temperature(now=1000.0)
            assert len(_errors(caplog)) == 1 and "82.0°C" in _errors(caplog)[0]
            caplog.clear()
    ...
    def test_unreadable_temperature_is_warning_not_crash(self, tmp_path, monkeypatch, caplog):
        self._setup(tmp_path, monkeypatch, "0")
        monkeypatch.setattr(server_watchdog, "THERMAL_ZONE_TEMP_FILE", tmp_path / "missing")
        with caplog.at_level(logging.DEBUG, logger=server_watchdog.logger.name):
            server_watchdog.check_cpu_temperature(now=1000.0)
        assert _errors(caplog) == []
        assert any("CPU温度を読み取れません" in r.getMessage() for r in caplog.records)
