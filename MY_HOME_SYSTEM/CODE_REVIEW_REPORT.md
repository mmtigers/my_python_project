...
### 今すぐ修正すべきもの（🔴 Critical）
1. 2.3 個人情報を含むDBバックアップファイルのGit追跡（`.gitignore`修正＋履歴からの除去検討）
2. 2.1 APIの認可欠如（`user_id`/`approver_id`のクライアント信頼）
3. 2.2 IPアドレス制限ヘッダーの詐称可能性

### 次のリリースまでに修正すべきもの（🟠 High）
4. 5.1 `test_unified_server.py`の実装乖離とCI未収集問題
5. 2.5 カメラ録画エンドポイントのパストラバーサル対策不足
6. 2.4 SwitchBot Webhookの署名検証欠如

### 将来的に改善すべきもの（🟡 Medium 〜 🔵 Low）
7. 2.6 AIツールのテーブルアクセス制御
8. 3.1 FOREIGN KEY未有効化
9. 3.2 高頻度書き込みテーブルのインデックス不足
10. 4.1 スケジューラの直列実行による監視遅延
11. 4.2 クエスト完了の冪等性不足
12. 5.2 テストカバレッジ不足
13. 3.3 実行時マイグレーションの脆さ
14. 6.1 例外詳細のクライアント露出
15. 7.1 型ヒントのタイポ
16. 8.1 LINEプロフィール取得の都度呼び出し
